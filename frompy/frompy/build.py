...
def load_plugins_from_config(
    ...
    for plugin_path in options.plugins:
        ...
        if not issubclass(plugin_type, Plugin):
            plugin_error(
                'Return value of "plugin" must be a subclass of "frompy.plugin.Plugin" '
...
class BuildManager:
    ...
    def log_fine_grained(self, *message: str) -> None:
        ...
        elif frompy.build.DEBUG_FINE_GRAINED:
...
class State:
    ...
    def compute_fine_grained_deps(self) -> Dict[str, Set[str]]:
        ...
        if "/typeshed/" in self.xpath or self.xpath.startswith("typeshed/"):
            ...
            return {}
        from frompy.server.deps import (
            get_dependencies,
        )  # Lazy import to speed up startup
...
def find_module_and_diagnose(
    ...
    if isinstance(result, str):
        ...
        if (
            ...
            or id in frompy.semanal_main.core_modules
...
def process_stale_scc(graph: Graph, scc: List[str], manager: BuildManager) -> None:
    ...
    frompy.semanal_main.semantic_analysis_for_scc(graph, scc, manager.errors)
