...
class SemanticAnalyzer(
    ...
    # Module name space
    modules = None  # type: Dict[str, FrompyFile]
    ...
    def prepare_file(self, file_node: FrompyFile) -> None:
    ...
    def prepare_typing_namespace(self, file_node: FrompyFile) -> None:
    ...
    def prepare_builtins_namespace(self, file_node: FrompyFile) -> None:
    ...
    def refresh_partial(
        self,
        node: Union[FrompyFile, FuncDef, OverloadedFuncDef],
        patches: List[Tuple[int, Callable[[], None]]],
        final_iteration: bool,
        file_node: FrompyFile,
        options: Options,
    ...
    def refresh_top_level(self, file_node: FrompyFile) -> None:
    ...
    def add_implicit_module_attrs(self, file_node: FrompyFile) -> None:
    ...
    def add_builtin_aliases(self, tree: FrompyFile) -> None:
        ...
        if i_id in self.modules:
            ...
            for name, node in m.names.items():
                ...
                if node.module_public and (
                    ...
                    if isinstance(node.node, FrompyFile):
    ...
    def process_module_assignment(
        ...
        if isinstance(rval, (TupleExpr, ListExpr)) and all(
            ...
            if rnode and isinstance(rnode.node, FrompyFile):
                for lval in lvals:
                    ...
                    if lnode:
                        if (
                            isinstance(lnode.node, FrompyFile)
    ...
    def visit_member_expr(self, expr: MemberExpr) -> None:
        base = expr.expr
        base.accept(self)
        if isinstance(base, RefExpr) and isinstance(base.node, FrompyFile):
            ...
            if type_info:
                ...
                if n is not None and isinstance(
                    n.node, (FrompyFile, TypeInfo, TypeAlias)
                ):
                    if not n:
                        return
                    expr.kind = n.kind
    ...
    def lookup(
        ...
        if b:
            assert isinstance(b.node, FrompyFile)
            table = b.node.names
            if name in table:
    ...
    def lookup_qualified(
        ...
        if sym:
            for i in range(1, len(parts)):
                ...
                elif isinstance(node, FrompyFile):
    ...
    def get_module_symbol(
        self, node: FrompyFile, name: str
    ) -> Optional[SymbolTableNode]:
    ...
    def lookup_fully_qualified(self, name: str) -> SymbolTableNode:
        ...
        for i in range(1, len(parts) - 1):
            ...
            assert isinstance(next_sym.node, FrompyFile)
            n = next_sym.node
