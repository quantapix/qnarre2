...
class ASTMergeSuite(DataSuite):
    ...
    ) -> Tuple[FrompyFile, Dict[Expression, Type]]:
    ...
    def dump_asts(self, modules: Dict[str, FrompyFile]) -> List[str]:
        a = []
    ...
    def dump_symbol_tables(self, modules: Dict[str, FrompyFile]) -> List[str]:
        a = []
    ...
    def dump_typeinfos(self, modules: Dict[str, FrompyFile]) -> List[str]:
        a = []
        for id in sorted(modules):
            if not is_dumped_module(id):
                continue
