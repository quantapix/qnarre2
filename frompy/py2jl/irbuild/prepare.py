...
from py2jl.ir.rtypes import RInstance, tuple_rprimitive, dict_rprimitive
...
from py2jl.common import PROPSET_PREFIX
from py2jl.irbuild.mapper import Mapper
from py2jl.irbuild.util import (
    get_func_def,
    is_dataclass,
    is_trait,
    is_extension_class,
...
def is_from_module(node: SymbolNode, module: FrompyFile) -> bool:
    return node.fullname == module.fullname + "." + node.name


def load_type_map(
    mapper: "Mapper", modules: List[FrompyFile], deser_ctx: DeserMaps
) -> None:
...
def get_module_func_defs(module: FrompyFile) -> Iterable[FuncDef]:
