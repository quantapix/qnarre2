...
export function cloneCell<T extends nbformat.IBaseCell>(cell: T): T {
  ...
  const source =
    Array.isArray(clonedCell.source) || typeof clonedCell.source === 'string'
      ? clonedCell.source
      : '';
  switch (cell.cell_type) {
    case 'code': {
      const codeCell: nbformat.ICodeCell = {
        ...
        execution_count:
          typeof clonedCell.execution_count === 'number'
            ? clonedCell.execution_count
            : null,
        outputs: Array.isArray(clonedCell.outputs)
          ? (clonedCell.outputs as nbformat.IOutput[])
          : [],
        source,
      };
      // tslint:disable-next-line: no-any
