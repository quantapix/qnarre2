...
export class LanguageServerExtensionActivationService
  ...
  constructor(
    ...
    disposables.push(
      commandManager.registerCommand(
        Commands.ClearAnalyisCache,
        this.onClearAnalysisCaches.bind(this)
      )
    );
  }
  ...
  public async activate(resource: Resource): Promise<void> {
    ...
    this.activatedServer = {
      key,
      server: result,
      jedi: result.type === LanguageServerType.Jedi,
    };
  ...
  public async sendTelemetryForChosenLanguageServer(jediEnabled: boolean): Promise<void> {
    const state = this.stateFactory.createGlobalPersistentState<boolean | undefined>(
      'SWITCH_LS',
      undefined
    );
  ...
  public isJediUsingDefaultConfiguration(resource: Resource): boolean {
    const settings = this.workspaceService
      ...
      .inspect<boolean>('jediEnabled');
    if (!settings) {
      traceError(
        'WorkspaceConfiguration.inspect returns `undefined` for setting `python.jediEnabled`'
      );
      return false;
    }
    return (
  ...
  public useJedi(): boolean {
    ...
    const languageServerType = configurationService.getSettings(this.resource)
      .languageServer;
  ...
  private async createRefCountedServer(
    ...
    let server = this.serviceContainer.get<ILanguageServerActivator>(
      ILanguageServerActivator,
      serverType
    );
    try {
      ...
      await this.logStartup(serverType);
      serverType = LanguageServerType.Jedi;
      server = this.serviceContainer.get<ILanguageServerActivator>(
        ILanguageServerActivator,
        serverType
      );
      await server.start(resource, interpreter);
  ...
  private async onDidChangeConfiguration(event: ConfigurationChangeEvent) {
    ...
    if (
      ...
      workspacesUris.findIndex((uri) =>
        event.affectsConfiguration(`python.${languageServerSetting}`, uri)
      ) === -1
    ) {
      return;
    }
  ...
  private async getKey(
    ...
    const interperterPortion = interpreter
