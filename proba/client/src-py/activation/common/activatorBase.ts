...
export abstract class LanguageServerActivatorBase implements ILanguageServerActivator {
  ...
  public abstract async ensureLanguageServerIsAvailable(
    resource: Resource
  ): Promise<void>;
  ...
  private async handleProvideRenameEdits(
    ...
    if (languageClient) {
      const args: vscodeLanguageClient.RenameParams = {
        textDocument: languageClient.code2ProtocolConverter.asTextDocumentIdentifier(
          document
        ),
  ...
  private async handleProvideCompletionItems(
    ...
    const languageClient = this.getLanguageClient();
    if (languageClient) {
      const args = languageClient.code2ProtocolConverter.asCompletionParams(
        document,
        position,
        context
      );
