...
@injectable()
export abstract class InteractiveBase extends WebViewHost<IInteractiveWindowMapping>
  implements IInteractiveBase {
  ...
  constructor(
    ...
    setTimeout(() => {
      this.getNotebookIdentity()
        .then((identity) =>
          this.listeners.forEach((l) =>
            l.onMessage(InteractiveWindowMessages.NotebookIdentity, identity)
          )
        )
        .ignoreErrors();
    }, 0);
    ...
    this.disposables.push(
      this.jupyterVariables.refreshRequired(this.refreshVariables.bind(this))
    );
  }
    ...
    switch (message) {
      case InteractiveWindowMessages.ConvertUriForUseInWebViewRequest:
        ...
        this.postMessageToListeners(
          InteractiveWindowMessages.ConvertUriForUseInWebViewResponse,
          response
        );
        break;
    ...
    switch (message) {
      case CssMessages.GetCssRequest:
        ...
        if (this._notebook) {
          this.isDark()
            .then((d) =>
              this._notebook ? this._notebook.setMatplotLibStyle(d) : Promise.resolve()
            )
            .ignoreErrors();
        }
        break;
  ...
  public dispose() {
    ...
    this.getNotebookIdentity()
      .then((r) => {
        ...
        this.listeners.forEach((l) =>
          l.onMessage(InteractiveWindowMessages.NotebookClose, r)
        );
      })
      .ignoreErrors();
  ...
  protected onViewStateChanged(args: WebViewViewChangeEventArgs) {
    ...
    this.listeners.forEach((l) =>
      l.onViewStateChanged ? l.onViewStateChanged(args) : noop()
    );
  }
  ...
  protected abstract getNotebookMetadata(): Promise<
    nbformat.INotebookMetadata | undefined
  >;
  ...
  protected async submitCode(
    ...
    const stopWatch =
      this._notebook && !this.perceivedJupyterStartupTelemetryCaptured
        ? new StopWatch()
        : undefined;
    let result = true;
    // Do not execute or render empty code cells
    const cellMatcher = new CellMatcher(
      this.configService.getSettings(await this.getOwningResource()).datascience
    );
      ...
      if (this._notebook) {
        ...
        if (debugInfo) {
          ...
          if (debugInfo.runByLine && debugInfo.hashFileName) {
            await this.jupyterDebugger.startRunByLine(
              this._notebook,
              debugInfo.hashFileName
            );
        ...
        observable.subscribe(
          (cells: ICell[]) => {
            ...
            if (this.configuration.getSettings(owningResource).datascience.stopOnError) {
              result =
                result && cells.find((c) => c.state === CellState.error) === undefined;
            }
          },
          (error) => {
            status.dispose();
  ...
  protected setStatus = (message: string, showInWebView: boolean): Disposable => {
    const result = this.statusProvider.set(
      message,
      showInWebView,
      undefined,
      undefined,
  ...
  private async disableAskForRestart(): Promise<void> {
    ...
    if (settings && settings.datascience) {
      ...
      this.configuration
        .updateSetting(
          'dataScience.askForKernelRestart',
          false,
          undefined,
  ...
  private async shouldAskForLargeData(): Promise<boolean> {
    ...
    return (
      settings &&
      settings.datascience &&
      settings.datascience.askForLargeDataFrames === true
    );
  }
  ...
  private async disableAskForLargeData(): Promise<void> {
    ...
    if (settings && settings.datascience) {
      ...
      this.configuration
        .updateSetting(
          'dataScience.askForLargeDataFrames',
          false,
          undefined,
          ConfigurationTarget.Global
        )
  ...
  private async checkColumnSize(columnSize: number): Promise<boolean> {
    if (columnSize > ColumnWarningSize && (await this.shouldAskForLargeData())) {
      ...
      const result = await this.applicationShell.showWarningMessage(
        message,
        yes,
        no,
        dontAskAgain
  ...
  protected async ensureNotebook(
  ...
  private getServerUri(
    serverConnection: INotebookProviderConnection | undefined
  ): string {
    let localizedUri = '';
  ...
  private async ensureNotebookImpl(
    serverConnection: INotebookProviderConnection
  ): Promise<void> {
  ...
  private async gotoCodeInternal(file: string, line: number) {
    ...
    if (await this.fileSystem.fileExists(file)) {
      editor = await this.documentManager.showTextDocument(Uri.file(file), {
  ...
  private async copyCodeInternal(source: string) {
    ...
    if (
      editor &&
      (editor.document.languageId === PYTHON_LANGUAGE || editor.document.isUntitled)
    ) {
      ...
      if (hasCellsAlready) {
        ...
        const matchingRange = ranges.find(
          (r) => r.range.start.line <= line && r.range.end.line >= line
        );

        // If in the middle, wrap the new code
        if (
          matchingRange &&
          matchingRange.range.start.line < line &&
      ...
      editor.revealRange(
        new Range(revealLine, 0, revealLine + source.split('\n').length + 3, 0)
      );
  ...
  private async kernelChangeHandler(kernel: IJupyterKernelSpec | LiveKernelModel) {
    ...
    return this.updateNotebookOptions(
      kernelSpec,
      this._notebook?.getMatchingInterpreter()
    );
  }
  ...
  private async handleKernelMessage(
  ...
  private async handleUpdateDisplayData(msg: KernelMessage.IUpdateDisplayDataMsg) {
    ...
    return this.postMessage(
      InteractiveWindowMessages.UpdateDisplayData,
      msg
    ).ignoreErrors();
  }
}
