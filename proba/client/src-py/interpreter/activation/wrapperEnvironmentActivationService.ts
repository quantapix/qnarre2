...
import { deferredFromPromise } from '../../common/utils/async';
...
@injectable()
export class WrapperEnvironmentActivationService
  implements IEnvironmentActivationService {
  private readonly cachePerResourceAndInterpreter = new Map<
    string,
    Promise<NodeJS.ProcessEnv | undefined>
  >();
  constructor(
    @inject(EnvironmentActivationService)
    private readonly procActivation: IEnvironmentActivationService,
    ...
    disposables.push(
      envVarsProvider.onDidEnvironmentVariablesChange(() =>
        this.cachePerResourceAndInterpreter.clear()
      )
    );
  }
  @captureTelemetry(
  ...
  public async getActivatedEnvironmentVariables(
    ...
    const terminalEnvVarsPromise = this.cacheCallback(terminalCacheKey, () =>
      this.getActivatedEnvVarsFromTerminal(
        procEnvVarsPromise,
        resource,
        interpreter,
        allowExceptions
    ...
    const procEnvVars = deferredFromPromise(procEnvVarsPromise);
    const terminalEnvVars = deferredFromPromise(terminalEnvVarsPromise);
  ...
  private async getDataCachedInFile(
    cacheKey: string
  ): Promise<EnvVariablesInCachedFile | undefined> {
   ...
   */
  private async writeDataToCacheFile(
    cacheKey: string,
    data: EnvVariablesInCachedFile
  ): Promise<void> {
    const cacheFile = this.getCacheFile(cacheKey);
  ...
  private async getActivatedEnvVarsFromProc(
    ...
    return this.procActivation.getActivatedEnvironmentVariables(
      resource,
      interpreter,
      allowExceptions
    );
