...
export class InterpreterService implements Disposable, IInterpreterService {
  ...
  constructor(
    ...
    @inject(InterpeterHashProviderFactory)
    private readonly hashProviderFactory: IInterpreterHashProviderFactory
  ) {
    this.locator = serviceContainer.get<IInterpreterLocatorService>(
      IInterpreterLocatorService,
      INTERPRETER_LOCATOR_SERVICE
    );
    this.persistentStateFactory = this.serviceContainer.get<IPersistentStateFactory>(
      IPersistentStateFactory
    );
    this.configService = this.serviceContainer.get<IConfigurationService>(
      IConfigurationService
    );
    this.interpreterPathService = this.serviceContainer.get<IInterpreterPathService>(
      IInterpreterPathService
    );
    this.experiments = this.serviceContainer.get<IExperimentsManager>(
      IExperimentsManager
    );
  }
  ...
  public initialize() {
    ...
    disposables.push(
      documentManager.onDidChangeActiveTextEditor((e) =>
        e ? this.refresh(e.document.uri) : undefined
      )
    );
    ...
    if (this.experiments.inExperiment(DeprecatePythonPath.experiment)) {
      ...
      const disposable = workspaceService.onDidChangeConfiguration((e) => {
        ...
        const workspaceUri =
          workspaceUriIndex === -1 ? undefined : workspacesUris[workspaceUriIndex];
  ...
  public async getDisplayName(
    ...
    if (
      !info.cachedEntry &&
      info.path &&
      this.inMemoryCacheOfDisplayNames.has(info.path)
    ) {
      return this.inMemoryCacheOfDisplayNames.get(info.path)!;
    }
    const fileHash =
      (info.path ? await this.getInterepreterFileHash(info.path).catch(() => '') : '') ||
      '';
    // Do not include dipslay name into hash as that changes.
    const interpreterHash = `${fileHash}-${md5(
      JSON.stringify({ ...info, displayName: '' })
    )}`;
    const store = this.persistentStateFactory.createGlobalPersistentState<{
      hash: string;
      displayName: string;
    }>(`${info.path}.interpreter.displayName.v7`, undefined, EXPITY_DURATION);
  ...
  public async getInterpreterCache(
    ...
    const fileHash =
      (pythonPath
        ? await this.getInterepreterFileHash(pythonPath).catch(() => '')
        : '') || '';
    const store = this.persistentStateFactory.createGlobalPersistentState<{
      fileHash: string;
  ...
  protected async updateCachedInterpreterInformation(
    info: PythonInterpreter,
    resource: Resource
  ): Promise<void> {
  ...
  protected async buildInterpreterDisplayName(
    info: Partial<PythonInterpreter>,
    resource?: Uri
  ): Promise<string> {
    ...
    if (!info.envName && info.path && info.type && info.type === InterpreterType.Pipenv) {
      ...
      const virtualEnvMgr = this.serviceContainer.get<IVirtualEnvironmentManager>(
  ...
  private async collectInterpreterDetails(pythonPath: string, resource: Uri | undefined) {
    ...
    const virtualEnvManager = this.serviceContainer.get<IVirtualEnvironmentManager>(
      IVirtualEnvironmentManager
    );
