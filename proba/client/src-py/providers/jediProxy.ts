...
export class JediProxy implements Disposable {
  ...
  public constructor(
    ...
    this.lastKnownPythonInterpreter = interpreter
      ? interpreter.path
      : this.pythonSettings.pythonPath;
  ...
  private async spawnProcess() {
    ...
    const pythonProcess = await this.serviceContainer
      ...
      .create({
        resource: Uri.file(this.workspacePath),
        pythonPath: this.lastKnownPythonInterpreter,
      });
  ...
  private getCommandHandler(
    ...
    | undefined
    | ((command: IExecutionCommand<ICommandResult>, response: object) => void) {
    switch (command) {
  ...
  private async getPathFromPython(
    getArgs = internalPython.getExecutable
  ): Promise<string> {
      ...
      const pythonPaths = await this.getEnvironmentVariablesProvider()
        ...
        .then((customEnvironmentVars) =>
          customEnvironmentVars
            ? JediProxy.getProperty<string>(customEnvironmentVars, 'PYTHONPATH')
            : ''
        )
        .then((pythonPath) =>
          typeof pythonPath === 'string' && pythonPath.trim().length > 0
            ? pythonPath.trim()
            : ''
        )
        .then((pythonPath) =>
          pythonPath.split(path.delimiter).filter((item) => item.trim().length > 0)
        );
