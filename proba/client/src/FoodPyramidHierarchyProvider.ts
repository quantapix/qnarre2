  ...
  async provideCallHierarchyOutgoingCalls(
    ...
    if (model.isVerb(item.name)) {
      ...
      outgoingCalls.forEach((relation) => {
        ...
        let verbItem = this.createCallHierarchyItem(
          relation.object,
          'noun',
          document,
          outgoingCallRange
        );
        let outgoingCallItem = new vscode.CallHierarchyOutgoingCall(verbItem, [
          outgoingCallRange,
        ]);
        outgoingCallItems.push(outgoingCallItem);
      });
    } else if (model.isNoun(item.name)) {
      let outgoingCallMap = groupBy(
      ...
      outgoingCallMap.forEach((relations, verb) => {
        ...
        let outgoingCallItem = new vscode.CallHierarchyOutgoingCall(
          verbItem,
          outgoingCallRanges
        );
        outgoingCallItems.push(outgoingCallItem);
  ...
  async provideCallHierarchyIncomingCalls(
    ...
    if (model.isVerb(item.name)) {
      ...
      outgoingCalls.forEach((relation) => {
        ...
        let outgoingCallItem = new vscode.CallHierarchyIncomingCall(verbItem, [
          outgoingCallRange,
        ]);
        outgoingCallItems.push(outgoingCallItem);
      });
      ...
      outgoingCallMap.forEach((relations, verb) => {
        ...
        let outgoingCallItem = new vscode.CallHierarchyIncomingCall(
          verbItem,
          outgoingCallRanges
        );
        outgoingCallItems.push(outgoingCallItem);
