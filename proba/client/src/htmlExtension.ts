  ...
  client.onReady().then(() => {
    ...
    toDispose.push(
      workspace.onDidChangeConfiguration(
        (e) =>
          e.affectsConfiguration(SettingIds.formatEnable) && updateFormatterRegistration()
      )
    );
  ...
  languages.registerCompletionItemProvider(documentSelector, {
    provideCompletionItems(doc, pos) {
      ...
      if (
        match2 &&
        doc
          .getText(new Range(new Position(0, 0), pos))
          .match(htmlSnippetCompletionRegExpr)
      ) {
        let range = new Range(new Position(pos.line, match2[1].length), pos);
        let snippetProposal = new CompletionItem(
          'HTML sample',
        ...
        snippetProposal.documentation = localize(
          'folding.html',
