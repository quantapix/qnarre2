...
export function activate(context: ExtensionContext) {
  ...
  const debugOptions = {
    execArgv: ['--nolazy', '--inspect=' + (9000 + Math.round(Math.random() * 10000))],
  };
        ...
        function updateHover(r: Hover | null | undefined): Hover | null | undefined {
          if (r && Array.isArray(r.contents)) {
            r.contents = r.contents.map((h) =>
              h instanceof MarkdownString ? updateMarkdownString(h) : h
            );
          }
          return r;
        }
  ...
  client.onReady().then(() => {
    ...
    client.onRequest(VSCodeContentRequest.type, (uriPath: string) => {
      ...
      if (uri.scheme === 'untitled') {
        return Promise.reject(
          new Error(localize('untitled.schema', 'Unable to load {0}', uri.toString()))
        );
      }
    ...
    client.onNotification(ResultLimitReachedNotification.type, (message) => {
      window.showInformationMessage(
        `${message}\n${localize(
          'configureLimit',
          "Use setting '{0}' to configure the limit.",
    ...
    function updateSchemaDownloadSetting() {
      schemaDownloadEnabled =
        workspace.getConfiguration().get(SettingIds.enableSchemaDownload) !== false;
      if (schemaDownloadEnabled) {
        schemaResolutionErrorStatusBarItem.tooltip = localize(
        ...
        schemaResolutionErrorStatusBarItem.tooltip = localize(
          'json.schemaResolutionDisabledMessage',
...
function getSettings(): Settings {
  ...
  const resultLimit: number =
    Math.trunc(
      Math.max(0, Number(workspace.getConfiguration().get(SettingIds.maxItemsComputed)))
    ) || 5000;
  ...
  const schemaSettingsById: { [schemaId: string]: JSONSchemaSettings } = Object.create(
    null
  );
...
function getSchemaId(schema: JSONSchemaSettings, folderUri?: Uri) {
  ...
  } else if (folderUri && (url.startsWith('.') || url.startsWith('/'))) {
