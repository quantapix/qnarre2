...
import { IConfigurationService, IHttpClient, IPythonSettings } from './types';
...
export const configUri =
  'https://raw.githubusercontent.com/microsoft/vscode-python/master/experiments.json';
export const EXPERIMENTS_EFFORT_TIMEOUT_MS = 2000;
// The old experiments which are working fine using the `SHA512` algorithm
export const oldExperimentSalts = [
...
export class ExperimentsManager implements IExperimentsManager {
  ...
  constructor(
    ...
    this.isDownloadedStorageValid = this.persistentStateFactory.createGlobalPersistentState<
      boolean
    >(isDownloadedStorageValidKey, false, EXPIRY_DURATION_MS);
    this.experimentStorage = this.persistentStateFactory.createGlobalPersistentState<
      ...
      for (const experiment of this.experimentStorage.value) {
        try {
          ...
          if (
            ...
            this.isUserInRange(experiment.min, experiment.max, experiment.salt)
          ) {
            this.userExperiments.push(experiment);
          }
        } catch (ex) {
          traceError(
            `Failed to populate experiment list for experiment '${experiment.name}'`,
            ex
          );
        }
      }
  ...
  public sendTelemetryIfInExperiment(experimentName: string): void {
    if (this.userExperiments.find((exp) => exp.name === experimentName)) {
      sendTelemetryEvent(EventName.PYTHON_EXPERIMENTS, undefined, {
  ...
  public async downloadAndStoreExperiments(
    storage: IPersistentState<ABExperiments | undefined> = this
      .downloadedExperimentsStorage
  ): Promise<void> {
    const downloadedExperiments = await this.httpClient.getJSON<ABExperiments>(
      configUri,
      false
    );
  ...
  public isUserInRange(min: number, max: number, salt: string): boolean {
    ...
    if (oldExperimentSalts.find((oldSalt) => oldSalt === salt)) {
      hash = this.crypto.createHash(
        `${this.appEnvironment.machineId}+${salt}`,
        'number',
        'SHA512'
      );
    } else {
  ...
  public async updateExperimentStorage(): Promise<void> {
    ...
    if (await this.fs.fileExists(configFile)) {
      ...
      try {
        const experiments = parse(content, [], {
        ...
        traceError(
          'Failed to parse experiments configuration file to update storage',
          ex
        );
      }
    }
  ...
  public async doBestEffortToPopulateExperiments(): Promise<boolean> {
    try {
      ...
      sendTelemetryEvent(EventName.PYTHON_EXPERIMENTS_DOWNLOAD_SUCCESS_RATE, undefined, {
        success,
      });
      return success;
    } catch (ex) {
  ...
  private cleanUpExperimentsOptList(): void {
    ...
    this._experimentsOptedOutFrom = this._experimentsOptedOutFrom.filter(
      (exp) => exp !== ''
    );
  }
}
