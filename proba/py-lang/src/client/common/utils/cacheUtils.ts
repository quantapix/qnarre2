...
function getCacheKey(
  ...
  if (serviceContainer) {
    interpreterPathService = serviceContainer.get<IInterpreterPathService>(
  ...
  const globalPythonPath =
    inExperiment && interpreterPathService
      ? interpreterPathService.inspect(vscode.Uri.file(__filename)).globalValue ||
...
export class InMemoryCache<T> {
  ...
  public get hasData() {
    if (
      !this.store.get(this.cacheKey) ||
      this.hasExpired(this.store.get(this.cacheKey)!.expiry)
    ) {
