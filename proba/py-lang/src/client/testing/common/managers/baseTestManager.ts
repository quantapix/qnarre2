...
export abstract class BaseTestManager implements ITestManager {
  ...
  constructor(
    ...
    this._testResultsService = this.serviceContainer.get<ITestResultsService>(
      ITestResultsService
    );
    ...
    this.unitTestDiagnosticService = serviceContainer.get<ITestDiagnosticService>(
      ITestDiagnosticService
    );
    this.testsStatusUpdaterService = serviceContainer.get<ITestsStatusUpdaterService>(
      ITestsStatusUpdaterService
    );
        ...
        tests.testFiles.forEach((file) => {
          if (file.errorsWhenDiscovering && file.errorsWhenDiscovering.length > 0) {
            ...
            this.outputChannel.append(
              `There was an error in identifying unit tests in ${file.nameToRun}`
            );
  ...
  public async runTest(
    ...
    const validCmdSourceValues = getNamesAndValues<CommandSource>(CommandSource).map(
      (item) => item.value
    );
    const telementryProperties: TestRunTelemetry = {
      ...
      triggerSource:
        validCmdSourceValues.indexOf(cmdSource) === -1 ? 'commandpalette' : cmdSource,
      failed: false,
    };
  ...
  private updateStatus(status: TestStatus): void {
    ...
    setTimeout(
      () => this._onDidStatusChange.fire({ workspace: this.workspaceFolder, status }),
      1
    );
  }
