...
export namespace CommonEffects {
  ...
  export function handleCss(
    arg: CommonReducerArg<CommonActionType, IGetCssResponse>
  ): IMainState {
    ...
    if (fontSizeIndex > -1) {
      const fontSizeEndIndex = arg.payload.data.css.indexOf(
        'px;',
        fontSizeIndex + sizeSetting.length
      );
      fontSize = parseInt(
        arg.payload.data.css.substring(
          fontSizeIndex + sizeSetting.length,
          fontSizeEndIndex
        ),
        10
    ...
    if (fontFamilyIndex > -1) {
      const fontFamilyEndIndex = arg.payload.data.css.indexOf(
        ';',
        fontFamilyIndex + familySetting.length
      );
      fontFamily = arg.payload.data.css.substring(
        fontFamilyIndex + familySetting.length,
        fontFamilyEndIndex
      );
    }
  ...
  export function monacoThemeChange<T>(
    arg: CommonReducerArg<T, IGetMonacoThemeResponse>
  ): IMainState {
    return {
      ...arg.prevState,
  ...
  export function openSettings(
    arg: CommonReducerArg<CommonActionType, IOpenSettingsAction>
  ): IMainState {
    postActionToExtension(
      arg,
      InteractiveWindowMessages.OpenSettings,
      arg.payload.data.setting
    );
    return arg.prevState;
  }
      ...
      if (c.cell.data.cell_type === 'code') {
        ...
        const changedOutputs = data.outputs.map((o) => {
          if (
            ...
            (o.transient as any).display_id ===
              arg.payload.data.content.transient.display_id
          ) {
  ...
  export function handleLoadIPyWidgetClassSuccess(
    ...
    postActionToExtension(
      arg,
      InteractiveWindowMessages.IPyWidgetLoadSuccess,
      arg.payload.data
    );
    return arg.prevState;
  }
  export function handleLoadIPyWidgetClassFailure(
    ...
    let index = arg.prevState.cellVMs.findIndex(
    ...
    if (index >= 0 && arg.prevState.cellVMs[index].cell.data.cell_type === 'code') {
      ...
      postActionToExtension(
        arg,
        InteractiveWindowMessages.IPyWidgetLoadFailure,
        arg.payload.data
      );
  ...
  export function handleIPyWidgetRenderFailure(
    arg: CommonReducerArg<CommonActionType, Error>
  ): IMainState {
    // Make sure to tell the extension so it can log telemetry.
    postActionToExtension(
      arg,
      InteractiveWindowMessages.IPyWidgetRenderFailure,
      arg.payload.data
    );
    return arg.prevState;
  }
