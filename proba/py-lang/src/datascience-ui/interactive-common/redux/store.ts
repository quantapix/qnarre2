...
import { getSelectedAndFocusedInfo, IMainState, ServerStatus } from '../mainState';
...
import {
  ...
  getDefaultSettings,
} from '../../react-common/settingsReactSide';
...
function createSendInfoMiddleware(): Redux.Middleware<{}, IStore> {
  return (store) => (next) => (action) => {
    ...
    const messageType =
      (action?.payload as BaseReduxActionPayload).messageType ?? MessageType.other;
    const isSyncMessage =
    ...
    if (
      ...
      getSelectedAndFocusedInfo(prevState.main).selectedCellId !==
      ...
      postActionToExtension(
        ...
        InteractiveWindowMessages.SendInfo,
        {
...
function createTestLogger() {
  ...
  if (logFileEnv) {
    ...
    const logFilePath = path.isAbsolute(logFileEnv)
      ? logFileEnv
      : path.join(EXTENSION_ROOT_DIR, logFileEnv);
    log4js.configure({
  ...
  return (store) => (next) => (action) => {
    ...
    const sendMessage = (message: any, payload?: any) => {
      setTimeout(() => {
        transformPromise
          .then(() =>
            postActionToExtension({ queueAction: store.dispatch }, message, payload)
          )
          .ignoreErrors();
      });
    };
    ...
    const currentSelection = getSelectedAndFocusedInfo(afterState.main);
    if (
      previousSelection.focusedCellId !== currentSelection.focusedCellId &&
      currentSelection.focusedCellId
    ) {
      // Send async so happens after render state changes (so our enzyme wrapper is up to date)
      sendMessage(InteractiveWindowMessages.FocusedCellEditor, {
    ...
    const prevFinished = prevState.main.cellVMs
      .filter(
        (c) => c.cell.state === CellState.finished || c.cell.state === CellState.error
      )
      .map((c) => c.cell.id);
    ...
    if (
      prevBreak !== newBreak ||
      !fastDeepEqual(prevBreak?.currentStack, newBreak?.currentStack)
    ) {
...
function createMiddleWare(testMode: boolean): Redux.Middleware<{}, IStore>[] {
  ...
  const logger = createLogger({
    ...
    stateTransformer: (state: any) => {
      ...
      if ('main' in rootState && typeof rootState.main === 'object') {
        // tslint:disable-next-line: no-any
        const main = (rootState.main = ({ ...rootState.main } as any) as Partial<
          IMainState
        >);
  ...
  const loggerMiddleware =
    process.env.VSC_PYTHON_FORCE_LOGGING !== undefined &&
 ...
 */
const addMessageDirectionMiddleware: Redux.Middleware = (_store) => (next) => (
