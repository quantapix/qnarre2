...
export function getJavaScriptMode(
  ...
  const workingFile =
    languageId === 'javascript' ? 'vscode://javascript/1.js' : 'vscode://javascript/2.ts'; // the same 'file' is used for all contents
  ...
  const host: ts.LanguageServiceHost = {
    ...
    getScriptKind: (fileName) =>
      fileName.substr(fileName.length - 2) === 'ts' ? ts.ScriptKind.TS : ts.ScriptKind.JS,
  ...
  return {
    ...
    doValidation(document: TextDocument, settings = workspace.settings): Diagnostic[] {
      ...
      host.getCompilationSettings()['experimentalDecorators'] =
        settings &&
        settings.javascript &&
        settings.javascript.implicitProjectConfig.experimentalDecorators;
    ...
    findDocumentSymbols(document: TextDocument): SymbolInformation[] {
      updateCurrentTextDocument(document);
      const items = jsLanguageService.getNavigationBarItems(workingFile);
      if (items) {
    ...
    findDefinition(document: TextDocument, position: Position): Definition | null {
      ...
      if (definition) {
        return definition
          .filter((d) => d.fileName === workingFile)
          .map((d) => {
            return {
              uri: document.uri,
    ...
    getSelectionRange(document: TextDocument, position: Position): SelectionRange {
      ...
      function convertSelectionRange(selectionRange: ts.SelectionRange): SelectionRange {
        ...
        return SelectionRange.create(
          convertRange(currentTextDocument, selectionRange.textSpan),
          parent
        );
      }
    ...
    getFoldingRanges(document: TextDocument): FoldingRange[] {
      updateCurrentTextDocument(document);
      const spans = jsLanguageService.getOutliningSpans(workingFile);
