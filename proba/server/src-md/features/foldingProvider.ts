...
export default class MarkdownFoldingProvider implements vscode.FoldingRangeProvider {
  ...
  private async getRegions(
    ...
    const regionMarkers = tokens
      .filter(isRegionMarker)
    ...
    return regionMarkers
      ...
      .filter(
        (region: vscode.FoldingRange | null): region is vscode.FoldingRange => !!region
      );
  }
  ...
  private async getBlockFoldingRanges(
    ...
    return multiLineListItems.map((listItem) => {
      ...
      return new vscode.FoldingRange(
        ...
        listItem.type === 'html_block' && listItem.content.startsWith('<!--')
