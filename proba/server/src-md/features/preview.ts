...
export class PreviewDocumentVersion {
  ...
  public equals(other: PreviewDocumentVersion): boolean {
    return (
      this.resource.fsPath === other.resource.fsPath && this.version === other.version
    );
  }
}
...
class MarkdownPreview extends Disposable implements WebviewResourceProvider {
    ...
    vscode.workspace
      ...
      .then(undefined, () => {
        vscode.window.showErrorMessage(
          localize(
            'preview.clickOpenFailed',
            'Could not open {0}',
            this._resource.toString()
          )
        );
      });
  }
  ...
  private async onDidClickPreviewLink(href: string) {
    ...
    vscode.commands.executeCommand('_markdown.openDocumentLink', {
      path: hrefPath,
      fragment,
  ...
  asWebviewUri(resource: vscode.Uri) {
    return this._webviewPanel.webview.asWebviewUri(
      normalizeResource(this._resource, resource)
    );
  }
...
export class StaticMarkdownPreview extends Disposable implements ManagedMarkdownPreview {
  public static revive(
    ...
    return new StaticMarkdownPreview(
      webview,
      resource,
      contentProvider,
...
export class DynamicMarkdownPreview extends Disposable implements ManagedMarkdownPreview {
  ...
  private constructor(
    ...
    this._preview = this.createPreview(
      input.resource,
      typeof input.line === 'number' ? new StartingScrollLine(input.line) : undefined
    );
  ...
  public toggleLock() {
    ...
    this._webviewPanel.title = DynamicMarkdownPreview.getPreviewTitle(
  ...
  private createPreview(
    ...
    return new MarkdownPreview(
      ...
      {
        getTitle: (resource) =>
          DynamicMarkdownPreview.getPreviewTitle(resource, this._locked),
        getAdditionalState: () => {
          return {
