...
export class MarkdownContentProvider {
  ...
  public async provideTextDocumentContent(
    ...
    return `<!DOCTYPE html>
          ...
          'pre.js'
			...
			<body class="vscode-body ${config.scrollBeyondLastLine ? 'scrollBeyondLastLine' : ''} ${
  ...
  private fixHref(
    ...
    return resourceProvider
      .asWebviewUri(vscode.Uri.file(path.join(path.dirname(resource.fsPath), href)))
      .toString();
  }
  ...
  private getSettingsOverrideStyles(config: MarkdownPreviewConfiguration): string {
    return [
      ...
      isNaN(config.lineHeight)
        ? ''
        : `--vscode-markdown-line-height: ${config.lineHeight};`,
    ].join(' ');
  }
