...
class VSCodeWorkspaceMarkdownDocumentProvider extends Disposable
  implements WorkspaceMarkdownDocumentProvider {
  private readonly _onDidChangeMarkdownDocumentEmitter = this._register(
    new vscode.EventEmitter<SkinnyTextDocument>()
  );
  private readonly _onDidCreateMarkdownDocumentEmitter = this._register(
    new vscode.EventEmitter<SkinnyTextDocument>()
  );
  private readonly _onDidDeleteMarkdownDocumentEmitter = this._register(
  ...
  private async getMarkdownDocument(
    resource: vscode.Uri
...
export default class MarkdownWorkspaceSymbolProvider extends Disposable
  ...
  public async provideWorkspaceSymbols(
    ...
    if (!this._symbolCachePopulated) {
      ...
      this._workspaceMarkdownDocumentProvider.onDidChangeMarkdownDocument(
        this.onDidChangeDocument,
        this,
        this._disposables
      );
      this._workspaceMarkdownDocumentProvider.onDidCreateMarkdownDocument(
        this.onDidChangeDocument,
        this,
        this._disposables
      );
      this._workspaceMarkdownDocumentProvider.onDidDeleteMarkdownDocument(
    ...
    const allSymbolsSets = await Promise.all(
      Array.from(this._symbolCache.values()).map((x) => x.value)
    );
