...
import { MarkdownFileExtensions, Schemes, isOfScheme } from './util/dispose';
...
export class MarkdownEngine {
    ...
    md.normalizeLink = (link: string) => {
      try {
        ...
        if (!/^[a-z\-]+:/i.test(link)) {
          ...
          if (
            uri.fragment &&
            (extname === '' || MarkdownFileExtensions.includes(extname))
          ) {
            uri = uri.with({
  ...
  private addNamedHeaders(md: any): void {
    ...
    md.renderer.rules.heading_open = (
      ...
      const title = tokens[idx + 1].children.reduce(
        (acc: string, t: any) => acc + t.content,
        ''
      );
