  ...
  return {
    onHtmlAttributeValue: ({ tag, attribute, value: valueBeforeCursor, range }) => {
      ...
      if (shouldDoPathCompletion(tag, attribute, fullValue)) {
        ...
        const paths = providePaths(
          valueBeforeCursor,
          URI.parse(document.uri).fsPath,
          workspaceRoot
        );
        result.push(
          ...paths.map((p) => pathToSuggestion(p, valueBeforeCursor, fullValue, range))
        );
      }
    },
  };
...
function providePaths(
  ...
  try {
    const paths = fs.readdirSync(parentDir).map((f) => {
      return isDir(path.resolve(parentDir, f)) ? f + '/' : f;
    });
