...
import { comparePositions, Position } from '../utils/text';
...
export class CompletionProvider {
  ...
  getCompletionsForPosition(): CompletionList | undefined {
    ...
    const lineTextRange = this._parseResults.tokenizerOutput.lines.getItemAt(
      this._position.line
    );
    const textOnLine = this._fileContents.substr(
      lineTextRange.start,
    ...
    while (true) {
      ...
      if (curNode.nodeType === ParseNodeType.Name) {
        ...
        if (curNode.parent && curNode.parent.nodeType === ParseNodeType.ModuleName) {
          ...
          curNode.parent &&
          curNode.parent.nodeType === ParseNodeType.ImportFromAs
        ) {
  ...
  resolveCompletionItem(completionItem: CompletionItem) {
    ...
    if (curIndex > 0) {
      ...
      CompletionProvider._mostRecentCompletions = CompletionProvider._mostRecentCompletions.splice(
        curIndex,
        1
      );
    }
  ...
  private _isWithinComment(offset: number, priorText: string): boolean {
    const tokenIndex = this._parseResults.tokenizerOutput.tokens.getItemAtPosition(
      offset
    );
    if (tokenIndex < 0) {
      return false;
  ...
  private _getMethodOverrideCompletions(
    ...
    symbolTable.forEach((symbol, name) => {
      ...
      if (decl && decl.type === DeclarationType.Function) {
        const isSimilar =
          StringUtils.computeCompletionSimilarity(partialName.value, name) >
          similarityLimit;
        if (isSimilar) {
          const range: Range = {
            start: {
              line: this._position.line,
              character: this._position.character - partialName.length,
            },
  ...
  private _printMethodSignature(node: FunctionNode): string {
    ...
    if (node.returnTypeAnnotation) {
      methodSignature +=
        ' -> ' + ParseTreeUtils.printExpression(node.returnTypeAnnotation);
    }
  ...
  private _getStringLiteralCompletions(
    ...
    if (
      !parentNode ||
      parentNode.nodeType !== ParseNodeType.StringList ||
      parentNode.strings.length > 1
    ) {
      return undefined;
    }
  ...
  private _getQuoteValueFromPriorText(priorText: string) {
    ...
    let quoteCharacter = this._parseResults.tokenizerOutput
  ...
  private _addStringLiteralToCompletionList(
    ...
    const isSimilar =
      StringUtils.computeCompletionSimilarity(priorString || '', value) > similarityLimit;
    if (isSimilar) {
  ...
  private _findMatchingKeywords(keywordList: string[], partialMatch: string): string[] {
    return keywordList.filter((keyword) => {
      if (partialMatch) {
        return (
          StringUtils.computeCompletionSimilarity(partialMatch, keyword) > similarityLimit
        );
      } else {
        return true;
      }
  ...
  private _addSymbol(
    ...
    if (primaryDecl) {
      ...
      primaryDecl = this._evaluator.resolveAliasDeclaration(
        primaryDecl,
  ...
  private _convertDeclarationTypeToItemKind(
    ...
    const resolvedDeclaration = this._evaluator.resolveAliasDeclaration(
      declaration,
      /* resolveLocalNames */ true
    );
