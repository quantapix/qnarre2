...
import { Position, Range } from '../utils/text';
...
export class HoverProvider {
  ...
  private static _addResultsForDeclaration(
    ...
    switch (resolvedDecl.type) {
      case DeclarationType.Intrinsic: {
        this._addResultsPart(
          parts,
          node.value + this._getTypeText(node, evaluator),
          true
        );
      ...
      case DeclarationType.Variable: {
        const label =
          resolvedDecl.isConstant || resolvedDecl.isFinal ? 'constant' : 'variable';
        ...
        this._addResultsPart(
          parts,
          `(${label}) ` + node.value + this._getTypeText(typeNode, evaluator),
          true
        );
        this._addDocumentationPart(parts, node, evaluator);
        break;
  ...
  private static _addDocumentationResultsPart(
    parts: HoverTextPart[],
    docString?: string
  ) {
    if (docString) {
