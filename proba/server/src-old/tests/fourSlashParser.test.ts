...
////    pass
  ...
  assert.equal(
    ...
    normalizeSlashes(combinePaths(factory.libFolder, 'file1.py'))
  );
});
...
////    [|pass|]
  ...
  assert.deepEqual(data.ranges, [
    { fileName: 'test.py', pos: 13, end: 17, marker: undefined },
  ]);
});
...
////    {| "data1":"1", "data2":"2" |}pass
  ...
  assert.deepEqual(data.markers, [
    { fileName: 'test.py', position: 13, data: { data1: '1', data2: '2' } },
  ]);
...
////    pass
  ...
  assert.equal(
    data.files.filter((f) => f.fileName === normalizeSlashes('./src/A.py'))[0].content,
    getContent('A')
  );
  assert.equal(
    data.files.filter(
      (f) => f.fileName === normalizeSlashes(combinePaths(factory.libFolder, 'src/B.py'))
    )[0].content,
    getContent('B')
  );
  assert.equal(
    data.files.filter((f) => f.fileName === normalizeSlashes('./src/C.py'))[0].content,
    getContent('C')
  );
});
...
////    pass
  ...
  assert.equal(
    data.files.filter((f) => f.fileName === normalizeSlashes('./src/B.py'))[0].content,
    getContent('B')
  );
  assert.equal(
    data.files.filter((f) => f.fileName === normalizeSlashes('./src/C.py'))[0].content,
...
////    pass
  ...
  for (const file of data.files) {
    assert.equal(
      fs.readFileSync(file.fileName, 'utf8'),
