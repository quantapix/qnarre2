...
async function updateConfig(
  ...
  for (const configKey of Object.keys(newConfig)) {
    ...
    await new Promise((resolve, reject) =>
      config
        .update(configKey, newConfig[configKey], vscode.ConfigurationTarget.Global)
        .then(() => resolve(), reject)
    );
  }
  return oldConfig;
...
suite('TypeScript References', () => {
  ...
  test('Should show on basic class', async () => {
    ...
    await createTestEditor(testDocumentUri, `class Foo {}`);
...
function getCodeLenses(
  document: vscode.Uri
): Thenable<readonly vscode.CodeLens[] | undefined> {
  return vscode.commands.executeCommand<readonly vscode.CodeLens[]>(
