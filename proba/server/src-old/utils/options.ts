...
export function cloneDiagnosticRuleSet(
  diagSettings: DiagnosticRuleSet
): DiagnosticRuleSet {
...
export class ConfigOptions {
  ...
  addExecEnvironmentForExtraPaths(
    fs: FileSystem,
    autoSearchPaths: boolean,
    extraPaths: string[]
  ) {
    ...
    if (autoSearchPaths) {
      ...
      if (
        ...
        !fs.existsSync(combinePaths(srcPath, '__init__.py'))
      ) {
        paths.push(srcPath);
      }
    }
  ...
  initializeFromJson(
      ...
      if (!Array.isArray(configObj.include)) {
        ...
        filesList.forEach((fileSpec, index) => {
          if (typeof fileSpec !== 'string') {
            ...
            console.log(
              `Ignoring path "${fileSpec}" in "include" array because it is not relative.`
            );
          } else {
      ...
      if (!Array.isArray(configObj.exclude)) {
        ...
        filesList.forEach((fileSpec, index) => {
          if (typeof fileSpec !== 'string') {
            ...
            console.log(
              `Ignoring path "${fileSpec}" in "exclude" array because it is not relative.`
            );
          } else {
      ...
      if (!Array.isArray(configObj.ignore)) {
        ...
        filesList.forEach((fileSpec, index) => {
          if (typeof fileSpec !== 'string') {
            ...
            console.log(
              `Ignoring path "${fileSpec}" in "ignore" array because it is not relative.`
            );
          } else {
      ...
      if (!Array.isArray(configObj.strict)) {
        ...
        filesList.forEach((fileSpec, index) => {
          if (typeof fileSpec !== 'string') {
            ...
            console.log(
              `Ignoring path "${fileSpec}" in "strict" array because it is not relative.`
            );
          } else {
    ...
    if (configObj.typeCheckingMode !== undefined) {
      if (
        ...
        console.log(
          `Config "typeCheckingMode" entry must contain "off", "basic", or "strict".`
        );
      }
    }

    const defaultSettings = ConfigOptions.getDiagnosticRuleSet(
      configTypeCheckingMode || typeCheckingMode
    );
    ...
    if (configObj.typingsPath !== undefined) {
      if (typeof configObj.typingsPath !== 'string') {
        ...
        this.typingsPath = normalizePath(
          combinePaths(this.projectRoot, configObj.typingsPath)
        );
      }
    }
  ...
  private _convertDiagnosticLevel(
    ...
    console.log(
      `Config "${fieldName}" entry must be true, false, "error", "warning" or "none".`
    );
    return defaultValue;
  }
    ...
    try {
          ...
          pathList.forEach((path, pathIndex) => {
            if (typeof path !== 'string') {
              ...
              newExecEnv.extraPaths.push(
                normalizePath(combinePaths(this.projectRoot, path))
              );
            }
          });
        }
      ...
      if (envObj.pythonVersion) {
        if (typeof envObj.pythonVersion === 'string') {
          ...
          console.log(
            `Config executionEnvironments index ${index} pythonVersion must be a string.`
          );
        }
      }
      ...
      if (envObj.pythonPlatform) {
        if (typeof envObj.pythonPlatform === 'string') {
          ...
          console.log(
            `Config executionEnvironments index ${index} pythonPlatform must be a string.`
          );
        }
      }
      ...
      if (envObj.venv) {
        if (typeof envObj.venv === 'string') {
          ...
          console.log(
            `Config executionEnvironments index ${index} venv must be a string.`
          );
        }
      }
