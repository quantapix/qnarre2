...
export abstract class BaseInstaller {
  ...
  protected getExecutableNameFromSettings(product: Product, resource?: Uri): string {
    ...
    const productPathService = this.serviceContainer.get<IProductPathService>(
      IProductPathService,
      productType
    );
  ...
  public async install(_product: Product, resource?: Uri): Promise<InstallerResponse> {
    if (this.serviceContainer.get<IPlatformService>(IPlatformService).isWindows) {
      ...
      terminalService
        ...
        .catch((ex) =>
          traceError(
            `Failed to install ctags. Script sent '${CTagsInsllationScript}', ${ex}`
          )
        );
    }
    return InstallerResponse.Ignore;
...
export class DataScienceInstaller extends BaseInstaller {
  ...
  public async install(
    ...
    if (!installerModule) {
      this.appShell
        .showErrorMessage(
          localize.DataScience.couldNotInstallLibrary().format(moduleName)
        )
        .then(noop, noop);
      return InstallerResponse.Ignore;
    }
  ...
  protected async promptToInstallImplementation(
    ...
    if (item === 'Yes') {
      ...
      try {
        ...
        const event =
          product === Product.jupyter
            ? Telemetry.UserInstalledJupyter
            : Telemetry.UserInstalledModule;
