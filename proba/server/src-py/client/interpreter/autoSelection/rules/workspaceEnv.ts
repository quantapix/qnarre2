...
export class WorkspaceVirtualEnvInterpretersAutoSelectionRule extends BaseRuleService {
  ...
  protected async onAutoSelectInterpreter(
    ...
    const pythonPathInConfig = this.experiments.inExperiment(
    ...
    const virtualEnvPromise = deferredFromPromise(
  ...
  protected async getWorkspaceVirtualEnvInterpreters(
    ...
    return interpreters.filter((interpreter) => {
      ...
      const fsPathToCompare =
        this.platform.osType === OSType.Windows ? fsPath.toUpperCase() : fsPath;
