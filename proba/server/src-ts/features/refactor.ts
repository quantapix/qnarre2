...
const Rewrite_Import = Object.freeze<CodeActionKind>({
  ...
  matches: (refactor) =>
    refactor.name.startsWith('Convert namespace import') ||
    refactor.name.startsWith('Convert named imports'),
});

const Rewrite_Export = Object.freeze<CodeActionKind>({
  kind: vscode.CodeActionKind.RefactorRewrite.append('export'),
  matches: (refactor) =>
    refactor.name.startsWith('Convert default export') ||
    refactor.name.startsWith('Convert named export'),
});
...
const Rewrite_Parameters_ToDestructured = Object.freeze<CodeActionKind>({
  ...
  matches: (refactor) =>
    refactor.name.startsWith('Convert parameters to destructured object'),
});
...
class TypeScriptRefactorProvider implements vscode.CodeActionProvider {
  ...
  constructor(
    ...
    const doRefactoringCommand = commandManager.register(
      new ApplyRefactoringCommand(this.client, telemetryReporter)
    );
  ...
  private static isPreferred(
    ...
    if (Extract_Constant.matches(action)) {
      ...
      return allActions
        .filter(
          (otherAtion) => otherAtion !== action && Extract_Constant.matches(otherAtion)
        )
        .every((otherAction) => {
  ...
  private appendInvalidActions(actions: vscode.CodeAction[]): vscode.CodeAction[] {
    if (
      !actions.some(
        (action) => action.kind && Extract_Constant.kind.contains(action.kind)
      )
    ) {
  ...
  return new VersionDependentRegistration(
    ...
    () => {
      return vscode.languages.registerCodeActionsProvider(
        selector,
        new TypeScriptRefactorProvider(
          client,
