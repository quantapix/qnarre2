...
class TsconfigLinkProvider implements vscode.DocumentLinkProvider {
      ...
      return new vscode.DocumentLink(
        ...
        vscode.Uri.file(
          join(
            ...
            extendsNode.value + (extendsNode.value.endsWith('.json') ? '' : '.json')
          )
        )
      );
    }

    const workspaceFolderPath = vscode.workspace.getWorkspaceFolder(document.uri)!.uri
      .fsPath;
    return new vscode.DocumentLink(
      this.getRange(document, extendsNode),
      vscode.Uri.file(
  ...
  private getReferencesLinks(document: vscode.TextDocument, root: jsonc.Node) {
    return mapChildren(jsonc.findNodeAtLocation(root, ['references']), (child) => {
      ...
      return new vscode.DocumentLink(
        this.getRange(document, pathNode),
  ...
  private pathNodeToLink(
    ...
    return this.isPathValue(node)
      ? new vscode.DocumentLink(
          ...
          this.getFileTarget(document, node)
        )
      : undefined;
  }
...
export function register() {
  const patterns: vscode.GlobPattern[] = ['**/[jt]sconfig.json', '**/[jt]sconfig.*.json'];
