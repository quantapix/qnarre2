...
import 'mocha';
import { RequestQueue, RequestQueueing } from '../tsServer/requestQueue';
...
suite('RequestQueue', () => {
  ...
  test('should queue normal requests in first in first out order', async () => {
    ...
    queue.enqueue({
      ...
      respond: true,
      isAsync: false,
      queueing: RequestQueueing.Normal,
    });
  ...
  test('should not push fence requests front of low priority requests', async () => {
    ...
    queue.enqueue({
      ...
      respond: true,
      isAsync: false,
      queueing: RequestQueueing.Fence,
    });
    queue.enqueue({
