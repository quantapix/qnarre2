...
import { PipeRequestCanceller, TsServerProcess, ProcessBasedTsServer } from '../server';
...
suite('Server', () => {
  ...
  test('should send requests with increasing sequence numbers', async () => {
    ...
    const server = new ProcessBasedTsServer(
      'semantic',
      process,
      undefined,
      new PipeRequestCanceller('semantic', undefined, tracer),
      undefined!,
