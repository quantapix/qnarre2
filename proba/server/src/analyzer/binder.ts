...
export class Binder extends ParseTreeWalker {
    ...
    this._createNewScope(ScopeType.Function, functionOrModuleScope, () => {
      ...
      this._deferBinding(() => {
        ...
        node.parameters.forEach((paramNode) => {
          if (paramNode.name) {
            const symbol = this._bindNameToScope(
              this._currentScope,
  ...
  visitRaise(node: RaiseNode): boolean {
    if (!node.typeExpression && this._nestedExceptDepth === 0) {
      this._addError(
        `"raise" requires one or more parameters when used outside of except clause`,
        node
      );
    }
  ...
  visitTry(node: TryNode): boolean {
    ...
    this._currentFinallyTarget = node.finallySuite
  ...
  visitStringList(node: StringListNode): boolean {
    for (const stringNode of node.strings) {
      if (stringNode.hasUnescapeErrors) {
        ...
        unescapedResult.unescapeErrors.forEach(
          (error: StringTokenUtils.UnescapeError) => {
            const start =
            ...
            if (
              error.errorType === StringTokenUtils.UnescapeErrorType.InvalidEscapeSequence
            ) {
              this._addDiagnostic(
  ...
  visitImportFrom(node: ImportFromNode): boolean {
    ...
    if (node.isWildcardImport) {
      if (
        ParseTreeUtils.getEnclosingClass(node) ||
        ParseTreeUtils.getEnclosingFunction(node)
      ) {
      ...
      node.imports.forEach((importSymbolNode) => {
        ...
        if (symbol) {
          ...
          if (importInfo && importInfo.implicitImports) {
            implicitImport = importInfo.implicitImports.find(
              (imp) => imp.name === importedName
            );
          }
  ...
  visitUnaryOperation(node: UnaryOperationNode): boolean {
    if (
      node.operator === OperatorType.Not &&
  ...
  private _getWildcardImportNames(lookupInfo: ImportLookupResult): string[] {
    ...
    if (allSymbol) {
      ...
      if (decls.length === 1 && decls[0].type === DeclarationType.Variable) {
        ...
        if (
          firstDecl.node.parent &&
          firstDecl.node.parent.nodeType === ParseNodeType.Assignment
        ) {
  ...
  private _isNarrowingExpression(
    expression: ExpressionNode,
    expressionList: NarrowingExpressionNode[]
  ): boolean {
    switch (expression.nodeType) {
      ...
      case ParseNodeType.BinaryOperation: {
        const isOrIsNotOperator =
          expression.operator === OperatorType.Is ||
        ...
        if (isOrIsNotOperator || equalsOrNotEqualsOperator) {
          ...
          if (
            ...
            expression.leftExpression.arguments[0].argumentCategory ===
  ...
  private _createAssignmentTargetFlowNodes(
  ...
  private _bindLoopStatement(
    preLoopLabel: FlowLabel,
    postLoopLabel: FlowLabel,
    callback: () => void
  ) {
  ...
  private _bindNameToScope(
    scope: Scope,
    name: string,
    addedSymbols?: Map<string, symbol>
  ) {
        ...
        if (memberAccessInfo) {
          ...
          if (!symbol) {
            ...
            const honorPrivateNaming =
              this._fileInfo.diagnosticRuleSet.reportPrivateUsage !== 'none';
