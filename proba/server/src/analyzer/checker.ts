...
export class Checker extends ParseTreeWalker {
  ...
  visitFunction(node: FunctionNode): boolean {
    ...
    if (functionTypeResult) {
      ...
      node.parameters.forEach((param, index) => {
        if (param.name) {
          const paramType =
            functionTypeResult.functionType.details.parameters[index].type;
          if (
            ...
            diagAddendum.addMessage(
              `Parameter type is "${this._evaluator.printType(paramType)}"`
            );
            this._evaluator.addDiagnostic(
              this._fileInfo.diagnosticRuleSet.reportUnknownParameterType,
              DiagnosticRule.reportUnknownParameterType,
              `Type of parameter "${param.name.value}" is partially unknown` +
    ...
    if (returnType) {
      if (returnType.category === TypeCategory.Unknown) {
        ...
        this._evaluator.addDiagnostic(
          this._fileInfo.diagnosticRuleSet.reportUnknownLambdaType,
          DiagnosticRule.reportUnknownLambdaType,
          `Return type of lambda, "${this._evaluator.printType(
            returnType
  ...
  visitCall(node: CallNode): boolean {
    ...
    if (
      ParseTreeUtils.isWithinDefaultParamInitializer(node) &&
        ...
        if (isNoReturnType(declaredReturnType)) {
          ...
          if (
            ...
            this._evaluator.addDiagnostic(
              this._fileInfo.diagnosticRuleSet.reportGeneralTypeIssues,
              DiagnosticRule.reportGeneralTypeIssues,
              `Expression of type "${this._evaluator.printType(
                returnType
              )}" cannot be assigned ` +
  ...
  visitYield(node: YieldNode) {
    const yieldType = node.expression
      ? this._evaluator.getType(node.expression)
      : NoneType.create();
    ...
    if (yieldType && iteratorType.category === TypeCategory.Class) {
      adjYieldType = ObjectType.create(
        ClassType.cloneForSpecialization(iteratorType, [yieldType])
      );
    } else {
  ...
  visitRaise(node: RaiseNode): boolean {
    const baseExceptionType = this._evaluator.getBuiltInType(
      node,
      'BaseException'
    ) as ClassType;
    ...
    if (node.typeExpression) {
      ...
      if (
        exceptionType &&
        baseExceptionType &&
        baseExceptionType.category === TypeCategory.Class
      ) {
        const diagAddendum = new DiagnosticAddendum();
  ...
  private _validateExceptionType(exceptionType: Type, errorNode: ParseNode) {
    ...
    const derivesFromBaseException = (classType: ClassType) => {
      ...
      return derivesFromClassRecursive(
        classType,
        baseExceptionType,
        /* ignoreUnknown */ false
      );
  ...
  private _reportMultipleFinalDeclarations(name: string, symbol: symbol) {
    ...
    if (!sawAssignment && !this._fileInfo.isStubFile) {
      const firstDecl = decls.find(
        (decl) => decl.type === DeclarationType.Variable && decl.isFinal
      );
      if (firstDecl) {
        this._evaluator.addError(
  ...
  private _reportIncompatibleDeclarations(name: string, symbol: symbol) {
    ...
    const addPrimaryDeclInfo = (diag?: Diagnostic) => {
      if (diag) {
        ...
        if (
          primaryDecl.type === DeclarationType.Function ||
        ...
        if (primaryDeclNode) {
          diag.addRelatedInfo(
            `See ${primaryDeclType}declaration`,
            primaryDecl.path,
            primaryDecl.range
          );
        }
  ...
  private _validateIsInstanceCallNecessary(node: CallNode) {
    ...
    if (
      classTypeList.some(
        (type) => ClassType.isProtocolClass(type) && !ClassType.isRuntimeCheckable(type)
      )
    ) {
      this._evaluator.addError(
    ...
    const filterType = (varType: ClassType): Type[] => {
      ...
      return filteredTypes.map((t) =>
        t.category === TypeCategory.Class ? ObjectType.create(t) : t
      );
    };
  ...
  private _conditionallyReportPrivateUsage(node: NameNode) {
    ...
    if (classOrModuleNode && classOrModuleNode.nodeType === ParseNodeType.Class) {
      if (isProtectedName) {
        ...
        if (
          declClassTypeInfo &&
          declClassTypeInfo.decoratedType.category === TypeCategory.Class
        ) {
          ...
          if (enclosingClassNode) {
            isProtectedAccess = true;
            const enclosingClassTypeInfo = this._evaluator.getTypeOfClass(
              enclosingClassNode
            );
    ...
    if (
      classOrModuleNode &&
      !ParseTreeUtils.isNodeContainedWithin(node, classOrModuleNode)
    ) {
      if (isProtectedAccess) {
        ...
        const scopeName =
          classOrModuleNode.nodeType === ParseNodeType.Class ? 'class' : 'module';
  ...
  private _validateFunctionReturn(node: FunctionNode, functionType: FunctionType) {
    ...
    if (node.returnTypeAnnotation) {
      ...
      const inferredReturnType = this._evaluator.getFunctionInferredReturnType(
        functionType
      );
      if (inferredReturnType.category === TypeCategory.Unknown) {
        ...
        this._evaluator.addDiagnostic(
          this._fileInfo.diagnosticRuleSet.reportUnknownParameterType,
          DiagnosticRule.reportUnknownParameterType,
          `Return type "${this._evaluator.printType(
            inferredReturnType
          )}" is partially unknown`,
          node.name
        );
    ...
    classType.details.fields.forEach((symbol, name) => {
      ...
      if (symbol.isClassMember() && !SymbolNameUtils.isDunderName(name)) {
        ...
        if (typeOfSymbol.category === TypeCategory.Function) {
          ...
          if (
            baseClassAndSymbol &&
            baseClassAndSymbol.classType.category === TypeCategory.Class
          ) {
  ...
  private _validateMethod(
    ...
    if (node.name && node.name.value === '__new__') {
      ...
      if (paramName !== 'cls') {
        if (
          !this._fileInfo.isStubFile ||
          (!paramName.startsWith('_') && paramName !== 'metacls')
        ) {
          this._evaluator.addDiagnostic(
            this._fileInfo.diagnosticRuleSet.reportSelfClsParameterName,
            DiagnosticRule.reportSelfClsParameterName,
  ...
  private _validateYieldType(node: YieldNode | YieldFromNode, adjustedYieldType: Type) {
    ...
    if (enclosingFunctionNode) {
      ...
      if (functionTypeResult) {
        ...
        const iteratorType = this._evaluator.getBuiltInType(node, 'Iterator');
        declaredYieldType = getDeclaredGeneratorYieldType(
