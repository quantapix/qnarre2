...
export class ImportResolver {
  ...
  private _resolveBestAbsoluteImport(
    ...
    if (allowPyi) {
      ...
      if (this._configOptions.typingsPath) {
        importFailureInfo.push(
          `Looking in typingsPath '${this._configOptions.typingsPath}'`
        );
  ...
  getModuleNameForImport(
    ...
    for (const extraPath of execEnv.extraPaths) {
      ...
      if (
        !moduleName ||
        (candidateModuleName && candidateModuleName.length < moduleName.length)
      ) {
    ...
    // Check for a typeshed file.
    const thirdPartyTypeshedPath = this._getTypeshedPath(
      false,
      execEnv,
      importFailureInfo
    );
  ...
  private _getModuleNameFromPath(
    ...
    if (filePathWithoutExtension.endsWith('__init__')) {
      filePathWithoutExtension = filePathWithoutExtension.substr(
        0,
        filePathWithoutExtension.length - 9
      );
    }
  ...
  private _findTypeshedPath(
    ...
    importFailureInfo.push(
      `Looking for typeshed ${isStdLib ? 'stdlib' : 'third_party'} path`
    );
    ...
    while (true) {
      const pythonVersionString =
        minorVersion > 0
          ? versionToString(0x300 + minorVersion)
          : minorVersion === 0
          ? '3'
          : '2and3';
  ...
  private _getTypeshedPath(
    ...
    if (
      !this.fileSystem.existsSync(typeshedPath) ||
      !isDirectory(this.fileSystem, typeshedPath)
    ) {
      return undefined;
    }
  ...
  protected resolveAbsoluteImport(
    ...
    if (moduleDescriptor.nameParts.length === 0) {
      ...
      if (
        ...
        this.fileSystem.existsSync(pyFilePath) &&
        isFile(this.fileSystem, pyFilePath)
      ) {
      ...
      for (let i = 0; i < moduleDescriptor.nameParts.length; i++) {
        ...
        if (allowPyi && allowStubsFolder) {
          ...
          foundDirectory =
            this.fileSystem.existsSync(stubsDirPath) &&
            isDirectory(this.fileSystem, stubsDirPath);
          if (foundDirectory) {
        ...
        if (!foundDirectory) {
          foundDirectory =
            this.fileSystem.existsSync(dirPath) && isDirectory(this.fileSystem, dirPath);
        }
        ...
        if (foundDirectory) {
          ...
          if (foundInit) {
            implicitImports = this._findImplicitImports(dirPath, [
              pyFilePath,
              pyiFilePath,
            ]);
            break;
  ...
  private _getCompletionSuggestionsAbsolute(
    ...
    if (nameParts.length === 0) {
      ...
      for (let i = 0; i < nameParts.length; i++) {
        ...
        if (
          !this.fileSystem.existsSync(dirPath) ||
          !isDirectory(this.fileSystem, dirPath)
        ) {
          break;
        }
      }
    ...
    entries.files.forEach((file) => {
      ...
      if (
        ...
        if (fileWithoutExtension !== '__init__') {
          if (
            !filter ||
            StringUtils.computeCompletionSimilarity(filter, fileWithoutExtension) >=
              similarityLimit
          ) {
  ...
  private _filterImplicitImports(
    importResult: ImportResult,
    importedSymbols: string[] | undefined
  ): ImportResult {
    ...
    const filteredImplicitImports = importResult.implicitImports.filter(
      (implicitImport) => {
