...
export function getTextEditsForAutoImportSymbolAddition(
  ...
  if (
    ...
    if (
      !importStatement.node.imports.some((importAs) => importAs.name.value === symbolName)
    ) {
      ...
      const insertionPosition = convertOffsetToPosition(
        insertionOffset,
...
export function getTextEditsForAutoImportInsertion(
  ...
  if (importStatements.orderedImports.length > 0) {
    ...
    for (const curImport of importStatements.orderedImports) {
      ...
      if (
        curImport ===
        importStatements.orderedImports[importStatements.orderedImports.length - 1]
      ) {
        if (importGroup > curImportGroup) {
          // Add an extra line to create a new group.
          newImportStatement =
    ...
    if (insertionImport) {
      if (insertBefore) {
        ...
        newImportStatement =
          parseResults.tokenizerOutput.predominantEndOfLineSequence + newImportStatement;
      }

      insertionPosition = convertOffsetToPosition(
        insertBefore
          ? insertionImport.node.start
          : TextRange.getEnd(insertionImport.node),
        parseResults.tokenizerOutput.lines
      );
    } else {
...
export function getContainingImportStatement(node: ParseNode | undefined) {
  while (node) {
    if (
      node.nodeType === ParseNodeType.Import ||
      node.nodeType === ParseNodeType.ImportFrom
    ) {
      break;
    }
