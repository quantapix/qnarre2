...
export function printExpression(
  node: ExpressionNode,
  flags = PrintExpressionFlags.None
): string {
  switch (node.nodeType) {
    ...
    case ParseNodeType.AssignmentExpression: {
      return (
        printExpression(node.name, flags) +
        ' := ' +
        printExpression(node.rightExpression, flags)
      );
    }
    ...
    case ParseNodeType.TypeAnnotation: {
      return (
        printExpression(node.valueExpression, flags) +
        ': ' +
...
export function getEnclosingClassOrModule(
  node: ParseNode,
  stopAtFunction = false
): ClassNode | ModuleNode | undefined {
  let curNode = node.parent;
  while (curNode) {
    if (curNode.nodeType === ParseNodeType.Class) {
      return curNode;
...
export function isNodeContainedWithin(
  node: ParseNode,
  potentialContainer: ParseNode
): boolean {
  let curNode: ParseNode | undefined = node;
  while (curNode) {
...
export function isMatchingExpression(
  ...
  if (
    expression1.nodeType === ParseNodeType.Name &&
    expression2.nodeType === ParseNodeType.Name
  ) {
...
export function getDocString(statements: StatementNode[]): string | undefined {
  ...
  if (
    statementList.statements.length === 0 ||
