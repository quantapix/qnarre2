...
export function getTypeShedFallbackPath(fs: FileSystem) {
  ...
  moduleDirectory = getDirectoryPath(
    ensureTrailingDirectorySeparator(normalizePath(moduleDirectory))
  );
  ...
  const debugTypeshedPath = combinePaths(
    getDirectoryPath(moduleDirectory),
...
export function findPythonSearchPaths(
  ...
  if (venvPath) {
    ...
    if (fs.existsSync(libPath)) {
      importFailureInfo.push(
        `Found path '${libPath}'; looking for ${pathConsts.sitePackages}`
      );
    } else {
    ...
    importFailureInfo.push(
      `Did not find '${pathConsts.sitePackages}'. Falling back on python interpreter.`
    );
  }

  // Fall back on the python interpreter.
  const pathResult = getPythonPathFromPythonInterpreter(
    fs,
  ...
  if (includeWatchPathsOnly && workspaceRoot) {
    const paths = pathResult.paths.filter(
      (p) =>
        !containsPath(workspaceRoot, p, true) || containsPath(pathResult.prefix, p, true)
    );
...
export function getPythonPathFromPythonInterpreter(
  ...
  try {
    ...
    if (interpreterPath) {
      ...
      execOutput = child_process.execFileSync(interpreterPath, commandLineArgs, {
        encoding: 'utf8',
      });
    } else {
      importFailureInfo.push(`Executing python interpreter`);
      execOutput = child_process.execFileSync('python', commandLineArgs, {
        encoding: 'utf8',
      });
    }
      ...
      for (let execSplitEntry of execSplit.path) {
        ...
        if (execSplitEntry) {
          ...
          if (fs.existsSync(normalizedPath) && isDirectory(fs, normalizedPath)) {
            ...
            importFailureInfo.push(
              `Skipping '${normalizedPath}' because it is not a valid directory`
            );
          }
        }
      }
