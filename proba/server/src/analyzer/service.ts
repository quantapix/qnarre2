...
import {
  ...
  getPythonPathFromPythonInterpreter,
} from './pythonPathUtils';
...
export class AnalyzerService {
  ...
  getReferencesForPosition(
    ...
    return this._program.getReferencesForPosition(
      filePath,
      position,
      includeDeclaration,
      token
  ...
  private _getConfigOptions(commandLineOptions: CommandLineOptions): ConfigOptions {
      ...
      if (commandLineOptions.executionRoot) {
        ...
        defaultExcludes.forEach((exclude) => {
          configOptions.exclude.push(
            getFileSpec(commandLineOptions.executionRoot, exclude)
          );
        });
      }
    }
    ...
    if (commandLineOptions.pythonPath) {
      this._console.log(
        `Setting pythonPath for service "${this._instanceName}": ` +
    ...
    if (configOptions.typingsPath === undefined) {
      configOptions.typingsPath = normalizePath(
        combinePaths(configOptions.projectRoot, 'typings')
      );
    }
    ...
    if (configOptions.venvPath) {
      if (
        !this._fs.existsSync(configOptions.venvPath) ||
        !isDirectory(this._fs, configOptions.venvPath)
      ) {
      ...
      configOptions.defaultVenv =
        configOptions.defaultVenv ?? this._configOptions.defaultVenv;
      if (configOptions.defaultVenv) {
        const fullVenvPath = combinePaths(
    ...
    if (
      configOptions.defaultVenv ||
      configOptions.executionEnvironments.find((e) => !!e.venv)
    ) {
      if (!configOptions.venvPath) {
    ...
    if (configOptions.typingsPath) {
      if (
        !this._fs.existsSync(configOptions.typingsPath) ||
        !isDirectory(this._fs, configOptions.typingsPath)
      ) {
        this._console.log(
  ...
  private _getTypeStubFolder() {
    ...
    if (!this._typeStubTargetPath || !this._typeStubTargetImportName) {
      const errMsg =
        `Import '${this._typeStubTargetImportName}'` + ` could not be resolved`;
  ...
  private _parseConfigFile(configPath: string): any | undefined {
    ...
    while (true) {
      ...
      if (parseAttemptCount++ >= 5) {
        this._console.log(
          `Config file "${configPath}" could not be parsed. Verify that JSON is correct.`
        );
        this._reportConfigParseError();
        return undefined;
      }
  ...
  private _getFileNamesFromFileSpecs(): string[] {
    ...
    timings.findFilesTime.timeCall(() => {
      const matchedFiles = this._matchFiles(
  ...
  private _matchFiles(include: FileSpec[], exclude: FileSpec[]): string[] {
    ...
    const visitDirectory = (absolutePath: string, includeRegExp: RegExp) => {
      if (this._configOptions.autoExcludeVenv) {
        if (
          envMarkers.some((f) => this._fs.existsSync(combinePaths(absolutePath, ...f)))
        ) {
      ...
      for (const file of files) {
        ...
        if (includeRegExp.test(filePath)) {
          if (
            !this._isInExcludePath(filePath, exclude) &&
            includeFileRegex.test(filePath)
          ) {
    ...
    include.forEach((includeSpec) => {
      ...
      if (!foundFileSpec) {
        this._console.log(
          `File or directory "${includeSpec.wildcardRoot}" does not exist.`
        );
      }
    });
  ...
  private _updateLibraryFileWatcher() {
    ...
    if (watchList && watchList.length > 0) {
      try {
        ...
        this._libraryFileWatcher = this._fs.createFileSystemWatcher(
          watchList,
          (event, path) => {
            if (this._verboseOutput) {
              this._console.log(`Received fs event '${event}' for path '${path}'`);
  ...
  private _reloadConfigFile() {
    if (this._configFilePath) {
      ...
      if (configJsonObj) {
        this._backgroundAnalysisProgram.initializeFromJson(
          configJsonObj,
          this._typeCheckingMode
        );
      }
