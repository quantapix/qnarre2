...
import {
  ReferencesProvider,
  ReferencesResult,
} from '../languageService/referencesProvider';
import {
  SignatureHelpProvider,
  SignatureHelpResults,
} from '../languageService/signatureHelpProvider';
...
export class SourceFile {
  ...
  getDiagnostics(
    ...
    if (
      options.diagnosticRuleSet.reportImportCycles !== 'none' &&
      this._circularDependencies.length > 0
    ) {
      const category =
    ...
    if (
      options.ignore.find((ignoreFileSpec) => ignoreFileSpec.regExp.test(this._filePath))
    ) {
      diagList = [];
    }
  ...
  parse(configOptions: ConfigOptions, importResolver: ImportResolver): boolean {
    ...
    try {
      ...
      // Resolve imports.
      timings.resolveImportsTime.timeCall(() => {
        [
          this._imports,
      ...
      const useStrict =
        configOptions.strict.find((strictFileSpec) =>
          strictFileSpec.regExp.test(this._filePath)
        ) !== undefined;
  ...
  bind(
    ...
    try {
      ...
      const message: string =
        ...
        JSON.stringify(e);
      this._console.log(
        `An internal error occurred while performing name binding for ${this.getFilePath()}: ` +
          message
      );

      const diagSink = new DiagnosticSink();
      diagSink.addError(
        `An internal error occurred while performing name binding`,
        getEmptyRange()
      );
  ...
  check(evaluator: TypeEvaluator) {
    ...
    try {
      timings.typeCheckerTime.timeCall(() => {
      ...
      if (!isCancellation) {
        ...
        const diagSink = new DiagnosticSink();
        diagSink.addError(
          `An internal error occurred while performing type checking`,
          getEmptyRange()
        );
    ...
    for (const moduleImport of moduleImports) {
      ...
      if (importResult.isImportFound && importResult.isTypeshedFile) {
        if (
          ...
          moduleImport.nameParts[0] === 'collections'
        ) {
