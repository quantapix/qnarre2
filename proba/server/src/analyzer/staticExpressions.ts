...
export function evaluateStaticBoolExpression(
  ...
  if (node.nodeType === ParseNodeType.UnaryOperation) {
    ...
    if (
      _isSysVersionInfoExpression(node.leftExpression) &&
      ...
      const comparisonPlatform = node.rightExpression.strings
        .map((s) => s.value)
        .join('');
...
function _getExpectedPlatformNameFromPlatform(
  execEnv: ExecutionEnvironment
): string | undefined {
...
function _getExpectedOsNameFromPlatform(
  execEnv: ExecutionEnvironment
): string | undefined {
  if (execEnv.pythonPlatform === 'Darwin') {
    return 'posix';
