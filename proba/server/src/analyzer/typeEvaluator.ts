...
export function createTypeEvaluator(importLookup: ImportLookup): TypeEvaluator {
  ...
  function setSymbolResolutionPartialType(
    symbol: symbol,
    declaration: Declaration,
    type: Type
  ) {
  ...
  function getSymbolResolutionPartialType(
    symbol: symbol,
    declaration: Declaration
  ): Type | undefined {
    const index = getIndexOfSymbolResolution(symbol, declaration);
  ...
  function getTypeOfExpression(
    ...
    switch (node.nodeType) {
      ...
      case ParseNodeType.StringList: {
        const expectingType =
          (flags & EvaluatorFlags.EvaluateStringLiteralAsType) !== 0 &&
      ...
      case ParseNodeType.Number: {
        ...
        typeResult = {
          node,
          type: cloneBuiltinTypeWithLiteral(node, builtInType, node.value),
        };
        break;
      }
      ...
      case ParseNodeType.Unpack: {
        ...
        const type = getTypeFromIterable(
          iterType,
          /* isAsync */ false,
          node,
  ...
  function getTypeOfAnnotation(node: ExpressionNode, allowFinal = true): Type {
    ...
    return convertClassToObject(
      getTypeOfExpression(node, undefined, evaluatorFlags).type
    );
  }
  ...
  function getCallSignatureInfo(
    ...
    while (curNode !== undefined) {
      if (
        curNode.nodeType === ParseNodeType.Call ||
        curNode.nodeType === ParseNodeType.Decorator
      ) {
        callNode = curNode;
        break;
      }
    ...
    if (symbol) {
      ...
      if (declaredType) {
        ...
        if (isProperty(declaredType)) {
          const setterInfo = lookUpClassMember(
            (declaredType as ObjectType).classType,
            'fset'
          );
  ...
  function getTypeFromIterable(
    ...
    if (
      ...
      type.subtypes.some((t) => isNoneOrNever(t))
    ) {
      if (errorNode) {
        addDiagnostic(
    ...
    const getIteratorReturnType = (
      ...
      const iterReturnType = metaclass
        ? getSpecializedReturnTypeForMetaclassMethod(
            metaclass,
            objType.classType,
            iterMethodName
          )
      ...
      if (!iterReturnType) {
        ...
        if (iterReturnType.category === TypeCategory.Object) {
          ...
          if (!nextReturnType) {
            diag.addMessage(
              `"${nextMethodName}" method not defined on type "${printType(
                iterReturnType
              )}"`
            );
          } else {
  ...
  function synthesizeDataClassMethods(
    ...
    if (dictType.category === TypeCategory.Class) {
      dictType = ObjectType.create(
        ClassType.cloneForSpecialization(dictType, [
          getBuiltInObject(node, 'str'),
          AnyType.create(),
        ])
      );
    }
    symbolTable.set(
      ...
      Symbol.createWithType(SymbolFlags.ClassMember, dictType)
    );
  }
    ...
    if (strClass.category === TypeCategory.Class) {
      ...
      if (getOverloads.length > 0) {
        ...
        if (mappingClass.category === TypeCategory.Class) {
          ...
          if (
            overriddenGet &&
            overriddenGet.type.category === TypeCategory.OverloadedFunction
          ) {
            getOverloads.push(
              overriddenGet.type.overloads[overriddenGet.type.overloads.length - 1]
            );
          }
        }
  ...
  function isAnnotationLiteralValue(node: StringListNode): boolean {
    if (node.parent && node.parent.nodeType === ParseNodeType.IndexItems) {
      ...
      if (
        indexItemsNode.parent &&
        indexItemsNode.parent.nodeType === ParseNodeType.Index
      ) {
    ...
    if (declaredType && srcExpression) {
      ...
      if (!canAssignType(declaredType, type, diagAddendum)) {
        addDiagnostic(
          ...
          `Expression of type "${printType(
            type
          )}" cannot be assigned to declared type "${printType(declaredType)}"` +
      ...
      if (scope.type === ScopeType.Class) {
        ...
        if (
          !isConstant &&
          (!isPrivate ||
            getFileInfo(nameNode).diagnosticRuleSet.reportPrivateUsage === 'none')
        ) {
  ...
  function assignTypeToMemberAccessNode(
    ...
    if (target.leftExpression.nodeType === ParseNodeType.Name) {
      ...
      if (enclosingClassNode) {
        ...
        if (
          classTypeResults &&
          classTypeResults.classType.category === TypeCategory.Class
        ) {
          if (baseTypeResult.type.category === TypeCategory.Object) {
            if (
              ClassType.isSameGenericClass(
                baseTypeResult.type.classType,
  ...
  function assignTypeToTupleNode(
    target: TupleNode,
    type: Type,
    srcExpr?: ExpressionNode
  ) {
    ...
    const unpackIndex = target.expressions.findIndex(
      (expr) => expr.nodeType === ParseNodeType.Unpack
    );
        ...
        if (sourceEntryCount === 2 && isEllipsisType(sourceEntryTypes[1])) {
          ...
          for (targetIndex = 0; targetIndex < target.expressions.length; targetIndex++) {
            if (targetIndex === unpackIndex) {
              ...
              let entriesToPack = Math.max(
                remainingSourceEntries - remainingTargetEntries,
                0
              );
    ...
    target.expressions.forEach((expr, index) => {
      ...
      let targetType =
        typeList.length === 0 ? UnknownType.create() : combineTypes(typeList);
      ...
      if (index === unpackIndex) {
        ...
        if (iterableType.category === TypeCategory.Class) {
          targetType = ObjectType.create(
            ClassType.cloneForSpecialization(iterableType, [targetType])
          );
        }
      }
  ...
  function getTypeFromMemberAccess(
    node: MemberAccessNode,
    ...
    if (isCodeFlowSupportedForReference(node)) {
      ...
      const codeFlowType = getFlowTypeOfReference(
        node,
  ...
  function getTypeFromMemberAccessWithBaseType(
    ...
    if (!type) {
      ...
      addDiagnostic(
        fileInfo.diagnosticRuleSet.reportGeneralTypeIssues,
        DiagnosticRule.reportGeneralTypeIssues,
        `Cannot ${operationName} member "${memberName}" for type "${printType(
          baseType
        )}"` + diag.getString(),
        node.memberName
      );
  ...
  function getClassFromPotentialTypeObject(
    potentialTypeObject: ObjectType
  ): Type | undefined {
    ...
    if (memberInfo) {
      ...
      if (usage.method === 'set') {
        ...
        if (memberInfo.symbol.hasTypedDeclarations()) {
          ...
          if (
            !memberInfo.symbol.getDeclarations().some((decl) => decl.node === errorNode)
          ) {
            enforceTargetType = true;
          }
        }
  ...
  function getTypeFromIndexWithBaseType(
    ...
    if (
      ...
      if (isUnionOfClasses && typeParameters.length > 0) {
        const typeArgs = getTypeArgs(node.items, flags).map((t) =>
          convertClassToObject(t.type)
        );
        ...
        if (diag.getMessageCount() > 0) {
          addError(
            `Could not specialize type "${printType(baseType)}"` + diag.getString(),
            node.items
          );
        }
  ...
  function getTypeFromIndexedObject(
    node: IndexNode,
    baseType: ObjectType,
    usage: EvaluatorUsage
  ): Type {
    ...
    if (node.nodeType === ParseNodeType.List) {
      typeResult = {
        ...
        typeList: node.entries.map((entry) =>
          getTypeOfExpression(entry, undefined, adjustedFlags)
        ),
        node,
      };
    } else {
  ...
  function getTypeFromTuple(node: TupleNode, expectedType?: Type): TypeResult {
    ...
    if (expectedType && expectedType.category === TypeCategory.Object) {
      ...
      if (ClassType.isBuiltIn(tupleClass, 'Tuple') && tupleClass.typeArguments) {
        ...
        if (
          tupleClass.typeArguments.length === 2 &&
          isEllipsisType(tupleClass.typeArguments[1])
        ) {
      ...
      for (const typeResult of entryTypeResults) {
        if (typeResult.unpackedType) {
          ...
          if (
            ...
            if (
              !typeArgs ||
              typeArgs.some((t) => t.category === TypeCategory.Any && t.isEllipsis)
            ) {
  ...
  function getTypeFromCall(
    ...
    if (
      ...
      node.leftExpression.value === 'super'
    ) {
      return {
    ...
    if (
      ...
      const exprString = ParseTreeUtils.printExpression(
  ...
  function getTypeFromSuperCall(node: CallNode): Type {
    ...
    if (node.arguments.length > 0) {
      ...
      if (
        !isAnyOrUnknown(targetClassType) &&
        !(targetClassType.category === TypeCategory.Class)
      ) {
        addError(
  ...
  function getTypeFromCallWithBaseType(
    ...
    switch (callType.category) {
      ...
      case TypeCategory.Function: {
        ...
        if (callType.details.builtInName === 'namedtuple') {
          ...
          type = callResult.argumentErrors
    ...
    if (!type) {
      ...
      addDiagnostic(
        fileInfo.diagnosticRuleSet.reportGeneralTypeIssues,
        DiagnosticRule.reportGeneralTypeIssues,
        `"${ParseTreeUtils.printExpression(errorNode)}" has type "${printType(
          callType
        )}" and is not callable`,
        errorNode
      );
        ...
        if (callResult.argumentErrors) {
          ...
          if (newReturnType) {
            if (
              ...
              if (
                (!containsUnknown(newReturnType) &&
                  !requiresSpecialization(newReturnType)) ||
  ...
  function validateFunctionArguments(
    ...
    typeParams.forEach((param) => {
      if (param.name) {
        paramMap.set(param.name, {
          argsNeeded:
            param.category === ParameterCategory.Simple && !param.hasDefault ? 1 : 0,
          argsReceived: 0,
        });
      }
    });
    ...
    if (!reportedArgError) {
      ...
      const foundUnpackedListArg =
        argList.find((arg) => arg.argumentCategory === ArgumentCategory.UnpackedList) !==
        undefined;
    ...
    if (
      ['cast', 'isinstance', 'issubclass'].some(
        (name) => name === type.details.builtInName
      )
    ) {
      skipUnknownArgCheck = true;
    }
    ...
    const typeVarMatchingCount = validateArgTypeParams.filter(
    ...
    if (!isSpeculativeMode(undefined)) {
      argList.forEach((arg) => {
        if (arg.valueExpression) {
          if (
            !validateArgTypeParams.some((validatedArg) => validatedArg.argument === arg)
          ) {
            getTypeOfExpression(arg.valueExpression);
          }
        }
  ...
  function validateArgType(
    ...
    if (argParam.argument.valueExpression) {
      ...
      if (
        ...
        !isSpeculativeMode(argParam.errorNode)
      ) {
    ...
    if (!canAssignType(argParam.paramType, argType, diag.createAddendum(), typeVarMap)) {
      ...
      if (simplifiedType.category === TypeCategory.Unknown) {
        ...
        if (
          !containsUnknown(argParam.paramType) &&
          simplifiedType.category !== TypeCategory.Class
        ) {
  ...
  function createTypeVarType(
    ...
    if (
      ...
      firstArg.valueExpression.nodeType === ParseNodeType.StringList
    ) {
      typeVarName = firstArg.valueExpression.strings.map((s) => s.value).join('');
    } else {
      addError(
        'Expected name of TypeVar as first parameter',
      ...
      if (paramName) {
        ...
        if (paramName === 'bound') {
          ...
          if (
            ...
            getBooleanValue(argList[i].valueExpression!)
          ) {
  ...
  function createEnumType(
    ...
    classFields.set(
      ...
      Symbol.createWithType(
        SymbolFlags.ClassMember | SymbolFlags.IgnoredForProtocolMatch,
        classType
      )
    );
  ...
  function createType(
    ...
    if (
      ...
      !ClassType.isBuiltIn(arg0Type.classType, 'str')
    ) {
      addError(
        'Expected name of type as first argument',
        argList[0].valueExpression || errorNode
      );
      return undefined;
    }
    ...
    if (
      ...
      addError(
        'Expected tuple of base class types as second argument',
  ...
  function createTypedDictType(
    ...
    if (argList.length > 3) {
      addError(
        'Extra TypedDict arguments not supported',
        argList[3].valueExpression || errorNode
      );
    }
  ...
  function createNamedTupleType(
    ...
    const classType = ClassType.create(className, ClassTypeFlags.None, errorNode.id);
    const builtInNamedTuple =
      getTypingType(errorNode, 'NamedTuple') || UnknownType.create();
    ...
    if (builtInTupleType.category === TypeCategory.Class) {
          ...
          if (
            ...
            entryList.entries.forEach((entry, index) => {
              ...
              if (includesTypes) {
                ...
                if (
                  entry.nodeType === ParseNodeType.Tuple &&
                  entry.expressions.length === 2
                ) {
                  ...
                  addError(
                    'Expected two-entry tuple specifying entry name and type',
                    entry
                  );
                }
              } else {
              ...
              if (entryNameNode && entryNameNode.nodeType === ParseNodeType.StringList) {
                ...
                addError(
                  'Expected string literal for entry name',
                  entryNameNode || entry
                );
              }
              ...
              if (entryMap.has(entryName)) {
                addError(
                  'Names within a named tuple must be unique',
                  entryNameNode || entry
                );
              }
      ...
      const keysItemType = FunctionType.create(
        'keys',
        FunctionTypeFlags.SynthesizedMethod
      );
      const itemsItemType = FunctionType.create(
        'items',
        FunctionTypeFlags.SynthesizedMethod
      );
      ...
      classFields.set(
        'items',
        Symbol.createWithType(SymbolFlags.InstanceMember, itemsItemType)
      );
    ...
    if (node.operator !== OperatorType.Not) {
      if (isOptionalType(exprType)) {
        addDiagnostic(
          ...
          `Operator "${ParseTreeUtils.printOperator(
            node.operator
          )}" not supported for "None" type`,
          node.expression
        );
    ...
    if (node.operator === OperatorType.BitwiseOr) {
      if (canUnionType(leftType) && canUnionType(rightType)) {
        ...
        const unionNotationSupported =
          fileInfo.isStubFile ||
          fileInfo.executionEnvironment.pythonVersion >= PythonVersion.V39;
        if (!unionNotationSupported) {
          addError(
            'Alternative syntax for unions requires Python 3.9 or newer',
            node,
            node.operatorToken
          );
        }
  ...
  function validateBinaryOperation(
    ...
    if (!type || type.category === TypeCategory.Never) {
      ...
      addDiagnostic(
        ...
        `Operator "${ParseTreeUtils.printOperator(
          operator
        )}" not supported for types "${printType(leftType)}" and "${printType(
          rightType
        )}"`,
        errorNode
      );
        ...
        if (subtype.category === TypeCategory.Object) {
          ...
          if (ClassType.isBuiltIn(classAlias, 'set') && subtype.classType.typeArguments) {
            ...
            for (const entryType of entryTypes) {
              if (
                !canAssignType(typeArg, entryType, new DiagnosticAddendum(), typeVarMap)
              ) {
                return undefined;
              }
            }
  ...
  function getTypeFromDictionary(node: DictionaryNode, expectedType?: Type): TypeResult {
    ...
    node.entries.forEach((entryNode) => {
      ...
      if (entryNode.nodeType === ParseNodeType.DictionaryKeyEntry) {
        const keyType = getTypeOfExpression(entryNode.keyExpression, expectedKeyType)
          .type;
        ...
        if (
          ...
          valueType = getTypeOfExpression(entryNode.valueExpression, expectedValueType)
            .type;
        }
      ...
      const narrowedExpectedType = doForSubtypes(expectedType, (subtype) => {
        ...
        if (ClassType.isBuiltIn(classAlias, 'dict') && subtype.classType.typeArguments) {
          ...
          for (const valueType of valueTypes) {
            if (
              !canAssignType(typeArg1, valueType, new DiagnosticAddendum(), typeVarMap)
            ) {
              return undefined;
            }
          }
  ...
  function getTypeFromList(node: ListNode, expectedType?: Type): TypeResult {
    ...
    const getListTypeArg = (potentialList: Type) => {
      ...
      const classAlias =
        potentialList.classType.details.aliasClass || potentialList.classType;
      if (
        !ClassType.isBuiltIn(classAlias, 'list') ||
        !potentialList.classType.typeArguments
      ) {
        return undefined;
      }
  ...
  function getTypeFromLambda(node: LambdaNode, expectedType?: Type): TypeResult {
    ...
    node.parameters.forEach((param, index) => {
      ...
      if (
        ...
        index < expectedFunctionType.details.parameters.length
      ) {
  ...
  function getTypeFromListComprehension(node: ListComprehensionNode): TypeResult {
    ...
    if (builtInIteratorType && builtInIteratorType.category === TypeCategory.Class) {
      type = ObjectType.create(
        ClassType.cloneForSpecialization(builtInIteratorType, [elementType])
      );
    }
  ...
  function cloneBuiltinTypeWithLiteral(
    node: ParseNode,
    builtInName: string,
    value: LiteralValue
  ): Type {
  ...
  function createLiteralType(node: IndexNode): Type {
    if (node.items.items.length === 0) {
      addError(
        `Expected one or more type arguments after "Literal"`,
        node.baseExpression
      );
      return UnknownType.create();
    }
  ...
  function createSpecialBuiltInClass(
    node: ParseNode,
    assignedName: string,
      ...
      if (!arg.name || arg.name.value === 'metaclass') {
        ...
        if (!isAnyOrUnknown(argType) && argType.category !== TypeCategory.Unbound) {
          ...
          if (argType.category !== TypeCategory.Class) {
            ...
            if (
              ClassType.isBuiltIn(argType, 'TypedDict') ||
              ClassType.isTypedDictClass(argType)
            ) {
        ...
        if (isMetaclass) {
          if (
            argType.category === TypeCategory.Class ||
            argType.category === TypeCategory.Unknown
          ) {
        ...
        if (
          argType.category === TypeCategory.Class &&
          ClassType.isBuiltIn(argType, 'Generic')
        ) {
          if (genericTypeParameters) {
            addError(
              `Class cannot derive from "Generic" more than once`,
              arg.valueExpression
            );
          } else {
        ...
        if (ClassType.isTypedDictClass(classType)) {
          ...
          if (constArgValue === undefined) {
            addError(
              'Value for total parameter must be "True" or "False"',
              arg.valueExpression
            );
    ...
    if (!fileInfo.isStubFile && classType.details.typeParameters.length === 0) {
      ...
      if (initMethod) {
        ...
        if (initDecls.length === 1 && initDecls[0].type === DeclarationType.Function) {
          ...
          if (
            initParams.length > 1 &&
            !initParams.some((param) => param.typeAnnotation)
          ) {
            const genericParams = initParams.filter(
              (param, index) =>
                index > 0 && param.name && param.category === ParameterCategory.Simple
            );
  ...
  function getTypeOfFunction(node: FunctionNode): FunctionTypeResult | undefined {
    ...
    node.parameters.forEach((param, index) => {
      ...
      if (param.name) {
        const variadicParamType = transformVariadicParamType(
          node,
          param.category,
          functionParam.type
        );
    ...
    const preDecoratedType = node.isAsync
      ? createAwaitableFunction(node, functionType)
      : functionType;
  ...
  function inferFirstParamType(
    ...
    if ((flags & FunctionTypeFlags.StaticMethod) === 0) {
      if (containingClassType) {
        ...
        if (
          flags &
          (FunctionTypeFlags.ClassMethod | FunctionTypeFlags.ConstructorMethod)
        ) {
  ...
  function transformVariadicParamType(
    node: ParseNode,
    ...
    switch (paramCategory) {
      ...
      case ParameterCategory.VarArgList: {
        ...
        if (tupleType && tupleType.category === TypeCategory.Class) {
          return ObjectType.create(
            ClassType.cloneForSpecialization(tupleType, [type, AnyType.create(true)])
          );
        }
      ...
      case ParameterCategory.VarArgDictionary: {
        ...
        if (
          dictType.category === TypeCategory.Class &&
          strType.category === TypeCategory.Object
        ) {
          return ObjectType.create(
            ClassType.cloneForSpecialization(dictType, [strType, type])
          );
        }
  ...
  function applyFunctionDecorator(
    ...
    if (
      decoratorType.category === TypeCategory.Class &&
    ...
    if (decoratorType.category === TypeCategory.Function) {
      ...
      if (decoratorNode.leftExpression.nodeType === ParseNodeType.MemberAccess) {
        const baseType = getTypeOfExpression(decoratorNode.leftExpression.leftExpression)
          .type;
    ...
    if (
      inputFunctionType.category === TypeCategory.Function &&
      returnType.category === TypeCategory.Function
    ) {
  ...
  function createProperty(
    ...
    const getFunction = FunctionType.create(
      '__get__',
      FunctionTypeFlags.SynthesizedMethod
    );
    getFunction.details.parameters.push({
      ...
      name: 'self',
      type:
        fget.details.parameters.length > 0
          ? fget.details.parameters[0].type
          : AnyType.create(),
    });
    getFunction.details.parameters.push({
      category: ParameterCategory.Simple,
    ...
    ['getter', 'setter', 'deleter'].forEach((accessorName) => {
      const accessorFunction = FunctionType.create(
        accessorName,
        FunctionTypeFlags.SynthesizedMethod
      );
      ...
      const accessorSymbol = Symbol.createWithType(
  ...
  function clonePropertyWithSetter(prop: Type, fset: FunctionType): Type {
    ...
    const setFunction = FunctionType.create(
      '__set__',
      FunctionTypeFlags.SynthesizedMethod
    );
    setFunction.details.parameters.push({
      ...
      name: 'self',
      type:
        fset.details.parameters.length > 0
          ? fset.details.parameters[0].type
          : AnyType.create(),
    });
    setFunction.details.parameters.push({
      category: ParameterCategory.Simple,
  ...
  function clonePropertyWithDeleter(prop: Type, fdel: FunctionType): Type {
    ...
    const delFunction = FunctionType.create(
      '__delete__',
      FunctionTypeFlags.SynthesizedMethod
    );
    delFunction.details.parameters.push({
      ...
      name: 'self',
      type:
        fdel.details.parameters.length > 0
          ? fdel.details.parameters[0].type
          : AnyType.create(),
    });
    delFunction.details.parameters.push({
      category: ParameterCategory.Simple,
      ...
      if (declIndex > 0) {
        ...
        while (declIndex > 0) {
          ...
          if (declTypeInfo.decoratedType.category === TypeCategory.Function) {
            ...
            declTypeInfo.decoratedType.category === TypeCategory.OverloadedFunction
          ) {
            // If the previous declaration was itself an overloaded function,
            // copy the last entry out of it.
            const lastOverload =
              declTypeInfo.decoratedType.overloads[
                declTypeInfo.decoratedType.overloads.length - 1
              ];
  ...
  function createAwaitableFunction(
    node: FunctionNode,
    functionType: FunctionType
  ): FunctionType {
              ...
              functionDecl.returnExpressions.forEach((returnNode) => {
                if (isNodeReachable(returnNode)) {
                  if (returnNode.returnExpression) {
                    const returnType = getTypeOfExpression(returnNode.returnExpression)
                      .type;
  ...
  function evaluateTypesForWithStatement(node: WithItemNode): void {
    ...
    let exprType = getTypeOfExpression(node.expression).type;
    const isAsync =
      node.parent && node.parent.nodeType === ParseNodeType.With && !!node.parent.isAsync;
  ...
  function evaluateTypesForImportAs(node: ImportAsNode): void {
    ...
    let symbolType =
      getAliasedSymbolTypeForName(node, symbolNameNode.value) || UnknownType.create();
    ...
    if (
      cachedModuleType &&
      cachedModuleType.category === TypeCategory.Module &&
      symbolType
    ) {
  ...
  function getAliasedSymbolTypeForName(node: ParseNode, name: string): Type | undefined {
    ...
    const aliasDecl = symbolWithScope.symbol
      .getDeclarations()
      .find((decl) => decl.type === DeclarationType.Alias);
    if (!aliasDecl) {
      return undefined;
    }
  ...
  function isCallNoReturn(node: CallNode) {
    ...
    if (callType) {
      ...
      if (callType.category === TypeCategory.Function) {
        ...
        functionType =
          overloadedFunction.overloads[overloadedFunction.overloads.length - 1];
      }
  ...
  function createCodeFlowAnalyzer(): CodeFlowAnalyzer {
    ...
    function getTypeFromCodeFlow(
      ...
      function evaluateAssignmentFlowNode(flowNode: FlowAssignment): Type | undefined {
        ...
        if (parentNode) {
          if (
            parentNode.nodeType === ParseNodeType.Function ||
    ...
    function isFlowNodeReachableRecursive(flowNode: FlowNode): boolean {
      ...
      while (true) {
        ...
        if (curFlowNode.flags & FlowFlags.PostFinally) {
          ...
          const wasGateClosed = postFinallyFlowNode.preFinallyGate.isGateClosed;
          postFinallyFlowNode.preFinallyGate.isGateClosed = true;
          const isReachable = isFlowNodeReachableRecursive(
            postFinallyFlowNode.antecedent
          );
          postFinallyFlowNode.preFinallyGate.isGateClosed = wasGateClosed;
          return isReachable;
  ...
  function getTypeNarrowingCallback(
    ...
    if (testExpression.nodeType === ParseNodeType.AssignmentExpression) {
      if (
        ParseTreeUtils.isMatchingExpression(reference, testExpression.rightExpression)
      ) {
    ...
    if (testExpression.nodeType === ParseNodeType.BinaryOperation) {
      const isOrIsNotOperator =
        testExpression.operator === OperatorType.Is ||
      ...
      if (isOrIsNotOperator || equalsOrNotEqualsOperator) {
        ...
        if (
          ...
          if (
            ParseTreeUtils.isMatchingExpression(reference, testExpression.leftExpression)
          ) {
        ...
        if (
          ...
          if (
            ...
            testExpression.leftExpression.arguments[0].argumentCategory ===
            ...
            if (ParseTreeUtils.isMatchingExpression(reference, arg0Expr)) {
              ...
              if (classType.category === TypeCategory.Class) {
                return (type: Type) => {
                  ...
                  return doForSubtypes(type, (subtype) => {
                    if (subtype.category === TypeCategory.Object) {
                      const matches = ClassType.isSameGenericClass(
                        subtype.classType,
                        classType
                      );
                      if (adjIsPositiveTest) {
    ...
    const filterType = (varType: ClassType): ObjectType[] | ClassType[] => {
      ...
      for (const filterType of classTypeList) {
        ...
        if (
          filterIsSubclass &&
          filterIsSuperclass &&
          !ClassType.isSameGenericClass(varType, filterType)
        ) {
    ...
    const narrowedType = doForSubtypes(referenceType, (subtype) => {
      if (
        ...
        if (subtype.literalValue !== undefined) {
          ...
          if (
            (literalValueMatches && !isPositiveTest) ||
            (!literalValueMatches && isPositiveTest)
          ) {
            return undefined;
          }
          ...
          if (allLiteralTypes) {
            return combineTypes(
              allLiteralTypes.filter(
                (type) => !ObjectType.isLiteralValueSame(type, literalType)
              )
            );
          }
        }
    ...
    return doForSubtypes(type, (subtype) => {
      switch (subtype.category) {
        ...
        case TypeCategory.Object: {
          ...
          if (
            classFromTypeObject &&
            classFromTypeObject.category === TypeCategory.Class
          ) {
  ...
  function createSpecializedClassType(
    ...
    const typeArgTypes = typeArgs
      ? typeArgs.map((t) => convertClassToObject(t.type))
      : [];
  ...
  function isSpeculativeMode(node: ParseNode | undefined) {
    ...
    return (
      node === undefined ||
      ParseTreeUtils.isNodeContainedWithin(node, speculativeRootNode)
    );
  }
  ...
  function getDeclarationFromFunctionNamedParameter(
    type: FunctionType,
    paramName: string
  ): Declaration | undefined {
    ...
    if (
      ...
      if (scope) {
        ...
        if (symbolInScope) {
          ...
          const declsForThisImport = symbolInScope.symbol
            .getDeclarations()
            .filter((decl) => {
      ...
      if (argNode.parent && argNode.parent.nodeType === ParseNodeType.Call) {
        ...
        if (baseType) {
          if (
            baseType.category === TypeCategory.Function &&
            baseType.details.declaration
          ) {
            const paramDecl = getDeclarationFromFunctionNamedParameter(
              baseType,
              paramName
            );
            if (paramDecl) {
    ...
    switch (declaration.type) {
      case DeclarationType.Intrinsic: {
        ...
        if (declaration.intrinsicType === 'class') {
          const classNode = ParseTreeUtils.getEnclosingClass(
            declaration.node
          ) as ClassNode;
        ...
        if (strType.category === TypeCategory.Object) {
          ...
          if (declaration.intrinsicType === 'Iterable[str]') {
            ...
            if (iterableType.category === TypeCategory.Class) {
              return ObjectType.create(
                ClassType.cloneForSpecialization(iterableType, [strType])
              );
            }
          }
      ...
      case DeclarationType.Parameter: {
        ...
        if (
          typeAnnotationNode &&
          typeAnnotationNode.nodeType === ParseNodeType.StringList
        ) {
    ...
    function applyLoaderActionsToModuleType(
      ...
      if (loaderActions.implicitImports) {
        loaderActions.implicitImports.forEach((implicitImport, name) => {
          ...
          const symbolType = applyLoaderActionsToModuleType(
            importedModuleType,
            implicitImport,
            importLookup
          );
  ...
  function getEffectiveTypeOfSymbol(symbol: symbol): Type {
    ...
    return typesToCombine.length > 0
  ...
  function getFunctionInferredReturnType(
    type: FunctionType,
    args?: ValidateArgTypeParams[]
  ) {
    let returnType: Type | undefined;
    ...
    if (type.inferredReturnType) {
      ...
      if (type.details.declaration) {
        ...
        disableSpeculativeMode(() => {
          returnType = inferFunctionReturnType(
            functionNode,
    ...
    if (
      ...
      const contextualReturnType = getFunctionInferredReturnTypeUsingArguments(
        type,
        args
      );
  ...
  function getFunctionInferredReturnTypeUsingArguments(
    ...
    if (contextualReturnType) {
      ...
      if (
        FunctionType.isWrapReturnTypeInAwait(type) &&
        !isNoReturnType(contextualReturnType)
      ) {
  ...
  function canAssignClassToProtocol(
    ...
    destType.details.baseClasses.forEach((baseClass) => {
      if (
        ...
        const specializedBaseClass = specializeForBaseClass(
          destType,
          baseClass,
          recursionCount + 1
        );
  ...
  function canAssignClass(
    ...
    if (
      (flags & CanAssignFlags.EnforceInvariance) === 0 ||
      ClassType.isSameGenericClass(srcType, destType)
    ) {
      ...
      if (ancestorIndex === 0 && ClassType.isSpecialBuiltIn(destType)) {
        ...
        if (destType.details.name === 'Tuple') {
          if (destType.typeArguments && curSrcType.typeArguments) {
            ...
            const isDestHomogenousTuple =
              destArgCount === 2 && isEllipsisType(destTypeArgs[1]);
            ...
            const isSrcHomogeneousType =
              srcArgCount === 2 && isEllipsisType(srcTypeArgs[1]);
            if (isSrcHomogeneousType) {
              srcArgCount = 1;
            }

            if (
              srcTypeArgs.length === destArgCount ||
              isDestHomogenousTuple ||
              isSrcHomogeneousType
            ) {
              for (let i = 0; i < Math.max(destArgCount, srcArgCount); i++) {
                const expectedDestType =
                  (isDestHomogenousTuple ? destTypeArgs[0] : destTypeArgs[i]) ||
                  AnyType.create();
                const expectedSrcType =
                  (isSrcHomogeneousType ? srcTypeArgs[0] : srcTypeArgs[i]) ||
                  AnyType.create();
  ...
  function verifyTypeArgumentsAssignable(
    ...
    if (srcTypeArgs) {
      if (
        ...
        srcTypeArgs.length === destTypeParams.length
      ) {
        for (let srcArgIndex = 0; srcArgIndex < srcTypeArgs.length; srcArgIndex++) {
          ...
          const destArgIndex =
            srcArgIndex >= destTypeArgs.length ? destTypeArgs.length - 1 : srcArgIndex;
  ...
  function assignTypeToTypeVar(
    ...
    if (destType.constraints.length > 0) {
      ...
      if (!constrainedType) {
        diag.addMessage(
          `Type "${printType(
            srcType
          )}" is not compatible with constraints imposed by TypeVar "${destType.name}"`
        );
        return false;
      }
      ...
      if (curTypeVarMapping) {
        if (!isTypeSame(curTypeVarMapping, constrainedType)) {
          diag.addMessage(
            `Type "${printType(
              srcType
            )}" is not compatible with constrained type "${printType(curTypeVarMapping)}"`
          );
          return false;
        }
      } else {
    ...
    const curTypeIsNarrowable =
      typeVarMap.isNarrowable(destType.name) && !typeVarMap.isLocked();
    ...
    if (curTypeVarMapping) {
      ...
      if (canNarrowType) {
        ...
        if (
          ...
          if (
            !isAnyOrUnknown(curTypeVarMapping) &&
            srcType.category !== TypeCategory.Unknown
          ) {
          ...
          diag.addMessage(
            `Type "${printType(srcType)}" cannot be assigned to type "${printType(
              curTypeVarMapping
            )}"`
          );
          return false;
        ...
        if (
          ...
          if (typeVarMap.isLocked()) {
            diag.addMessage(
              `Type "${printType(
                curTypeVarMapping
              )}" cannot be assigned to type "${printType(srcType)}"`
            );
            return false;
          }
    ...
    if (destType.boundType) {
      if (
        ...
        diag.addMessage(
          `Type "${printType(
            updatedType
          )}" is not compatible with bound type "${printType(
            destType.boundType
  ...
  function canAssignType(
    ...
    if (
      destType.category === TypeCategory.Unbound ||
    ...
    if (flags & CanAssignFlags.EnforceInvariance) {
      if (
        srcType.category === TypeCategory.Union ||
        destType.category === TypeCategory.Union
      ) {
        if (!isTypeSame(srcType, destType)) {
          diag.addMessage(
            `Type "${printType(srcType)}" cannot be assigned to type "${printType(
    ...
    if (srcType.category === TypeCategory.Union) {
      ...
      srcType.subtypes.forEach((t) => {
        if (
          ...
          diag.addMessage(
            `Type "${printType(t)}" cannot be assigned to type "${printType(destType)}"`
          );
          isIncompatible = true;
        }
      });
    ...
    if (
      destType.category === TypeCategory.None &&
      srcType.category === TypeCategory.None
    ) {
      return true;
    }
    ...
    if (
      srcType.category === TypeCategory.Object &&
      ...
      if (srcType.category === TypeCategory.Object) {
        if (destType.literalValue !== undefined) {
          ...
          if (
            ...
            !ObjectType.isLiteralValueSame(srcType, destType)
          ) {
            diag.addMessage(
              `"${
        ...
        if (
          ClassType.isBuiltIn(destType.classType) &&
    ...
    if (destType.category === TypeCategory.Function) {
      ...
      if (srcType.category === TypeCategory.OverloadedFunction) {
        ...
        if (overloadIndex < 0) {
          diag.addMessage(
            `No overloaded function matches type "${printType(destType)}".`
          );
          return false;
        }
  ...
  function canAssignFunction(
    ...
    const destParamCount = destType.details.parameters.filter((param) => param.name)
      .length;
    ...
    for (let paramIndex = 0; paramIndex < minParamCount; paramIndex++) {
      ...
      if (
        srcParam.category !== ParameterCategory.Simple ||
      ...
      srcParams.forEach((param) => {
        if (srcHasNamedParam) {
          if (param.name && param.category === ParameterCategory.Simple) {
            ...
            if (!destParam) {
              paramDiag.addMessage(
                `Named parameter "${param.name}" is missing in destination`
              );
              canAssign = false;
            } else {
              const specializedDestParamType = specializeType(
    ...
    if (!isAnyOrUnknown(destReturnType)) {
      ...
      if (
        ...
        diag.addMessage(
          `Function return type "${printType(
            srcReturnType
  ...
  function narrowDeclaredTypeBasedOnAssignedType(
    declaredType: Type,
    assignedType: Type
  ): Type {
    const diagAddendum = new DiagnosticAddendum();
    ...
    if (declaredType.category === TypeCategory.Union) {
      return doForSubtypes(declaredType, (subtype) => {
        if (assignedType.category === TypeCategory.Union) {
          if (
            !assignedType.subtypes.some((t) => canAssignType(subtype, t, diagAddendum))
          ) {
            return undefined;
          } else {
            return subtype;
  ...
  function canOverrideMethod(
    ...
    for (let i = 0; i < paramCount; i++) {
      ...
      if (baseParam.name !== overrideParam.name) {
        ...
        const overrideParamType = FunctionType.getEffectiveParameterType(
          overrideMethod,
          i
        );
  ...
  function canAssignToTypeVar(
    ...
    if (boundType) {
      if (
        ...
        diag.addMessage(
          `Type "${printType(
            effectiveSrcType
          )}" is not compatible with bound type "${printType(boundType)}" for TypeVar "${
            destType.name
          }"`
        );
        return false;
    ...
    for (const constraint of constraints) {
      if (isAnyOrUnknown(constraint)) {
        ...
        if (
          effectiveSrcType.subtypes.find((t) => isSameWithoutLiteralValue(constraint, t))
        ) {
          return true;
        }
  ...
  function getTypedDictMembersForClassRecursive(
    ...
    classType.details.baseClasses.forEach((baseClassType) => {
      if (
        baseClassType.category === TypeCategory.Class &&
    ...
    const paramTypeStrings = type.details.parameters.map((param, index) => {
      ...
      if (type.details.declaration) {
        ...
        if (paramNode.defaultValue) {
          paramString +=
            defaultValueAssignment +
            ParseTreeUtils.printExpression(paramNode.defaultValue);
        }
      }
