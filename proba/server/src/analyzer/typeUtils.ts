...
export function enumerateLiteralsForType(type: ObjectType): ObjectType[] | undefined {
  if (ClassType.isBuiltIn(type.classType, 'bool')) {
    ...
    return [
      ObjectType.cloneWithLiteral(type, true),
      ObjectType.cloneWithLiteral(type, false),
    ];
  }
  ...
  if (ClassType.isEnumClass(type.classType)) {
    ...
    fields.forEach((symbol, name) => {
      if (!symbol.isIgnoredForProtocolMatch() && !symbol.isInstanceMember()) {
        enumList.push(
          ObjectType.cloneWithLiteral(
            type,
            new EnumLiteral(type.classType.details.name, name)
          )
        );
      }
    });
      ...
      const strippedSpecializedTypes: SpecializedFunctionTypes = {
        ...
        returnType: type.specializedTypes.returnType
          ? stripLiteralTypeArgsValue(
              stripLiteralValue(type.specializedTypes.returnType),
              recursionCount + 1
            )
          : undefined,
      };
...
export function isProperty(type: Type): boolean {
  return (
    type.category === TypeCategory.Object && ClassType.isPropertyClass(type.classType)
  );
}
...
export function specializeType(
  ...
  if (type.category === TypeCategory.Union) {
    ...
    type.subtypes.forEach((typeEntry) => {
      subtypes.push(
        specializeType(typeEntry, typeVarMap, makeConcrete, recursionLevel + 1)
      );
    });
  ...
  if (type.category === TypeCategory.Object) {
    const classType = _specializeClassType(
      type.classType,
      typeVarMap,
      makeConcrete,
      recursionLevel + 1
  ...
  if (type.category === TypeCategory.OverloadedFunction) {
    return _specializeOverloadedFunctionType(
      type,
      typeVarMap,
      makeConcrete,
      recursionLevel + 1
...
export function addDefaultFunctionParameters(
...
export function selfSpecializeClassType(
  type: ClassType,
  setSkipAbstractClassTest = false
): ClassType {
...
// of true is assumed.
export function derivesFromClassRecursive(
  classType: ClassType,
...
export function getDeclaredGeneratorYieldType(
  functionType: FunctionType,
  iteratorType: Type
): Type | undefined {
  ...
  if (returnType) {
    ...
    if (
      ...
      return ObjectType.create(
        ClassType.cloneForSpecialization(iteratorType, [generatorTypeArgs[0]])
      );
    }
...
export function getDeclaredGeneratorSendType(
  functionType: FunctionType
): Type | undefined {
  const returnType = FunctionType.getSpecializedReturnType(functionType);
  if (returnType) {
...
export function containsUnknown(
  type: Type,
  ...
  if (type.category === TypeCategory.Class) {
    if (
      ...
      for (const argType of type.typeArguments) {
        if (
          containsUnknown(argType, allowUnknownTypeArgsForClasses, recursionCount + 1)
        ) {
          return true;
        }
      }
  ...
  if (type.category === TypeCategory.Function) {
    ...
    if (
      ...
      containsUnknown(type.details.declaredReturnType)
    ) {
      return true;
    }
...
function _specializeClassType(
  ...
  if (classType.typeArguments) {
    newTypeArgs = classType.typeArguments.map((oldTypeArgType) => {
      const newTypeArgType = specializeType(
        oldTypeArgType,
        typeVarMap,
        makeConcrete,
        recursionLevel + 1
...
export function requiresSpecialization(type: Type, recursionCount = 0): boolean {
  switch (type.category) {
    ...
    case TypeCategory.OverloadedFunction: {
      return (
        type.overloads.find((overload) =>
          requiresSpecialization(overload, recursionCount + 1)
        ) !== undefined
      );
    }
    ...
    case TypeCategory.Union: {
      return (
        type.subtypes.find((type) => requiresSpecialization(type, recursionCount + 1)) !==
        undefined
      );
    }
  ...
  const isInTail = (searchClass: ClassType, classLists: Type[][]) => {
    return classLists.some((classList) => {
      return (
        classList.findIndex(
          (value) =>
            value.category === TypeCategory.Class &&
            ClassType.isSameGenericClass(value, searchClass, false)
        ) > 0
      );
    });
  };
  ...
  const filterClass = (classToFilter: ClassType, classLists: Type[][]) => {
    for (let i = 0; i < classLists.length; i++) {
      classLists[i] = classLists[i].filter(
        (value) =>
          value.category !== TypeCategory.Class ||
          !ClassType.isSameGenericClass(value, classToFilter, false)
      );
    }
  };
