...
export namespace ClassType {
  ...
  export function isGeneric(classType: ClassType) {
    return (
      classType.details.typeParameters.length > 0 && classType.typeArguments === undefined
    );
  }
  ...
  export function isSameGenericClass(
    ...
    const class1Details =
      treatAliasAsSame && classType.details.aliasClass
        ? classType.details.aliasClass.details
        : classType.details;
    const class2Details =
      treatAliasAsSame && type2.details.aliasClass
        ? type2.details.aliasClass.details
        : type2.details;
    ...
    for (let i = 0; i < dataClassEntries1.length; i++) {
      if (
        ...
        !isTypeSame(
          dataClassEntries1[i].type,
          dataClassEntries2[i].type,
          recursionCount + 1
        )
      ) {
        return false;
  ...
  export function clone(type: FunctionType, deleteFirstParam = false): FunctionType {
    ...
    if (deleteFirstParam) {
      newFunction.details.flags &= ~(
        FunctionTypeFlags.ConstructorMethod | FunctionTypeFlags.ClassMethod
      );
...
export namespace UnionType {
  ...
  export function containsType(
    ...
    return (
      unionType.subtypes.find((t) => isTypeSame(t, subtype, recursionCount + 1)) !==
      undefined
    );
  }
}
...
export function isTypeSame(type1: Type, type2: Type, recursionCount = 0): boolean {
  ...
  switch (type1.category) {
    ...
    case TypeCategory.Union: {
      ...
      return (
        subtypes1.find(
          (t) => !UnionType.containsType(unionType2, t, recursionCount + 1)
        ) === undefined
      );
    }
...
export function combineTypes(types: Type[]): Type {
  ...
  expandedTypes = expandedTypes.sort((type1, type2) => {
    if (type1.category === TypeCategory.Object && type1.literalValue !== undefined) {
      ...
      type2.category === TypeCategory.Object &&
      type2.literalValue !== undefined
    ) {
      return -1;
    }
    return 0;
...
function _addTypeIfUnique(types: Type[], typeToAdd: Type) {
  for (let i = 0; i < types.length; i++) {
    ...
    if (
      ...
      typeToAdd.category === TypeCategory.Object
    ) {
      ...
      if (ClassType.isBuiltIn(type.classType, 'bool')) {
        if (
          ...
          !typeToAdd.literalValue === type.literalValue
        ) {
