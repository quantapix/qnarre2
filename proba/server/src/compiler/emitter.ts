...
export function emitFiles(resolver: EmitResolver, host: EmitHost, targetSourceFile: SourceFile | undefined, { scriptTransformers, declarationTransformers }: EmitTransformers, emitOnlyDtsFiles?: boolean, onlyBuildInfo?: boolean, forceDtsEmit?: boolean): EmitResult {
  ...
  function collectLinkedAliases(node: qt.Node) {
  ...
  forEach(jsBundle.sources && jsBundle.sources.prologues, (prologueInfo) => {
    ...
    sourceFile.statements = createNodeArray(
      prologueInfo.directives.map((directive) => {
        const statement = createNode(SyntaxKind.ExpressionStatement, directive.pos, directive.end);
        statement.expression = createNode(SyntaxKind.StringLiteral, directive.expression.pos, directive.expression.end);
...
export function createPrinter(printerOptions: PrinterOptions = {}, handlers: PrintHandlers = {}): Printer {
  ...
  function printNode(hint: EmitHint, node: qt.Node, sourceFile: SourceFile): string {
    switch (hint) {
  ...
  function writeNode(hint: EmitHint, node: qt.Node, sourceFile: SourceFile, output: qt.EmitTextWriter): void;
  function writeNode(hint: EmitHint, node: qt.Node, sourceFile: SourceFile | undefined, output: qt.EmitTextWriter) {
  ...
  function recordBundleFileInternalSectionStart(node: qt.Node) {
  ...
  function print(hint: EmitHint, node: qt.Node, sourceFile: SourceFile | undefined) {
    if (sourceFile) {
      setSourceFile(sourceFile);
    }
  ...
  function reset() {
    ...
    generatedNames = qc.createMap<true>();
    tempFlagsStack = [];
  ...
  function emit(node: qt.Node | undefined): Node | undefined;
  function emit(node: qt.Node | undefined) {
  ...
  function pipelineEmit(emitHint: EmitHint, node: qt.Node) {
  ...
  function getPipelinePhase(phase: PipelinePhase, emitHint: EmitHint, node: qt.Node) {
    switch (phase) {
  ...
  function getNextPipelinePhase(currentPhase: PipelinePhase, emitHint: EmitHint, node: qt.Node) {
    return getPipelinePhase(currentPhase + 1, emitHint, node);
  }

  function pipelineEmitWithNotification(hint: EmitHint, node: qt.Node) {
    Debug.assert(lastNode === node);
  ...
  function pipelineEmitWithHint(hint: EmitHint, node: qt.Node): void {
  ...
  function pipelineEmitWithSubstitution(hint: EmitHint, node: qt.Node) {
    Debug.assert(lastNode === node || lastSubstitution === node);
  ...
  function getSortedEmitHelpers(node: qt.Node) {
  ...
  function emitPropertyAccessExpression(node: PropertyAccessExpression) {
    ...
    const token = node.questionDotToken || createNode(SyntaxKind.DotToken, node.expression.end, node.name.pos);
  ...
  function emitCaseOrDefaultClauseRest(parentNode: Node, statements: qt.NodeArray<Statement>, colonPos: number) {
    const emitAsSingleStatement =
      ...
      (nodeIsSynthesized(parentNode) || nodeIsSynthesized(statements[0]) || rangeStartPositionsAreOnSameLine(parentNode, statements[0], currentSourceFile!));
  ...
  function emitPrologueDirectivesIfNeeded(sourceFileOrBundle: Bundle | SourceFile) {
    if (isSourceFile(sourceFileOrBundle)) {
      ...
      const seenPrologueDirectives = qc.createMap<true>();
  ...
  function emitNodeWithWriter(node: qt.Node | undefined, writer: typeof write) {
    if (!node) return;
  ...
  function emitWithLeadingSpace(node: qt.Node | undefined) {
    if (node) {
      writeSpace();
      emit(node);
    }
  ...
  function emitWithTrailingSpace(node: qt.Node | undefined) {
    if (node) {
      emit(node);
      writeSpace();
    }
  ...
  function writeTokenNode(node: qt.Node, writer: (s: string) => void) {
  ...
  function getLeadingLineTerminatorCount(parentNode: qt.TextRange, children: readonly Node[], format: ListFormat): number {
  ...
  function getClosingLineTerminatorCount(parentNode: qt.TextRange, children: readonly Node[], format: ListFormat): number {
    if (format & ListFormat.PreserveLines || preserveSourceNewlines) {
      if (format & ListFormat.PreferNewLine) {
        return 1;
      }
  ...
  function writeLineSeparatorsAndIndentBefore(node: qt.Node, parent: Node): boolean {
  ...
  function writeLineSeparatorsAfter(node: qt.Node, parent: Node) {
  ...
  function synthesizedNodeStartsOnNewLine(node: qt.Node, format: ListFormat) {
  ...
  function skipSynthesizedParentheses(node: qt.Node) {
  ...
  function generateNames(node: qt.Node | undefined) {
    if (!node) return;
    switch (node.kind) {
      case qt.SyntaxKind.Block:
  ...
  function generateMemberNames(node: qt.Node | undefined) {
    if (!node) return;
    switch (node.kind) {
      case qt.SyntaxKind.PropertyAssignment:
      case qt.SyntaxKind.ShorthandPropertyAssignment:
  ...
  function makeUniqueName(baseName: string, checkFn: (name: string) => boolean = isUniqueName, optimistic?: boolean, scoped?: boolean): string {
    ...
    if (baseName.charCodeAt(baseName.length - 1) !== qt.CharacterCodes._) {
      baseName += '_';
    }
    let i = 1;
    while (true) {
  ...
  function pipelineEmitWithComments(hint: EmitHint, node: qt.Node) {
    Debug.assert(lastNode === node || lastSubstitution === node);
    enterComment();
  ...
  function pipelineEmitWithSourceMap(hint: EmitHint, node: qt.Node) {
    Debug.assert(lastNode === node || lastSubstitution === node);
   ...
   */
  function emitTokenWithSourceMap(node: qt.Node | undefined, token: qt.SyntaxKind, writer: (s: string) => void, tokenPos: number, emitCallback: (token: qt.SyntaxKind, writer: (s: string) => void, tokenStartPos: number) => number) {
