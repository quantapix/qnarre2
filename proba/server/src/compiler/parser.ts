...
let SourceFileConstructor: new (kind: qt.SyntaxKind.SourceFile, pos?: number, end?: number) => qt.Node;
let IdentifierConstructor: new (kind: qt.SyntaxKind.Identifier, pos?: number, end?: number) => qt.Node;
let PrivateIdentifierConstructor: new (kind: qt.SyntaxKind.PrivateIdentifier, pos?: number, end?: number) => qt.Node;
...
export function createNode(kind: qt.SyntaxKind, pos?: number, end?: number): qt.Node {
  if (kind === qt.SyntaxKind.SourceFile) {
    return new (SourceFileConstructor || (SourceFileConstructor = qu.objectAllocator.getSourceFileConstructor()))(kind, pos, end);
  } else if (kind === qt.SyntaxKind.Identifier) {
    return new (IdentifierConstructor || (IdentifierConstructor = qu.objectAllocator.getIdentifierConstructor()))(kind, pos, end);
  } else if (kind === qt.SyntaxKind.PrivateIdentifier) {
    return new (PrivateIdentifierConstructor || (PrivateIdentifierConstructor = qu.objectAllocator.getPrivateIdentifierConstructor()))(kind, pos, end);
  } else if (!qpu.isNodeKind(kind)) {
    return new (TokenConstructor || (TokenConstructor = qu.objectAllocator.getTokenConstructor()))(kind, pos, end);
  } else {
    return new (NodeConstructor || (NodeConstructor = qu.objectAllocator.getNodeConstructor()))(kind, pos, end);
  }
}
...
export function forEachChild<T>(n: qt.Node, cbNode: (_: qt.Node) => T | undefined, cbNodes?: (_: qt.NodeArray<qt.Node>) => T | undefined): T | undefined {
  if (!n || n.kind <= qt.SyntaxKind.LastToken) return;
  const node = n as any;
  switch (node.kind) {
    ...
    case qt.SyntaxKind.JSDocVariadicType:
      return visitNode(cbNode, (node as qt.OptionalTypeNode | qt.RestTypeNode | qt.JSDocTypeExpression | qt.JSDocTypeReferencingNode).type);
    ...
    case qt.SyntaxKind.JSDocEnumTag:
      return visitNode(cbNode, node.tagName) || visitNode(cbNode, (node as qt.JSDocReturnTag | qt.JSDocTypeTag | qt.JSDocThisTag | qt.JSDocEnumTag).typeExpression);
    case qt.SyntaxKind.JSDocSignature:
      return qc.forEach(node.typeParameters, cbNode) || qc.forEach(node.parameters, cbNode) || visitNode(cbNode, node.type);
    case qt.SyntaxKind.JSDocTypeLiteral:
      return qc.forEach(node.jsDocPropertyTags, cbNode);
...
export function createSourceFile(fileName: string, sourceText: string, languageVersion: qt.ScriptTarget, setParentNodes = false, scriptKind?: qt.ScriptKind): qt.SourceFile {
  performance.mark('beforeParse');
  let result: qt.SourceFile;
...
export function parseIsolatedEntityName(text: string, languageVersion: qt.ScriptTarget): qt.EntityName | undefined {
  return Parser.parseIsolatedEntityName(text, languageVersion);
}

export function parseJsonText(fileName: string, sourceText: string): qt.JsonSourceFile {
  return Parser.parseJsonText(fileName, sourceText);
}

export function isExternalModule(file: qt.SourceFile): boolean {
  return file.externalModuleIndicator !== undefined;
}

export function updateSourceFile(sourceFile: qt.SourceFile, newText: string, textChangeRange: qt.TextChangeRange, aggressiveChecks = false): qt.SourceFile {
...
export namespace Parser {
  const scanner = qs.createScanner(qt.ScriptTarget.Latest, /*skipTrivia*/ true);
  ...
  let identifiers: qpc.Map<string>;
  let privateIdentifiers: qpc.Map<string>;
  let identifierCount: number;

  let parsingContext: ParsingContext;

  let notParenthesizedArrow: qpc.Map<true> | undefined;
  ...
  export function parseSourceFile(fileName: string, sourceText: string, languageVersion: qt.ScriptTarget, syntaxCursor: IncrementalParser.SyntaxCursor | undefined, setParentNodes = false, scriptKind?: qt.ScriptKind): qt.SourceFile {
    scriptKind = qu.ensureScriptKind(fileName, scriptKind);
    if (scriptKind === qt.ScriptKind.JSON) {
      ...
      result.referencedFiles = qc.emptyArray;
      result.typeReferenceDirectives = qc.emptyArray;
      result.libReferenceDirectives = qc.emptyArray;
      result.amdDependencies = qc.emptyArray;
      result.hasNoDefaultLib = false;
      result.pragmas = qu.emptyMap;
      return result;
    }
  ...
  export function parseIsolatedEntityName(content: string, languageVersion: qt.ScriptTarget): qt.EntityName | undefined {
  ...
  export function parseJsonText(fileName: string, sourceText: string, languageVersion: qt.ScriptTarget = qt.ScriptTarget.ES2015, syntaxCursor?: IncrementalParser.SyntaxCursor, setParentNodes?: boolean): qt.JsonSourceFile {
    ...
    if (token() === qt.SyntaxKind.EndOfFileToken) {
      ...
      sourceFile.endOfFileToken = parseTokenNode<qt.EndOfFileToken>();
    } else {
      const statement = createNode(qt.SyntaxKind.ExpressionStatement) as qt.JsonObjectExpressionStatement;
      switch (token()) {
        ...
        case qt.SyntaxKind.NullKeyword:
          statement.expression = parseTokenNode<qt.BooleanLiteral | qt.NullLiteral>();
          break;
        ...
        case qt.SyntaxKind.StringLiteral:
          if (lookAhead(() => nextToken() !== qt.SyntaxKind.ColonToken)) {
            statement.expression = parseLiteralNode() as qt.StringLiteral | qt.NumericLiteral;
            break;
          }
        // falls through
        default:
    ...
    clearState();
    return result as qt.JsonSourceFile;
  }

  function getLanguageVariant(scriptKind: qt.ScriptKind) {
    // .tsx and .jsx files are treated as jsx language variant.
    return scriptKind === qt.ScriptKind.TSX || scriptKind === qt.ScriptKind.JSX || scriptKind === qt.ScriptKind.JS || scriptKind === qt.ScriptKind.JSON ? qt.LanguageVariant.JSX : qt.LanguageVariant.Standard;
  }

  function initializeState(_sourceText: string, languageVersion: qt.ScriptTarget, _syntaxCursor: IncrementalParser.SyntaxCursor | undefined, scriptKind: qt.ScriptKind) {
    NodeConstructor = qu.objectAllocator.getNodeConstructor();
    TokenConstructor = qu.objectAllocator.getTokenConstructor();
    IdentifierConstructor = qu.objectAllocator.getIdentifierConstructor();
    PrivateIdentifierConstructor = qu.objectAllocator.getPrivateIdentifierConstructor();
    SourceFileConstructor = qu.objectAllocator.getSourceFileConstructor();
  ...
  function parseSourceFileWorker(fileName: string, languageVersion: qt.ScriptTarget, setParentNodes: boolean, scriptKind: qt.ScriptKind): qt.SourceFile {
    ...
    function reportPragmaDiagnostic(pos: number, end: number, diagnostic: qt.DiagnosticMessage) {
      parseDiagnostics.push(qu.createFileDiagnostic(sourceFile, pos, end, diagnostic));
    }
  }

  function addJSDocComment<T extends qt.HasJSDoc>(node: T): T {
    Debug.assert(!node.jsDoc); // Should only be called once per node
    const jsDoc = qc.mapDefined(qu.getJSDocCommentRanges(node, sourceFile.text), (comment) => JSDocParser.parseJSDocComment(node, comment.pos, comment.end - comment.pos));
  ...
  function createSourceFile(fileName: string, languageVersion: qt.ScriptTarget, scriptKind: qt.ScriptKind, isDeclarationFile: boolean): qt.SourceFile {
    // code from createNode is inlined here so createNode won't have to deal with special case of creating source files
    // this is quite rare comparing to other nodes and createNode should be as fast as possible
    const sourceFile = <qt.SourceFile>new SourceFileConstructor(qt.SyntaxKind.SourceFile, /*pos*/ 0, /* end */ sourceText.length);
    nodeCount++;
  ...
  function parseErrorAtPosition(start: number, length: number, message: qt.DiagnosticMessage, arg0?: any): void {
    ...
    if (!lastError || start !== lastError.start) {
      parseDiagnostics.push(qu.createFileDiagnostic(sourceFile, start, length, message, arg0));
    }
  ...
  function nextToken(): qt.SyntaxKind {
    ...
    if (qu.isKeyword(currentToken) && (scanner.hasUnicodeEscape() || scanner.hasExtendedUnicodeEscape())) {
  ...
  function parseExpected(kind: qt.SyntaxKind, diagnosticMessage?: qt.DiagnosticMessage, shouldAdvance = true): boolean {
    ...
    if (diagnosticMessage) {
      ...
      parseErrorAtCurrentToken(Diagnostics._0_expected, qs.tokenToString(kind));
    }
    return false;
  }
  ...
  function parseExpectedToken(t: qt.SyntaxKind, diagnosticMessage?: qt.DiagnosticMessage, arg0?: any): qt.Node {
    return parseOptionalToken(t) || createMissingNode(t, /*reportAtCurrentPosition*/ false, diagnosticMessage || Diagnostics._0_expected, arg0 || qs.tokenToString(t));
  }
  ...
  function parseExpectedTokenJSDoc(t: qt.JSDocSyntaxKind): qt.Node {
    return parseOptionalTokenJSDoc(t) || createMissingNode(t, /*reportAtCurrentPosition*/ false, Diagnostics._0_expected, qs.tokenToString(t));
  }
  ...
  function createNode(kind: qt.SyntaxKind, pos?: number): qt.Node {
    ...
    return qpu.isNodeKind(kind) || kind === qt.SyntaxKind.Unknown ? new NodeConstructor(kind, p, p) : kind === qt.SyntaxKind.Identifier ? new IdentifierConstructor(kind, p, p) : kind === qt.SyntaxKind.PrivateIdentifier ? new PrivateIdentifierConstructor(kind, p, p) : new TokenConstructor(kind, p, p);
  }

  function createNodeWithJSDoc(kind: qt.SyntaxKind, pos?: number): qt.Node {
    const node = createNode(kind, pos);
    if (scanner.getTokenFlags() & qt.TokenFlags.PrecedingJSDocComment && (kind !== qt.SyntaxKind.ExpressionStatement || token() !== qt.SyntaxKind.OpenParenToken)) {
      addJSDocComment(<qt.HasJSDoc>node);
    }
    return node;
  }
  ...
  function createNodeArray<T extends qt.Node>(elements: T[], pos: number, end?: number): qt.NodeArray<T> {
    ...
    const array = <qt.MutableNodeArray<T>>(length >= 1 && length <= 4 ? elements.slice() : elements);
    array.pos = pos;
  ...
  function createMissingNode<T extends qt.Node>(kind: T['kind'], reportAtCurrentPosition: boolean, diagnosticMessage: qt.DiagnosticMessage, arg0?: any): T {
    ...
    } else if (qpu.isLiteralKind(kind) || qpu.isTemplateLiteralKind(kind)) {
      (result as qt.LiteralLikeNode).text = '';
    }
  ...
  function createIdentifier(isIdentifier: boolean, diagnosticMessage?: qt.DiagnosticMessage, privateIdentifierDiagnosticMessage?: qt.DiagnosticMessage): qt.Identifier {
    ...
    if (isIdentifier) {
      ...
      node.escapedText = qpu.escapeLeadingUnderscores(internIdentifier(scanner.getTokenValue()));
  ...
  function parseIdentifierName(diagnosticMessage?: qt.DiagnosticMessage): qt.Identifier {
    return createIdentifier(qs.tokenIsIdentifierOrKeyword(token()), diagnosticMessage);
  }

  function isLiteralPropertyName(): boolean {
    return qs.tokenIsIdentifierOrKeyword(token()) || token() === qt.SyntaxKind.StringLiteral || token() === qt.SyntaxKind.NumericLiteral;
  }

  function parsePropertyNameWorker(allowComputedPropertyNames: boolean): qt.PropertyName {
    if (token() === qt.SyntaxKind.StringLiteral || token() === qt.SyntaxKind.NumericLiteral) {
      const node = <qt.StringLiteral | qt.NumericLiteral>parseLiteralNode();
  ...
  function parsePropertyName(): qt.PropertyName {
    return parsePropertyNameWorker(/*allowComputedPropertyNames*/ true);
  }

  function parseComputedPropertyName(): qt.ComputedPropertyName {
    // qt.PropertyName [Yield]:
    //      LiteralPropertyName
    //      qt.ComputedPropertyName[?Yield]
    const node = <qt.ComputedPropertyName>createNode(qt.SyntaxKind.ComputedPropertyName);
  ...
  function parsePrivateIdentifier(): qt.PrivateIdentifier {
    const node = createNode(qt.SyntaxKind.PrivateIdentifier) as qt.PrivateIdentifier;
    node.escapedText = qpu.escapeLeadingUnderscores(internPrivateIdentifier(scanner.getTokenText()));
    nextToken();
    return finishNode(node);
  }
  ...
  function parseAnyContextualModifier(): boolean {
    return qpu.isModifierKind(token()) && tryParse(nextTokenCanFollowModifier);
  }
  ...
  function isListElement(parsingContext: ParsingContext, inErrorRecovery: boolean): boolean {
    ...
    switch (parsingContext) {
      ...
      case ParsingContext.ImportOrExportSpecifiers:
        return qs.tokenIsIdentifierOrKeyword(token());
      case ParsingContext.JsxAttributes:
        return qs.tokenIsIdentifierOrKeyword(token()) || token() === qt.SyntaxKind.OpenBraceToken;
  ...
  function nextTokenIsIdentifierOrKeywordOrGreaterThan() {
    nextToken();
    return qs.tokenIsIdentifierOrKeywordOrGreaterThan(token());
  }
  ...
  function currentNode(parsingContext: ParsingContext): qt.Node | undefined {
    ...
    if (qu.nodeIsMissing(node) || node.intersectsChange || qu.containsParseError(node)) {
      return undefined;
    }
    ...
    if ((node as qt.JSDocContainer).jsDocCache) {
      // jsDocCache may include tags from parent nodes, which might have been modified.
      (node as qt.JSDocContainer).jsDocCache = undefined;
    }
    return node;
  }
  ...
  function canReuseNode(node: qt.Node, parsingContext: ParsingContext): boolean {
    switch (parsingContext) {
      ...
      // Note: these are almost certainly not safe to ever reuse.  qt.Expressions commonly
    ...
    if (node) {
      switch (node.kind) {
        ...
        case qt.SyntaxKind.MethodDeclaration:
          ...
          const methodDeclaration = node as qt.MethodDeclaration;
  ...
  function isReusableVariableDeclaration(node: qt.Node) {
    ...
    // has an initializer.
    const variableDeclarator = node as qt.VariableDeclaration;
  ...
  function isReusableParameter(node: qt.Node) {
    ...
    const parameter = node as qt.ParameterDeclaration;
  ...
  function parseEntityName(allowReservedWords: boolean, diagnosticMessage?: qt.DiagnosticMessage): qt.EntityName {
    let entity: qt.EntityName = allowReservedWords ? parseIdentifierName(diagnosticMessage) : parseIdentifier(diagnosticMessage);
  ...
  function createQualifiedName(entity: qt.EntityName, name: qt.Identifier): qt.QualifiedName {
    const node = createNode(qt.SyntaxKind.QualifiedName, entity.pos) as qt.QualifiedName;
    node.left = entity;
    node.right = name;
    return finishNode(node);
  }

  function parseRightSideOfDot(allowIdentifierNames: boolean, allowPrivateIdentifiers: boolean): qt.Identifier | qt.PrivateIdentifier {
    ...
    if (scanner.hasPrecedingLineBreak() && qs.tokenIsIdentifierOrKeyword(token())) {
  ...
  function parseTemplateExpression(isTaggedTemplate: boolean): qt.TemplateExpression {
    const template = <qt.TemplateExpression>createNode(qt.SyntaxKind.TemplateExpression);
    ...
    } while (qc.last(list).literal.kind === qt.SyntaxKind.TemplateMiddle);
  ...
  function parseTemplateSpan(isTaggedTemplate: boolean): qt.TemplateSpan {
    const span = <qt.TemplateSpan>createNode(qt.SyntaxKind.TemplateSpan);
    span.expression = allowInAnd(parseExpression);

    let literal: qt.TemplateMiddle | qt.TemplateTail;
    if (token() === qt.SyntaxKind.CloseBraceToken) {
      reScanTemplateToken(isTaggedTemplate);
      literal = parseTemplateMiddleOrTemplateTail();
    } else {
      literal = parseExpectedToken(qt.SyntaxKind.TemplateTail, Diagnostics._0_expected, qs.tokenToString(qt.SyntaxKind.CloseBraceToken));
    }
  ...
  function parseLiteralNode(): qt.LiteralExpression {
    return parseLiteralLikeNode(token());
  }

  function parseTemplateHead(isTaggedTemplate: boolean): qt.TemplateHead {
    if (isTaggedTemplate) {
  ...
  function parseTemplateMiddleOrTemplateTail(): qt.TemplateMiddle | qt.TemplateTail {
    const fragment = parseLiteralLikeNode(token());
  ...
  function parseLiteralLikeNode(kind: qt.SyntaxKind): qt.LiteralLikeNode {
    const node = createNode(kind) as qt.LiteralLikeNode;
    ...
    if (node.kind === qt.SyntaxKind.NumericLiteral) {
      node.numericLiteralFlags = scanner.getTokenFlags() & qt.TokenFlags.NumericLiteralFlags;
    }
    if (isTemplateLiteralKind(node.kind)) {
      (<qt.TemplateHead | qt.TemplateMiddle | qt.TemplateTail | qt.NoSubstitutionTemplateLiteral>node).templateFlags = scanner.getTokenFlags() & qt.TokenFlags.ContainsInvalidEscape;
    }
  ...
  function parseTypeReference(): qt.TypeReferenceNode {
    const node = <qt.TypeReferenceNode>createNode(qt.SyntaxKind.TypeReference);
  ...
  function typeHasArrowFunctionBlockingParseError(node: qt.TypeNode): boolean {
    switch (node.kind) {
      case qt.SyntaxKind.TypeReference:
        return qu.nodeIsMissing(node.typeName);
      case qt.SyntaxKind.FunctionType:
      case qt.SyntaxKind.ConstructorType: {
        const { parameters, type } = node as qt.ConstructorTypeNode;
  ...
  function parseThisTypePredicate(lhs: qt.ThisTypeNode): qt.TypePredicateNode {
    nextToken();
    const node = createNode(qt.SyntaxKind.TypePredicate, lhs.pos) as qt.TypePredicateNode;
    node.parameterName = lhs;
    node.type = parseType();
    return finishNode(node);
  }

  function parseThisTypeNode(): qt.ThisTypeNode {
    const node = createNode(qt.SyntaxKind.ThisType) as qt.ThisTypeNode;
    nextToken();
    return finishNode(node);
  }

  function parseJSDocAllType(postFixEquals: boolean): qt.JSDocAllType | qt.JSDocOptionalType {
    const result = createNode(qt.SyntaxKind.JSDocAllType) as qt.JSDocAllType;
    if (postFixEquals) {
      return createPostfixType(qt.SyntaxKind.JSDocOptionalType, result) as qt.JSDocOptionalType;
    } else {
      nextToken();
    }
  ...
  function parseJSDocNonNullableType(): qt.TypeNode {
    const result = createNode(qt.SyntaxKind.JSDocNonNullableType) as qt.JSDocNonNullableType;
    nextToken();
    result.type = parseNonArrayType();
    return finishNode(result);
  }

  function parseJSDocUnknownOrNullableType(): qt.JSDocUnknownType | qt.JSDocNullableType {
    ...
    if (token() === qt.SyntaxKind.CommaToken || token() === qt.SyntaxKind.CloseBraceToken || token() === qt.SyntaxKind.CloseParenToken || token() === qt.SyntaxKind.GreaterThanToken || token() === qt.SyntaxKind.EqualsToken || token() === qt.SyntaxKind.BarToken) {
      const result = <qt.JSDocUnknownType>createNode(qt.SyntaxKind.JSDocUnknownType, pos);
      return finishNode(result);
    } else {
      const result = <qt.JSDocNullableType>createNode(qt.SyntaxKind.JSDocNullableType, pos);
      result.type = parseType();
      return finishNode(result);
    }
  }

  function parseJSDocFunctionType(): qt.JSDocFunctionType | qt.TypeReferenceNode {
    if (lookAhead(nextTokenIsOpenParen)) {
      const result = <qt.JSDocFunctionType>createNodeWithJSDoc(qt.SyntaxKind.JSDocFunctionType);
      nextToken();
  ...
  function parseJSDocParameter(): qt.ParameterDeclaration {
    const parameter = createNode(qt.SyntaxKind.Parameter) as qt.ParameterDeclaration;
  ...
  function parseJSDocType(): qt.TypeNode {
    scanner.setInJSDocType(true);
    const moduleSpecifier = parseOptionalToken(qt.SyntaxKind.ModuleKeyword);
    if (moduleSpecifier) {
      const moduleTag = createNode(qt.SyntaxKind.JSDocNamepathType, moduleSpecifier.pos) as qt.JSDocNamepathType;
    ...
    scanner.setInJSDocType(false);
    if (dotdotdot) {
      const variadic = createNode(qt.SyntaxKind.JSDocVariadicType, dotdotdot.pos) as qt.JSDocVariadicType;
      variadic.type = type;
  ...
  function parseTypeQuery(): qt.TypeQueryNode {
    const node = <qt.TypeQueryNode>createNode(qt.SyntaxKind.TypeQuery);
    parseExpected(qt.SyntaxKind.TypeOfKeyword);
    node.exprName = parseEntityName(/*allowReservedWords*/ true);
    return finishNode(node);
  }

  function parseTypeParameter(): qt.TypeParameterDeclaration {
    const node = <qt.TypeParameterDeclaration>createNode(qt.SyntaxKind.TypeParameter);
  ...
  function parseTypeParameters(): qt.NodeArray<qt.TypeParameterDeclaration> | undefined {
    if (token() === qt.SyntaxKind.LessThanToken) {
  ...
  function parseParameterType(): qt.TypeNode | undefined {
  ...
  function isStartOfParameter(isJSDocParameter: boolean): boolean {
    return token() === qt.SyntaxKind.DotDotDotToken || isIdentifierOrPrivateIdentifierOrPattern() || qpu.isModifierKind(token()) || token() === qt.SyntaxKind.AtToken || isStartOfType(/*inStartOfParameter*/ !isJSDocParameter);
  }

  function parseParameter(): qt.ParameterDeclaration {
    ...
    if (qu.getFullWidth(node.name) === 0 && !node.modifiers && qpu.isModifierKind(token())) {
      // in cases like
      // 'use strict'
      // function foo(static)
      // isParameter('static') === true, because of isModifier('static')
      // however 'static' is not a legal identifier in a strict mode.
      // so result of this function will be qt.ParameterDeclaration (flags = 0, name = missing, type = undefined, initializer = undefined)
   ...
   */
  function fillSignature(returnToken: qt.SyntaxKind.ColonToken | qt.SyntaxKind.EqualsGreaterThanToken, flags: SignatureFlags, signature: qt.SignatureDeclaration): boolean {
  ...
  function shouldParseReturnType(returnToken: qt.SyntaxKind.ColonToken | qt.SyntaxKind.EqualsGreaterThanToken, isType: boolean): boolean {
    if (returnToken === qt.SyntaxKind.EqualsGreaterThanToken) {
      ...
      parseErrorAtCurrentToken(Diagnostics._0_expected, qs.tokenToString(qt.SyntaxKind.ColonToken));
      nextToken();
      return true;
    }
    return false;
  ...
  function parseParameterList(signature: qt.SignatureDeclaration, flags: SignatureFlags): boolean {
  ...
  function parseSignatureMember(kind: qt.SyntaxKind.CallSignature | qt.SyntaxKind.ConstructSignature): qt.CallSignatureDeclaration | qt.ConstructSignatureDeclaration {
    const node = <qt.CallSignatureDeclaration | qt.ConstructSignatureDeclaration>createNodeWithJSDoc(kind);
  ...
  function parseIndexSignatureDeclaration(node: qt.IndexSignatureDeclaration): qt.IndexSignatureDeclaration {
  ...
  function parsePropertyOrMethodSignature(node: qt.PropertySignature | qt.MethodSignature): qt.PropertySignature | qt.MethodSignature {
    ...
    if (token() === qt.SyntaxKind.OpenParenToken || token() === qt.SyntaxKind.LessThanToken) {
      ...
      // [Yield] nor [Await]
      fillSignature(qt.SyntaxKind.ColonToken, SignatureFlags.Type, <qt.MethodSignature>node);
    } else {
  ...
  function parseTypeMember(): qt.TypeElement {
    if (token() === qt.SyntaxKind.OpenParenToken || token() === qt.SyntaxKind.LessThanToken) {
    ...
    const node = <qt.TypeElement>createNodeWithJSDoc(qt.SyntaxKind.Unknown);
    node.modifiers = parseModifiers();
    if (isIndexSignature()) {
      return parseIndexSignatureDeclaration(node);
    }
    return parsePropertyOrMethodSignature(<qt.PropertySignature | qt.MethodSignature>node);
  }
  ...
  function parseTypeLiteral(): qt.TypeLiteralNode {
    const node = <qt.TypeLiteralNode>createNode(qt.SyntaxKind.TypeLiteral);
    node.members = parseObjectTypeMembers();
    return finishNode(node);
  }

  function parseObjectTypeMembers(): qt.NodeArray<qt.TypeElement> {
    let members: qt.NodeArray<qt.TypeElement>;
    if (parseExpected(qt.SyntaxKind.OpenBraceToken)) {
      members = parseList(ParsingContext.TypeMembers, parseTypeMember);
      parseExpected(qt.SyntaxKind.CloseBraceToken);
    } else {
      members = createMissingList<qt.TypeElement>();
    }
  ...
  function parseMappedType() {
    const node = <qt.MappedTypeNode>createNode(qt.SyntaxKind.MappedType);
    ...
    if (token() === qt.SyntaxKind.ReadonlyKeyword || token() === qt.SyntaxKind.PlusToken || token() === qt.SyntaxKind.MinusToken) {
      node.readonlyToken = parseTokenNode<qt.ReadonlyToken | qt.PlusToken | qt.MinusToken>();
    ...
    if (token() === qt.SyntaxKind.QuestionToken || token() === qt.SyntaxKind.PlusToken || token() === qt.SyntaxKind.MinusToken) {
      node.questionToken = parseTokenNode<qt.QuestionToken | qt.PlusToken | qt.MinusToken>();
  ...
  function parseTupleElementType() {
    ...
    if (parseOptional(qt.SyntaxKind.DotDotDotToken)) {
      const node = <qt.RestTypeNode>createNode(qt.SyntaxKind.RestType, pos);
      node.type = parseType();
      return finishNode(node);
    }
  ...
  function isTupleElementName() {
    if (token() === qt.SyntaxKind.DotDotDotToken) {
      return qs.tokenIsIdentifierOrKeyword(nextToken()) && isNextTokenColonOrQuestionColon();
    }
    return qs.tokenIsIdentifierOrKeyword(token()) && isNextTokenColonOrQuestionColon();
  }

  function parseTupleElementNameOrTupleElementType() {
    if (lookAhead(isTupleElementName)) {
      const node = <qt.NamedTupleMember>createNode(qt.SyntaxKind.NamedTupleMember);
      node.dotDotDotToken = parseOptionalToken(qt.SyntaxKind.DotDotDotToken);
  ...
  function parseTupleType(): qt.TupleTypeNode {
    const node = <qt.TupleTypeNode>createNode(qt.SyntaxKind.TupleType);
  ...
  function parseParenthesizedType(): qt.TypeNode {
    const node = <qt.ParenthesizedTypeNode>createNode(qt.SyntaxKind.ParenthesizedType);
  ...
  function parseFunctionOrConstructorType(): qt.TypeNode {
    const pos = getNodePos();
    const kind = parseOptional(qt.SyntaxKind.NewKeyword) ? qt.SyntaxKind.ConstructorType : qt.SyntaxKind.FunctionType;
    const node = <qt.FunctionOrConstructorTypeNode>createNodeWithJSDoc(kind, pos);
    fillSignature(qt.SyntaxKind.EqualsGreaterThanToken, SignatureFlags.Type, node);
    return finishNode(node);
  }

  function parseKeywordAndNoDot(): qt.TypeNode | undefined {
    const node = parseTokenNode<qt.TypeNode>();
    return token() === qt.SyntaxKind.DotToken ? undefined : node;
  }

  function parseLiteralTypeNode(negative?: boolean): qt.LiteralTypeNode {
    const node = createNode(qt.SyntaxKind.LiteralType) as qt.LiteralTypeNode;
    let unaryMinusExpression!: qt.PrefixUnaryExpression;
    if (negative) {
      unaryMinusExpression = createNode(qt.SyntaxKind.PrefixUnaryExpression) as qt.PrefixUnaryExpression;
      unaryMinusExpression.operator = qt.SyntaxKind.MinusToken;
      nextToken();
    }
    let expression: qt.BooleanLiteral | qt.LiteralExpression | qt.PrefixUnaryExpression = token() === qt.SyntaxKind.TrueKeyword || token() === qt.SyntaxKind.FalseKeyword ? parseTokenNode<qt.BooleanLiteral>() : parseLiteralLikeNode(token());
    if (negative) {
  ...
  function parseImportType(): qt.ImportTypeNode {
    sourceFile.flags |= qt.NodeFlags.PossiblyContainsDynamicImport;
    const node = createNode(qt.SyntaxKind.ImportType) as qt.ImportTypeNode;
  ...
  function parseNonArrayType(): qt.TypeNode {
    switch (token()) {
      ...
      case qt.SyntaxKind.NullKeyword:
        return parseTokenNode<qt.TypeNode>();
  ...
  function parsePostfixTypeOrHigher(): qt.TypeNode {
    ...
    while (!scanner.hasPrecedingLineBreak()) {
      switch (token()) {
        ...
        case qt.SyntaxKind.OpenBracketToken:
          parseExpected(qt.SyntaxKind.OpenBracketToken);
          if (isStartOfType()) {
            const node = createNode(qt.SyntaxKind.IndexedAccessType, type.pos) as qt.IndexedAccessTypeNode;
            node.objectType = type;
            node.indexType = parseType();
            parseExpected(qt.SyntaxKind.CloseBracketToken);
            type = finishNode(node);
          } else {
            const node = createNode(qt.SyntaxKind.ArrayType, type.pos) as qt.ArrayTypeNode;
  ...
  function createPostfixType(kind: qt.SyntaxKind, type: qt.TypeNode) {
    nextToken();
    const postfix = createNode(kind, type.pos) as qt.OptionalTypeNode | qt.JSDocOptionalType | qt.JSDocNonNullableType | qt.JSDocNullableType;
    postfix.type = type;
  ...
  function parseTypeOperator(operator: qt.SyntaxKind.KeyOfKeyword | qt.SyntaxKind.UniqueKeyword | qt.SyntaxKind.ReadonlyKeyword) {
    const node = <qt.TypeOperatorNode>createNode(qt.SyntaxKind.TypeOperator);
  ...
  function parseInferType(): qt.InferTypeNode {
    const node = <qt.InferTypeNode>createNode(qt.SyntaxKind.InferType);
    parseExpected(qt.SyntaxKind.InferKeyword);
    const typeParameter = <qt.TypeParameterDeclaration>createNode(qt.SyntaxKind.TypeParameter);
    typeParameter.name = parseIdentifier();
    node.typeParameter = finishNode(typeParameter);
    return finishNode(node);
  }

  function parseTypeOperatorOrHigher(): qt.TypeNode {
  ...
  function parseUnionOrIntersectionType(kind: qt.SyntaxKind.UnionType | qt.SyntaxKind.IntersectionType, parseConstituentType: () => qt.TypeNode, operator: qt.SyntaxKind.BarToken | qt.SyntaxKind.AmpersandToken): qt.TypeNode {
    ...
    if (token() === operator || hasLeadingOperator) {
      ...
      const node = <qt.UnionOrIntersectionTypeNode>createNode(kind, start);
  ...
  function parseIntersectionTypeOrHigher(): qt.TypeNode {
  ...
  function parseUnionTypeOrHigher(): qt.TypeNode {
  ...
  function parseTypeOrTypePredicate(): qt.TypeNode {
    const typePredicateVariable = isIdentifier() && tryParse(parseTypePredicatePrefix);
    const type = parseType();
    if (typePredicateVariable) {
      const node = <qt.TypePredicateNode>createNode(qt.SyntaxKind.TypePredicate, typePredicateVariable.pos);
  ...
  function parseAssertsTypePredicate(): qt.TypeNode {
    const node = <qt.TypePredicateNode>createNode(qt.SyntaxKind.TypePredicate);
  ...
  function parseTypeWorker(noConditionalTypes?: boolean): qt.TypeNode {
    ...
    if (!noConditionalTypes && !scanner.hasPrecedingLineBreak() && parseOptional(qt.SyntaxKind.ExtendsKeyword)) {
      const node = <qt.ConditionalTypeNode>createNode(qt.SyntaxKind.ConditionalType, type.pos);
      node.checkType = type;
  ...
  function parseTypeAnnotation(): qt.TypeNode | undefined {
  ...
  function parseExpression(): qt.Expression {
    // qt.Expression[in]:
    //      AssignmentExpression[in]
    //      qt.Expression[in] , AssignmentExpression[in]

    // clear the decorator context when parsing qt.Expression, as it should be unambiguous when parsing a decorator
    ...
    let operatorToken: qt.BinaryOperatorToken;
  ...
  function parseInitializer(): qt.Expression | undefined {
    return parseOptional(qt.SyntaxKind.EqualsToken) ? parseAssignmentExpressionOrHigher() : undefined;
  }

  function parseAssignmentExpressionOrHigher(): qt.Expression {
    //  AssignmentExpression[in,yield]:
    //      1) ConditionalExpression[?in,?yield]
    //      2) qt.LeftHandSideExpression = AssignmentExpression[?in,?yield]
    //      3) qt.LeftHandSideExpression AssignmentOperator AssignmentExpression[?in,?yield]
    //      4) qt.ArrowFunctionExpression[?in,?yield]
    //      5) AsyncArrowFunctionExpression[in,yield,await]
    //      6) [+Yield] qt.YieldExpression[?In]
    //
    // Note: for ease of implementation we treat productions '2' and '3' as the same thing.
    // (i.e. they're both qt.BinaryExpressions with an assignment operator in it).

    // First, do the simple check if we have a qt.YieldExpression (production '6').
    ...
    //
    // If we do successfully parse arrow-function, we must *not* recurse for productions 1, 2 or 3. An qt.ArrowFunction is
    // not a qt.LeftHandSideExpression, nor does it start a ConditionalExpression.  So we are done
    ...
    // qt.LeftHandSideExpressions.
    //
    // So, first, we try to just parse out a qt.BinaryExpression.  If we get something that is a
  ...
  function parseYieldExpression(): qt.YieldExpression {
    const node = <qt.YieldExpression>createNode(qt.SyntaxKind.YieldExpression);

    // qt.YieldExpression[In] :
    //      yield
  ...
  function parseSimpleArrowFunctionExpression(identifier: qt.Identifier, asyncModifier?: qt.NodeArray<qt.Modifier> | undefined): qt.ArrowFunction {
    ...
    let node: qt.ArrowFunction;
    if (asyncModifier) {
      node = <qt.ArrowFunction>createNode(qt.SyntaxKind.ArrowFunction, asyncModifier.pos);
      node.modifiers = asyncModifier;
    } else {
      node = <qt.ArrowFunction>createNode(qt.SyntaxKind.ArrowFunction, identifier.pos);
    }
  ...
  function tryParseParenthesizedArrowFunctionExpression(): qt.Expression | undefined {
  ...
  function isParenthesizedArrowFunctionExpressionWorker() {
    ...
    if (first === qt.SyntaxKind.OpenParenToken) {
      ...
      // JSX overrides
      if (sourceFile.languageVariant === qt.LanguageVariant.JSX) {
  ...
  function parsePossibleParenthesizedArrowFunctionExpressionHead(): qt.ArrowFunction | undefined {
  ...
  function tryParseAsyncSimpleArrowFunctionExpression(): qt.ArrowFunction | undefined {
  ...
  function parseParenthesizedArrowFunctionExpressionHead(allowAmbiguity: boolean): qt.ArrowFunction | undefined {
    const node = <qt.ArrowFunction>createNodeWithJSDoc(qt.SyntaxKind.ArrowFunction);
  ...
  function parseArrowFunctionExpressionBody(isAsync: boolean): qt.Block | qt.Expression {
    if (token() === qt.SyntaxKind.OpenBraceToken) {
  ...
  function parseConditionalExpressionRest(leftOperand: qt.Expression): qt.Expression {
    ...
    node.whenFalse = qu.nodeIsPresent(node.colonToken) ? parseAssignmentExpressionOrHigher() : createMissingNode(qt.SyntaxKind.Identifier, /*reportAtCurrentPosition*/ false, Diagnostics._0_expected, qs.tokenToString(qt.SyntaxKind.ColonToken));
    return finishNode(node);
  }

  function parseBinaryExpressionOrHigher(precedence: number): qt.Expression {
  ...
  function parseBinaryExpressionRest(precedence: number, leftOperand: qt.Expression): qt.Expression {
    while (true) {
  ...
  function makeBinaryExpression(left: qt.Expression, operatorToken: qt.BinaryOperatorToken, right: qt.Expression): qt.BinaryExpression {
    const node = <qt.BinaryExpression>createNode(qt.SyntaxKind.BinaryExpression, left.pos);
    node.left = left;
  ...
  function makeAsExpression(left: qt.Expression, right: qt.TypeNode): qt.AsExpression {
    const node = <qt.AsExpression>createNode(qt.SyntaxKind.AsExpression, left.pos);
  ...
  function parsePrefixUnaryExpression() {
    const node = <qt.PrefixUnaryExpression>createNode(qt.SyntaxKind.PrefixUnaryExpression);
    node.operator = token();
    nextToken();
  ...
  function parseDeleteExpression() {
    const node = <qt.DeleteExpression>createNode(qt.SyntaxKind.DeleteExpression);
    nextToken();
  ...
  function parseTypeOfExpression() {
    const node = <qt.TypeOfExpression>createNode(qt.SyntaxKind.TypeOfExpression);
    nextToken();
    node.expression = parseSimpleUnaryExpression();
    return finishNode(node);
  }
  ...
  function parseVoidExpression() {
    const node = <qt.VoidExpression>createNode(qt.SyntaxKind.VoidExpression);
    nextToken();
    node.expression = parseSimpleUnaryExpression();
    return finishNode(node);
  }
  ...
  function parseAwaitExpression() {
    const node = <qt.AwaitExpression>createNode(qt.SyntaxKind.AwaitExpression);
    nextToken();
    node.expression = parseSimpleUnaryExpression();
    return finishNode(node);
  }
   ...
   *      1) qt.UnaryExpression[?Yield]
   *      2) qt.UpdateExpression[?Yield] ** ExponentiationExpression[?Yield]
   *
   */
  function parseUnaryExpressionOrHigher(): qt.UnaryExpression | qt.BinaryExpression {
    /**
     * ES7 qt.UpdateExpression:
     *      1) qt.LeftHandSideExpression[?Yield]
     *      2) qt.LeftHandSideExpression[?Yield][no LineTerminator here]++
     *      3) qt.LeftHandSideExpression[?Yield][no LineTerminator here]--
    ...
    /**
     * ES7 qt.UnaryExpression:
     *      1) qt.UpdateExpression[?yield]
     *      2) delete qt.UpdateExpression[?yield]
     *      3) void qt.UpdateExpression[?yield]
     *      4) typeof qt.UpdateExpression[?yield]
     *      5) + qt.UpdateExpression[?yield]
     *      6) - qt.UpdateExpression[?yield]
     *      7) ~ qt.UpdateExpression[?yield]
     *      8) ! qt.UpdateExpression[?yield]
     */
    const unaryOperator = token();
    const simpleUnaryExpression = parseSimpleUnaryExpression();
    if (token() === qt.SyntaxKind.AsteriskAsteriskToken) {
      const pos = qs.skipTrivia(sourceText, simpleUnaryExpression.pos);
      ...
      if (simpleUnaryExpression.kind === qt.SyntaxKind.TypeAssertionExpression) {
        ...
        parseErrorAt(pos, end, Diagnostics.An_unary_expression_with_the_0_operator_is_not_allowed_in_the_left_hand_side_of_an_exponentiation_expression_Consider_enclosing_the_expression_in_parentheses, qs.tokenToString(unaryOperator));
      }
    }
   ...
   * ES7 qt.UnaryExpression:
   *      1) qt.UpdateExpression[?yield]
   *      2) delete qt.UnaryExpression[?yield]
   *      3) void qt.UnaryExpression[?yield]
   *      4) typeof qt.UnaryExpression[?yield]
   *      5) + qt.UnaryExpression[?yield]
   *      6) - qt.UnaryExpression[?yield]
   *      7) ~ qt.UnaryExpression[?yield]
   *      8) ! qt.UnaryExpression[?yield]
   *      9) [+Await] await qt.UnaryExpression[?yield]
   */
  function parseSimpleUnaryExpression(): qt.UnaryExpression {
    switch (token()) {
      case qt.SyntaxKind.PlusToken:
      case qt.SyntaxKind.MinusToken:
      case qt.SyntaxKind.TildeToken:
      ...
      case qt.SyntaxKind.LessThanToken:
        // This is modified qt.UnaryExpression grammar in TypeScript
        //  qt.UnaryExpression (modified):
        //      < type > qt.UnaryExpression
   ...
   *
   * ES7 qt.UpdateExpression:
   *      qt.LeftHandSideExpression[?Yield]
   *      qt.LeftHandSideExpression[?Yield][no LineTerminator here]++
   *      qt.LeftHandSideExpression[?Yield][no LineTerminator here]--
  ...
  function isUpdateExpression(): boolean {
    ...
    switch (token()) {
      ...
      case qt.SyntaxKind.AwaitKeyword:
        return false;
      case qt.SyntaxKind.LessThanToken:
        // If we are not in JSX context, we are parsing qt.TypeAssertion which is an qt.UnaryExpression
        if (sourceFile.languageVariant !== qt.LanguageVariant.JSX) {
          return false;
        }
  ...
  /**
   * Parse ES7 qt.UpdateExpression. qt.UpdateExpression is used instead of ES6's PostFixExpression.
   *
   * ES7 qt.UpdateExpression[yield]:
   *      1) qt.LeftHandSideExpression[?yield]
   *      2) qt.LeftHandSideExpression[?yield] [[no LineTerminator here]]++
   *      3) qt.LeftHandSideExpression[?yield] [[no LineTerminator here]]--
   *      4) ++LeftHandSideExpression[?yield]
   *      5) --LeftHandSideExpression[?yield]
   * In TypeScript (2), (3) are parsed as qt.PostfixUnaryExpression. (4), (5) are parsed as qt.PrefixUnaryExpression
   */
  function parseUpdateExpression(): qt.UpdateExpression {
    if (token() === qt.SyntaxKind.PlusPlusToken || token() === qt.SyntaxKind.MinusMinusToken) {
      ...
      return finishNode(node);
    } else if (sourceFile.languageVariant === qt.LanguageVariant.JSX && token() === qt.SyntaxKind.LessThanToken && lookAhead(nextTokenIsIdentifierOrKeywordOrGreaterThan)) {
    ...
    if ((token() === qt.SyntaxKind.PlusPlusToken || token() === qt.SyntaxKind.MinusMinusToken) && !scanner.hasPrecedingLineBreak()) {
      const node = <qt.PostfixUnaryExpression>createNode(qt.SyntaxKind.PostfixUnaryExpression, expression.pos);
  ...
  function parseLeftHandSideExpressionOrHigher(): qt.LeftHandSideExpression {
    // Original Ecma:
    // qt.LeftHandSideExpression: See 11.2
    //      qt.NewExpression
    //      CallExpression
    //
    // Our simplification:
    ...
    //      CallExpression
    //
    // See comment in parseMemberExpressionOrHigher on how we replaced qt.NewExpression with
    // qt.MemberExpression to make our lives easier.
    //
    // to best understand the below code, it's important to see how CallExpression expands
    // out into its own productions:
    //
    // CallExpression:
    //      qt.MemberExpression Arguments
    ...
    // 3)we have a qt.MemberExpression which either completes the qt.LeftHandSideExpression,
    ...
    if (token() === qt.SyntaxKind.ImportKeyword) {
      if (lookAhead(nextTokenIsOpenParenOrLessThan)) {
        ...
        const node = createNode(qt.SyntaxKind.MetaProperty, fullStart) as qt.MetaProperty;
  ...
  function parseMemberExpressionOrHigher(): qt.MemberExpression {
    // Note: to make our lives simpler, we decompose the qt.NewExpression productions and
    // place ObjectCreationExpression and qt.FunctionExpression into qt.PrimaryExpression.
    // like so:
    //
    //   qt.PrimaryExpression : See 11.1
    //      this
    //      qt.Identifier
    //      Literal
    //      ArrayLiteral
    ...
    //      new qt.MemberExpression Arguments?
    //
    //   qt.MemberExpression : See 11.2
    //      qt.PrimaryExpression
    //      qt.MemberExpression[Expression]
    //      qt.MemberExpression.IdentifierName
    //
    ...
    //
    // Because there are no other places in the grammar now that refer to qt.FunctionExpression
    // or ObjectCreationExpression, it is safe to push down into the qt.PrimaryExpression
    // production.
    //
    // Because CallExpression and qt.MemberExpression are left recursive, we need to bottom out
    // of the recursion immediately.  So we parse out a primary expression to start with.
    const expression = parsePrimaryExpression();
    return parseMemberExpressionRest(expression, /*allowOptionalChain*/ true);
  }

  function parseSuperExpression(): qt.MemberExpression {
    ...
    const node = <qt.PropertyAccessExpression>createNode(qt.SyntaxKind.PropertyAccessExpression, expression.pos);
  ...
  function parseJsxElementOrSelfClosingElementOrFragment(inExpressionContext: boolean): qt.JsxElement | qt.JsxSelfClosingElement | qt.JsxFragment {
    const opening = parseJsxOpeningOrSelfClosingElementOrOpeningFragment(inExpressionContext);
    let result: qt.JsxElement | qt.JsxSelfClosingElement | qt.JsxFragment;
    if (opening.kind === qt.SyntaxKind.JsxOpeningElement) {
      const node = <qt.JsxElement>createNode(qt.SyntaxKind.JsxElement, opening.pos);
      ...
      if (!tagNamesAreEquivalent(node.openingElement.tagName, node.closingElement.tagName)) {
        parseErrorAtRange(node.closingElement, Diagnostics.Expected_corresponding_JSX_closing_tag_for_0, qu.getTextOfNodeFromSourceText(sourceText, node.openingElement.tagName));
      }

      result = finishNode(node);
    } else if (opening.kind === qt.SyntaxKind.JsxOpeningFragment) {
      const node = <qt.JsxFragment>createNode(qt.SyntaxKind.JsxFragment, opening.pos);
    ...
    if (inExpressionContext && token() === qt.SyntaxKind.LessThanToken) {
      ...
      if (invalidElement) {
        ...
        const badNode = <qt.BinaryExpression>createNode(qt.SyntaxKind.BinaryExpression, result.pos);
  ...
  function parseJsxText(): qt.JsxText {
    const node = <qt.JsxText>createNode(qt.SyntaxKind.JsxText);
    node.text = scanner.getTokenValue();
  ...
  function parseJsxChild(openingTag: qt.JsxOpeningElement | qt.JsxOpeningFragment, token: qt.JsxTokenSyntaxKind): qt.JsxChild | undefined {
    switch (token) {
      case qt.SyntaxKind.EndOfFileToken:
        ...
        if (isJsxOpeningFragment(openingTag)) {
          ...
          const start = qs.skipTrivia(sourceText, tag.pos);
          parseErrorAt(start, tag.end, Diagnostics.JSX_element_0_has_no_corresponding_closing_tag, qu.getTextOfNodeFromSourceText(sourceText, openingTag.tagName));
        }
        return undefined;
  ...
  function parseJsxChildren(openingTag: qt.JsxOpeningElement | qt.JsxOpeningFragment): qt.NodeArray<qt.JsxChild> {
    const list = [];
    const listPos = getNodePos();
    const saveParsingContext = parsingContext;
  ...
  function parseJsxAttributes(): qt.JsxAttributes {
  ...
  function parseJsxOpeningOrSelfClosingElementOrOpeningFragment(inExpressionContext: boolean): qt.JsxOpeningElement | qt.JsxSelfClosingElement | qt.JsxOpeningFragment {
    const fullStart = scanner.getStartPos();
    ...
    if (token() === qt.SyntaxKind.GreaterThanToken) {
      ...
      const node: qt.JsxOpeningFragment = <qt.JsxOpeningFragment>createNode(qt.SyntaxKind.JsxOpeningFragment, fullStart);
      scanJsxText();
      return finishNode(node);
    }
    ...
    let node: qt.JsxOpeningLikeElement;

    if (token() === qt.SyntaxKind.GreaterThanToken) {
      // Closing tag, so scan the immediately-following text with the JSX scanning instead
      // of regular scanning to avoid treating illegal characters (e.g. '#') as immediate
      // scanning errors
      node = <qt.JsxOpeningElement>createNode(qt.SyntaxKind.JsxOpeningElement, fullStart);
      scanJsxText();
    } else {
      ...
      node = <qt.JsxSelfClosingElement>createNode(qt.SyntaxKind.JsxSelfClosingElement, fullStart);
    }
  ...
  function parseJsxElementName(): qt.JsxTagNameExpression {
    scanJsxIdentifier();
    // qt.JsxElement can have name in the form of
    //      propertyAccessExpression
    //      primaryExpression in the form of an identifier and "this" keyword
    // We can't just simply use parseLeftHandSideExpressionOrHigher because then we will start consider class,function etc as a keyword
    // We only want to consider "this" as a primaryExpression
    let expression: qt.JsxTagNameExpression = token() === qt.SyntaxKind.ThisKeyword ? parseTokenNode<qt.ThisExpression>() : parseIdentifierName();
    while (parseOptional(qt.SyntaxKind.DotToken)) {
  ...
  function parseJsxExpression(inExpressionContext: boolean): qt.JsxExpression | undefined {
    const node = <qt.JsxExpression>createNode(qt.SyntaxKind.JsxExpression);
  ...
  function parseJsxAttribute(): qt.JsxAttribute | qt.JsxSpreadAttribute {
    if (token() === qt.SyntaxKind.OpenBraceToken) {
      return parseJsxSpreadAttribute();
    }

    scanJsxIdentifier();
    const node = <qt.JsxAttribute>createNode(qt.SyntaxKind.JsxAttribute);
    node.name = parseIdentifierName();
    if (token() === qt.SyntaxKind.EqualsToken) {
  ...
  function parseJsxSpreadAttribute(): qt.JsxSpreadAttribute {
    const node = <qt.JsxSpreadAttribute>createNode(qt.SyntaxKind.JsxSpreadAttribute);
    parseExpected(qt.SyntaxKind.OpenBraceToken);
  ...
  function parseJsxClosingElement(inExpressionContext: boolean): qt.JsxClosingElement {
    const node = <qt.JsxClosingElement>createNode(qt.SyntaxKind.JsxClosingElement);
  ...
  function parseJsxClosingFragment(inExpressionContext: boolean): qt.JsxClosingFragment {
    const node = <qt.JsxClosingFragment>createNode(qt.SyntaxKind.JsxClosingFragment);
    parseExpected(qt.SyntaxKind.LessThanSlashToken);
    if (qs.tokenIsIdentifierOrKeyword(token())) {
  ...
  function parseTypeAssertion(): qt.TypeAssertion {
    const node = <qt.TypeAssertion>createNode(qt.SyntaxKind.TypeAssertionExpression);
    parseExpected(qt.SyntaxKind.LessThanToken);
    node.type = parseType();
    parseExpected(qt.SyntaxKind.GreaterThanToken);
    node.expression = parseSimpleUnaryExpression();
  ...
  function nextTokenIsIdentifierOrKeywordOrOpenBracketOrTemplate() {
    nextToken();
    return qs.tokenIsIdentifierOrKeyword(token()) || token() === qt.SyntaxKind.OpenBracketToken || isTemplateStartOfTaggedTemplate();
  }
  ...
  function tryReparseOptionalChain(node: qt.Expression) {
  ...
  function parsePropertyAccessExpressionRest(expression: qt.LeftHandSideExpression, questionDotToken: qt.QuestionDotToken | undefined) {
  ...
  function parseElementAccessExpressionRest(expression: qt.LeftHandSideExpression, questionDotToken: qt.QuestionDotToken | undefined) {
  ...
  function parseMemberExpressionRest(expression: qt.LeftHandSideExpression, allowOptionalChain: boolean): qt.MemberExpression {
    while (true) {
      ...
      if (allowOptionalChain && isStartOfOptionalPropertyOrElementAccessChain()) {
        ...
        isPropertyAccess = qs.tokenIsIdentifierOrKeyword(token());
      } else {
      ...
      // when in the [Decorator] context, we do not parse ElementAccess as it could be part of a qt.ComputedPropertyName
  ...
  function parseTaggedTemplateRest(tag: qt.LeftHandSideExpression, questionDotToken: qt.QuestionDotToken | undefined, typeArguments: qt.NodeArray<qt.TypeNode> | undefined) {
  ...
  function parseCallExpressionRest(expression: qt.LeftHandSideExpression): qt.LeftHandSideExpression {
    while (true) {
  ...
  function parsePrimaryExpression(): qt.PrimaryExpression {
    switch (token()) {
      case qt.SyntaxKind.NumericLiteral:
      case qt.SyntaxKind.BigIntLiteral:
      case qt.SyntaxKind.StringLiteral:
  ...
  function parseParenthesizedExpression(): qt.ParenthesizedExpression {
    const node = <qt.ParenthesizedExpression>createNodeWithJSDoc(qt.SyntaxKind.ParenthesizedExpression);
    parseExpected(qt.SyntaxKind.OpenParenToken);
    node.expression = allowInAnd(parseExpression);
    parseExpected(qt.SyntaxKind.CloseParenToken);
    return finishNode(node);
  ...
  function parseSpreadElement(): qt.Expression {
    const node = <qt.SpreadElement>createNode(qt.SyntaxKind.SpreadElement);
    parseExpected(qt.SyntaxKind.DotDotDotToken);
    node.expression = parseAssignmentExpressionOrHigher();
    return finishNode(node);
  }

  function parseArgumentOrArrayLiteralElement(): qt.Expression {
  ...
  function parseArgumentExpression(): qt.Expression {
    return doOutsideOfContext(disallowInAndDecoratorContext, parseArgumentOrArrayLiteralElement);
  }

  function parseArrayLiteralExpression(): qt.ArrayLiteralExpression {
    const node = <qt.ArrayLiteralExpression>createNode(qt.SyntaxKind.ArrayLiteralExpression);
    parseExpected(qt.SyntaxKind.OpenBracketToken);
    if (scanner.hasPrecedingLineBreak()) {
      node.multiLine = true;
    }
  ...
  function parseObjectLiteralElement(): qt.ObjectLiteralElementLike {
    const node = <qt.ObjectLiteralElementLike>createNodeWithJSDoc(qt.SyntaxKind.Unknown);
  ...
  function parseObjectLiteralExpression(): qt.ObjectLiteralExpression {
    const node = <qt.ObjectLiteralExpression>createNode(qt.SyntaxKind.ObjectLiteralExpression);
  ...
  function parseFunctionExpression(): qt.FunctionExpression {
    // GeneratorExpression:
  ...
  function parseNewExpressionOrNewDotTarget(): qt.NewExpression | qt.MetaProperty {
    const fullStart = scanner.getStartPos();
    parseExpected(qt.SyntaxKind.NewKeyword);
    if (parseOptional(qt.SyntaxKind.DotToken)) {
      const node = <qt.MetaProperty>createNode(qt.SyntaxKind.MetaProperty, fullStart);
      node.keywordToken = qt.SyntaxKind.NewKeyword;
      node.name = parseIdentifierName();
      return finishNode(node);
    }

    let expression: qt.MemberExpression = parsePrimaryExpression();
    let typeArguments;
    while (true) {
    ...
    const node = <qt.NewExpression>createNode(qt.SyntaxKind.NewExpression, fullStart);
    node.expression = expression;
    node.typeArguments = typeArguments;
  ...
  // STATEMENTS
  function parseBlock(ignoreMissingOpenBrace: boolean, diagnosticMessage?: qt.DiagnosticMessage): qt.Block {
    const node = <qt.Block>createNode(qt.SyntaxKind.Block);
    const openBracePosition = scanner.getTokenPos();
    if (parseExpected(qt.SyntaxKind.OpenBraceToken, diagnosticMessage) || ignoreMissingOpenBrace) {
      ...
      node.statements = createMissingList<qt.Statement>();
    }
    return finishNode(node);
  }

  function parseFunctionBlock(flags: SignatureFlags, diagnosticMessage?: qt.DiagnosticMessage): qt.Block {
    const savedYieldContext = inYieldContext();
    setYieldContext(!!(flags & SignatureFlags.Yield));
  ...
  function parseEmptyStatement(): qt.Statement {
    const node = <qt.Statement>createNode(qt.SyntaxKind.EmptyStatement);
    parseExpected(qt.SyntaxKind.SemicolonToken);
    return finishNode(node);
  }

  function parseIfStatement(): qt.IfStatement {
    const node = <qt.IfStatement>createNode(qt.SyntaxKind.IfStatement);
  ...
  function parseDoStatement(): qt.DoStatement {
    const node = <qt.DoStatement>createNode(qt.SyntaxKind.DoStatement);
  ...
  function parseWhileStatement(): qt.WhileStatement {
    const node = <qt.WhileStatement>createNode(qt.SyntaxKind.WhileStatement);
    parseExpected(qt.SyntaxKind.WhileKeyword);
    parseExpected(qt.SyntaxKind.OpenParenToken);
    node.expression = allowInAnd(parseExpression);
    parseExpected(qt.SyntaxKind.CloseParenToken);
  ...
  function parseForOrForInOrForOfStatement(): qt.Statement {
    const pos = getNodePos();
    parseExpected(qt.SyntaxKind.ForKeyword);
    const awaitToken = parseOptionalToken(qt.SyntaxKind.AwaitKeyword);
    parseExpected(qt.SyntaxKind.OpenParenToken);

    let initializer!: qt.VariableDeclarationList | qt.Expression;
    if (token() !== qt.SyntaxKind.SemicolonToken) {
    ...
    let forOrForInOrForOfStatement: qt.IterationStatement;
    if (awaitToken ? parseExpected(qt.SyntaxKind.OfKeyword) : parseOptional(qt.SyntaxKind.OfKeyword)) {
      const forOfStatement = <qt.ForOfStatement>createNode(qt.SyntaxKind.ForOfStatement, pos);
      ...
      const forInStatement = <qt.ForInStatement>createNode(qt.SyntaxKind.ForInStatement, pos);
      forInStatement.initializer = initializer;
      forInStatement.expression = allowInAnd(parseExpression);
      parseExpected(qt.SyntaxKind.CloseParenToken);
      forOrForInOrForOfStatement = forInStatement;
    } else {
      const forStatement = <qt.ForStatement>createNode(qt.SyntaxKind.ForStatement, pos);
  ...
  function parseBreakOrContinueStatement(kind: qt.SyntaxKind): qt.BreakOrContinueStatement {
    const node = <qt.BreakOrContinueStatement>createNode(kind);
  ...
  function parseReturnStatement(): qt.ReturnStatement {
    const node = <qt.ReturnStatement>createNode(qt.SyntaxKind.ReturnStatement);
  ...
  function parseWithStatement(): qt.WithStatement {
    const node = <qt.WithStatement>createNode(qt.SyntaxKind.WithStatement);
  ...
  function parseCaseClause(): qt.CaseClause {
    const node = <qt.CaseClause>createNode(qt.SyntaxKind.CaseClause);
  ...
  function parseDefaultClause(): qt.DefaultClause {
    const node = <qt.DefaultClause>createNode(qt.SyntaxKind.DefaultClause);
  ...
  function parseCaseOrDefaultClause(): qt.CaseOrDefaultClause {
    return token() === qt.SyntaxKind.CaseKeyword ? parseCaseClause() : parseDefaultClause();
  }

  function parseSwitchStatement(): qt.SwitchStatement {
    const node = <qt.SwitchStatement>createNode(qt.SyntaxKind.SwitchStatement);
  ...
  function parseThrowStatement(): qt.ThrowStatement {
    // qt.ThrowStatement[Yield] :
    ...
    // grammar walker.
    const node = <qt.ThrowStatement>createNode(qt.SyntaxKind.ThrowStatement);
  ...
  function parseTryStatement(): qt.TryStatement {
    const node = <qt.TryStatement>createNode(qt.SyntaxKind.TryStatement);
  ...
  function parseCatchClause(): qt.CatchClause {
    const result = <qt.CatchClause>createNode(qt.SyntaxKind.CatchClause);
  ...
  function parseDebuggerStatement(): qt.Statement {
    const node = <qt.Statement>createNode(qt.SyntaxKind.DebuggerStatement);
    parseExpected(qt.SyntaxKind.DebuggerKeyword);
    parseSemicolon();
    return finishNode(node);
  }

  function parseExpressionOrLabeledStatement(): qt.ExpressionStatement | qt.LabeledStatement {
    // Avoiding having to do the lookahead for a labeled statement by just trying to parse
    // out an expression, seeing if it is identifier and then seeing if it is followed by
    // a colon.
    const node = <qt.ExpressionStatement | qt.LabeledStatement>createNodeWithJSDoc(token() === qt.SyntaxKind.Identifier ? qt.SyntaxKind.Unknown : qt.SyntaxKind.ExpressionStatement);
    ...
    if (expression.kind === qt.SyntaxKind.Identifier && parseOptional(qt.SyntaxKind.ColonToken)) {
      ...
      (<qt.LabeledStatement>node).label = expression;
      (<qt.LabeledStatement>node).statement = parseStatement();
    } else {
      node.kind = qt.SyntaxKind.ExpressionStatement;
      (<qt.ExpressionStatement>node).expression = expression;
      parseSemicolon();
    }
    return finishNode(node);
  }
  ...
  function nextTokenIsIdentifierOrKeywordOnSameLine() {
    nextToken();
    return qs.tokenIsIdentifierOrKeyword(token()) && !scanner.hasPrecedingLineBreak();
  }
  ...
  function nextTokenIsIdentifierOrKeywordOrLiteralOnSameLine() {
    nextToken();
    return (qs.tokenIsIdentifierOrKeyword(token()) || token() === qt.SyntaxKind.NumericLiteral || token() === qt.SyntaxKind.BigIntLiteral || token() === qt.SyntaxKind.StringLiteral) && !scanner.hasPrecedingLineBreak();
  }
  ...
  function isDeclaration(): boolean {
    while (true) {
      switch (token()) {
        ...
        case qt.SyntaxKind.ImportKeyword:
          nextToken();
          return token() === qt.SyntaxKind.StringLiteral || token() === qt.SyntaxKind.AsteriskToken || token() === qt.SyntaxKind.OpenBraceToken || qs.tokenIsIdentifierOrKeyword(token());
        case qt.SyntaxKind.ExportKeyword:
  ...
  function parseStatement(): qt.Statement {
    switch (token()) {
      case qt.SyntaxKind.SemicolonToken:
      ...
      case qt.SyntaxKind.VarKeyword:
        return parseVariableStatement(<qt.VariableStatement>createNodeWithJSDoc(qt.SyntaxKind.VariableDeclaration));
      case qt.SyntaxKind.LetKeyword:
        if (isLetDeclaration()) {
          return parseVariableStatement(<qt.VariableStatement>createNodeWithJSDoc(qt.SyntaxKind.VariableDeclaration));
        }
      ...
      case qt.SyntaxKind.ClassKeyword:
        return parseClassDeclaration(<qt.ClassDeclaration>createNodeWithJSDoc(qt.SyntaxKind.ClassDeclaration));
      case qt.SyntaxKind.IfKeyword:
  ...
  function isDeclareModifier(modifier: qt.Modifier) {
    return modifier.kind === qt.SyntaxKind.DeclareKeyword;
  }

  function parseDeclaration(): qt.Statement {
    const modifiers = lookAhead(() => (parseDecorators(), parseModifiers()));
    // `parseListElement` attempted to get the reused node at this position,
    // but the ambient context flag was not yet set, so the node appeared
    // not reusable in that context.
    const isAmbient = qc.some(modifiers, isDeclareModifier);
    if (isAmbient) {
    ...
    const node = <qt.Statement>createNodeWithJSDoc(qt.SyntaxKind.Unknown);
    node.decorators = parseDecorators();
    node.modifiers = parseModifiers();
    if (isAmbient) {
  ...
  function tryReuseAmbientDeclaration(): qt.Statement | undefined {
  ...
  function parseDeclarationWorker(node: qt.Statement): qt.Statement {
    switch (token()) {
      case qt.SyntaxKind.VarKeyword:
      case qt.SyntaxKind.LetKeyword:
      case qt.SyntaxKind.ConstKeyword:
      ...
      case qt.SyntaxKind.ImportKeyword:
        return parseImportDeclarationOrImportEqualsDeclaration(<qt.ImportDeclaration | qt.ImportEqualsDeclaration>node);
      case qt.SyntaxKind.ExportKeyword:
        nextToken();
        switch (token()) {
          case qt.SyntaxKind.DefaultKeyword:
      ...
      default:
        if (node.decorators || node.modifiers) {
          ...
          const missing = createMissingNode<qt.Statement>(qt.SyntaxKind.MissingDeclaration, /*reportAtCurrentPosition*/ true, Diagnostics.Declaration_expected);
  ...
  function parseFunctionBlockOrSemicolon(flags: SignatureFlags, diagnosticMessage?: qt.DiagnosticMessage): qt.Block | undefined {
  ...
  function parseArrayBindingElement(): qt.ArrayBindingElement {
    if (token() === qt.SyntaxKind.CommaToken) {
      return <qt.OmittedExpression>createNode(qt.SyntaxKind.OmittedExpression);
    }
    const node = <qt.BindingElement>createNode(qt.SyntaxKind.BindingElement);
    node.dotDotDotToken = parseOptionalToken(qt.SyntaxKind.DotDotDotToken);
  ...
  function parseObjectBindingPattern(): qt.ObjectBindingPattern {
    const node = <qt.ObjectBindingPattern>createNode(qt.SyntaxKind.ObjectBindingPattern);
    parseExpected(qt.SyntaxKind.OpenBraceToken);
  ...
  function parseArrayBindingPattern(): qt.ArrayBindingPattern {
    const node = <qt.ArrayBindingPattern>createNode(qt.SyntaxKind.ArrayBindingPattern);
    parseExpected(qt.SyntaxKind.OpenBracketToken);
  ...
  function parseVariableDeclaration(allowExclamation?: boolean): qt.VariableDeclaration {
    const node = <qt.VariableDeclaration>createNode(qt.SyntaxKind.VariableDeclaration);
    ...
    if (allowExclamation && node.name.kind === qt.SyntaxKind.Identifier && token() === qt.SyntaxKind.ExclamationToken && !scanner.hasPrecedingLineBreak()) {
      node.exclamationToken = parseTokenNode<qt.Token<qt.SyntaxKind.ExclamationToken>>();
    }
    node.type = parseTypeAnnotation();
    if (!isInOrOfKeyword(token())) {
      node.initializer = parseInitializer();
  ...
  function parseVariableDeclarationList(inForStatementInitializer: boolean): qt.VariableDeclarationList {
    const node = <qt.VariableDeclarationList>createNode(qt.SyntaxKind.VariableDeclarationList);
    ...
    if (token() === qt.SyntaxKind.OfKeyword && lookAhead(canFollowContextualOfKeyword)) {
      node.declarations = createMissingList<qt.VariableDeclaration>();
    } else {
  ...
  function parseVariableStatement(node: qt.VariableStatement): qt.VariableStatement {
  ...
  function parseFunctionDeclaration(node: qt.FunctionDeclaration): qt.FunctionDeclaration {
  ...
  function parseMethodDeclaration(node: qt.MethodDeclaration, asteriskToken: qt.AsteriskToken, diagnosticMessage?: qt.DiagnosticMessage): qt.MethodDeclaration {
  ...
  function parsePropertyDeclaration(node: qt.PropertyDeclaration): qt.PropertyDeclaration {
  ...
  function parsePropertyOrMethodDeclaration(node: qt.PropertyDeclaration | qt.MethodDeclaration): qt.PropertyDeclaration | qt.MethodDeclaration {
    const asteriskToken = parseOptionalToken(qt.SyntaxKind.AsteriskToken);
    node.name = parsePropertyName();
  ...
  function parseAccessorDeclaration(node: qt.AccessorDeclaration, kind: qt.AccessorDeclaration['kind']): qt.AccessorDeclaration {
    node.kind = kind;
    node.name = parsePropertyName();
    fillSignature(qt.SyntaxKind.ColonToken, SignatureFlags.None, node);
   ...
   */
  function parseModifiers(permitInvalidConstAsModifier?: boolean): qt.NodeArray<qt.Modifier> | undefined {
    let list: qt.Modifier[] | undefined;
    const listPos = getNodePos();
    while (true) {
      ...
      const modifier = finishNode(<qt.Modifier>createNode(modifierKind, modifierStart));
      (list || (list = [])).push(modifier);
    }
    return list && createNodeArray(list, listPos);
  }

  function parseModifiersForArrowFunction(): qt.NodeArray<qt.Modifier> | undefined {
    let modifiers: qt.NodeArray<qt.Modifier> | undefined;
    if (token() === qt.SyntaxKind.AsyncKeyword) {
      const modifierStart = scanner.getStartPos();
      const modifierKind = token();
      nextToken();
      const modifier = finishNode(<qt.Modifier>createNode(modifierKind, modifierStart));
      modifiers = createNodeArray<qt.Modifier>([modifier], modifierStart);
    }
    return modifiers;
  }

  function parseClassElement(): qt.ClassElement {
    if (token() === qt.SyntaxKind.SemicolonToken) {
      const result = <qt.SemicolonClassElement>createNode(qt.SyntaxKind.SemicolonClassElement);
      nextToken();
      return finishNode(result);
    }

    const node = <qt.ClassElement>createNodeWithJSDoc(qt.SyntaxKind.Unknown);
    node.decorators = parseDecorators();
    ...
    if (qs.tokenIsIdentifierOrKeyword(token()) || token() === qt.SyntaxKind.StringLiteral || token() === qt.SyntaxKind.NumericLiteral || token() === qt.SyntaxKind.AsteriskToken || token() === qt.SyntaxKind.OpenBracketToken) {
      const isAmbient = node.modifiers && qc.some(node.modifiers, isDeclareModifier);
      if (isAmbient) {
        for (const m of node.modifiers!) {
          m.flags |= qt.NodeFlags.Ambient;
        }
        return doInsideOfContext(qt.NodeFlags.Ambient, () => parsePropertyOrMethodDeclaration(node as qt.PropertyDeclaration | qt.MethodDeclaration));
      } else {
        return parsePropertyOrMethodDeclaration(node as qt.PropertyDeclaration | qt.MethodDeclaration);
      }
    }
  ...
  function parseClassExpression(): qt.ClassExpression {
  ...
  function parseClassDeclaration(node: qt.ClassLikeDeclaration): qt.ClassDeclaration {
    return parseClassDeclarationOrExpression(node, qt.SyntaxKind.ClassDeclaration);
  }

  function parseClassDeclarationOrExpression(node: qt.ClassLikeDeclaration, kind: qt.ClassLikeDeclaration['kind']): qt.ClassLikeDeclaration {
    node.kind = kind;
    ...
    if (parseExpected(qt.SyntaxKind.OpenBraceToken)) {
      ...
      node.members = createMissingList<qt.ClassElement>();
    }
  ...
  function parseHeritageClauses(): qt.NodeArray<qt.HeritageClause> | undefined {
    // ClassTail[Yield,Await] : (Modified) See 14.5
    //      ClassHeritage[?Yield,?Await]opt { ClassBody[?Yield,?Await]opt }
  ...
  function parseHeritageClause(): qt.HeritageClause {
    const tok = token();
    Debug.assert(tok === qt.SyntaxKind.ExtendsKeyword || tok === qt.SyntaxKind.ImplementsKeyword); // isListElement() should ensure this.
    const node = <qt.HeritageClause>createNode(qt.SyntaxKind.HeritageClause);
    node.token = tok;
    nextToken();
  ...
  function parseExpressionWithTypeArguments(): qt.ExpressionWithTypeArguments {
    const node = <qt.ExpressionWithTypeArguments>createNode(qt.SyntaxKind.ExpressionWithTypeArguments);
    node.expression = parseLeftHandSideExpressionOrHigher();
    node.typeArguments = tryParseTypeArguments();
    return finishNode(node);
  }

  function tryParseTypeArguments(): qt.NodeArray<qt.TypeNode> | undefined {
  ...
  function parseClassMembers(): qt.NodeArray<qt.ClassElement> {
    return parseList(ParsingContext.ClassMembers, parseClassElement);
  }

  function parseInterfaceDeclaration(node: qt.InterfaceDeclaration): qt.InterfaceDeclaration {
  ...
  function parseTypeAliasDeclaration(node: qt.TypeAliasDeclaration): qt.TypeAliasDeclaration {
  ...
  function parseEnumMember(): qt.EnumMember {
    const node = <qt.EnumMember>createNodeWithJSDoc(qt.SyntaxKind.EnumMember);
    node.name = parsePropertyName();
    node.initializer = allowInAnd(parseInitializer);
    return finishNode(node);
  }

  function parseEnumDeclaration(node: qt.EnumDeclaration): qt.EnumDeclaration {
  ...
  function parseModuleBlock(): qt.ModuleBlock {
    const node = <qt.ModuleBlock>createNode(qt.SyntaxKind.ModuleBlock);
    if (parseExpected(qt.SyntaxKind.OpenBraceToken)) {
      node.statements = parseList(ParsingContext.BlockStatements, parseStatement);
      parseExpected(qt.SyntaxKind.CloseBraceToken);
    } else {
  ...
  function parseModuleOrNamespaceDeclaration(node: qt.ModuleDeclaration, flags: qt.NodeFlags): qt.ModuleDeclaration {
  ...
  function parseAmbientExternalModuleDeclaration(node: qt.ModuleDeclaration): qt.ModuleDeclaration {
    node.kind = qt.SyntaxKind.ModuleDeclaration;
  ...
  function parseModuleDeclaration(node: qt.ModuleDeclaration): qt.ModuleDeclaration {
  ...
  function parseNamespaceExportDeclaration(node: qt.NamespaceExportDeclaration): qt.NamespaceExportDeclaration {
  ...
  function parseImportDeclarationOrImportEqualsDeclaration(node: qt.ImportEqualsDeclaration | qt.ImportDeclaration): qt.ImportEqualsDeclaration | qt.ImportDeclaration {
    parseExpected(qt.SyntaxKind.ImportKeyword);
    ...
    if (
      ...
      (<qt.ImportDeclaration>node).importClause = parseImportClause(identifier, afterImportPos, isTypeOnly);
      parseExpected(qt.SyntaxKind.FromKeyword);
    }

    (<qt.ImportDeclaration>node).moduleSpecifier = parseModuleSpecifier();
    parseSemicolon();
    return finishNode(node);
  }
  ...
  function tokenAfterImportedIdentifierDefinitelyProducesImportDeclaration() {
    ...
    // an qt.ImportDeclaration or qt.ImportEqualsDeclaration.
    return token() === qt.SyntaxKind.CommaToken || token() === qt.SyntaxKind.FromKeyword;
  }

  function parseImportEqualsDeclaration(node: qt.ImportEqualsDeclaration, identifier: qt.Identifier, isTypeOnly: boolean): qt.ImportEqualsDeclaration {
  ...
  function parseImportClause(identifier: qt.Identifier | undefined, fullStart: number, isTypeOnly: boolean) {
    ...
    //  ImportedDefaultBinding, qt.NamedImports

    const importClause = <qt.ImportClause>createNode(qt.SyntaxKind.ImportClause, fullStart);
  ...
  function parseExternalModuleReference() {
    const node = <qt.ExternalModuleReference>createNode(qt.SyntaxKind.ExternalModuleReference);
  ...
  function parseModuleSpecifier(): qt.Expression {
    if (token() === qt.SyntaxKind.StringLiteral) {
  ...
  function parseNamespaceImport(): qt.NamespaceImport {
    // NameSpaceImport:
    //  * as ImportedBinding
  ...
  function parseNamedImportsOrExports(kind: qt.SyntaxKind.NamedImports): qt.NamedImports;
  function parseNamedImportsOrExports(kind: qt.SyntaxKind.NamedExports): qt.NamedExports;
  function parseNamedImportsOrExports(kind: qt.SyntaxKind): qt.NamedImportsOrExports {
    const node = <qt.NamedImports | qt.NamedExports>createNode(kind);
    ...
    node.elements = <qt.NodeArray<qt.ImportSpecifier> | qt.NodeArray<qt.ExportSpecifier>>parseBracketedList(ParsingContext.ImportOrExportSpecifiers, kind === qt.SyntaxKind.NamedImports ? parseImportSpecifier : parseExportSpecifier, qt.SyntaxKind.OpenBraceToken, qt.SyntaxKind.CloseBraceToken);
    return finishNode(node);
  }
  ...
  function parseImportOrExportSpecifier(kind: qt.SyntaxKind): qt.ImportOrExportSpecifier {
    const node = <qt.ImportSpecifier>createNode(kind);
    // ImportSpecifier:
    //   BindingIdentifier
  ...
  function parseNamespaceExport(pos: number): qt.NamespaceExport {
    const node = <qt.NamespaceExport>createNode(qt.SyntaxKind.NamespaceExport, pos);
    node.name = parseIdentifier();
    return finishNode(node);
  }

  function parseExportDeclaration(node: qt.ExportDeclaration): qt.ExportDeclaration {
  ...
  function parseExportAssignment(node: qt.ExportAssignment): qt.ExportAssignment {
  ...
  function setExternalModuleIndicator(sourceFile: qt.SourceFile) {
    // Try to use the first top-level import/export when available, then
    // fall back to looking for an 'import.meta' somewhere in the tree if necessary.
    sourceFile.externalModuleIndicator = qc.forEach(sourceFile.statements, isAnExternalModuleIndicatorNode) || getImportMetaIfNecessary(sourceFile);
  }
  ...
  function getImportMetaIfNecessary(sourceFile: qt.SourceFile) {
  ...
  function walkTreeForExternalModuleIndicators(node: qt.Node): qt.Node | undefined {
    return qu.isImportMeta(node) ? node : forEachChild(node, walkTreeForExternalModuleIndicators);
  }

  /** Do not use hasModifier inside the parser; it relies on parent pointers. Use this instead. */
  function hasModifierOfKind(node: qt.Node, kind: qt.SyntaxKind) {
    return qc.some(node.modifiers, (m) => m.kind === kind);
  }
  ...
  const enum ParsingContext {
    ...
    BlockStatements, // qt.Statements in block
    SwitchClauses, // Clauses in switch statement
    SwitchClauseStatements, // qt.Statements in switch clause
    ...
    ArgumentExpressions, // qt.Expressions in argument list
  ...
  export namespace JSDocParser {
    export function parseJSDocTypeExpressionForTests(content: string, start: number | undefined, length: number | undefined): { jsDocTypeExpression: qt.JSDocTypeExpression; diagnostics: Diagnostic[] } | undefined {
    ...
    export function parseJSDocTypeExpression(mayOmitBraces?: boolean): qt.JSDocTypeExpression {
      const result = <qt.JSDocTypeExpression>createNode(qt.SyntaxKind.JSDocTypeExpression);
    ...
    export function parseIsolatedJSDocComment(content: string, start: number | undefined, length: number | undefined): { jsDoc: JSDoc; diagnostics: Diagnostic[] } | undefined {
      initializeState(content, qt.ScriptTarget.Latest, /*_syntaxCursor:*/ undefined, qt.ScriptKind.JS);
      sourceFile = <qt.SourceFile>{ languageVariant: qt.LanguageVariant.Standard, text: content };
    ...
    export function parseJSDocComment(parent: qt.HasJSDoc, start: number, length: number): JSDoc | undefined {
      const saveToken = currentToken;
      const saveParseDiagnosticsLength = parseDiagnostics.length;
      const saveParseErrorBeforeNextFinishedNode = parseErrorBeforeNextFinishedNode;
    ...
    function parseJSDocCommentWorker(start = 0, length: number | undefined): JSDoc | undefined {
      ...
      function parseTagComments(indent: number, initialMargin?: string): string | undefined {
        ...
        loop: while (true) {
          switch (tok) {
            ...
            case qt.SyntaxKind.OpenBraceToken:
              state = JSDocState.SavingComments;
              if (lookAhead(() => nextTokenJSDoc() === qt.SyntaxKind.AtToken && qs.tokenIsIdentifierOrKeyword(nextTokenJSDoc()) && scanner.getTokenText() === 'link')) {
                pushComment(scanner.getTokenText());
                nextTokenJSDoc();
                pushComment(scanner.getTokenText());
                nextTokenJSDoc();
      ...
      function addTag(tag: qt.JSDocTag | undefined): void {
        if (!tag) {
          return;
        }
        if (!tags) {
      ...
      function tryParseTypeExpression(): qt.JSDocTypeExpression | undefined {
        skipWhitespaceOrAsterisk();
        return token() === qt.SyntaxKind.OpenBraceToken ? parseJSDocTypeExpression() : undefined;
      }

      function parseBracketNameInPropertyAndParamTag(): { name: qt.EntityName; isBracketed: boolean } {
      ...
      function isObjectOrObjectArrayTypeReference(node: qt.TypeNode): boolean {
        switch (node.kind) {
          case qt.SyntaxKind.ObjectKeyword:
            return true;
          case qt.SyntaxKind.ArrayType:
      ...
      function parseNestedTypeLiteral(typeExpression: qt.JSDocTypeExpression | undefined, name: qt.EntityName, target: PropertyLikeParse, indent: number) {
        if (typeExpression && isObjectOrObjectArrayTypeReference(typeExpression.type)) {
          const typeLiteralExpression = <qt.JSDocTypeExpression>createNode(qt.SyntaxKind.JSDocTypeExpression, scanner.getTokenPos());
          let child: JSDocPropertyLikeTag | qt.JSDocTypeTag | false;
      ...
      function parseReturnTag(start: number, tagName: qt.Identifier): qt.JSDocReturnTag {
      ...
      function parseTypeTag(start: number, tagName: qt.Identifier): qt.JSDocTypeTag {
      ...
      function parseExpressionWithTypeArgumentsForAugments(): qt.ExpressionWithTypeArguments & { expression: qt.Identifier | PropertyAccessEntityNameExpression } {
        const usedBrace = parseOptional(qt.SyntaxKind.OpenBraceToken);
        const node = createNode(qt.SyntaxKind.ExpressionWithTypeArguments) as qt.ExpressionWithTypeArguments & { expression: qt.Identifier | PropertyAccessEntityNameExpression };
      ...
      function parseSimpleTag(start: number, kind: qt.SyntaxKind, tagName: qt.Identifier): qt.JSDocTag {
        const tag = <JSDocTag>createNode(kind, start);
        tag.tagName = tagName;
        return finishNode(tag);
      }

      function parseThisTag(start: number, tagName: qt.Identifier): qt.JSDocThisTag {
      ...
      function parseEnumTag(start: number, tagName: qt.Identifier): qt.JSDocEnumTag {
      ...
      function parseTypedefTag(start: number, tagName: qt.Identifier, indent: number): JSDocTypedefTag {
        ...
        if (!typeExpression || isObjectOrObjectArrayTypeReference(typeExpression.type)) {
          let child: qt.JSDocTypeTag | JSDocPropertyTag | false;
          let jsdocTypeLiteral: JSDocTypeLiteral | undefined;
          let childTypeTag: qt.JSDocTypeTag | undefined;
          while ((child = tryParse(() => parseChildPropertyTag(indent)))) {
            ...
            if (child.kind === qt.SyntaxKind.JSDocTypeTag) {
              if (childTypeTag) {
                ...
                const lastError = qc.lastOrUndefined(parseDiagnostics);
                if (lastError) {
                  addRelatedInfo(lastError, qu.createDiagnosticForNode(sourceFile, Diagnostics.The_tag_was_first_specified_here));
                }
                break;
              } else {
                childTypeTag = child;
        ...
        const returnTag = tryParse(() => {
          if (parseOptionalJsdoc(qt.SyntaxKind.AtToken)) {
            ...
            if (tag && tag.kind === qt.SyntaxKind.JSDocReturnTag) {
              return tag as qt.JSDocReturnTag;
            }
          }
        });
        if (returnTag) {
      ...
      function parseChildParameterOrPropertyTag(target: PropertyLikeParse, indent: number, name?: qt.EntityName): qt.JSDocTypeTag | JSDocPropertyTag | JSDocParameterTag | false {
      ...
      function tryParseChildTag(target: PropertyLikeParse, indent: number): qt.JSDocTypeTag | JSDocPropertyTag | JSDocParameterTag | false {
        Debug.assert(token() === qt.SyntaxKind.AtToken);
        const start = scanner.getStartPos();
        nextTokenJSDoc();
      ...
      function parseTemplateTag(start: number, tagName: qt.Identifier): JSDocTemplateTag {
        ...
        let constraint: qt.JSDocTypeExpression | undefined;
        if (token() === qt.SyntaxKind.OpenBraceToken) {
      ...
      function parseJSDocEntityName(): qt.EntityName {
        let entity: qt.EntityName = parseJSDocIdentifierName();
      ...
      function parseJSDocIdentifierName(message?: qt.DiagnosticMessage): qt.Identifier {
        ...
        result.escapedText = qpu.escapeLeadingUnderscores(internIdentifier(scanner.getTokenValue()));
...
namespace IncrementalParser {
  export function updateSourceFile(sourceFile: qt.SourceFile, newText: string, textChangeRange: qt.TextChangeRange, aggressiveChecks: boolean): qt.SourceFile {
  ...
  function getNewCommentDirectives(oldDirectives: qt.CommentDirective[] | undefined, newDirectives: qt.CommentDirective[] | undefined, changeStart: number, changeRangeOldEnd: number, delta: number, oldText: string, newText: string, aggressiveChecks: boolean): qt.CommentDirective[] | undefined {
  ...
  function extendToAffectedRange(sourceFile: qt.SourceFile, changeRange: qt.TextChangeRange): qt.TextChangeRange {
  ...
  function findNearestNodeStartingBeforeOrAtPosition(sourceFile: qt.SourceFile, position: number): qt.Node {
  ...
  function checkChangeRange(sourceFile: qt.SourceFile, newText: string, textChangeRange: qt.TextChangeRange, aggressiveChecks: boolean) {
    const oldText = sourceFile.text;
    if (textChangeRange) {
  ...
  function createSyntaxCursor(sourceFile: qt.SourceFile): SyntaxCursor {
...
export function processCommentPragmas(context: PragmaContext, sourceText: string): void {
  const pragmas: qt.PragmaPseudoMapEntry[] = [];

  for (const range of qs.getLeadingCommentRanges(sourceText, 0) || qc.emptyArray) {
  ...
  context.pragmas.forEach((entryOrList, key) => {
    ...
    switch (key) {
      case 'reference': {
        ...
        qc.forEach(toArray(entryOrList) as qt.PragmaPseudoMap['reference'][], (arg) => {
      ...
      case 'amd-dependency': {
        context.amdDependencies = map(toArray(entryOrList) as qt.PragmaPseudoMap['amd-dependency'][], (x) => ({ name: x.arguments.name, path: x.arguments.path }));
        break;
      }
      case 'amd-module': {
        if (entryOrList instanceof Array) {
          for (const entry of entryOrList) {
            ...
            context.moduleName = (entry as qt.PragmaPseudoMap['amd-module']).arguments.name;
          }
        } else {
          context.moduleName = (entryOrList as qt.PragmaPseudoMap['amd-module']).arguments.name;
        }
        break;
      }
      case 'ts-nocheck':
      case 'ts-check': {
        ...
        qc.forEach(toArray(entryOrList), (entry) => {
...
function extractPragmas(pragmas: qt.PragmaPseudoMapEntry[], range: qt.CommentRange, text: string) {
  ...
  if (tripleSlash) {
    const name = tripleSlash[1].toLowerCase() as keyof qt.PragmaPseudoMap; // Technically unsafe cast, but we do it so the below check to make it safe typechecks
    const pragma = commentPragmas[name];
    if (!pragma || !(pragma.kind! & qt.PragmaKindFlags.TripleSlashXML)) {
      return;
    }
    if (pragma.args) {
      ...
      pragmas.push({ name, args: { arguments: argument, range } } as qt.PragmaPseudoMapEntry);
    } else {
      pragmas.push({ name, args: { arguments: {}, range } } as qt.PragmaPseudoMapEntry);
    }
    return;
  }
  ...
  if (singleLine) {
    return addPragmaForMatch(pragmas, range, qt.PragmaKindFlags.SingleLine, singleLine);
  }
  ...
  if (range.kind === qt.SyntaxKind.MultiLineCommentTrivia) {
    ...
    while ((multiLineMatch = multiLinePragmaRegEx.exec(text))) {
      addPragmaForMatch(pragmas, range, qt.PragmaKindFlags.MultiLine, multiLineMatch);
    }
  }
}

function addPragmaForMatch(pragmas: qt.PragmaPseudoMapEntry[], range: qt.CommentRange, kind: PragmaKindFlags, match: RegExpExecArray) {
  if (!match) return;
  const name = match[1].toLowerCase() as keyof qt.PragmaPseudoMap; // Technically unsafe cast, but we do it so they below check to make it safe typechecks
  const pragma = commentPragmas[name];
...
function getNamedPragmaArguments(pragma: qt.PragmaDefinition, text: string | undefined): { [index: string]: string } | 'fail' {
  if (!text) return {};
...
export function tagNamesAreEquivalent(lhs: qt.JsxTagNameExpression, rhs: qt.JsxTagNameExpression): boolean {
  ...
  // take forms of qt.JsxTagNameExpression which includes an identifier, "this" expression, or another propertyAccessExpression
