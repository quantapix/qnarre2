...
export function createCompilerHost(options: qt.CompilerOptions, setParentNodes?: boolean): CompilerHost {
...
export function createCompilerHostWorker(options: qt.CompilerOptions, setParentNodes?: boolean, system = sys): CompilerHost {
  ...
  function writeFileWorker(fileName: string, data: string, writeByteOrderMark: boolean) {
    ...
    if (!outputFingerprints) {
      outputFingerprints = qc.createMap<OutputFingerprint>();
    }
...
export function changeCompilerHostLikeToUseCache(host: CompilerHostLikeForCache, toPath: (fileName: string) => Path, getSourceFile?: CompilerHost['getSourceFile']) {
  ...
  const readFileCache = qc.createMap<string | false>();
  const fileExistsCache = qc.createMap<boolean>();
  const directoryExistsCache = qc.createMap<boolean>();
  const sourceFileCache = qc.createMap<SourceFile>();
...
function formatCodeSpan(file: SourceFile, start: number, length: number, indent: string, squiggleColor: ForegroundColorEscapeSequences, host: FormatDiagnosticsHost) {
  const { line: firstLine, character: firstLineChar } = qs.getLineAndCharacterOfPosition(file, start);
  const { line: lastLine, character: lastLineChar } = qs.getLineAndCharacterOfPosition(file, start + length);
  const lastLineInFile = qs.getLineAndCharacterOfPosition(file, file.text.length).line;
  ...
  for (let i = firstLine; i <= lastLine; i++) {
    ...
    const lineStart = qs.getPositionOfLineAndCharacter(file, i, 0);
    const lineEnd = i < lastLineInFile ? qs.getPositionOfLineAndCharacter(file, i + 1, 0) : file.text.length;
...
export function flattenDiagnosticMessageText(diag: string | qt.DiagnosticMessageChain | undefined, newLine: string, indent = 0): string {
  if (isString(diag)) {
    return diag;
...
interface RefFile extends qt.TextRange {
  kind: RefFileKind;
  index: number;
  file: SourceFile;
}
...
export function isProgramUptoDate(program: Program | undefined, rootFileNames: string[], newOptions: qt.CompilerOptions, getSourceVersion: (path: Path, fileName: string) => string | undefined, fileExists: (fileName: string) => boolean, hasInvalidatedResolution: HasInvalidatedResolution, hasChangedAutomaticTypeDirectiveNames: boolean, projectReferences: readonly qt.ProjectReference[] | undefined): boolean {
  ...
  function projectReferenceUptoDate(oldRef: qt.ProjectReference, newRef: qt.ProjectReference, index: number) {
    if (!qu.projectReferenceIsEqualTo(oldRef, newRef)) {
      return false;
    }
 ...
 */
function shouldProgramCreateNewSourceFiles(program: Program | undefined, newOptions: qt.CompilerOptions): boolean {
...
function createCreateProgramOptions(rootNames: readonly string[], options: qt.CompilerOptions, host?: CompilerHost, oldProgram?: Program, configFileParsingDiagnostics?: readonly Diagnostic[]): CreateProgramOptions {
  return {
    rootNames,
    options,
    host,
 ...
 * @returns A 'Program' object.
 */
export function createProgram(rootNames: readonly string[], options: qt.CompilerOptions, host?: CompilerHost, oldProgram?: Program, configFileParsingDiagnostics?: readonly Diagnostic[]): Program;
export function createProgram(rootNamesOrOptions: readonly string[] | CreateProgramOptions, _options?: qt.CompilerOptions, _host?: CompilerHost, _oldProgram?: Program, _configFileParsingDiagnostics?: readonly Diagnostic[]): Program {
  ...
  let symlinks: qpc.ReadonlyMap<string> | undefined;
  let commonSourceDirectory: string;
  let diagnosticsProducingTypeChecker: TypeChecker;
  let noDiagnosticsTypeChecker: TypeChecker;
  let classifiableNames: qt.UnderscoreEscapedMap<true>;
  const ambientModuleNameToUnmodifiedFileName = qc.createMap<string>();
  ...
  let resolvedTypeReferenceDirectives = qc.createMap<ResolvedTypeReferenceDirective | undefined>();
  ...
  const modulesWithElidedImports = qc.createMap<boolean>();

  // Track source files that are source files found by searching under node_modules, as these shouldn't be compiled.
  const sourceFilesFoundSearchingNodeModules = qc.createMap<boolean>();
  ...
  const hasEmitBlockingDiagnostics = qc.createMap<boolean>();
  let _compilerOptionsObjectLiteralSyntax: ObjectLiteralExpression | null | undefined;

  let moduleResolutionCache: ModuleResolutionCache | undefined;
  let actualResolveModuleNamesWorker: (moduleNames: string[], containingFile: string, reusedNames?: string[], redirectedReference?: ResolvedProjectReference) => qt.ResolvedModuleFull[];
  ...
  if (host.resolveModuleNames) {
    actualResolveModuleNamesWorker = (moduleNames, containingFile, reusedNames, redirectedReference) =>
      host.resolveModuleNames!(Debug.checkEachDefined(moduleNames), containingFile, reusedNames, redirectedReference, options).map((resolved) => {
        ...
        if (!resolved || (resolved as qt.ResolvedModuleFull).extension !== undefined) {
          return resolved as qt.ResolvedModuleFull;
        }
    ...
    actualResolveModuleNamesWorker = (moduleNames, containingFile, _reusedNames, redirectedReference) => loadWithLocalCache<qt.ResolvedModuleFull>(Debug.checkEachDefined(moduleNames), containingFile, redirectedReference, loader);
  }
  ...
  let sourceFileToPackageName = qc.createMap<string>();
  // Key is a file name. Value is the (non-empty, or undefined) list of files that redirect to it.
  let redirectTargetsMap = qc.createMultiMap<string>();
  ...
  function resolveModuleNamesReusingOldState(moduleNames: string[], containingFile: string, file: SourceFile) {
     ...
     * * qt.ResolvedModuleFull instance: can be reused.
     */
    ...
    for (let i = 0; i < result.length; i++) {
      if (result[i]) {
        // `result[i]` is either a `qt.ResolvedModuleFull` or a marker.
    ...
    function moduleNameResolvesToAmbientModuleInNonModifiedFile(moduleName: string): boolean {
      const resolutionToFile = qu.getResolvedModule(oldSourceFile, moduleName);
  ...
  function tryReuseStructureFromOldProgram(): StructureIsReused {
    ...
    if (qu.changesAffectModuleResolution(oldOptions, options)) {
    ...
    const seenPackageNames = qc.createMap<SeenPackageName>();

    for (const oldSourceFile of oldSourceFiles) {
      let newSourceFile = host.getSourceFileByPath ? host.getSourceFileByPath(oldSourceFile.fileName, oldSourceFile.resolvedPath, options.target!, /*onError*/ undefined, shouldCreateNewSourceFile) : host.getSourceFile(oldSourceFile.fileName, options.target!, /*onError*/ undefined, shouldCreateNewSourceFile); // TODO: GH#18217
    ...
    for (const { oldFile: oldSourceFile, newFile: newSourceFile } of modifiedSourceFiles) {
      ...
      const resolutionsChanged = qu.hasChangesInResolutions(moduleNames, resolutions, oldSourceFile.resolvedModules, moduleResolutionIsEqualTo);
      ...
      if (resolveTypeReferenceDirectiveNamesWorker) {
        ...
        const resolutionsChanged = qu.hasChangesInResolutions(typesReferenceDirectives, resolutions, oldSourceFile.resolvedTypeReferenceDirectiveNames, typeDirectiveIsEqualTo);
        if (resolutionsChanged) {
          oldProgram.structureIsReused = StructureIsReused.SafeModules;
  ...
  function emitBuildInfo(writeFileCallback?: WriteFileCallback): EmitResult {
    ...
    const emitResult = emitFiles(notImplementedResolver, getEmitHost(writeFileCallback), /*targetSourceFile*/ undefined, /*transformers*/ noTransformers, /*emitOnlyDtsFiles*/ false, /*onlyBuildInfo*/ true);
  ...
  function emitWorker(program: Program, sourceFile: SourceFile | undefined, writeFileCallback: WriteFileCallback | undefined, cancellationToken: CancellationToken | undefined, emitOnlyDtsFiles?: boolean, customTransformers?: CustomTransformers, forceDtsEmit?: boolean): EmitResult {
    ...
    const emitResult = emitFiles(emitResolver, getEmitHost(writeFileCallback), sourceFile, getTransformers(options, customTransformers, emitOnlyDtsFiles), emitOnlyDtsFiles, /*onlyBuildInfo*/ false, forceDtsEmit);
  ...
  function getSyntacticDiagnostics(sourceFile?: SourceFile, cancellationToken?: CancellationToken): readonly qt.DiagnosticWithLocation[] {
  ...
  function getDeclarationDiagnostics(sourceFile?: SourceFile, cancellationToken?: CancellationToken): readonly qt.DiagnosticWithLocation[] {
  ...
  function getSyntacticDiagnosticsForFile(sourceFile: SourceFile): readonly qt.DiagnosticWithLocation[] {
  ...
  function getMergedBindAndCheckDiagnostics(sourceFile: SourceFile, ...allDiagnostics: (readonly Diagnostic[] | undefined)[]) {
    ...
    for (const errorExpectation of directives.getUnusedExpectations()) {
      diagnostics.push(qu.createDiagnosticForRange(sourceFile, errorExpectation.range, Diagnostics.Unused_ts_expect_error_directive));
    }
  ...
  function getDiagnosticsWithPrecedingDirectives(sourceFile: SourceFile, commentDirectives: qt.CommentDirective[], flatDiagnostics: Diagnostic[]) {
    ...
    const directives = qu.createCommentDirectivesMap(sourceFile, commentDirectives);
  ...
  function getSuggestionDiagnostics(sourceFile: SourceFile, cancellationToken: CancellationToken): readonly qt.DiagnosticWithLocation[] {
    return runWithCancellationToken(() => {
  ...
  function markPrecedingCommentDirectiveLine(diagnostic: Diagnostic, directives: qt.CommentDirectivesMap) {
    ...
    const lineStarts = qs.getLineStarts(file);
  ...
  function getJSSyntacticDiagnosticsForFile(sourceFile: SourceFile): qt.DiagnosticWithLocation[] {
    return runWithCancellationToken(() => {
      ...
      function walk(node: qt.Node, parent: Node) {
        ...
        switch (parent.kind) {
          ...
          case qt.SyntaxKind.MethodDeclaration:
            if ((<ParameterDeclaration | PropertyDeclaration | MethodDeclaration>parent).questionToken === node) {
              diagnostics.push(qu.createDiagnosticForNode(node, Diagnostics.The_0_modifier_can_only_be_used_in_TypeScript_files, '?'));
              return 'skip';
            }
          // falls through
          ...
          case qt.SyntaxKind.VariableDeclaration:
            ...
            if ((<FunctionLikeDeclaration | qt.VariableDeclaration | ParameterDeclaration | PropertyDeclaration>parent).type === node) {
              diagnostics.push(qu.createDiagnosticForNode(node, Diagnostics.Type_annotations_can_only_be_used_in_TypeScript_files));
              return 'skip';
            }
        }
        ...
        switch (node.kind) {
          case qt.SyntaxKind.ImportClause:
            if (node.isTypeOnly) {
              diagnostics.push(qu.createDiagnosticForNode(node.parent, Diagnostics._0_declarations_can_only_be_used_in_TypeScript_files, 'import type'));
              return 'skip';
            }
            break;
          case qt.SyntaxKind.ExportDeclaration:
            if (node.isTypeOnly) {
              diagnostics.push(qu.createDiagnosticForNode(node, Diagnostics._0_declarations_can_only_be_used_in_TypeScript_files, 'export type'));
              return 'skip';
            }
            break;
          case qt.SyntaxKind.ImportEqualsDeclaration:
            diagnostics.push(qu.createDiagnosticForNode(node, Diagnostics.import_can_only_be_used_in_TypeScript_files));
            return 'skip';
          case qt.SyntaxKind.ExportAssignment:
            if (node.isExportEquals) {
              diagnostics.push(qu.createDiagnosticForNode(node, Diagnostics.export_can_only_be_used_in_TypeScript_files));
              return 'skip';
            }
            break;
          case qt.SyntaxKind.HeritageClause:
            const heritageClause = node;
            if (heritageClause.token === qt.SyntaxKind.ImplementsKeyword) {
              diagnostics.push(qu.createDiagnosticForNode(node, Diagnostics.implements_clauses_can_only_be_used_in_TypeScript_files));
              return 'skip';
            }
            break;
          case qt.SyntaxKind.InterfaceDeclaration:
            const interfaceKeyword = qs.tokenToString(SyntaxKind.InterfaceKeyword);
          ...
          case qt.SyntaxKind.TypeAliasDeclaration:
            diagnostics.push(qu.createDiagnosticForNode(node, Diagnostics.Type_aliases_can_only_be_used_in_TypeScript_files));
            return 'skip';
          case qt.SyntaxKind.EnumDeclaration:
            const enumKeyword = Debug.checkDefined(qs.tokenToString(SyntaxKind.EnumKeyword));
            diagnostics.push(qu.createDiagnosticForNode(node, Diagnostics._0_declarations_can_only_be_used_in_TypeScript_files, enumKeyword));
            return 'skip';
          case qt.SyntaxKind.NonNullExpression:
            diagnostics.push(qu.createDiagnosticForNode(node, Diagnostics.Non_null_assertions_can_only_be_used_in_TypeScript_files));
            return 'skip';
          case qt.SyntaxKind.AsExpression:
            diagnostics.push(qu.createDiagnosticForNode(node.type, Diagnostics.Type_assertion_expressions_can_only_be_used_in_TypeScript_files));
            return 'skip';
      ...
      function walkArray(nodes: qt.NodeArray<Node>, parent: Node) {
        if (parent.decorators === nodes && !options.experimentalDecorators) {
          diagnostics.push(qu.createDiagnosticForNode(parent, Diagnostics.Experimental_support_for_decorators_is_a_feature_that_is_subject_to_change_in_a_future_release_Set_the_experimentalDecorators_option_in_your_tsconfig_or_jsconfig_to_remove_this_warning));
        }
        ...
        switch (parent.kind) {
          ...
          case qt.SyntaxKind.ArrowFunction:
            ...
            if (nodes === (<DeclarationWithTypeParameterChildren>parent).typeParameters) {
              diagnostics.push(qu.createDiagnosticForNodeArray(nodes, Diagnostics.Type_parameter_declarations_can_only_be_used_in_TypeScript_files));
              return 'skip';
            }
          // falls through
          ...
          case qt.SyntaxKind.VariableStatement:
            ...
            if (nodes === parent.modifiers) {
              checkModifiers(parent.modifiers, parent.kind === qt.SyntaxKind.VariableStatement);
              return 'skip';
            }
            break;
          case qt.SyntaxKind.PropertyDeclaration:
            ...
            if (nodes === (<qt.PropertyDeclaration>parent).modifiers) {
              for (const modifier of nodes) {
                if (modifier.kind !== qt.SyntaxKind.StaticKeyword) {
                  diagnostics.push(qu.createDiagnosticForNode(modifier, Diagnostics.The_0_modifier_can_only_be_used_in_TypeScript_files, qs.tokenToString(modifier.kind)));
                }
              }
              return 'skip';
            }
          ...
          case qt.SyntaxKind.Parameter:
            ...
            if (nodes === (<qt.ParameterDeclaration>parent).modifiers) {
              diagnostics.push(qu.createDiagnosticForNodeArray(nodes, Diagnostics.Parameter_modifiers_can_only_be_used_in_TypeScript_files));
              return 'skip';
            }
            break;
          ...
          case qt.SyntaxKind.TaggedTemplateExpression:
            ...
            if (nodes === (<NodeWithTypeArguments>parent).typeArguments) {
              diagnostics.push(qu.createDiagnosticForNodeArray(nodes, Diagnostics.Type_arguments_can_only_be_used_in_TypeScript_files));
              return 'skip';
            }
            break;
        }
      ...
      function qu.createDiagnosticForNodeArray(nodes: qt.NodeArray<Node>, message: qt.DiagnosticMessage, arg0?: string | number, arg1?: string | number, arg2?: string | number): qt.DiagnosticWithLocation {
      ...
      function qu.createDiagnosticForNode(node: qt.Node, message: qt.DiagnosticMessage, arg0?: string | number, arg1?: string | number, arg2?: string | number): qt.DiagnosticWithLocation {
        return qu.createDiagnosticForNodeInSourceFile(sourceFile, node, message, arg0, arg1, arg2);
      }
    });
  }

  function getDeclarationDiagnosticsWorker(sourceFile: SourceFile | undefined, cancellationToken: CancellationToken | undefined): readonly qt.DiagnosticWithLocation[] {
    return getAndCacheDiagnostics(sourceFile, cancellationToken, cachedDeclarationDiagnosticsForFile, getDeclarationDiagnosticsForFileNoCache);
  }

  function getDeclarationDiagnosticsForFileNoCache(sourceFile: SourceFile | undefined, cancellationToken: CancellationToken | undefined): readonly qt.DiagnosticWithLocation[] {
    return runWithCancellationToken(() => {
  ...
  function getDeclarationDiagnosticsForFile(sourceFile: SourceFile, cancellationToken: CancellationToken): readonly qt.DiagnosticWithLocation[] {
  ...
  function moduleNameIsEqualTo(a: StringLiteralLike | Identifier, b: StringLiteralLike | Identifier): boolean {
    return a.kind === qt.SyntaxKind.Identifier ? b.kind === qt.SyntaxKind.Identifier && a.escapedText === b.escapedText : b.kind === qt.SyntaxKind.StringLiteral && a.text === b.text;
  }
  ...
  function collectExternalModuleReferences(file: SourceFile): void {
    ...
    function collectModuleReferences(node: qt.Statement, inAmbientModule: boolean): void {
      if (qu.isAnyImportOrReExport(node)) {
        ...
        if (qu.isAmbientModule(node) && (inAmbientModule || qu.hasSyntacticModifier(node, qt.ModifierFlags.Ambient) || file.isDeclarationFile)) {
    ...
    function collectDynamicImportOrRequireCalls(file: SourceFile) {
      ...
      while (r.exec(file.text) !== null) {
        ...
        else if (qu.isImportCall(node) && node.arguments.length === 1 && isStringLiteralLike(node.arguments[0])) {
          imports = append(imports, node.arguments[0]);
        } else if (qu.isLiteralImportTypeNode(node)) {
    ...
    function getNodeAtPosition(sourceFile: SourceFile, position: number): Node {
      ...
      const getContainingChild = (child: Node) => {
        if (child.pos <= position && (position < child.end || (position === child.end && child.kind === qt.SyntaxKind.EndOfFileToken))) {
          return child;
        }
      };
      while (true) {
  ...
  function findSourceFile(fileName: string, path: Path, isDefaultLib: boolean, ignoreNoDefaultLib: boolean, refFile: RefFile | undefined, packageId: qt.PackageId | undefined): SourceFile | undefined {
    ...
    if (packageId) {
      const packageIdKey = qu.packageIdToString(packageId);
  ...
  function processReferencedFiles(file: SourceFile, isDefaultLib: boolean) {
    forEach(file.referencedFiles, (ref, index) => {
      ...
      processSourceFile(referencedFileName, isDefaultLib, /*ignoreNoDefaultLib*/ false, /*packageId*/ undefined, {
  ...
  function processTypeReferenceDirectives(file: SourceFile) {
    ...
    for (let i = 0; i < typeDirectives.length; i++) {
      ...
      qu.setResolvedTypeReferenceDirective(file, fileName, resolvedTypeReferenceDirective);
  ...
  function processTypeReferenceDirective(typeReferenceDirective: string, resolvedTypeReferenceDirective?: qt.ResolvedTypeReferenceDirective, refFile?: RefFile): void {
  ...
  function processImportedModules(file: SourceFile) {
    ...
    if (file.imports.length || file.moduleAugmentations.length) {
      ...
      for (let i = 0; i < moduleNames.length; i++) {
        ...
        qu.setResolvedModule(file, moduleNames[i], resolution);
        ...
        if (elideImport) {
          ...
          const pos = qs.skipTrivia(file.text, file.imports[i].pos);
          findSourceFile(
            resolvedFileName,
            path,
            /*isDefaultLib*/ false,
  ...
  function verifyCompilerOptions() {
    ...
    if (options.isolatedModules) {
      ...
      if (firstNonExternalModuleSourceFile) {
        const span = qu.getErrorSpanForNode(firstNonExternalModuleSourceFile, firstNonExternalModuleSourceFile);
      ...
      const span = qu.getErrorSpanForNode(firstNonAmbientExternalModuleSourceFile, firstNonAmbientExternalModuleSourceFile.externalModuleIndicator!);
    ...
    } else if (options.reactNamespace && !qs.isIdentifierText(options.reactNamespace, languageVersion)) {
    ...
    if (!options.noEmit && !options.suppressOutputPathCheck) {
      ...
      const emitFilesSeen = qc.createMap<true>();
    ...
    function verifyEmitFilePath(emitFileName: string | undefined, emitFilesSeen: Map<true>) {
      if (emitFileName) {
        ...
        if (filesByName.has(emitFilePath)) {
          let chain: qt.DiagnosticMessageChain | undefined;
  ...
  function createFileDiagnosticAtReference(refPathToReportErrorOn: ts.RefFile, message: qt.DiagnosticMessage, ...args: (string | number | undefined)[]) {
    ...
    switch (kind) {
      case RefFileKind.Import:
        pos = qs.skipTrivia(refFile.text, refFile.imports[index].pos);
  ...
  function createDiagnosticForOptionPathKeyValue(key: string, valueIndex: number, message: qt.DiagnosticMessage, arg0: string | number, arg1: string | number, arg2?: string | number) {
    let needCompilerDiagnostic = true;
    const pathsSyntax = getOptionPathsSyntax();
    for (const pathProp of pathsSyntax) {
      if (isObjectLiteralExpression(pathProp.initializer)) {
        for (const keyProps of qu.getPropertyAssignment(pathProp.initializer, key)) {
          const initializer = keyProps.initializer;
          if (isArrayLiteralExpression(initializer) && initializer.elements.length > valueIndex) {
            programDiagnostics.add(qu.createDiagnosticForNodeInSourceFile(options.configFile!, initializer.elements[valueIndex], message, arg0, arg1, arg2));
  ...
  function createDiagnosticForOptionPaths(onKey: boolean, key: string, message: qt.DiagnosticMessage, arg0: string | number) {
    let needCompilerDiagnostic = true;
    const pathsSyntax = getOptionPathsSyntax();
    for (const pathProp of pathsSyntax) {
  ...
  function getOptionsSyntaxByName(name: string): object | undefined {
    ...
    if (compilerOptionsObjectLiteralSyntax) {
      return qu.getPropertyAssignment(compilerOptionsObjectLiteralSyntax, name);
    }
    return undefined;
  }
  ...
  function createOptionValueDiagnostic(option1: string, message: qt.DiagnosticMessage, arg0: string) {
    createDiagnosticForOption(/*onKey*/ false, option1, /*option2*/ undefined, message, arg0);
  }

  function createDiagnosticForReference(sourceFile: JsonSourceFile | undefined, index: number, message: qt.DiagnosticMessage, arg0?: string | number, arg1?: string | number) {
    const referencesSyntax = firstDefined(qu.getTsConfigPropArray(sourceFile || options.configFile, 'references'), (property) => (isArrayLiteralExpression(property.initializer) ? property.initializer : undefined));
    if (referencesSyntax && referencesSyntax.elements.length > index) {
      programDiagnostics.add(qu.createDiagnosticForNodeInSourceFile(sourceFile || options.configFile!, referencesSyntax.elements[index], message, arg0, arg1));
    } else {
      programDiagnostics.add(createCompilerDiagnostic(message, arg0, arg1));
    }
  }

  function createDiagnosticForOption(onKey: boolean, option1: string, option2: string | undefined, message: qt.DiagnosticMessage, arg0: string | number, arg1?: string | number, arg2?: string | number) {
    const compilerOptionsObjectLiteralSyntax = getCompilerOptionsObjectLiteralSyntax();
  ...
  function getCompilerOptionsObjectLiteralSyntax() {
    if (_compilerOptionsObjectLiteralSyntax === undefined) {
      ...
      const jsonObjectLiteral = qu.getTsConfigObjectLiteralExpression(options.configFile);
      if (jsonObjectLiteral) {
        for (const prop of qu.getPropertyAssignment(jsonObjectLiteral, 'compilerOptions')) {
  ...
  function createOptionDiagnosticInObjectLiteralSyntax(objectLiteral: ObjectLiteralExpression, onKey: boolean, key1: string, key2: string | undefined, message: qt.DiagnosticMessage, arg0: string | number, arg1?: string | number, arg2?: string | number): boolean {
    const props = qu.getPropertyAssignment(objectLiteral, key1, key2);
    for (const prop of props) {
      programDiagnostics.add(qu.createDiagnosticForNodeInSourceFile(options.configFile!, onKey ? prop.name : prop.initializer, message, arg0, arg1, arg2));
    }
    return !!props.length;
  }
...
export function handleNoEmitOptions(program: ProgramToEmitFilesAndReportErrors, sourceFile: SourceFile | undefined, cancellationToken: CancellationToken | undefined): EmitResult | undefined {
  ...
  let diagnostics: readonly Diagnostic[] = [...program.getOptionsDiagnostics(cancellationToken), ...program.getSyntacticDiagnostics(sourceFile, cancellationToken), ...program.getGlobalDiagnostics(cancellationToken), ...program.getSemanticDiagnostics(sourceFile, cancellationToken)];
...
/**
 * Returns a qt.DiagnosticMessage if we won't include a resolved module due to its extension.
 * The qt.DiagnosticMessage's parameters are the imported module name, and the filename it resolved to.
 * This returns a diagnostic even if the module will be an untyped module.
 */
export function getResolutionDiagnostic(options: qt.CompilerOptions, { extension }: qt.ResolvedModuleFull): qt.DiagnosticMessage | undefined {
  switch (extension) {
    case Extension.Ts:
    case Extension.Dts:
...
function getModuleNames({ imports, moduleAugmentations }: SourceFile): string[] {
  ...
  for (const aug of moduleAugmentations) {
    if (aug.kind === qt.SyntaxKind.StringLiteral) {
      res.push(aug.text);
    }
