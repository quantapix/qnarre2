...
import { ParserWeakMap, ParserWeakMapESTreeToTSNode } from './parser';
import {
  AST_NODE_TYPES,
  AST_TOKEN_TYPES,
  TSESTree,
...
export function convertError(error: any): TSError {
  return createError(error.file, error.start, error.message || error.messageText);
}
...
export class Converter {
  ...
  private converter(
    ...
    const result = this.convertNode(node as TSNode, (parent ?? node.parent) as TSNode);
  ...
  private registerTSNodeInNodeMap(node: ts.Node, result: TSESTree.BaseNode | null): void {
    ...
    switch (node.kind) {
      case SyntaxKind.PropertyAccessExpression:
        ...
        result = this.createNode<TSESTree.JSXMemberExpression>(node, {
          ...
          property: this.convertJSXTagName(node.name, parent) as TSESTree.JSXIdentifier,
        });
        break;
  ...
  private convertNode(node: TSNode, parent: TSNode): TSESTree.Node | null {
    switch (node.kind) {
      ...
      case SyntaxKind.FunctionDeclaration: {
         ...
         */
        if (node.decorators) {
          (result as any).decorators = node.decorators.map((el) => this.convertChild(el));
        }
      ...
      case SyntaxKind.Parameter: {
        ...
        if (node.questionToken) {
          if (node.questionToken.end > parameter.range[1]) {
            ...
            parameter.loc.end = getLineAndCharacterFor(parameter.range[1], this.ast);
          }
        ...
        if (node.exportClause?.kind === SyntaxKind.NamedExports) {
          ...
          return this.createNode<TSESTree.ExportAllDeclaration>(node, {
            ...
            exported:
              ...
              node.exportClause && node.exportClause.kind === SyntaxKind.NamespaceExport
      ...
      case SyntaxKind.BinaryExpression: {
        ...
        if (isComma(node.operatorToken)) {
          ...
          if (this.allowPattern && type === AST_NODE_TYPES.AssignmentExpression) {
            return this.createNode<TSESTree.AssignmentPattern>(node, {
              type: AST_NODE_TYPES.AssignmentPattern,
...
export function convertComments(ast: ts.SourceFile, code: string): TSESTree.Comment[] {
