...
export class Parser {
  ...
  parseSourceFile(
    ...
    timings.tokenizeFileTime.timeCall(() => {
    ...
    timings.parseFileTime.timeCall(() => {
      while (!this._atEof()) {
  ...
  private _parseIfStatement(
    keywordType: KeywordType.If | KeywordType.Elif = KeywordType.If
  ): IfNode {
  ...
  private _tryParseListComprehension(
    target: ParseNode
  ): ListComprehensionNode | undefined {
  ...
  private _tryParseCompForStatement(): ListComprehensionForNode | undefined {
    ...
    if (startTokenKeywordType === KeywordType.Async) {
      ...
      if (
        ...
        nextToken.keywordType !== KeywordType.For
      ) {
        return undefined;
      }
    ...
    const compForNode = ListComprehensionForNode.create(
      asyncToken || forToken,
      targetExpr,
      seqExpr!
    );
  ...
  private _parseTryStatement(): TryNode {
    ...
    while (true) {
      ...
      if (!typeExpr) {
        ...
        if (sawCatchAllExcept) {
          this._addError(
            'A named except clause cannot appear after catch-all except clause',
            typeExpr
          );
        }
      }
  ...
  private _parseParameter(allowAnnotations: boolean): ParameterNode {
    ...
    if (this._consumeTokenIfOperator(OperatorType.Multiply)) {
      ...
      if (this._getLanguageVersion() < PythonVersion.V38) {
        this._addError(
          `Position-only argument separator requires Python 3.8 or greater`,
          firstToken
        );
      }
      slashCount = 1;
    ...
    if (!paramName) {
      ...
      if (this._peekTokenType() === TokenType.OpenParenthesis) {
        ...
        this._addError(
          `Sublist parameters are not supported in Python 3.x`,
          sublistStart
        );
      } else {
    ...
    if (this._consumeTokenIfOperator(OperatorType.Assign)) {
      ...
      if (starCount > 0) {
        this._addError(
          `Parameter with "*" or "**" cannot have default value`,
    ...
    if (!this._consumeTokenIfKeyword(KeywordType.Import)) {
      ...
      if (this._consumeTokenIfOperator(OperatorType.Multiply)) {
        ...
        if (importFromNode.imports.length === 0) {
          this._addError(
            'Expected one or more symbol names after import',
            this._peekToken()
          );
        }
  ...
  private _parseOrTest(): ExpressionNode {
    ...
    while (true) {
      ...
      leftExpr = BinaryOperationNode.create(
        leftExpr,
        rightExpr,
        peekToken,
        OperatorType.Or
  ...
  private _parseShiftExpression(): ExpressionNode {
    ...
    while (
      nextOperator === OperatorType.LeftShift ||
      nextOperator === OperatorType.RightShift
    ) {
      this._getNextToken();
  ...
  private _parseAtomExpression(): ExpressionNode {
    ...
    while (true) {
      ...
      if (this._consumeTokenIfType(TokenType.OpenParenthesis)) {
        ...
        const indexItemsNode = IndexItemsNode.create(
          nextToken,
          closingToken,
          indexExpressions
        );
  ...
  private _parseSubscript(): ExpressionNode {
    const firstToken = this._peekToken();
    const sliceExpressions: (ExpressionNode | undefined)[] = [
      undefined,
      undefined,
      undefined,
    ];
  ...
  private _parseDictionaryOrSetAtom(): DictionaryNode | SetNode {
    ...
    while (true) {
      ...
      if (keyExpression && valueExpression) {
        ...
        if (isSet) {
          this._addError(
            'Unpack operator not allowed within a set',
        ...
        if (keyExpression) {
          if (isDictionary) {
            ...
            if (listComp) {
              ...
              if (!isFirstEntry) {
                this._addError(
                  'Comprehension cannot be used with other set entries',
                  keyExpression
                );
              }
            }
  ...
  private _parseExpressionStatement(): ExpressionNode {
    ...
    if (this._consumeTokenIfType(TokenType.Colon)) {
      ...
      if (
        ...
        this._getLanguageVersion() < PythonVersion.V36
      ) {
        this._addError(
  ...
  private _parseTypeAnnotation(allowUnionNotation = true): ExpressionNode {
    ...
    while (true) {
      ...
      if (result === undefined || bitwiseOrToken === undefined) {
        ...
        result = BinaryOperationNode.create(
          result,
          annotationExpr,
          bitwiseOrToken,
          OperatorType.BitwiseOr
  ...
  private _reportStringTokenErrors(
    ...
    if (stringToken.flags & StringTokenFlags.Format) {
      if (this._getLanguageVersion() < PythonVersion.V36) {
        this._addError(
          'Format string literals (f-strings) require Python 3.6 or newer',
          stringToken
        );
      }
      ...
      if (stringToken.flags & StringTokenFlags.Bytes) {
        this._addError(
          'Format string literals (f-strings) cannot be binary',
          stringToken
        );
      }
      ...
      if (stringToken.flags & StringTokenFlags.Unicode) {
        this._addError(
          'Format string literals (f-strings) cannot be unicode',
          stringToken
        );
      }
    }
  ...
  private _getTypeAnnotationComment(): ExpressionNode | undefined {
    ...
    const interTokenContents = this._fileContents!.substring(
      curToken.start + curToken.length,
      nextToken.start
    );
      ...
      if (segment.isExpression) {
        ...
        parseResults.diagnostics.forEach((diag) => {
          ...
          const textRangeEnd =
            ...
            stringToken.start + stringToken.length;
          const textRange = {
            start: textRangeStart,
            length: textRangeEnd - textRangeStart,
          };
  ...
  private _parseStringList(): StringListNode {
    ...
    if (this._isParsingTypeAnnotation) {
      ...
      if (stringNode.strings.length > 1) {
        this._addError(
          'Type annotations cannot span multiple string literals',
          stringNode
        );
  ...
  private _getTokenIfIdentifier(
    disallowedKeywords: KeywordType[] = []
