...
class SemanticTokens implements vsc.SemanticTokens, vsc.DocumentRangeSemanticTokensProvider {
  ...
  async provideDocumentSemanticTokens(document: vsc.TextDocument, ct: vsc.CancellationToken): Promise<vsc.SemanticTokens | null> {
    const file = this.client.toOpenedPath(document);
    if (!file || document.getText().length > CONTENT_LENGTH_LIMIT) {
      return null;
    }
    return this._provideSemanticTokens(document, { file, start: 0, length: document.getText().length }, ct);
  }

  async provideDocumentRangeSemanticTokens(document: vsc.TextDocument, range: vsc.Range, ct: vsc.CancellationToken): Promise<vsc.SemanticTokens | null> {
    const file = this.client.toOpenedPath(document);
  ...
  async _provideSemanticTokens(document: vsc.TextDocument, requestArg: Experimentalprotocol.EncodedSemanticClassificationsRequestArgs, ct: vsc.CancellationToken): Promise<vsc.SemanticTokens | null> {
    const file = this.client.toOpenedPath(document);
    if (!file) {
      return null;
    }
    ...
    const response = await (this.client as Experimentalprotocol.IExtendedServiceClient).execute('encodedSemanticClassifications-full', requestArg, ct);
...
namespace Experimentalprotocol {
  export interface IExtendedServiceClient {
    ...
    ): Promise<qs.ServerResponse.Response<Experimentalprotocol.ExtendedTsServerRequests[K][1]>>;
  }
...
export function register(selector: vsc.DocumentSelector, client: qs.IServiceClient) {
  return new qr.VersionDependent(client, qr.API.default, () => {
    ...
    return vsc.Disposable.from(vsc.languages.registerDocumentRangeSemanticTokensProvider(selector, provider, provider.getLegend()));
  });
}
