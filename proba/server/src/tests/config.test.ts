...
test('FindFilesVirtualEnvAutoDetectExclude', () => {
  ...
  commandLineOptions.configFilePath =
    'src/tests/samples/project_with_venv_auto_detect_exclude';
...
test('FindFilesVirtualEnvAutoDetectInclude', () => {
  ...
  const commandLineOptions = new CommandLineOptions(cwd, true);
  commandLineOptions.configFilePath =
    'src/tests/samples/project_with_venv_auto_detect_include';
...
test('FileSpecNotAnArray', () => {
  ...
  const service = new AnalyzerService(
    '<default>',
    createFromRealFileSystem(nullConsole),
    nullConsole
  );
...
test('FindExecEnv1', () => {
  ...
  assert.equal(
    normalizeSlashes(defaultExecEnv.root),
    normalizeSlashes(configOptions.projectRoot)
  );
});
...
test('AutoSearchPathsOn', () => {
  const cwd = normalizePath(
    combinePaths(process.cwd(), '../server/src/tests/samples/project_src')
  );
  const nullConsole = new NullConsole();
  const service = new AnalyzerService(
    '<default>',
...
test('AutoSearchPathsOnSrcIsPkg', () => {
  const cwd = normalizePath(
    combinePaths(process.cwd(), '../server/src/tests/samples/project_src_is_pkg')
  );
  const nullConsole = new NullConsole();
  const service = new AnalyzerService(
    '<default>',
...
test('AutoSearchPathsOnWithConfigExecEnv', () => {
  const cwd = normalizePath(
    combinePaths(
      process.cwd(),
      '../server/src/tests/samples/project_src_with_config_exec_env'
    )
  );
  const nullConsole = new NullConsole();
  const service = new AnalyzerService(
...
test('AutoSearchPathsOnAndExtraPaths', () => {
  const cwd = normalizePath(
    combinePaths(
      process.cwd(),
      '../server/src/tests/samples/project_src_with_extra_paths'
    )
  );
  const nullConsole = new NullConsole();
  const service = new AnalyzerService(
