...
suite('Completions', () => {
  ...
  const assertCompletion = function (
    ...
    assert.equal(
      matches.length,
      1,
      `${expected.label} should only existing once: Actual: ${completions.items
        .map((c) => c.label)
        .join(', ')}`
    );
  ...
  function assertCompletions(
    ...
    const participantResult = CompletionList.create([]);
    cssLanguageService.setCompletionParticipants([
      getPathCompletionParticipant(document, workspaceFolders, participantResult),
    ]);

    const stylesheet = cssLanguageService.parseStylesheet(document);
    const list = cssLanguageService.doComplete(document, position, stylesheet)!;
  ...
  test('CSS url() Path completion', function () {
    const testUri = URI.file(
      path.resolve(__dirname, '../../test/pathCompletionFixtures/about/about.css')
    ).toString();
    const folders = [
      { name: 'x', uri: URI.file(path.resolve(__dirname, '../../test')).toString() },
    ];
    ...
    assertCompletions(
      `html { background-image: url('/pathCompletionFixtures/|')`,
      {
        items: [
          {
            label: 'about/',
  ...
  test('CSS url() Path Completion - Unquoted url', function () {
    ...
    assertCompletions(
      'html { background-image: url(../s|rc/)',
      {
        items: [
          { label: 'about/', resultText: 'html { background-image: url(../about/)' },
        ],
  ...
  test('CSS @import Path completion', function () {
    ...
    assertCompletions(
      `@import './|'`,
      {
        items: [{ label: 'about.html', resultText: `@import './about.html'` }],
      },
      testUri,
      folders
    );
  ...
  test('SCSS @import Path completion', function () {
    ...
    const testSCSSUri = URI.file(
      path.resolve(__dirname, '../../test/pathCompletionFixtures/scss/main.scss')
    ).toString();
    assertCompletions(
      `@import './|'`,
      {
