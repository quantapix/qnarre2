...
import { combinePaths, normalizeSlashes } from '../utils/pathUtils';
...
test('createFromFileSystem2', () => {
  const fs = factory.createFromFileSystem(host.HOST, /* ignoreCase */ true, {
    cwd: factory.srcFolder,
  });
...
test('createFromFileSystemWithCustomTypeshedPath', () => {
  const invalidpath = normalizeSlashes(
    combinePaths(host.HOST.getWorkspaceRoot(), '../docs')
  );
