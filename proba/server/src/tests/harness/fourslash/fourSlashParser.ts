...
import { toBoolean } from '../../../utils/misc';
import {
  combinePaths,
  getRelativePath,
  isRootedDiskPath,
...
export function parseTestData(
  ...
  for (let line of lines) {
    ...
    if (line.substr(0, 4) === '////') {
      ...
      currentFileContent =
        currentFileContent === undefined ? text : currentFileContent + '\n' + text;
...
function parseFileContent(
    ...
    for (let i = 1; i < content.length; i++) {
      ...
      switch (state) {
        ...
        case State.inObjectMarker:
          ...
          if (previousChar === '|' && currentChar === '}') {
            // Record the marker
            const objectMarkerNameText = content
              .substring(openMarker!.sourcePosition + 2, i - 1)
              .trim();
  ...
  if (openRanges.length > 0) {
    ...
    reportError(
      fileName,
      openRange.sourceLine,
      openRange.sourceColumn,
      'Unterminated range.'
    );
  }
  ...
  localRanges = localRanges.sort((a, b) =>
    a.pos < b.pos ? -1 : a.pos === b.pos && a.end > b.end ? -1 : 1
  );
