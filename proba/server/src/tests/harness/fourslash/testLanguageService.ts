...
export class TestLanguageService implements LanguageServerInterface {
  ...
  async getWorkspaceForFile(filePath: string): Promise<WorkspaceServiceInstance> {
    debug.assertDefined(this._workspace.service.test_program.getSourceFile(filePath));
    return this._workspace;
  }

  async getSettings(workspace: WorkspaceServiceInstance): Promise<ServerSettings> {
    const settings: ServerSettings = {
      venvPath: this._workspace.service.test_configOptions.venvPath,
      pythonPath: this._workspace.service.test_configOptions.pythonPath,
      typeshedPath: this._workspace.service.test_configOptions.typeshedPath,
      openFilesOnly: this._workspace.service.test_configOptions.checkOnlyOpenFiles,
      useLibraryCodeForTypes: this._workspace.service.test_configOptions
        .useLibraryCodeForTypes,
      disableServices: this._workspace.disableServices,
    };
