...
import { FileEditAction } from '../../../utils/text';
import {
  combinePaths,
  comparePaths,
  pathToUri,
...
import { getStringComparer } from '../../../utils/strings';
import {
  DocumentRange,
  Position,
...
export class TestState {
  ...
  constructor(
    ...
    const ignoreCase = toBoolean(
      testData.globalOptions[GlobalMetadataOptionNames.ignoreCase]
    );

    this._cancellationToken = new TestCancellationToken();
    const configOptions = this._convertGlobalOptionsToConfigOptions(
      this.testData.globalOptions
    );
    ...
    for (const file of testData.files) {
      ...
      if (this._isConfig(file, ignoreCase)) {
        ...
        files[file.fileName] = new vfs.File(file.content, {
    ...
    this.workspace = {
      ...
      rootUri: pathToUri(this.fs.getModulePath()),
      service: service,
    ...
    this.asyncTest = toBoolean(
      testData.globalOptions[GlobalMetadataOptionNames.asynctest]
    );
  ...
  get configOptions(): ConfigOptions {
    return this.workspace.service.test_configOptions;
  }
  ...
  goToMarker(nameOrMarker: string | Marker = '') {
    const marker = isString(nameOrMarker)
      ? this.getMarkerByName(nameOrMarker)
      : nameOrMarker;
    ...
    if (marker.position === -1 || marker.position > content.length) {
      throw new Error(
        `Marker "${nameOrMarker}" has been invalidated by unrecoverable edits to the file.`
      );
    }
  ...
  goToEachMarker(
    ...
    action: (marker: Marker, index: number) => void
  ) {
  ...
  selectLine(index: number) {
    const lineStart = this._convertPositionToOffset(this.activeFile.fileName, {
      line: index,
      character: 0,
    });
  ...
  getRangesByText(): Map<string, Range[]> {
    ...
    const result = this._createMultiMap<Range>(this.getRanges(), (r) =>
      this._rangeText(r)
    );
  ...
  printCurrentFileState(showWhitespace: boolean, makeCaretVisible: boolean) {
    for (const file of this.testData.files) {
      ...
      host.HOST.log(
        `=== Script (${file.fileName}) ${active ? '(active, cursor at |)' : ''} ===`
      );
  ...
  deleteLineRange(startIndex: number, endIndexInclusive: number) {
    const startPos = this._convertPositionToOffset(this.activeFile.fileName, {
      line: startIndex,
      character: 0,
    });
  ...
  verifyDiagnostics(map?: {
    [marker: string]: { category: string; message: string };
  }): void {
    this._analyze();
    ...
    for (const [file, ranges] of rangePerFile.entries()) {
      ...
      for (const [category, expected] of rangesPerCategory.entries()) {
        ...
        if (expected.length !== actual.length) {
          this._raiseError(
            `contains unexpected result - expected: ${stringify(
              expected
  ...
  async verifyCodeActions(map: {
    [marker: string]: {
      codeActions: { title: string; kind: string; command: Command }[];
    };
  }): Promise<any> {
    this._analyze();
  ...
  async verifyCommand(
    ...
    if (command.command === 'pyright.createtypestub') {
      ...
      if (actualText !== expectedText) {
        this._raiseError(
          `doesn't contain expected result: ${stringify(
            expectedText
          )}, actual: ${stringify(actualText)}`
        );
      }
    }
  ...
  verifyHover(map: { [marker: string]: { value: string; kind: string } }): void {
    ...
    for (const range of this.getRanges()) {
      ...
      const actual = convertHoverResults(
        this.program.getHoverForPosition(
          range.fileName,
          rangePos.start,
          CancellationToken.None
        )
      ...
      if (MarkupContent.is(actual.contents)) {
        ...
        assert.fail(
          `Unexpected type of contents object "${actual.contents}", should be MarkupContent.`
        );
      }
    }
  }
  ...
  verifyRangeIs(expectedText: string, includeWhiteSpace?: boolean) {
    this._verifyTextMatches(
      this._rangeText(this._getOnlyRange()),
      !!includeWhiteSpace,
      expectedText
    );
  }
  ...
  async verifyCompletion(
    ...
    map: {
      [marker: string]: {
        completions: { label: string; documentation?: { kind: string; value: string } }[];
      };
    }
  ): Promise<void> {
    this._analyze();
      ...
      if (result) {
        ...
        if (verifyMode === 'exact') {
          if (result.items.length !== 0) {
            ...
            assert.fail(
              `Completion items unexpected: ${stringify(
                result.items.map((r) => r.label)
              )}`
            );
          }
  ...
  verifyFindAllReferences(map: {
    ...
    for (const marker of this.getMarkers()) {
      ...
      const position = this._convertOffsetToPosition(fileName, marker.position);
      const actual = this.program.getReferencesForPosition(
        fileName,
        position,
        true,
        CancellationToken.None
  ...
  private _isConfig(file: FourSlashFile, ignoreCase: boolean): boolean {
    ...
    return configFileNames.some(
      (f) => comparer(getBaseFileName(file.fileName), f) === Comparison.EqualTo
    );
  }
  ...
  private _convertOffsetsToRange(
    fileName: string,
    startOffset: number,
    endOffset: number
  ): PositionRange {
  ...
  private _editScriptAndUpdateMarkers(
    fileName: string,
    editStart: number,
    editEnd: number,
    newText: string
    ...
    for (const marker of this.testData.markers) {
      if (marker.fileName === fileName) {
        marker.position = this._updatePosition(
          marker.position,
          editStart,
          editEnd,
          newText
  ...
  private _verifyTextMatches(
    ...
    const removeWhitespace = (s: string): string =>
      includeWhitespace ? s : this._removeWhitespace(s);
  ...
  private _getLineContent(index: number) {
    ...
    const pos = this._convertPositionToOffset(this.activeFile.fileName, {
      line: index,
      character: 0,
    });
    let startPos = pos;
  ...
  private _getCurrentLineContent() {
    return this._getLineContent(
      this._convertOffsetToPosition(this.activeFile.fileName, this.currentCaretPosition)
        .line
    );
  }
  ...
  private _getLineColStringAtPosition(
    position: number,
    file: FourSlashFile = this.activeFile
  ) {
  ...
  private _updatePosition(
    ...
    return position <= editStart
      ? position
      : position < editEnd
      ? -1
      : position + length - +(editEnd - editStart);
  }
