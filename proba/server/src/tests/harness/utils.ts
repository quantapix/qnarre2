...
import { binarySearch, insertAt } from '../../utils/collection';
...
export class SortedMap<K, V> {
  ...
  constructor(
    ...
    this._order =
      typeof comparer === 'object' && comparer.sort === 'insertion' ? [] : undefined;
    if (iterable) {
...
export class Metadata {
  ...
  private static _escapeKey(text: string) {
    return text.length >= 2 && text.startsWith('_') && text.charAt(1) === '_'
      ? '_' + text
      : text;
  }
...
export function createIOError(code: keyof typeof IO_ERROR_MESSAGE, details = '') {
  const err: NodeJS.ErrnoException = new Error(
    `${code}: ${IO_ERROR_MESSAGE[code]} ${details}`
  );
  err.code = code;
