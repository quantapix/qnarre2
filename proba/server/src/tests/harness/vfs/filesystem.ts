...
export class TestFileSystem implements FileSystem {
  ...
  mountSync(source: string, target: string, resolver: FileSystemResolver) {
    ...
    const { parent, links, node: existingNode, basename } = this._walk(
    ...
    const printLinks = (dirname: string | undefined, links: SortedMap<string, Inode>) => {
      ...
      try {
        for (let i = nextResult(iterator); i; i = nextResult(iterator)) {
          ...
          const marker =
            pathUtil.comparePaths(this._cwd, path, this.ignoreCase) === 0 ? '*' : ' ';
          if (result) {
            result += '\n';
          }
          result += marker;
  ...
  linkSync(oldpath: string, newpath: string) {
    ...
    const { parent, links, basename, node: existingNode } = this._walk(
  ...
  static diff(changed: TestFileSystem, base: TestFileSystem, options: DiffOptions = {}) {
    const differences: FileSet = {};
    return TestFileSystem._rootDiff(differences, changed, base, options)
      ? differences
      : undefined;
  }
      ...
      changedLinks.forEach((changedNode, basename) => {
        ...
        if (baseNode) {
          ...
          if (isSymlink(changedNode) && isSymlink(baseNode)) {
            return (hasChanges =
              TestFileSystem._symlinkDiff(container, basename, changedNode, baseNode) ||
              hasChanges);
          }
        }
        return (hasChanges =
          TestFileSystem._trackCreatedInode(container, basename, changed, changedNode) ||
          hasChanges);
      });
      return hasChanges;
    }
  ...
  private static _trackCreatedInode(
    container: FileSet,
    basename: string,
    changed: TestFileSystem,
    node: Inode
  ...
  private static _trackDeletedInodes(
    container: FileSet,
  ...
  private _walk(
    ...
    while (true) {
      ...
      if (lastStep && (noFollow || !isSymlink(node))) {
        return {
          realpath: pathUtil.combinePathComponents(components),
          basename,
          parent,
          links,
          node,
      ...
      if (isSymlink(node)) {
        ...
        parent = undefined;
        components = pathUtil
          .getPathComponents(symlink)
          .concat(components.slice(step + 1));
        step = 0;
        depth++;
        retry = false;
        continue;
  ...
  private _resolve(path: string) {
    return this._cwd
      ? pathUtil.resolvePaths(
          ...
          validate(
            path,
            ValidationFlags.RelativeOrAbsolute | ValidationFlags.AllowWildcard
          )
        )
...
function formatPatchWorker(dirname: string, container: FileSet): string {
  ...
  for (const name of Object.keys(container)) {
    ...
    if (
      ...
      const content =
        typeof entry.data === 'string' ? entry.data : entry.data.toString('utf8');
      text += `//// [${file}]\r\n${content}\r\n\r\n`;
    } else if (entry instanceof Link) {
