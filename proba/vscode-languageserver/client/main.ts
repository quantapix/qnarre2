...
namespace Transport {
  export function isSocket(value: Transport): value is SocketTransport {
    ...
    return (
      candidate && candidate.kind === TransportKind.socket && Is.number(candidate.port)
    );
  }
}
...
namespace ChildProcessInfo {
  export function is(value: any): value is ChildProcessInfo {
    ...
    return (
      candidate && candidate.process !== void 0 && typeof candidate.detached === 'boolean'
    );
  }
}
...
export class LanguageClient extends BaseLanguageClient {
  ...
  protected createMessageTransports(encoding: string): Promise<MessageTransports> {
    ...
    function startedInDebugMode(): boolean {
      ...
      if (args) {
        return args.some((arg) => {
          return (
            debugStartWith.some((value) => arg.startsWith(value)) ||
    ...
    function assertStdio(
      process: cp.ChildProcess
    ): asserts process is cp.ChildProcessWithoutNullStreams {
    ...
    return this._getServerWorkingDir(json.options).then((serverWorkingDir) => {
          ...
          return new Promise<MessageTransports>((resolve, _reject) => {
            ...
            if (transport === TransportKind.ipc || transport === TransportKind.stdio) {
              ...
              if (transport === TransportKind.ipc) {
                ...
                resolve({
                  reader: new StreamMessageReader(sp.stdout),
                  writer: new StreamMessageWriter(sp.stdin),
                });
              }
            } else if (transport === TransportKind.pipe) {
      ...
      return Promise.reject<MessageTransports>(
        new Error(`Unsupported server configuration ` + JSON.stringify(server, null, 4))
      );
    });
  }
