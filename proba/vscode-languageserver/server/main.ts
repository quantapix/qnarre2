...
export class TextDocuments<T> {
  ...
  public listen(connection: IConnection): void {
    ...
    connection.onDidOpenTextDocument((event: DidOpenTextDocumentParams) => {
      ...
      let document = this._configuration.create(
        td.uri,
        td.languageId,
        td.version,
        td.text
      ...
      (event: WillSaveTextDocumentParams, token: CancellationToken) => {
        ...
        if (document && this._willSaveWaitUntil) {
          return this._willSaveWaitUntil(
            Object.freeze({ document, reason: event.reason }),
            token
          );
        } else {
          return [];
...
class RemoteConsoleImpl implements Logger, RemoteConsole, Remote {
  ...
  private send(type: MessageType, message: string) {
    if (this._rawConnection) {
      this._rawConnection.sendNotification(LogMessageNotification.type, {
        type,
        message,
      });
    }
...
const RemoteWindowImpl: new () => RemoteWindow = ProgressFeature(
  _RemoteWindowImpl
) as new () => RemoteWindow;
...
class BulkUnregistrationImpl implements BulkUnregistration {
  private _unregistrations: Map<string, Unregistration> = new Map<
    string,
    Unregistration
  >();
    ...
    this._connection!.sendRequest(UnregistrationRequest.type, params).then(
      ...
      (_error) => {
        this._connection!.console.info(
          `Unregistering request handler for ${unregistration.id} failed.`
        );
      }
    );
    return true;
...
class RemoteClientImpl implements RemoteClient, Remote {
  ...
  public register(
    ...
    registerOptions?: any
  ): Promise<any> /* Promise<Disposable | BulkUnregistration> */ {
    if (typeOrRegistrations instanceof BulkRegistrationImpl) {
      return this.registerMany(typeOrRegistrations);
    } else if (typeOrRegistrations instanceof BulkUnregistrationImpl) {
      return this.registerSingle1(
        <BulkUnregistrationImpl>typeOrRegistrations,
        <string | RPCMessageType>registerOptionsOrType,
        registerOptions
      );
    } else {
      return this.registerSingle2(
        <string | RPCMessageType>typeOrRegistrations,
  ...
  private unregisterSingle(id: string, method: string): Promise<void> {
    ...
    return this.connection
      .sendRequest(UnregistrationRequest.type, params)
...
class _RemoteWorkspaceImpl implements _RemoteWorkspace, Remote {
  ...
  public applyEdit(
    ...
    let params: ApplyWorkspaceEditParams = isApplyWorkspaceEditParams(paramOrEdit)
      ? paramOrEdit
      : { edit: paramOrEdit };
...
const RemoteWorkspaceImpl: new () => RemoteWorkspace = WorkspaceFoldersFeature(
...
export interface _Languages {
  ...
  ): ResultProgressReporter<PR> | undefined;
}
...
export class _LanguagesImpl implements Remote, _Languages {
  ...
  public attachPartialResultProgress<PR>(
    _type: ProgressType<PR>,
    params: PartialResultParams
  ): ResultProgressReporter<PR> | undefined {
...
const LanguagesImpl: new () => Languages = CallHierarchyFeature(
  _LanguagesImpl
) as new () => Languages;
...
> {
   ...
   */
  onHover(
    handler: ServerRequestHandler<HoverParams, Hover | undefined | null, never, void>
  ): void;
  ...
  onDocumentHighlight(
    handler: ServerRequestHandler<
      DocumentHighlightParams,
      DocumentHighlight[] | undefined | null,
      DocumentHighlight[],
      void
    >
  ): void;
  ...
  onDocumentSymbol(
    handler: ServerRequestHandler<
      DocumentSymbolParams,
      SymbolInformation[] | DocumentSymbol[] | undefined | null,
  ...
  onWorkspaceSymbol(
    handler: ServerRequestHandler<
      WorkspaceSymbolParams,
      SymbolInformation[] | undefined | null,
      SymbolInformation[],
      void
    >
  ): void;
  ...
  onPrepareRename(
    handler: RequestHandler<
      PrepareRenameParams,
      Range | { range: Range; placeholder: string } | undefined | null,
      void
    >
  ): void;
  ...
  onDocumentLinkResolve(
    handler: RequestHandler<DocumentLink, DocumentLink | undefined | null, void>
  ): void;
  ...
  onColorPresentation(
    handler: ServerRequestHandler<
      ColorPresentationParams,
      ColorPresentation[] | undefined | null,
      ColorPresentation[],
      void
    >
  ): void;
...
> {
  function combine<O, T>(
    one: O | undefined,
    two: T | undefined,
    func: (one: O, two: T) => any
  ): any {
    if (one && two) {
      return func(one, two);
    } else if (one) {
...
>(
  ...
  const logger = (factories && factories.console
    ...
    : new RemoteConsoleImpl()) as RemoteConsoleImpl & PConsole;
  ...
  const telemetry = (factories && factories.telemetry
    ...
    : new TelemetryImpl()) as TelemetryImpl & PTelemetry;
  const client = (factories && factories.client
    ? new (factories.client(RemoteClientImpl))()
    : new RemoteClientImpl()) as RemoteClientImpl & PClient;
  const remoteWindow = (factories && factories.window
    ? new (factories.window(RemoteWindowImpl))()
    : new RemoteWindowImpl()) as Remote & RemoteWindow & PWindow;
  const workspace = (factories && factories.workspace
    ? new (factories.workspace(RemoteWorkspaceImpl))()
    : new RemoteWorkspaceImpl()) as Remote & RemoteWorkspace & PWorkspace;
  const languages = (factories && factories.languages
    ? new (factories.languages(LanguagesImpl))()
    : new LanguagesImpl()) as Remote & Languages & PLanguages;
  ...
  let initializeHandler:
    | ServerRequestHandler<InitializeParams, InitializeResult, never, InitializeError>
    | undefined = undefined;
  let exitHandler: NotificationHandler0 | undefined = undefined;
  let protocolConnection: Connection<
    PConsole,
    PTracer,
    PTelemetry,
    PClient,
  ...
  > &
    ...
    onDidOpenTextDocument: (handler) =>
      connection.onNotification(DidOpenTextDocumentNotification.type, handler),
    onDidChangeTextDocument: (handler) =>
      connection.onNotification(DidChangeTextDocumentNotification.type, handler),
    onDidCloseTextDocument: (handler) =>
      connection.onNotification(DidCloseTextDocumentNotification.type, handler),
    onWillSaveTextDocument: (handler) =>
      connection.onNotification(WillSaveTextDocumentNotification.type, handler),
    ...
    onDidSaveTextDocument: (handler) =>
      connection.onNotification(DidSaveTextDocumentNotification.type, handler),

    sendDiagnostics: (params) =>
      connection.sendNotification(PublishDiagnosticsNotification.type, params),

    onHover: (handler) =>
      connection.onRequest(HoverRequest.type, (params, cancel) => {
        return handler(params, cancel, attachWorkDone(connection, params), undefined);
      }),
    onCompletion: (handler) =>
      connection.onRequest(CompletionRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
          attachPartialResult(connection, params)
        );
      }),
    onCompletionResolve: (handler) =>
      connection.onRequest(CompletionResolveRequest.type, handler),
    onSignatureHelp: (handler) =>
      connection.onRequest(SignatureHelpRequest.type, (params, cancel) => {
        return handler(params, cancel, attachWorkDone(connection, params), undefined);
      }),
    onDeclaration: (handler) =>
      connection.onRequest(DeclarationRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
    ...
    onDefinition: (handler) =>
      connection.onRequest(DefinitionRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
    ...
    onTypeDefinition: (handler) =>
      connection.onRequest(TypeDefinitionRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
    ...
    onImplementation: (handler) =>
      connection.onRequest(ImplementationRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
    ...
    onReferences: (handler) =>
      connection.onRequest(ReferencesRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
    ...
    onDocumentHighlight: (handler) =>
      connection.onRequest(DocumentHighlightRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
    ...
    onDocumentSymbol: (handler) =>
      connection.onRequest(DocumentSymbolRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
    ...
    onWorkspaceSymbol: (handler) =>
      connection.onRequest(WorkspaceSymbolRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
    ...
    onCodeAction: (handler) =>
      connection.onRequest(CodeActionRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
    ...
    onCodeLens: (handler) =>
      connection.onRequest(CodeLensRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
    ...
    onCodeLensResolve: (handler) =>
      connection.onRequest(CodeLensResolveRequest.type, (params, cancel) => {
        return handler(params, cancel);
      }),
    onDocumentFormatting: (handler) =>
      connection.onRequest(DocumentFormattingRequest.type, (params, cancel) => {
        return handler(params, cancel, attachWorkDone(connection, params), undefined);
      }),
    onDocumentRangeFormatting: (handler) =>
      connection.onRequest(DocumentRangeFormattingRequest.type, (params, cancel) => {
        return handler(params, cancel, attachWorkDone(connection, params), undefined);
      }),
    onDocumentOnTypeFormatting: (handler) =>
      connection.onRequest(DocumentOnTypeFormattingRequest.type, (params, cancel) => {
        return handler(params, cancel);
      }),
    onRenameRequest: (handler) =>
      connection.onRequest(RenameRequest.type, (params, cancel) => {
        return handler(params, cancel, attachWorkDone(connection, params), undefined);
      }),
    onPrepareRename: (handler) =>
      connection.onRequest(PrepareRenameRequest.type, (params, cancel) => {
        return handler(params, cancel);
      }),
    onDocumentLinks: (handler) =>
      connection.onRequest(DocumentLinkRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
    ...
    onDocumentLinkResolve: (handler) =>
      connection.onRequest(DocumentLinkResolveRequest.type, (params, cancel) => {
        return handler(params, cancel);
      }),
    onDocumentColor: (handler) =>
      connection.onRequest(DocumentColorRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
    ...
    onColorPresentation: (handler) =>
      connection.onRequest(ColorPresentationRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
    ...
    onFoldingRanges: (handler) =>
      connection.onRequest(FoldingRangeRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
    ...
    onSelectionRanges: (handler) =>
      connection.onRequest(SelectionRangeRequest.type, (params, cancel) => {
        return handler(
          params,
          cancel,
          attachWorkDone(connection, params),
    ...
    onExecuteCommand: (handler) =>
      connection.onRequest(ExecuteCommandRequest.type, (params, cancel) => {
        return handler(params, cancel, attachWorkDone(connection, params), undefined);
      }),
