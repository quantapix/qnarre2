...
export namespace DiagnosticCode {
   ...
   */
  export function is(
    value: string | number | DiagnosticCode | undefined | null
...
export namespace Command {
  ...
  export function is(value: any): value is Command {
    ...
    return (
      Is.defined(candidate) && Is.string(candidate.title) && Is.string(candidate.command)
    );
  }
}
      ...
      (candidate.documentChanges === void 0 ||
        candidate.documentChanges.every((change) => {
          if (Is.string((change as ResourceOperation).kind)) {
            return (
              CreateFile.is(change) || RenameFile.is(change) || DeleteFile.is(change)
            );
          } else {
...
export class WorkspaceChange {
  ...
  public getTextEditChange(
    key: DocumentUri | VersionedTextDocumentIdentifier
  ): TextEditChange {
...
export namespace InsertTextFormat {
   ...
   *
   * See also: https://github.com/Microsoft/vscode/blob/master/src/vs/editor/contrib/snippet/utils/snippet.md
   */
...
export namespace CodeAction {
  ...
  export function create(
    title: string,
    commandOrEdit: Command | WorkspaceEdit,
    kind?: CodeActionKind
  ): CodeAction {
