...
describe('NavItemComponent', () => {
  ...
  describe('(class-only)', () => {
    ...
    let selectedNodes: NavNode[];
    let setClassesSpy: jasmine.Spy;

    function initialize(nd: NavNode) {
      component.node = nd;
  ...
  describe('(via TestBed)', () => {
    ...
    it('should pass the `isWide` property to all displayed child nav-items', () => {
      ...
      children.forEach(c => expect(c.componentInstance.isWide).toBe(true));
      ...
      children.forEach(c => expect(child.componentInstance.isWide).toBe(false));
    });
  });
});
