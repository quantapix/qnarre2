...
module.exports = function exampleInlineTagDef(
  ...
  return {
    ...
    handler: function(doc, tagName, tagDescription) {
      ...
      var regionName =
        tagArgs.region || (unnamedArgs.length > 1 ? unnamedArgs[1] : '');
      // eslint-disable-next-line quotes
      if (regionName === "''" || regionName === '""') regionName = '';
      var header =
        tagArgs.header ||
        (unnamedArgs.length > 2 ? unnamedArgs.slice(2).join(' ') : '');
        ...
        throw new Error(
          createDocMessage(
            `Missing required "path" on @${tagName} inline tag "{@${tagName} ${tagDescription}}".\n` +
              'Usage: {@example some/path [some-region [Some header [linenums="true|false"]]]}',
            doc
          )
        );
      }
