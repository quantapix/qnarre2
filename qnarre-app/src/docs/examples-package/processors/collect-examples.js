...
module.exports = function collectExamples(
  exampleMap,
  regionParser,
  log,
  createDocMessage
  ...
  return {
    ...
    registerIgnoredExamples(paths, gitIgnorePath) {
      paths.forEach(path => {
        this.ignoredExamples[path] = gitIgnorePath;
      });
    },
    /**
    ...
    $process(docs) {
      ...
      exampleFolders.forEach(
        folder => (exampleMap[folder] = exampleMap[folder] || {})
      );
      const regionDocs = [];
          ...
          try {
            ...
            exampleFolders.some(folder => {
              if (doc.fileInfo.relativePath.indexOf(folder) === 0) {
                const relativePath = doc.fileInfo.relativePath
                ...
                if (fileType === 'annotated') {
                  fileType =
                    extname(doc.fileInfo.baseName).substr(1) + '.' + fileType;
                }
