...
describe('collectExampleRegions processor', () => {
  ...
  beforeEach(function() {
    regionParser = jasmine
      .createSpy('regionParser')
      .and.callFake(function(contents, extension) {
        return {
          contents: 'PARSED:' + contents,
          regions: { dummy: extension }
        };
      });

    const dgeni = new Dgeni([
      testPackage('examples-package', true).factory('regionParser', function() {
        return regionParser;
      })
    ]);
  ...
  describe('$process', () => {
    ...
    it('should remove example files from the docs collection', () => {
      ...
      expect(
        processedDocs.filter(doc => doc.docType === 'example-file')
      ).toEqual([]);
    });
    ...
    it('should attach parsed content as renderedContent to the example file docs', () => {
      ...
      expect(exampleMap['examples-1']['x/app.js'].renderedContent).toEqual(
        'PARSED:A'
      );
      expect(exampleMap['examples-1']['y/index.html'].renderedContent).toEqual(
        'PARSED:B'
      );
      expect(exampleMap['examples-2']['s/app.js'].renderedContent).toEqual(
        'PARSED:C'
      );
      expect(exampleMap['examples-2']['t/style.css'].renderedContent).toEqual(
        'PARSED:D'
      );
    });
  ...
  describe('filtered examples', () => {
    it('should indicate if an example was filtered', () => {
      processor.registerIgnoredExamples(
        ['c/d/e', 'e/f/g'],
        'path/to/gitignore'
      );
      ...
      expect(processor.isExampleIgnored('x/y/z')).toEqual(
...
function createDoc(content, relativePath, docType) {
  return {
    fileInfo: {
      ...
      extension: path.extname(relativePath).substr(1)
    },
    content: content,
