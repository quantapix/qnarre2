...
module.exports = function regionParser() {
  ...
  function regionParserImpl(contents, fileType) {
    ...
    if (regionMatcher) {
      ...
      const lines = contents.split(/\r?\n/).filter((line, index) => {
        ...
        if (startRegion) {
          ...
          if (openRegions.length === 0) {
            throw new RegionParserError(
              'Tried to close a region when none are open',
              index
            );
          }
          ...
          plaster = plasterString
            ? regionMatcher.createPlasterComment(plasterString)
            : '';

          // simple line of content processing
        } else {
          openRegions.forEach(regionName =>
            regionMap[regionName].lines.push(line)
          );
      ...
      return {
        contents: lines.join('\n'),
        regions: mapObject(regionMap, (regionName, region) =>
