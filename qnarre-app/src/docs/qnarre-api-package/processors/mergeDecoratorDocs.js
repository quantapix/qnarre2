  ...
  return {
    ...
    makeDecoratorCalls: [
      { type: '', description: 'toplevel', functionName: 'makeDecorator' },
      {
        ...
        functionName: 'makePropDecorator'
      },
      {
        type: 'Param',
        description: 'parameter',
        functionName: 'makeParamDecorator'
      }
    ],
    $process(docs) {
        ...
        if (initializer) {
          this.makeDecoratorCalls.forEach(function(call) {
            if (
              ...
              initializer.expression.text === call.functionName
            ) {
              ...
              log.debug(
                'mergeDecoratorDocs: found decorator',
                doc.docType,
                doc.name,
                decoratorType
              );
      ...
      docs = docs.filter(doc => {
        if (decoratorDocs[doc.name]) {
          ...
          doc.moduleDoc.exports = doc.moduleDoc.exports.filter(
            exportDoc => exportDoc !== doc
          );
        }
...
function getInitializer(doc) {
  var initializer =
    doc.symbol &&
    doc.symbol.valueDeclaration &&
    doc.symbol.valueDeclaration.initializer;
