  ...
  describe('$process', () => {
    describe('docs without members', () => {
      it('should ignore the docs', () => {
        ...
        const docs = [{ id: 'some-doc' }, { id: 'some-other' }];
        processor.$process(docs);
        expect(docs).toEqual([{ id: 'some-doc' }, { id: 'some-other' }]);
      });
    });
    ...
    describe('docs with members', () => {
      it('should compute an anchor for each instance member', () => {
        ...
        const docs = [
          {
            id: 'some-doc',
            members: [{ name: 'foo' }, { name: 'new' }, { name: '' }]
          }
        ];
        processor.$process(docs);
        expect(docs[0].members.map(member => member.anchor)).toEqual([
          'foo',
          'new',
          'call'
        ]);
      ...
      it('should compute a path for each instance member', () => {
        ...
        processor.$process(docs);
        expect(docs[0].members.map(member => member.path)).toEqual([
          'a/b/c#foo',
          'a/b/c#new',
          'a/b/c#call'
        ]);
    ...
    describe('docs with static members', () => {
      it('should compute an anchor for each static member', () => {
        ...
        const docs = [
          {
            id: 'some-doc',
            statics: [{ name: 'foo' }, { name: 'new' }, { name: '' }]
          }
        ];
        processor.$process(docs);
        expect(docs[0].statics.map(member => member.anchor)).toEqual([
          'foo',
          'new',
          'call'
        ]);
      ...
      it('should compute a path for each static member', () => {
        ...
        processor.$process(docs);
        expect(docs[0].statics.map(member => member.path)).toEqual([
          'a/b/c#foo',
          'a/b/c#new',
          'a/b/c#call'
        ]);
