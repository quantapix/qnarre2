 ...
 * Usually set to "code" but also "code-example" for qnarre.
 */
module.exports = function autoLinkCode(getDocFromAlias) {
  ...
  autoLinkCodeImpl.ignoredLanguages = [
    'bash',
    'sh',
    'shell',
    'json',
  ...
  function autoLinkCodeImpl() {
    return ast => {
      visit(ast, 'element', (node, ancestors) => {
        ...
        if (
          autoLinkCodeImpl.codeElements.some(elementType =>
            is(node, elementType)
          ) &&
          (!node.properties.className ||
            !node.properties.className.includes('no-auto-link')) &&
          !autoLinkCodeImpl.ignoredLanguages.includes(
