...
describe('autoLinkCode post-processor', () => {
  ...
  beforeEach(() => {
    const testPackage = createTestPackage('qnarre-base-package');
  ...
  it('should insert an anchor into every code item that matches the id of an API doc', () => {
    ...
    const doc = {
      docType: 'test-doc',
      renderedContent: '<code>MyClass</code>'
    };
    processor.$process([doc]);
    expect(doc.renderedContent).toEqual(
      '<code><a href="a/b/myclass" class="code-anchor">MyClass</a></code>'
    );
  });
  ...
  it('should insert an anchor into every code item that matches an alias of an API doc', () => {
    ...
    const doc = {
      docType: 'test-doc',
      renderedContent: '<code>foo.MyClass</code>'
    };
    processor.$process([doc]);
    expect(doc.renderedContent).toEqual(
      '<code><a href="a/b/myclass" class="code-anchor">foo.MyClass</a></code>'
    );
  });
  ...
  it('should match code items within a block of code that contain a dot in their identifier', () => {
    ...
    const doc = {
      docType: 'test-doc',
      renderedContent: '<code>someFn(): MyEnum.Value</code>'
    };
    processor.$process([doc]);
    expect(doc.renderedContent).toEqual(
  ...
  it('should ignore code items that are already inside a link', () => {
    ...
    processor.$process([doc]);
    expect(doc.renderedContent).toEqual(
      '<a href="..."><div><code>MyClass</code></div></a>'
    );
  });
  ...
  it('should ignore code items that match an API doc but are attached to other text via a dash', () => {
    ...
    const doc = {
      docType: 'test-doc',
      renderedContent: '<code>xyz-MyClass</code>'
    };
    processor.$process([doc]);
  ...
  it('should work with custom elements', () => {
    ...
    const doc = {
      docType: 'test-doc',
      renderedContent: '<code-example>MyClass</code-example>'
    };
    processor.$process([doc]);
    expect(doc.renderedContent).toEqual(
  ...
  it('should ignore code blocks that are marked with a `no-auto-link` class', () => {
    ...
    const doc = {
      docType: 'test-doc',
      renderedContent: '<code class="no-auto-link">MyClass</code>'
    };
    processor.$process([doc]);
    expect(doc.renderedContent).toEqual(
      '<code class="no-auto-link">MyClass</code>'
    );
  });
  ...
  it('should ignore code blocks that are marked with an "ignored" language', () => {
    ...
    const doc = {
      docType: 'test-doc',
      renderedContent: '<code language="bash">MyClass</code>'
    };
    processor.$process([doc]);
