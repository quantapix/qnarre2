...
describe('h1Checker postprocessor', () => {
  ...
  it('should complain if there is more than one h1 in a document', () => {
    ...
    expect(() => processor.$process([doc])).toThrowError(
      createDocMessage('More than one h1 found in ' + doc.renderedContent, doc)
    );
  });
  ...
  it('should clean aria-hidden anchors from h1 text added to the vFile', () => {
    const doc = {
      docType: 'a',
      renderedContent:
        '<h1 class="no-toc" id="what-is-qnarre">' +
        '<a title="Link to this heading" class="header-link" aria-hidden="true" href="docs#what-is-qnarre">' +
        '<i class="material-icons">link</i>' +
        '</a>What is Qnarre?' +
        '</h1>'
    };
    processor.$process([doc]);
    expect(doc.vFile.title).toEqual('What is Qnarre?');
  });
