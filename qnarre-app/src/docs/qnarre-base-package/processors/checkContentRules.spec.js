...
describe('checkContentRules processor', function() {
  ...
  beforeEach(function() {
    const dgeni = new Dgeni([testPackage('qnarre-base-package')]);
  ...
  it('should run configured rules against matching docs', () => {
    ...
    expect(descriptionSpy1).toHaveBeenCalledWith(
      docs[0],
      'description',
      'test doc 1'
    );
    ...
    expect(descriptionSpy3).toHaveBeenCalledWith(
      docs[0],
      'description',
      'test doc 1'
    );
  ...
  it('should log warnings if the rule returns error messages and `failOnContentErrors` is false', () => {
    const nameSpy1 = jasmine
      .createSpy('name 1')
      .and.returnValue('name error message');
    const descriptionSpy1 = jasmine
      .createSpy('description 1')
      .and.returnValue('description error message');
  ...
  it('should log errors and then throw if `failOnContentErrors` is true and errors are found', () => {
    ...
    expect(() => processor.$process(docs)).toThrowError(
      'Stopping due to content errors.'
    );
  ...
  it('should ignore docs whose id contains a barred-o', () => {
    ...
    const docs = [
      {
        docType: 'doc-type',
        id: 'package/class/property/param',
        name: 'name-1'
      },
      {
        docType: 'doc-type',
        id: 'package/class/property/ɵparam',
        name: 'name-2'
      },
      {
        docType: 'doc-type',
        id: 'package/class/ɵproperty/param',
        name: 'name-3'
      },
      {
        docType: 'doc-type',
        id: 'package/ɵclass/property/param',
        name: 'name-4'
      }
    ];
    processor.$process(docs);
