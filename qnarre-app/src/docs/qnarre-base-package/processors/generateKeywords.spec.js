...
describe('generateKeywords processor', () => {
    ...
    const docs = [
      {
        ...
        title: 'title A',
        vFile: { headings: { h1: ['vFile A'] } }
      },
      {
        docType: 'class',
        name: 'B',
        title: 'title B',
        vFile: { headings: { h1: ['vFile B'] } }
      },
      {
        docType: 'class',
        name: 'C',
        vFile: { title: 'vFile C', headings: { h1: ['vFile C'] } }
      },
  ...
  it('should add heading words to the search terms', () => {
    ...
    processor.$process(docs);
    const keywordsDoc = docs[docs.length - 1];
    expect(keywordsDoc.data[0].headingWords).toEqual(
      'heading important secondary'
    );
  });
