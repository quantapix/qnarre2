...
module.exports = function cliCommandFileReader() {
  ...
  return {
    ...
    defaultPattern: /\.json$/,
    getDocs(fileInfo) {
      fileInfo.realProjectRelativePath =
        'packages/angular/cli/commands/' + fileInfo.relativePath;
      try {
  ...
  function createLongDescriptionDoc(fileInfo) {
    ...
    const schemaJsonPath = path.resolve(
      fileInfo.basePath,
      '../commands',
    ...
    try {
      ...
      if (schema.$longDescription) {
        return {
          ...
          startingLine: 0,
          fileInfo: {
            realProjectRelativePath: path.join(
              path.dirname(fileInfo.realProjectRelativePath),
