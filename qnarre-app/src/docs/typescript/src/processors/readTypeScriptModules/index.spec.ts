...
describe('readTypeScriptModules', () => {
  ...
  beforeEach(() => {
    ...
    processor.basePath = path.resolve(
      __dirname,
      '../../mocks/readTypeScriptModules'
    );
  });
  ...
  describe('exportDocs', () => {
    ...
    it('should extract the starting and ending lines from the comments', () => {
      ...
      const docs: DocCollection = [];
      processor.$process(docs);
      const someClassDoc = docs.find(
        doc => doc.name === 'SomeClass'
      ) as ClassExportDoc;
    ...
    it('should put static members into the `.statics` property of the export doc', () => {
      ...
      const classDoc: ClassExportDoc = docs.find(
        doc => doc.docType === 'class'
      );
      expect(classDoc.statics.length).toEqual(2);
      expect(classDoc.statics.map(staticMember => staticMember.name)).toEqual([
        'staticA',
        'num'
      ]);
    ...
    it('should add each exportDoc to the exportSymbolsToDocsMap', () => {
      ...
      const map = injector.get('exportSymbolsToDocsMap') as Map<
        Symbol,
        ExportDoc
      >;
  ...
  describe('interfaces', () => {
    ...
    it('should handle "call" type interfaces', () => {
      ...
      const callMember = exportedInterface.members.find(
        member => member.isCallMember
      )! as MethodMemberDoc;
      ...
      const newMember = exportedInterface.members.find(
        member => member.isNewMember
      )! as MethodMemberDoc;
  ...
  describe('export aliases', () => {
    function getProcessedDocs() {
      ...
      processor.$process(docs);
      const [originalExport, aliasedExport] = docs.filter(
        doc => doc.docType === 'class'
      ) as ClassExportDoc[];
    ...
    it('should have the proper aliases resolved', () => {
      ...
      expect(originalExport.aliases).toEqual([
        'OriginalExport',
        'exportAliases/OriginalExport'
      ]);
      expect(aliasedExport.aliases).toEqual([
        'AliasedExport',
        'exportAliases/AliasedExport'
      ]);
    });
    ...
    it('should expose the original symbols members', () => {
      ...
      expect(originalExport.members.map(memberDoc => memberDoc.id)).toEqual([
        'exportAliases/OriginalExport.sayHello()'
      ]);
      expect(aliasedExport.members.map(memberDoc => memberDoc.id)).toEqual([
        'exportAliases/AliasedExport.sayHello()'
      ]);
    });
  });
  ...
  describe('members', () => {
    ...
    describe('overloaded constructors', () => {
      it('should create a member doc for the "real" constructor, which includes an overloads property', () => {
        ...
        const foo: MethodMemberDoc = docs.find(
          doc => doc.name === 'constructor'
        );
        expect(foo.parameters).toEqual([
          'x: string',
          'y: string | number',
    ...
    describe('return types', () => {
      ...
      it('should return the text of the type if initialized', () => {
        ...
        expect(propDocs[1].type).toEqual(
          ['Object.assign(this.someProp, {', "    bar: 'baz'", '})'].join('\n')
        );
      });
    });
    ...
    describe('getters and setters', () => {
      ...
      beforeEach(() => {
        ...
        decoratorPropGetter = docs.find(
          doc => doc.name === 'decoratorProp:get'
        );
        decoratorPropSetter = docs.find(
          doc => doc.name === 'decoratorProp:set'
        );
      });
    ...
    describe('method parameters', () => {
      it('should process method parameters', () => {
        ...
        const docs: DocCollection = [];
        processor.$process(docs);
        const param1: ParameterDoc = docs.find(
          doc => doc.name === 'param1' && doc.container.name === 'method1'
        );
        expect(param1.docType).toEqual('parameter');
        expect(param1.id).toEqual(
          'methodParameters/TestClass.method1()~param1'
        );
        ...
        const param2: ParameterDoc = docs.find(
          doc => doc.name === 'param2' && doc.container.name === 'method1'
        );
        expect(param2.docType).toEqual('parameter');
        expect(param2.id).toEqual(
          'methodParameters/TestClass.method1()~param2'
        );
        ...
        const param3: ParameterDoc = docs.find(
          doc => doc.name === 'param3' && doc.container.name === 'method1'
        );
        expect(param3.docType).toEqual('parameter');
        expect(param3.id).toEqual(
          'methodParameters/TestClass.method1()~param3'
        );
        ...
        const param4: ParameterDoc = docs.find(
          doc => doc.name === 'param4' && doc.container.name === 'method1'
        );
        expect(param4.docType).toEqual('parameter');
        expect(param4.id).toEqual(
          'methodParameters/TestClass.method1()~param4'
        );
        ...
        const method1: MethodMemberDoc = docs.find(
          doc => doc.name === 'method1'
        )!;
  ...
  describe('namespaces', () => {
    ...
    it('should cope with spread operator', () => {
      ...
      const docs: DocCollection = [];
      processor.$process(docs);
      const functionDoc: FunctionExportDoc = docs.find(
        doc => doc.docType === 'function'
      );
