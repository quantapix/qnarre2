...
describe('FileInfo', () => {
  ...
  let basePath: string;
  beforeEach(() => {
    spyOn(fs, 'realpathSync').and.callFake(
      (filePath: string) => filePath + '.real'
    );
  ...
  it('should compute the file path properties from the parsed file', () => {
    ...
    const fileInfo1 = new FileInfo(
      module.exportArray[0].declarations![0],
      basePath
    );
    ...
    expect(fileInfo1.location).toEqual(
    ...
    const fileInfo2 = new FileInfo(
      module.exportArray[1].declarations![0],
      basePath
    );
    ...
    expect(fileInfo2.location).toEqual(
      jasmine.objectContaining({
        start: {line: 33, character: 10},
        end: {line: 33, character: 42}
      })
    );
    ...
    const fileInfo3 = new FileInfo(
      module.exportArray[2].declarations![0],
      basePath
    );
    ...
    expect(fileInfo3.location).toEqual(
      jasmine.objectContaining({
    ...
    expect(fileInfo3.realProjectRelativePath).toBe(
      'tsParser/importedSrc.ts.real'
    );
