...
describe('getDecoratorSpec', () => {
  ...
  beforeEach(() => {
    ...
    const parseInfo = parser.parse(
      ['tsParser/getDecorators.test.ts'],
      basePath
    );
    const moduleExports = parseInfo.moduleSymbols[0].exportArray;
    testClass = moduleExports[0];
    testMethodDeclaration = testClass
      .members!.get('method' as __String)!
      .getDeclarations()![0];
  ...
  it('should return the decorators of a class declaration', () => {
    ...
    expect(classDecoratorFactory.argumentInfo).toEqual([
      "'foo'",
      "'bar'",
      {value: "'xxx'"}
    ]);
    expect(classDecoratorFactory.arguments).toEqual([
      "'foo'",
      "'bar'",
      "{\n    value: 'xxx'\n}"
    ]);
  ...
  it('should return the decorators of a property declaration', () => {
    const propertyDecorators = getDecorators(
      testClass.members!.get('property' as __String)!.getDeclarations()![0]
    )!;
