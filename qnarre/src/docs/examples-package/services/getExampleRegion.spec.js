...
describe('getExampleRegion', () => {
  ...
  beforeEach(function() {
    ...
    collectExamples.registerIgnoredExamples(
      ['filtered/path'],
      'some/gitignore'
    );
    exampleMap['examples'] = {
  ...
  it('should contain the region contents from the example file if a region is specified', () => {
    expect(getExampleRegion({}, 'test/url', 'region-1')).toEqual(
      'region 1 contents'
    );
  });
  ...
  it('should throw an error if an example has been filtered out', () => {
    expect(() => {
      ...
      'Ignored example file... relativePath: "filtered/path" - doc\n' +
