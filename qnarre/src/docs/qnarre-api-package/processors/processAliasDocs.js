 ...
 */
module.exports = function processAliasDocs(
  getDocFromAlias,
  log,
  createDocMessage
) {
      ...
      docs.forEach(doc => {
        if (doc.aliasDocId) {
          ...
          if (aliasDocs.length === 1) {
            ...
            throw new Error(
              createDocMessage(
                `There is no doc that matches "@alias ${doc.aliasDocId}"`,
                doc
              )
            );
          } else {
            throw new Error(
              createDocMessage(
                `There is more than one doc that matches "@alias ${
                  doc.aliasDocId
                }": ${aliasDocs.map(d => d.id).join(', ')}.`,
                doc
              )
            );
          }
