      ...
      docs.forEach(doc => {
        if (this.exportDocTypes.indexOf(doc.docType) !== -1) {
          if (!doc.ngModules || doc.ngModules.length === 0) {
            errors.push(
              createDocMessage(
                `"${doc.id}" has no @ngModule tag. Docs of type "${doc.docType}" must have this tag.`,
                doc
              )
            );
            return;
          ...
          doc.ngModules.forEach((ngModule, index) => {
            ...
            if (ngModuleDocs.length === 0) {
              errors.push(
                createDocMessage(
                  `"@ngModule ${ngModule}" does not match a public NgModule`,
                  doc
                )
              );
              return;
            ...
            if (ngModuleDocs.length > 1) {
              errors.push(
                createDocMessage(
                  `"@ngModule ${ngModule}" is ambiguous. Matches: ${ngModuleDocs
                    .map(d => d.id)
                    .join(', ')}`,
                  doc
                )
            ...
            const container = (ngModuleDoc[containerName] =
              ngModuleDoc[containerName] || []);
            container.push(doc);
