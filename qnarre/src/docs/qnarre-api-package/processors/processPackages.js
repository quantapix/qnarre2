  ...
  return {
    ...
    $process(docs) {
      const packageContentFiles =
        collectPackageContentDocsProcessor.packageContentFiles;
      const packageMap = {};
      ...
      docs.forEach(doc => {
        if (doc.docType === 'module') {
          ...
          if (doc.exports) {
            ...
            doc.ngmodules = publicExports
              .filter(doc => doc.docType === 'ngmodule')
              .sort(byId);
            doc.classes = publicExports
              .filter(doc => doc.docType === 'class')
              .sort(byId);
            doc.decorators = publicExports
              .filter(doc => doc.docType === 'decorator')
              .sort(byId);
            ...
            doc.structures = publicExports
              .filter(
                doc => doc.docType === 'enum' || doc.docType === 'interface'
              )
              .sort(byId);
            doc.directives = publicExports
              .filter(doc => doc.docType === 'directive')
              .sort(byId);
            doc.pipes = publicExports
              .filter(doc => doc.docType === 'pipe')
              .sort(byId);
            doc.types = publicExports
              .filter(
                doc => doc.docType === 'type-alias' || doc.docType === 'const'
              )
              .sort(byId);
            if (publicExports.every(doc => !!doc.deprecated)) {
              doc.deprecated =
                'all exports of this entry point are deprecated.';
            }
          }
          ...
          doc.packageInfo = packageMap[primaryPackageName] = packageMap[
            primaryPackageName
