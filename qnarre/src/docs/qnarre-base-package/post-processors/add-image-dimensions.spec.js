...
describe('addImageDimensions post-processor', () => {
  ...
  beforeEach(() => {
    const testPackage = createTestPackage('qnarre-base-package').factory(
      'getImageDimensions',
  ...
  it('should log a warning for images with no src attribute', () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      'Missing src in image tag `<img attr="value">` - doc (a) '
    );
  });
  ...
  it('should fail for images whose source cannot be loaded', () => {
    ...
    expect(() => processor.$process(docs)).toThrowError(
      'Unable to load src in image tag `<img src="missing">` - doc (a) '
    );
