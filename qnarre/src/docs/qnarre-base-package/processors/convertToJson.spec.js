...
describe('convertToJson processor', () => {
  ...
  beforeEach(function() {
    dgeni = new Dgeni([testPackage('qnarre-base-package')]);
    ...
    const docs = [
      {
        ...
        renderedContent:
          '<div><h1 class="title">Some title</h1><article><h1>Article 1</h1></article></div>'
      }
    ];
    processor.$process(docs);
    expect(JSON.parse(docs[0].renderedContent).contents).toEqual(
  ...
  it('should log a warning', () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      'Title property expected - doc (test-doc) '
    );
  });
});
