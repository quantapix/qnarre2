  ...
  return {
    ...
    $process: function(docs) {
      ...
      if (this.ignoreWordsFile) {
        var ignoreWordsPath = path.resolve(
          readFilesProcessor.basePath,
          this.ignoreWordsFile
        );
        wordsToIgnore = fs
          .readFileSync(ignoreWordsPath, 'utf8')
          .toString()
          .split(/[,\s\n\r]+/gm);
      ...
      const filteredDocs = docs
        ...
        .filter(function(doc) {
          return !docTypesToIgnore[doc.docType];
        })
        // Ignore internals and private exports (indicated by the ɵ prefix)
        .filter(function(doc) {
          return !doc.internal && !doc.privateExport;
        });
...
function tokenize(text) {
  const rawTokens = text.split(/[\s/]+/gm);
  const tokens = [];
...
function extractMemberWords(doc, members, membersMap) {
  ...
  if (doc.extendsClauses) {
    doc.extendsClauses.forEach(clause =>
      extractMemberWords(clause.doc, members, membersMap)
    );
  }
  if (doc.implementsClauses) {
    doc.implementsClauses.forEach(clause =>
      extractMemberWords(clause.doc, members, membersMap)
    );
  }
}
