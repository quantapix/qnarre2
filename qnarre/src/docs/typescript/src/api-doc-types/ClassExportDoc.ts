...
export class ClassExportDoc extends ClassLikeExportDoc {
  ...
  isAbstract =
    ...
    this.declaration.modifiers.some(
  ...
  constructor(
    ...
    if (symbol.exports) {
      this.statics = this.getMemberDocs(
        symbol.exports,
        moduleDoc.hidePrivateMembers
      );
    }

    if (symbol.members) {
      // Get the constructor
      const constructorSymbol = symbol.members.get(
        InternalSymbolName.Constructor
      );
      if (
        constructorSymbol &&
        constructorSymbol.getFlags() & SymbolFlags.Constructor
      ) {
        this.constructorDoc = this.getConstructorDoc(constructorSymbol);
      }
      // Get the instance members
      this.members = this.getMemberDocs(
        symbol.members,
        moduleDoc.hidePrivateMembers
      );
    }
  ...
  private getConstructorDoc(constructorSymbol: Symbol) {
    ...
    constructorSymbol.getDeclarations()!.forEach(declaration => {
      if ((declaration as FunctionLikeDeclaration).body) {
        ...
        constructorDoc = new MethodMemberDoc(
          this.host,
          this,
          constructorSymbol,
          declaration,
