...
describe('getContent', () => {
  ...
  it('should get the leading jsdoc comments for exports', () => {
    ...
    const comments = module.exportArray.map(e =>
      getContent(e.getDeclarations()![0])
    );
  ...
  it('should get the leading jsdoc comments for class members and their parameters', () => {
    ...
    expect(
      getContent(
        classDoc.members!.get('property' as __String)!.valueDeclaration
      )
    ).toEqual('Some property');

    const method: MethodDeclaration = classDoc.members!.get(
      'method' as __String
    )!.valueDeclaration as MethodDeclaration;
  ...
  it('should properly concatenate multiple leading comments', () => {
    const parseInfo = parser.parse(
      ['tsParser/multipleLeadingComments.ts'],
      basePath
    );
    const module = parseInfo.moduleSymbols[0];

    expect(getContent(module.exportArray[0].getDeclarations()![0])).toEqual(
  ...
  describe('leading comments concatenation disabled', () => {
    it('should be able to disable concatenation', () => {
      ...
      expect(
        getContent(module.exportArray[0].getDeclarations()![0], false)
    ...
    it('should not throw if node does not have any leading comment', () => {
      ...
      expect(() =>
        getContent(module.exportArray[1].getDeclarations()![0], false)
      ).not.toThrow();
    });
    ...
    it('should skip leading single-line comments', () => {
      ...
      expect(
        getContent(module.exportArray[2].getDeclarations()![0], false)
    ...
    it('should skip leading non-js multi line comments', () => {
      ...
      expect(
        getContent(module.exportArray[3].getDeclarations()![0], false)
