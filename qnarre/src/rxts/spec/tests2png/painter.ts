...
const MESSAGES_WIDTH: number =
  CANVAS_WIDTH - 2 * CANVAS_PADDING - OBSERVABLE_END_PADDING;
...
function stringToColor(str: string) {
  ...
  let hash = str
    ...
    .reduce(function (x, y) {
      return x * smallPrime1 + y * smallPrime2;
    }, 1);
...
function areEqualStreamData(
  leftStreamData: TestStream,
  rightStreamData: TestStream
): boolean {
...
function measureObservableArrow(
  maxFrame: number,
  ...
  let lastMessageFrame = streamData.messages.reduce(function (acc, msg) {
    let frame = msg.frame;
    return frame > acc ? frame : acc;
  }, 0);
  let subscriptionEndX =
    typeof streamData.subscription!.end === 'number'
      ? CANVAS_PADDING +
  ...
  let streamEndX =
    ...
    OBSERVABLE_END_PADDING;
  let endX =
    streamData.subscription!.end === '100%'
      ? MAX_MESSAGES_WIDTH
...
function measureNestedStreamHeight(
  maxFrame: number,
  streamData: TestStream
): number {
...
function amountPriorOverlaps(
  message: TestMessage,
  messageIndex: number,
  otherMessages: TestMessage[]
): number {
  return otherMessages.reduce(function (
    acc: number,
    otherMessage: TestMessage,
    otherIndex
  ) {
    if (
      otherIndex < messageIndex &&
...
function measureStreamHeight(
  maxFrame: number
): (streamData: TestStream) => number {
  return function measureStreamHeightWithMaxFrame(
    streamData: TestStream
  ): number {
    ...
    let maxMessageHeight = messages
      .map(function (msg: TestMessage, index) {
        let height = isNestedStreamData(msg)
          ? measureNestedStreamHeight(maxFrame, msg.notification.value) +
            OBSERVABLE_HEIGHT * 0.25
          : OBSERVABLE_HEIGHT * 0.5;
        let overlapHeightBonus =
          amountPriorOverlaps(msg, index, messages) * MESSAGE_OVERLAP_HEIGHT;
...
function drawObservableArrow(
  ...
  out = out.draw(
    'translate',
    String(endX) + ',' + String(y + inclination),
    'rotate ' + String(angle),
    'line',
    String(0) + ',' + String(0),
...
function drawMarble(
  ...
  out = out.draw(
    'translate ' +
      (x - CANVAS_WIDTH * 0.5) +
      ',' +
      (y + inclination - canvasHeight! * 0.5),
    'gravity Center',
    'text 0,0',
...
function drawObservableMessages(
  out: GMObject,
  maxFrame: number,
  baseY: number,
  angle: number,
  ...
  messages
    .slice()
    .reverse()
    .forEach(function (message: TestMessage, reversedIndex: number) {
      if (message.frame < 0) {
        // ignore messages with negative frames
        return;
      }
      let index = messages.length - reversedIndex - 1;
      let x = startX + MESSAGES_WIDTH * (message.frame / maxFrame);
      if (x - MARBLE_RADIUS < 0) {
        // out of screen, on the left
        x += MARBLE_RADIUS;
      }
      let y =
        baseY +
        amountPriorOverlaps(message, index, messages) * MESSAGE_OVERLAP_HEIGHT;
      let inclination = measureInclination(startX, x, angle);
      switch (message.notification.kind) {
        case 'N':
          if (isNestedStreamData(message)) {
            out = drawNestedObservable(
              out,
              maxFrame,
              y,
              message.notification.value
...
function removeDuplicateInputs(
  ...
  return inputStreams.filter(function (inputStream) {
    return !inputStreams
      .concat(outputStreams)
      .some(function (otherStream: TestStream) {
        return otherStream.messages.some(function (msg) {
          let passes =
            isNestedStreamData(msg) &&
            inputStream.cold &&
    ...
    for (let j = 0; j < inputStream.messages.length; j++) {
      ...
      if (isNestedStreamData(message) && typeof message.isGhost !== 'boolean') {
        ...
        if (!message.notification.value.subscription) {
          ...
          message.notification.value.subscription = {
            start: referenceTime,
            end: 0
          };
          continue;
        ...
        if (referenceTime !== subscriptionTime) {
          ...
          ghost.notification.value.subscription.end -=
            subscriptionTime - referenceTime;
...
function sanitizeHigherOrderInputStreams(
  inputStreams: TestStream[],
  outputStreams: TestStream[]
): TestStream[] {
...
export function painter(
  ...
  const maxFrame =
    getMaxFrame(inputStreams.concat(outputStreams)) || DEFAULT_MAX_FRAME;
  let allStreamsHeight = inputStreams
    .concat(outputStreams)
  ...
  outputStreams.forEach(function (streamData) {
    let isSpecial =
      ...
      areEqualStreamData(inputStreams[0], streamData);
