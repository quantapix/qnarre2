    ...
    binaryLikeExpressionWorker(left: Expression, operatorToken: Node, right: Expression, leftType: qc.Type, rightType: qc.Type, errorNode?: Node): qc.Type {
      ...
      switch (operator) {
        ...
        case Syntax.EqualsToken:
          const declKind = qf.is.kind(qc.BinaryExpression, left.parent) ? getAssignmentDeclarationKind(left.parent) : AssignmentDeclarationKind.None;
      ...
      for (const typeParameter of typeParameters) {
        ...
        if (parent.kind !== Syntax.InferType && parent.typeParameters!.every(isTypeParameterUnused)) {
          if (seenParentsWithEveryUnused.tryAdd(parent)) {
            ...
            addDiagnostic(typeParameter, UnusedKind.Parameter, createFileDiagnostic(qf.get.sourceFileOf(parent), range.pos, range.end - range.pos, message, arg0));
          }
        } else {
          ...
          exports.forEach(({ declarations, flags }, id) => {
            ...
            if (exportedDeclarationsCount > 1) {
              for (const declaration of declarations) {
                if (isNotOverload(declaration)) diagnostics.add(createDiagnosticForNode(declaration, qd.msgs.Cannot_redeclare_exported_variable_0, qy.qf.get.unescUnderscores(id)));
              }
            }
          });
        }
