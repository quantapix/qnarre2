  ...
  return (qf.decl = new (class Fdecl {
    check = new (class extends Fdecl {
      ...
      param(n: qt.ParamDeclaration) {
        ...
        if (n.dot3Token && !n.name.kind === Syntax.BindingPattern && !qf.type.is.assignableTo(qf.type.get.reduced(n.symbol.typeOfSymbol()), anyReadonlyArrayType))
      ...
      classLikeDeclaration(n: qt.ClassLikeDeclaration) {
        ...
        const typeWithThis = qf.type.get.withThisArg(type);
        ...
        if (baseTypeNode) {
          ...
          if (baseTypes.length && produceDiagnostics) {
            ...
            const staticBaseType = qf.type.get.apparent(baseConstructorType);
            ...
            if (qu.some(baseTypeNode.typeArgs)) {
              forEach(baseTypeNode.typeArgs, checkSourceElem);
              for (const constructor of qf.type.get.constructorsForArgs(staticBaseType, baseTypeNode.typeArgs, baseTypeNode)) {
                if (!this.typeArgConstraints(baseTypeNode, constructor.typeParams!)) break;
              }
            }
            const baseWithThis = qf.type.get.withThisArg(baseType, type.thisType);
            if (!qf.type.check.assignableTo(typeWithThis, baseWithThis, undefined)) issueMemberSpecificError(n, typeWithThis, baseWithThis, qd.msgs.Class_0_incorrectly_extends_base_class_1);
              ...
              qf.type.check.assignableTo(staticType, qf.type.get.withoutSignatures(staticBaseType), n.name || n, qd.msgs.Class_static_side_0_incorrectly_extends_base_class_static_side_1);
            }
            ...
            if (!(staticBaseType.symbol && staticBaseType.symbol.flags & SymbolFlags.Class) && !baseConstructorType.isa(TypeFlags.TypeVariable)) {
              const constructors = qf.type.get.instantiatedConstructorsForArgs(staticBaseType, baseTypeNode.typeArgs, baseTypeNode);
        ...
        if (implementedTypeNodes) {
          for (const typeRefNode of implementedTypeNodes) {
            ...
            if (produceDiagnostics) {
              const t = qf.type.get.reduced(qf.get.typeFromTypeNode(typeRefNode));
              if (t !== errorType) {
                if (qf.type.is.validBase(t)) {
                  ...
                  const baseWithThis = qf.type.get.withThisArg(t, type.thisType);
        ...
        for (const member of n.members) {
          ...
          if (isInstancePropertyWithoutIniter(member)) {
            ...
            if (propName.kind === Syntax.Identifier || propName.kind === Syntax.PrivateIdentifier) {
              ...
              if (!(type.isa(TypeFlags.AnyOrUnknown) || qf.type.get.falsyFlags(type) & TypeFlags.Undefined)) {
        ...
        if (produceDiagnostics) {
          ...
          if (n === firstInterfaceDecl) {
            ...
            if (this.inheritedPropertiesAreIdentical(type, n.name)) {
              for (const baseType of qf.type.get.bases(type)) {
                qf.type.check.assignableTo(typeWithThis, qf.type.get.withThisArg(baseType, type.thisType), n.name, qd.msgs.Interface_0_incorrectly_extends_interface_1);
              }
              qf.type.check.qf.type.check.indexConstraints(type);
            }
          }
        ...
        forEach(n.caseBlock.clauses, (clause) => {
          ...
          if (produceDiagnostics && clause.kind === Syntax.CaseClause) {
            ...
            if (!caseIsLiteral || !expressionIsLiteral) {
              caseType = caseIsLiteral ? qf.type.get.baseOfLiteral(caseType) : caseType;
              comparedExpressionType = qf.type.get.baseOfLiteral(expressionType);
            }
    ...
    check = new (class extends Ftype {
      ...
      relatedTo(
        ...
        function reportRelationError(message: qd.Message | undefined, source: Type, target: Type) {
          if (incompatibleStack.length) reportIncompatibleStack();
          const [sourceType, targetType] = qf.type.get.namesForErrorDisplay(source, target);
          let generalizedSource = source;
          let generalizedSourceType = sourceType;
          if (qf.type.is.literal(source) && !typeCouldHaveTopLevelSingletonTypes(target)) {
            generalizedSource = qf.type.get.baseOfLiteral(source);
            generalizedSourceType = qf.type.get.ameForErrorDisplay(generalizedSource);
          }
          if (qf.type.is.param(target)) {
            const constraint = qf.get.qf.type.get.baseConstraint(target);
        ...
        function isRelatedTo(originalSource: Type, originalTarget: Type, reportErrors = false, headMessage?: qd.Message, intersectionState = IntersectionState.None): qt.Ternary {
          ...
          const source = qf.type.get.normalize(originalSource, false);
          let target = qf.type.get.normalize(originalTarget, true);
          if (source === target) return qt.Ternary.True;
          if (relation === identityRelation) return isIdenticalTo(source, target);
          if (qf.type.is.param(source) && qf.type.get.constraint(source) === target) return qt.Ternary.True;
          ...
          if (qf.type.is.union(source)) {
            ...
            if (qf.type.is.union(target))
              result = qf.type.check.relatedToSomeType(
                qf.type.get.regularOfObjectLiteral(source),
                <qt.UnionType>target,
                reportErrors && !source.isa(TypeFlags.Primitive) && !target.isa(TypeFlags.Primitive)
              );
            else if (qf.type.is.intersection(target)) {
              result = qf.type.check.relatedToEachType(qf.type.get.regularOfObjectLiteral(source), target as qt.IntersectionType, reportErrors, IntersectionState.Target);
          ...
          if (
            ...
            ((qf.type.is.intersection(target) && (isPerformingExcessPropertyChecks || isPerformingCommonPropertyChecks)) ||
              (qf.type.is.nonGenericObject(target) &&
                ...
                qf.type.get.apparent(source).flags & TypeFlags.StructuredType &&
        ...
        function getTypeOfPropertyInTypes(ts: Type[], name: qu.__String) {
          const appendPropType = (ps: Type[] | undefined, t: Type) => {
            t = qf.type.get.apparent(t);
            const prop = qf.type.is.unionOrIntersection(t) ? getPropertyOfqt.UnionOrIntersectionType(t, name) : qf.type.get.propertyOfObject(t, name);
        ...
        function hasExcessProperties(source: qt.FreshObjectLiteralType, target: Type, reportErrors: boolean): boolean {
          ...
          for (const prop of qf.type.get.properties(source)) {
        ...
        function relatedToSomeType(source: Type, target: qt.UnionOrIntersectionType, reportErrors: boolean): qt.Ternary {
          ...
          if (reportErrors) {
            const bestMatchingType = qf.type.get.bestMatching(source, target, isRelatedTo);
        ...
        function recursiveTypeRelatedTo(source: Type, target: Type, reportErrors: boolean, intersectionState: IntersectionState): qt.Ternary {
          ...
          const id = qf.type.get.relationKey(source, target, intersectionState | (inPropertyCheck ? IntersectionState.InPropertyCheck : 0), relation);
        ...
        function structuredTypeRelatedTo(source: Type, target: Type, reportErrors: boolean, intersectionState: IntersectionState): qt.Ternary {
          ...
          if (qf.type.is.param(target)) {
            if (source.isobj(ObjectFlags.Mapped) && isRelatedTo(qf.type.get.index(target), getConstraintTypeFromMappedType(<qt.MappedType>source))) {
              if (!(getMappedTypeModifiers(<qt.MappedType>source) & MappedTypeModifiers.IncludeOptional)) {
                ...
                const indexedAccessType = qf.type.get.indexedAccess(target, getTypeParamFromMappedType(<qt.MappedType>source));
            ...
            const constraint = qf.type.get.simplifiedOrConstraint(target.type);
            if (constraint) {
              if (isRelatedTo(source, qf.type.get.index(constraint, target.stringsOnly), reportErrors) === qt.Ternary.True) return qt.Ternary.True;
            }
          } else if (qf.type.is.indexedAccess(target)) {
            if (relation !== identityRelation) {
              const objectType = target.objectType;
              const indexType = target.indexType;
              const baseObjectType = qf.get.qf.type.get.baseConstraint(objectType) || objectType;
            ...
            if (!(modifiers & MappedTypeModifiers.ExcludeOptional)) {
              ...
              if (!qf.type.is.genericMapped(source)) {
                ...
                const sourceKeys = qf.type.get.index(source, true);
                ...
                if (includeOptional ? !(filteredByApplicability!.flags & TypeFlags.Never) : isRelatedTo(targetConstraint, sourceKeys)) {
                  ...
                  const indexedAccessType = qf.type.get.indexedAccess(source, indexingType);
          ...
          if (qf.type.is.variable(source)) {
            if (qf.type.is.indexedAccess(source) && qf.type.is.indexedAccess(target)) {
              ...
              const constraint = qf.type.get.constraint(<TypeVariable>source);
              if (!constraint || (qf.type.is.param(source) && constraint.isa(TypeFlags.Any))) {
                ...
                resetErrorInfo(saveErrorInfo);
                return result;
              } else if ((result = isRelatedTo(qf.type.get.withThisArg(constraint, source), target, reportErrors, undefined, intersectionState))) {
                resetErrorInfo(saveErrorInfo);
                return result;
              }
            }
            ...
            if (relation !== identityRelation) source = qf.type.get.apparent(source);
        ...
        function relatedToDiscriminatedType(source: Type, target: qt.UnionType) {
          const sourceProperties = qf.type.get.properties(source);
          ...
          for (const combination of discriminantCombinations) {
            ...
            outer: for (const type of target.types) {
              for (let i = 0; i < sourcePropertiesFiltered.length; i++) {
                const sourceProperty = sourcePropertiesFiltered[i];
                const targetProperty = qf.type.get.property(type, sourceProperty.escName);
        ...
        function reportUnmatchedProperty(source: Type, target: Type, unmatchedProperty: qt.Symbol, requireOptionalProperties: boolean) {
          ...
          if (
            ...
            if (symbolTableKey && qf.type.get.property(source, symbolTableKey)) {
          ...
          const props = arrayFrom(qf.type.get.unmatchedProperties(source, target, requireOptionalProperties, false));
          if (
            !m ||
          ...
          if (props.length === 1) {
            ...
            reportError(qd.msgs.Property_0_is_missing_in_type_1_but_required_in_type_2, propName, ...qf.type.get.namesForErrorDisplay(source, target));
        ...
        function propertiesRelatedTo(source: Type, target: Type, reportErrors: boolean, excludedProperties: EscapedMap<true> | undefined, intersectionState: IntersectionState): qt.Ternary {
          ...
          const unmatchedProperty = qf.type.get.unmatchedProperty(source, target, requireOptionalProperties, false);
          ...
          if (target.isobj(ObjectFlags.ObjectLiteral)) {
            for (const sourceProp of excludeProperties(qf.type.get.properties(source), excludedProperties)) {
              if (!qf.type.get.propertyOfObject(target, sourceProp.escName)) {
          ...
          for (const targetProp of excludeProperties(properties, excludedProperties)) {
            ...
            if (!(targetProp.flags & qt.SymbolFlags.Prototype) && (!numericNamesOnly || qt.NumericLiteral.name(name) || name === 'length')) {
              const sourceProp = qf.type.get.property(source, name);
        ...
        function propertiesIdenticalTo(source: Type, target: Type, excludedProperties: EscapedMap<true> | undefined): qt.Ternary {
          ...
          const sourceProperties = excludeProperties(qf.type.get.propertiesOfObject(source), excludedProperties);
          const targetProperties = excludeProperties(qf.type.get.propertiesOfObject(target), excludedProperties);
          if (sourceProperties.length !== targetProperties.length) return qt.Ternary.False;
          let result = qt.Ternary.True;
          for (const sourceProp of sourceProperties) {
            const targetProp = qf.type.get.propertyOfObject(target, sourceProp.escName);
        ...
        function eachPropertyRelatedTo(source: Type, target: Type, kind: IndexKind, reportErrors: boolean): qt.Ternary {
          let result = qt.Ternary.True;
          const props = qf.type.is.intersection(source) ? getPropertiesOfqt.UnionOrIntersectionType(<qt.IntersectionType>source) : qf.type.get.propertiesOfObject(source);
        ...
        function indexTypesIdenticalTo(t: Type, to: Type, k: IndexKind): qt.Ternary {
          const targetInfo = qf.type.get.indexInfo(to, k);
          const sourceInfo = qf.type.get.indexInfo(t, k);
      ...
      spreadPropOverrides(t: Type, ps: qt.SymbolTable, s: qt.SpreadAssignment | qt.JsxSpreadAttribute) {
        for (const right of qf.type.get.properties(t)) {
      ...
      nonNullWithReporter(t: Type, n: Node, cb: (n: Node, k: TypeFlags) => void): Type {
        ...
        const k = (strictNullChecks ? qf.type.get.falsyFlags(t) : t.flags) & TypeFlags.Nullable;
        if (k) {
          cb(n, k);
          const u = qf.type.get.nonNullable(t);
      ...
      privateIdentifierPropAccess(left: Type, right: qt.PrivateIdentifier, s?: qt.Symbol) {
        ...
        const ps = qf.type.get.properties(left);
        if (ps) {
      ...
      indexedAccessIndex(t: Type, n: qt.IndexedAccessTyping | qt.ElemAccessExpression) {
        ...
        if (qf.type.is.assignableTo(indexType, qf.type.get.index(objectType, false))) {
          if (
        ...
        const apparentObjectType = qf.type.get.apparent(objectType);
        if (qf.type.get.indexInfo(apparentObjectType, IndexKind.Number) && qf.type.is.assignableToKind(indexType, TypeFlags.NumberLike)) return t;
        if (qf.type.is.genericObject(objectType)) {
          const propertyName = getPropertyNameFromIndex(indexType, n);
          if (propertyName) {
            const propertySymbol = forEachType(apparentObjectType, (t) => qf.type.get.property(t, propertyName));
      ...
      indexConstraints(t: Type) {
        ...
        const checkIndexConstraintForProperty = (
          ...
          if (propDeclaration && name && (propDeclaration.kind === Syntax.BinaryExpression || name.kind === Syntax.ComputedPropertyName || s.parent === containingType.symbol))
            ...
            const someBaseClassHasBothPropertyAndIndexer = forEach(
              qf.type.get.bases(<qt.InterfaceType>containingType),
              (b) => qf.type.get.propertyOfObject(b, s.escName) && qf.get.indexTypeOfType(b, indexKind)
            );
        ...
        if (stringIndexType || numberIndexType) {
          forEach(qf.type.get.propertiesOfObject(t), (p) => {
        ...
        if (stringIndexType && numberIndexType) {
          ...
          if (!errorNode && t.isobj(ObjectFlags.Interface)) {
            const someBaseTypeHasBothIndexers = forEach(qf.type.get.bases(<qt.InterfaceType>t), (b) => qf.get.indexTypeOfType(b, IndexKind.String) && qf.get.indexTypeOfType(b, IndexKind.Number));
      ...
      kindsOfPropertyMemberOverrides(type: qt.InterfaceType, baseType: qt.BaseType): void {
        const baseProperties = qf.type.get.properties(baseType);
        basePropertyCheck: for (const baseProperty of baseProperties) {
          const base = getTargetSymbol(baseProperty);
          if (base.flags & qt.SymbolFlags.Prototype) continue;
          const baseSymbol = qf.type.get.propertyOfObject(type, base.escName);
          ...
          if (derived === base) {
            ...
            if (baseDeclarationFlags & ModifierFlags.Abstract && (!derivedClassDecl || !qf.has.syntacticModifier(derivedClassDecl, ModifierFlags.Abstract))) {
              for (const otherBaseType of qf.type.get.bases(type)) {
                if (otherBaseType === baseType) continue;
                const baseSymbol = qf.type.get.propertyOfObject(otherBaseType, base.escName);
      ...
      inheritedPropertiesAreIdentical(type: qt.InterfaceType, typeNode: Node): boolean {
        ...
        for (const base of baseTypes) {
          const properties = qf.type.get.properties(qf.type.get.withThisArg(base, type.thisType));
      ...
      typeArgs(signature: qt.Signature, typeArgNodes: readonly qt.Typing[], reportErrors: boolean, headMessage?: qd.Message): qt.Type[] | undefined {
        ...
        for (let i = 0; i < typeArgNodes.length; i++) {
          ...
          if (constraint) {
            ...
            if (
              !qf.type.check.assignableTo(typeArg, qf.type.get.withThisArg(instantiateType(constraint, mapper), typeArg), reportErrors ? typeArgNodes[i] : undefined, typeArgHeadMessage, errorInfo)
            ) {
              return;
            }
          }
