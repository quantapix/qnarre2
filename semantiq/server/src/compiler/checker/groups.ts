  ...
  return (qf.sign = new (class extends _Fsign {
    get = new (class extends Base {
      ...
      signatureInstantiationWithoutFillingInTypeArgs(s: Signature, typeArgs: readonly Type[] | undefined): Signature {
        ...
        const id = qf.type.get.listId(typeArgs);
    ...
    get = new (class extends Base {
      ...
      widenedTypeForAssignmentDeclaration(symbol: Symbol, resolvedSymbol?: Symbol) {
        ...
        if (container) {
          ...
          return containerObjectType || qf.type.get.widenedLiteral(qf.check.expressionCached(container));
        }
        let type;
        ...
        if (!type) {
          ...
          if (!type) {
            ...
            type = qf.type.get.union(sourceTypes!, UnionReduction.Subtype);
          }
        }
        const widened = qf.type.get.widened(addOptionality(t, definedInMethod && !definedInConstructor));
      ...
      initerTypeFromAssignmentDeclaration(symbol: Symbol, resolvedSymbol: Symbol | undefined, expression: qt.BinaryExpression | qt.CallExpression, kind: qt.AssignmentDeclarationKind) {
        ...
        const type = resolvedSymbol ? this.typeOfSymbol(resolvedSymbol) : qf.type.get.widenedLiteral(qf.check.expressionCached(expression.right));
          ...
          (resolvedSymbol || symbol).exports!.forEach((s, name) => {
            ...
            if (exportedMember && exportedMember !== s) {
              if (s.flags & SymbolFlags.Value) {
                ...
                union.type = qf.type.get.union([this.typeOfSymbol(s), this.typeOfSymbol(exportedMember)]);
      ...
      widenedProperty(prop: Symbol, context: qt.WideningContext | undefined): Symbol {
        ...
        const widened = qf.type.get.widenedWithContext(original, propContext);
  ...
  return (qf.type = new (class Base extends _Ftype {
    _get = new (class extends Base {
      ...
      intersectionType(types: readonly Type[], aliasSymbol?: Symbol, aliasTypeArgs?: readonly Type[]): Type {
        ...
        const id = qf.type.get.listId(typeSet);
        ...
        if (!result) {
          if (includes & TypeFlags.Union) {
            if (intersectUnionsOfPrimitiveTypes(typeSet)) result = this.intersectionType(typeSet, aliasSymbol, aliasTypeArgs);
              ...
              result = qf.type.get.union([this.intersectionType(typeSet), undefinedType], UnionReduction.Literal, aliasSymbol, aliasTypeArgs);
            } else if (extractIrreducible(typeSet, TypeFlags.Null)) {
              result = qf.type.get.union([this.intersectionType(typeSet), nullType], UnionReduction.Literal, aliasSymbol, aliasTypeArgs);
            } else {
        ...
        if (!type.resolvedBaseConstructorType) {
          ...
          if (!(baseConstructorType.isa(TypeFlags.Any)) && baseConstructorType !== nullWideningType && !qf.type.is.constructr(baseConstructorType)) {
            ...
            if (qf.type.is.param(baseConstructorType)) {
              const constraint = qf.type.get.constraintFromParam(baseConstructorType);
      ...
      constraintFromIndexedAccess(t: qt.IndexedAccessType) {
        ...
        if (indexConstraint && indexConstraint !== t.indexType) {
          const indexedAccess = qf.type.get.indexedAccessOrUndefined(t.objectType, indexConstraint);
          if (indexedAccess) return indexedAccess;
        }
        const objectConstraint = qf.type.get.simplifiedOrConstraint(t.objectType);
        if (objectConstraint && objectConstraint !== t.objectType) return qf.type.get.indexedAccessOrUndefined(objectConstraint, t.indexType);
        return;
      }
      defaultConstraintOfConditionalType(t: qt.ConditionalType) {
        if (!type.resolvedDefaultConstraint) {
          ...
          t.resolvedDefaultConstraint = qf.type.is.any(trueConstraint) ? falseConstraint : qf.type.is.any(falseConstraint) ? trueConstraint : qf.type.get.union([trueConstraint, falseConstraint]);
        }
      ...
      constraintOfDistributiveConditionalType(t: qt.ConditionalType): Type | undefined {
        if (t.root.isDistributive && t.restrictive !== type) {
          const simplified = qf.type.get.simplified(t.checkType, false);
      ...
      resolvedBaseConstraint(t: qt.InstantiableType | qt.UnionOrIntersectionType): Type {
        ...
        const immediateBaseConstraint = (t: Type): Type => {
          if (!t.immediateBaseConstraint) {
            ...
            constraintDepth++;
            let result = computeBaseConstraint(qf.type.get.simplified(t, false));
            constraintDepth--;
            if (!popTypeResolution()) {
        ...
        const computeBaseConstraint = (t: Type): Type | undefined => {
          if (qf.type.is.param(t)) {
            const constraint = qf.type.get.constraintFromParam(<TypeParam>t);
          ...
          if (t.isa(TypeFlags.IndexedAccess)) {
            ...
            const baseIndexedAccess = baseObjectType && baseIndexType && qf.type.get.indexedAccessOrUndefined(baseObjectType, baseIndexType);
      ...
      reducedUnionType(unionType: qt.UnionType) {
        ...
        const reduced = qf.type.get.union(reducedTypes);
      ...
      simplifiedIndexedAccessType(t: qt.IndexedAccessType, writing: boolean): Type {
        ...
        const objectType = unwrapSubstitution(qf.type.get.simplified(t.objectType, writing));
        const indexType = qf.type.get.simplified(t.indexType, writing);
        ...
        if (qf.type.is.genericMapped(objectType)) return (type[cache] = mapType(substituteIndexedMappedType(objectType, t.indexType), (t) => qf.type.get.simplified(t, writing)));
      ...
      simplifiedConditionalType(t: qt.ConditionalType, writing: boolean) {
        ...
        if (falseType.isa(TypeFlags.Never) && qf.type.get.actualVariable(trueType) === qf.type.get.actualVariable(checkType)) {
          if (checkType.isa(TypeFlags.Any) || qf.type.is.assignableTo(this.restrictive(checkType), this.restrictive(extendsType))) return qf.type.get.simplified(trueType, writing);
          else if (qf.type.is.intersectionEmpty(checkType, extendsType)) return neverType;
        } else if (trueType.isa(TypeFlags.Never) && qf.type.get.actualVariable(falseType) === qf.type.get.actualVariable(checkType)) {
          ...
          else if (checkType.isa(TypeFlags.Any) || qf.type.is.intersectionEmpty(checkType, extendsType)) return qf.type.get.simplified(falseType, writing);
        }
        return type;
      }
      ...
      propertyTypeForIndexType(
        ...
        if (propName !== undefined) {
          ...
          if (everyType(objectType, qf.is.tupleType) && qt.NumericLiteral.name(propName) && +propName >= 0) {
            ...
            errorIfWritingToReadonlyIndex(qf.type.get.indexInfo(objectType, qt.IndexKind.Number));
        ...
        if (!(indexType.isa(TypeFlags.Nullable)) && qf.type.is.assignableToKind(indexType, TypeFlags.StringLike | TypeFlags.NumberLike | TypeFlags.ESSymbolLike)) {
          ...
          const stringIndexInfo = qf.type.get.indexInfo(objectType, qt.IndexKind.String);
          const indexInfo = (qf.type.is.assignableToKind(indexType, TypeFlags.NumberLike) && qf.type.get.indexInfo(objectType, qt.IndexKind.Number)) || stringIndexInfo;
          if (indexInfo) {
      ...
      indexedAccessOrUndefined(
        ...
        const apparentObjectType = qf.type.get.reducedApparent(objectType);
        if (qf.type.is.union(indexType) && !(indexType.isa(TypeFlags.Boolean))) {
          ...
          return accessFlags & AccessFlags.Writing ? this.intersectionType(propTypes) : qf.type.get.union(propTypes);
        }
      ...
      objectTypeInstantiation(t: qt.AnonymousType | qt.DeferredTypeReference, mapper: TypeMapper) {
        ...
        if (!typeParams) {
          ...
          if (typeParams.length) {
            ...
            links.instantiations.set(qf.type.get.listId(typeParams), target);
          }
        }
        if (typeParams.length) {
          const combinedMapper = combineTypeMappers(t.mapper, mapper);
          const typeArgs = map(typeParams, (t) => qf.type.get.mapped(t, combinedMapper));
          const id = qf.type.get.listId(typeArgs);
      ...
      homomorphicTypeVariable(t: qt.MappedType) {
        ...
        if (constraintType.isa(TypeFlags.Index)) {
          const typeVariable = qf.type.get.actualVariable((<qt.IndexType>constraintType).type);
      ...
      conditionalTypeInstantiation(t: qt.ConditionalType, mapper: TypeMapper): Type {
        ...
        if (root.outerTypeParams) {
          const typeArgs = map(root.outerTypeParams, (t) => qf.type.get.mapped(t, mapper));
          const id = qf.type.get.listId(typeArgs);
      ...
      uninstantiatedSignaturesOfType(elemType: Type, caller: qt.JsxOpeningLikeElem): readonly Signature[] {
        ...
        const apparentElemType = qf.type.get.apparent(elemType);
      ...
      iteratedTypeOrElemType(use: IterationUse, inputType: Type, sentType: Type, errorNode: Node | undefined, checkAssignability: boolean): Type | undefined {
        ...
        if (uplevelIteration || downlevelIteration || allowAsyncIterables) {
          const iterationTypes = qf.type.get.itersOfIterable(inputType, use, uplevelIteration ? errorNode : undefined);
        ...
        if (use & IterationUse.AllowsStringInputFlag) {
          if (qf.type.is.union(arrayType)) {
            ...
            if (filteredTypes !== arrayTypes) arrayType = qf.type.get.union(filteredTypes, UnionReduction.Subtype);
        ...
        if (hasStringConstituent && arrayElemType) {
          ...
          return qf.type.get.union([arrayElemType, stringType], UnionReduction.Subtype);
        }
      ...
      iterationTypeOfIterable(use: IterationUse, typeKind: IterationTypeKind, inputType: Type, errorNode: Node | undefined): Type | undefined {
        ...
        const iterationTypes = qf.type.get.itersOfIterable(inputType, use, errorNode);
      ...
      flow = new (class extends Base {
        narrowTypeByAssertion(t: Type, expr: qt.Expression): Type {
          ...
          if (n.kind === Syntax.BinaryExpression) {
            ...
            if (n.operatorToken.kind === Syntax.Bar2Token) return qf.type.get.union([narrowTypeByAssertion(t, n.left), narrowTypeByAssertion(t, n.right)]);
          }
        ...
        narrowTypeByTruthiness(t: Type, expr: qt.Expression, assumeTrue: boolean): Type {
          if (qf.is.matchingReference(reference, expr)) return qf.type.get.withFacts(t, assumeTrue ? TypeFacts.Truthy : TypeFacts.Falsy);
          if (strictNullChecks && assumeTrue && optionalChainContainsReference(expr, reference)) type = qf.type.get.withFacts(t, TypeFacts.NEUndefinedOrNull);
          if (qf.is.matchingReferenceDiscriminant(expr, type))
            return narrowTypeByDiscriminant(t, <qt.AccessExpression>expr, (t) => qf.type.get.withFacts(t, assumeTrue ? TypeFacts.Truthy : TypeFacts.Falsy));
          return type;
        }
        isTypePresencePossible(t: Type, propName: qu.__String, assumeTrue: boolean) {
          if (qf.type.get.indexInfo(t, qt.IndexKind.String)) return true;
        ...
        narrowTypeByOptionalChainContainment(t: Type, operator: Syntax, value: qt.Expression, assumeTrue: boolean): Type {
          ...
          return removeNullable ? qf.type.get.withFacts(t, TypeFacts.NEUndefinedOrNull) : type;
        }
        narrowTypeByEquality(t: Type, operator: Syntax, value: qt.Expression, assumeTrue: boolean): Type {
          ...
          if (valueType.isa(TypeFlags.Nullable)) {
            ...
            return qf.type.get.withFacts(t, facts);
          }
          ...
          if (valueType.isa(TypeFlags.Unit)) {
            const regularType = qf.type.get.regularOfLiteral(valueType);
            return filterType(t, (t) => (t.isa(TypeFlags.Unit) ? !areTypesComparable(t, valueType) : qf.type.get.regularOfLiteral(t) !== regularType));
          }
          return type;
        }
        narrowTypeByTypeof(t: Type, typeOfExpr: TypeOfExpression, operator: Syntax, literal: qt.LiteralExpression, assumeTrue: boolean): Type {
          ...
          if (!qf.is.matchingReference(reference, target)) {
            if (strictNullChecks && optionalChainContainsReference(target, reference) && assumeTrue === (literal.text !== 'undefined')) return qf.type.get.withFacts(t, TypeFacts.NEUndefinedOrNull);
            return type;
          }
          ...
          if (assumeTrue && t.isa(TypeFlags.Unknown) && literal.text === 'object') {
            ...
            return qf.type.get.union([nonPrimitiveType, nullType]);
          }
          ...
          return qf.type.get.withFacts(assumeTrue && impliedType ? mapType(t, narrowUnionMemberByTypeof(impliedType)) : type, facts);
        }
        narrowTypeBySwitchOptionalChainContainment(t: Type, switchStatement: qt.SwitchStatement, clauseStart: number, clauseEnd: number, clauseCheck: (t: Type) => boolean) {
          ...
          return everyClauseChecks ? qf.type.get.withFacts(t, TypeFacts.NEUndefinedOrNull) : type;
        }
        narrowTypeBySwitchOnDiscriminant(t: Type, switchStatement: qt.SwitchStatement, clauseStart: number, clauseEnd: number) {
          ...
          if (t.isa(TypeFlags.Unknown) && !hasDefaultClause) {
            ...
            return qf.type.get.union(groundClauseTypes === undefined ? clauseTypes : groundClauseTypes);
          }
          const discriminantType = qf.type.get.union(clauseTypes);
          const caseType =
          ...
          const defaultType = filterType(t, (t) => !(t.isa(TypeFlags.Unit) && contains(switchTypes, qf.type.get.regularOfLiteral(t))));
          return caseType.isa(TypeFlags.Never) ? defaultType : qf.type.get.union([caseType, defaultType]);
        }
        impliedTypeFromTypeofGuard(t: Type, text: string) {
          switch (text) {
            ...
            case 'object':
              return t.isa(TypeFlags.Unknown) ? qf.type.get.union([nonPrimitiveType, nullType]) : type;
            default:
        ...
        narrowBySwitchOnTypeOf(t: Type, switchStatement: qt.SwitchStatement, clauseStart: number, clauseEnd: number): Type {
          ...
          if (hasDefaultClause) return filterType(t, (t) => (qf.type.get.facts(t) & switchFacts) === switchFacts);
          const impliedType = qf.type.get.withFacts(qf.type.get.union(clauseWitnesses.map((text) => this.impliedTypeFromTypeofGuard(t, text) || type)), switchFacts);
          return qf.type.get.withFacts(mapType(t, narrowUnionMemberByTypeof(impliedType)), switchFacts);
        }
        ...
        narrowTypeByInstanceof(t: Type, expr: qt.BinaryExpression, assumeTrue: boolean): Type {
          ...
          if (!qf.is.matchingReference(reference, left)) {
            if (assumeTrue && strictNullChecks && optionalChainContainsReference(left, reference)) return qf.type.get.withFacts(t, TypeFacts.NEUndefinedOrNull);
            return type;
          }
          ...
          if (!targetType) {
            ...
            targetType = constrSignatures.length ? qf.type.get.union(map(constrSignatures, (s) => this.returnTypSignature(s.erased()))) : qu.emptyObjectType;
          }
        ...
        narrowTypeByTypePredicate(t: Type, predicate: TypePredicate, callExpression: qt.CallExpression, assumeTrue: boolean): Type {
          if (predicate.type && !(qf.type.is.any(t) && (predicate.type === globalObjectType || predicate.type === globalFunctionType))) {
            ...
            if (predicateArg) {
              ...
              if (strictNullChecks && assumeTrue && optionalChainContainsReference(predicateArg, reference) && !(qf.type.get.facts(predicate.type) & TypeFacts.EQUndefined))
                type = qf.type.get.withFacts(t, TypeFacts.NEUndefinedOrNull);
        ...
        narrowTypeByOptionality(t: Type, expr: qt.Expression, assumePresent: boolean): Type {
          if (qf.is.matchingReference(reference, expr)) return qf.type.get.withFacts(t, assumePresent ? TypeFacts.NEUndefinedOrNull : TypeFacts.EQUndefinedOrNull);
          if (qf.is.matchingReferenceDiscriminant(expr, type))
            return narrowTypeByDiscriminant(t, <qt.AccessExpression>expr, (t) => qf.type.get.withFacts(t, assumePresent ? TypeFacts.NEUndefinedOrNull : TypeFacts.EQUndefinedOrNull));
          return type;
        }
      })();
        */
    ...
    _is = new (class extends Base {
      literalTypesWithSameBaseType(ts: qt.Type[]): boolean {
        ...
        for (const t of ts) {
          const b = qf.type.get.baseOfLiteral(t);
          if (!c) c = b;
    ...
    removeOptionalityFromDeclaredType(declaredType: qt.Type, declaration: qt.VariableLikeDeclaration): qt.Type {
      if (pushTypeResolution(declaration.symbol, TypeSystemPropertyName.DeclaredType)) {
        const annotationIncludesUndefined =
          ...
          declaration.initer &&
          qf.type.get.falsyFlags(declaredType) & qt.TypeFlags.Undefined &&
          !(qf.type.get.falsyFlags(check.expression(declaration.initer)) & qt.TypeFlags.Undefined);
        popTypeResolution();
        return annotationIncludesUndefined ? qf.type.get.withFacts(declaredType, TypeFacts.NEUndefined) : declaredType;
      } else {
    ...
    addEvolvingArrayElemType(evolvingArrayType: qt.EvolvingArrayType, node: qt.Expression): qt.EvolvingArrayType {
      const elemType = qf.type.get.baseOfLiteral(getContextFreeTypeOfExpression(node));
      return this.is.subsetOf(elemType, evolvingArrayType.elemType) ? evolvingArrayType : qf.type.get.evolvingArray(qf.type.get.union([evolvingArrayType.elemType, elemType]));
    }
    finalizeEvolvingArrayType(t: qt.Type): qt.Type {
      return this.is.evolvingArray(t) ? qf.type.get.finalArray(<qt.EvolvingArrayType>t) : t;
    }
    ...
    removeRedundantLiteralTypes(ts: qt.Type[], f: qt.TypeFlags) {
      ...
      while (i > 0) {
        ...
        const remove =
          ...
          (this.is.freshLiteral(t) && qf.type.has.type(ts, (<qt.LiteralType>t).regularType));
    ...
    intersectUnionsOfPrimitiveTypes(types: qt.Type[]) {
      ...
      types[index] = qf.type.get.unionFromSortedList(result, ObjectFlags.PrimitiveUnion);
      return true;
    }
    ...
    tryMergeUnionOfObjectTypeAndEmptyObject(type: qt.UnionType, readonly: boolean): qt.Type | undefined {
      ...
      function getAnonymousPartialType(type: qt.Type) {
        ...
        const spread = qf.make.anonymousType(type.symbol, members, qu.empty, qu.empty, qf.type.get.indexInfo(type, IndexKind.String), qf.type.get.indexInfo(type, IndexKind.Number));
