...
export function create(host: qt.TypeCheckerHost, produceDiagnostics: boolean): qt.TypeChecker {
  ...
  function lookupOrIssueError(location: Node | undefined, message: qd.Message, arg0?: string | number, arg1?: string | number, arg2?: string | number, arg3?: string | number): qd.Diagnostic {
    const diagnostic = location ? qf.make.diagForNode(location, message, arg0, arg1, arg2, arg3) : createCompilerDiagnostic(message, arg0, arg1, arg2, arg3);
  ...
  function errorOrSuggestion(
    ...
    addErrorOrSuggestion(isError, 'message' in message ? qf.make.diagForNode(location, message, arg0, arg1, arg2, arg3) : qf.make.diagForNodeFromMessageChain(location, message));
  }
  function errorAndMaybeSuggestAwait(
    ...
    if (maybeMissingAwait) {
      const related = qf.make.diagForNode(location, qd.msgs.Did_you_forget_to_use_await);
  ...
  function addDuplicateDeclarationError(node: qt.Declaration, message: qd.Message, symbolName: string, relatedNodes: readonly qt.Declaration[] | undefined) {
    ...
    for (const relatedNode of relatedNodes || empty) {
      ...
      const leadingMessage = qf.make.diagForNode(adjustedNode, qd.msgs._0_was_also_declared_here, symbolName);
      const followOnMessage = qf.make.diagForNode(adjustedNode, qd.msgs.and_here);
      if (
  ...
  function reportNonExportedMember(node: qt.ImportDeclaration | qt.ExportDeclaration, name: qt.Identifier, declarationName: string, moduleSymbol: qt.Symbol, moduleName: string): void {
    ...
    if (localSymbol) {
      ...
      if (exportedEqualsSymbol) {
        ...
        addRelatedInfo(diagnostic, ...map(localSymbol.declarations, (decl, index) => qf.make.diagForNode(decl, index === 0 ? qd.msgs._0_is_declared_here : qd.msgs.and_here, declarationName)));
      }
    } else {
      error(name, qd.msgs.Module_0_has_no_exported_member_1, moduleName, declarationName);
    }
      ...
      if (!(t.isa(qt.TypeFlags.Nullable))) {
        if (t.flags & (TypeFlags.BooleanLiteral | qt.TypeFlags.EnumLiteral)) {
          ...
          if (baseType.isa(qt.TypeFlags.Union)) {
            ...
            if (i + count <= types.length && qf.type.get.regularOfLiteral(types[i + count - 1]) === qf.type.get.regularOfLiteral((<qt.UnionType>baseType).types[count - 1])) {
  ...
  function intersectIndexInfos(info1: qt.IndexInfo | undefined, info2: qt.IndexInfo | undefined): qt.IndexInfo | undefined {
    return !info1 ? info2 : !info2 ? info1 : qf.make.indexInfo(qf.get.intersectionType([info1.type, info2.type]), info1.isReadonly && info2.isReadonly);
  }
  function unionSpreadIndexInfos(info1: qt.IndexInfo | undefined, info2: qt.IndexInfo | undefined): qt.IndexInfo | undefined {
    return info1 && info2 && qf.make.indexInfo(qf.type.get.union([info1.type, info2.type]), info1.isReadonly || info2.isReadonly);
  }
  function findMixins(types: readonly qt.Type[]): readonly boolean[] {
    const constructorTypeCount = countWhere(types, (t) => qf.type.get.signatures(t, qt.SignatureKind.Construct).length > 0);
  ...
  function addTypeToIntersection(typeSet: qu.QMap<qt.Type>, includes: qt.TypeFlags, type: qt.Type) {
    ...
    if (type.isa(qt.TypeFlags.Intersection)) return addTypesToIntersection(typeSet, includes, (<qt.IntersectionType>type).types);
    if (qf.type.is.emptyAnonymousObject(type)) {
      ...
      if (type.isa(qt.TypeFlags.AnyOrUnknown))
        if (type === wildcardType) includes |= qt.TypeFlags.IncludesWildcard;
        else if ((strictNullChecks || !(type.isa(qt.TypeFlags.Nullable))) && !typeSet.has(type.id.toString())) {
          if (type.isa(qt.TypeFlags.Unit) && includes & qt.TypeFlags.Unit) includes |= qt.TypeFlags.NonPrimitive;
          typeSet.set(type.id.toString(), type);
        }
      includes |= type.isa(qt.TypeFlags.IncludesMask);
    }
    return includes;
  }
  function addTypesToIntersection(typeSet: qu.QMap<qt.Type>, includes: qt.TypeFlags, types: readonly qt.Type[]) {
    for (const type of types) {
      includes = addTypeToIntersection(typeSet, includes, qf.type.get.regularOfLiteral(type));
    }
    return includes;
  }
  ...
  function cloneTypeParam(typeParam: qt.TypeParam): qt.TypeParam {
    const result = qf.make.typeParam(typeParam.symbol);
  ...
  function elaborateDidYouMeanToCallOrConstruct(
    ...
    const callSignatures = qf.type.get.signatures(source, qt.SignatureKind.Call);
    const constructSignatures = qf.type.get.signatures(source, qt.SignatureKind.Construct);
    for (const signatures of [constructSignatures, callSignatures]) {
      if (
        ...
        addRelatedInfo(
          diagnostic,
          qf.make.diagForNode(node, signatures === constructSignatures ? qd.msgs.Did_you_mean_to_use_new_with_this_expression : qd.msgs.Did_you_mean_to_call_this_expression)
        );
        return true;
      }
    }
  ...
  function elaborateArrowFunction(
    ...
    const targetSignatures = qf.type.get.signatures(target, qt.SignatureKind.Call);
    ...
    const targetReturn = qf.type.get.union(map(targetSignatures, qf.get.returnTypeOfSignature));
      ...
      if (resultObj.errors) {
        if (target.symbol && length(target.symbol.declarations)) {
          addRelatedInfo(
            ...
            qf.make.diagForNode(target.symbol.declarations[0], qd.msgs.The_expected_type_comes_from_the_return_type_of_this_signature)
          );
        }
        if (
  ...
  function elaborateElemwise(
    ...
    for (let status = iterator.next(); !status.done; status = iterator.next()) {
      ...
      const targetPropType = qf.type.get.bestMatchIndexedAccessOrUndefined(source, target, nameType);
      if (!targetPropType || targetPropType.isa(qt.TypeFlags.IndexedAccess)) continue;
      const sourcePropType = qf.type.get.indexedAccessOrUndefined(source, nameType);
      if (sourcePropType && !qf.type.check.relatedTo(sourcePropType, targetPropType, relation, undefined)) {
        ...
        if (elaborated) reportedError = true;
          ...
          if (resultObj.errors) {
            ...
            const targetProp = propertyName !== undefined ? qf.type.get.property(target, propertyName) : undefined;
            let issuedElaboration = false;
            if (!targetProp) {
              const indexInfo =
                (qf.type.is.assignableToKind(nameType, qt.TypeFlags.NumberLike) && qf.type.get.indexInfo(target, IndexKind.Number)) || qf.type.get.indexInfo(target, IndexKind.String) || undefined;
              if (indexInfo && indexInfo.declaration && !indexInfo.declaration.sourceFile.hasNoDefaultLib) {
                issuedElaboration = true;
                addRelatedInfo(reportedDiag, qf.make.diagForNode(indexInfo.declaration, qd.msgs.The_expected_type_comes_from_this_index_signature));
              }
            }
              ...
              if (!targetNode.sourceFile.hasNoDefaultLib) {
                addRelatedInfo(
                  ...
                  qf.make.diagForNode(
                    ...
                    propertyName && !(nameType.isa(qt.TypeFlags.UniqueESSymbol)) ? qy.get.unescUnderscores(propertyName) : typeToString(nameType),
                    typeToString(target)
                  )
                );
              }
  ...
  function elaborateJsxComponents(
    ...
    if (node.parent.kind === Syntax.JsxOpeningElem && node.parent.parent.kind === Syntax.JsxElem) {
      ...
      const childrenTargetType = qf.type.get.indexedAccess(target, childrenNameType);
      ...
      if (moreThanOneRealChildren) {
        ...
        } else if (!qf.type.is.relatedTo(qf.type.get.indexedAccess(source, childrenNameType), childrenTargetType, relation)) {
          result = true;
          const diag = error(
    ...
    function getInvalidTextualChildDiagnostic() {
      if (!invalidTextDiagnostic) {
        ...
        const childPropName = getJsxElemChildrenPropertyName(getJsxNamespaceAt(node));
        const childrenPropName = childPropName === undefined ? 'children' : qy.get.unescUnderscores(childPropName);
        const childrenTargetType = qf.type.get.indexedAccess(target, qf.get.literalType(childrenPropName));
  ...
  function* generateLimitedTupleElems(node: qt.ArrayLiteralExpression, target: qt.Type): ElaborationIterator {
    ...
    if (!len) return;
    for (let i = 0; i < len; i++) {
      if (qf.type.is.tupleLike(target) && !qf.type.get.property(target, ('' + i) as qu.__String)) continue;
  ...
  function elaborateArrayLiteral(
    ...
    if (target.isa(qt.TypeFlags.Primitive)) return false;
  ...
  function* generateObjectLiteralElems(node: qt.ObjectLiteralExpression): ElaborationIterator {
    ...
    for (const prop of node.properties) {
      ...
      const type = qf.type.get.literalFromProperty(qf.get.symbolOfNode(prop), qt.TypeFlags.StringOrNumberLiteralOrUnique);
      if (!type || type.isa(qt.TypeFlags.Never)) continue;
      switch (prop.kind) {
  ...
  function reportImplicitAny(declaration: qt.Declaration, type: qt.Type, wideningKind?: WideningKind) {
    const typeAsString = typeToString(qf.type.get.widened(type));
  ...
  function reportErrorsFromWidening(declaration: qt.Declaration, type: qt.Type, wideningKind?: WideningKind) {
    if (
      produceDiagnostics &&
      noImplicitAny &&
      type.isobj(ObjectFlags.ContainsWideningType) &&
  ...
  function inferTypes(inferences: qt.InferenceInfo[], originalSource: qt.Type, originalTarget: qt.Type, priority: InferencePriority = 0, contravariant = false) {
    ...
    function inferFromTypes(source: qt.Type, target: qt.Type): void {
      ...
      if (source === target && source.isa(qt.TypeFlags.UnionOrIntersection)) {
        for (const t of (<qt.UnionOrIntersectionType>source).types) {
          inferFromTypes(t, t);
        }
        return;
      }
      if (target.isa(qt.TypeFlags.Union)) {
        const [tempSources, tempTargets] = inferFromMatchingTypes(source.isa(qt.TypeFlags.Union) ? (<qt.UnionType>source).types : [source], (<qt.UnionType>target).types, qf.type.is.orBaseIdenticalTo);
        const [sources, targets] = inferFromMatchingTypes(tempSources, tempTargets, qf.type.is.closelyMatchedBy);
        if (targets.length === 0) return;
        target = qf.type.get.union(targets);
        if (sources.length === 0) {
          inferWithPriority(source, target, InferencePriority.NakedTypeVariable);
          return;
        }
        source = qf.type.get.union(sources);
      } else if (
        target.isa(qt.TypeFlags.Intersection) &&
        ...
        if (!(source.isa(qt.TypeFlags.Union))) {
          const [sources, targets] = inferFromMatchingTypes(
            source.isa(qt.TypeFlags.Intersection) ? (<qt.IntersectionType>source).types : [source],
        ...
        target = qf.type.get.actualVariable(target);
      }
      if (target.isa(qt.TypeFlags.TypeVariable)) {
        if (
          source.isobj(ObjectFlags.NonInferrableType) ||
        ...
        if (inference) {
          if (!inference.isFixed) {
            ...
            if (
              ...
              target.isa(qt.TypeFlags.TypeParam) &&
              inference.topLevel &&
          ...
          const simplified = qf.type.get.simplified(target, false);
          if (simplified !== target) invokeOnce(source, simplified, inferFromTypes);
          else if (target.isa(qt.TypeFlags.IndexedAccess)) {
            const indexType = qf.type.get.simplified((target as qt.IndexedAccessType).indexType, false);
            if (indexType.isa(qt.TypeFlags.Instantiable)) {
              const simplified = distributeIndexOverObjectType(qf.type.get.simplified((target as qt.IndexedAccessType).objectType, false), indexType, false);
      ...
      } else if (source.isa(qt.TypeFlags.Index) && target.isa(qt.TypeFlags.Index)) {
        contravariant = !contravariant;
        inferFromTypes((<qt.IndexType>source).type, (<qt.IndexType>target).type);
        contravariant = !contravariant;
      } else if ((qf.type.is.literal(source) || source.isa(qt.TypeFlags.String)) && target.isa(qt.TypeFlags.Index)) {
        const empty = createEmptyObjectTypeFromStringLiteral(source);
        contravariant = !contravariant;
        inferWithPriority(empty, (target as qt.IndexType).type, InferencePriority.LiteralKeyof);
        contravariant = !contravariant;
      } else if (source.isa(qt.TypeFlags.IndexedAccess) && target.isa(qt.TypeFlags.IndexedAccess)) {
      ...
      } else if (source.isa(qt.TypeFlags.Conditional) && target.isa(qt.TypeFlags.Conditional)) {
      ...
      } else if (target.isa(qt.TypeFlags.Conditional)) {
      ...
      if (source.isa(qt.TypeFlags.Simplifiable)) {
        const simplified = qf.type.get.simplified(source, contravariant);
    ...
    function getInferenceInfoForType(type: qt.Type) {
      if (type.isa(qt.TypeFlags.TypeVariable)) {
        for (const inference of inferences) {
    ...
    function getSingleTypeVariableFromIntersectionTypes(types: qt.Type[]) {
      ...
      for (const type of types) {
        const t = type.isa(qt.TypeFlags.Intersection) && qf.find.up((<qt.IntersectionType>type).types, (t) => !!getInferenceInfoForType(t));
    ...
    function inferToMultipleTypes(source: qt.Type, targets: qt.Type[], f: qt.TypeFlags) {
      ...
      if (f & qt.TypeFlags.Union) {
        ...
        const sources = source.isa(qt.TypeFlags.Union) ? (<qt.UnionType>source).types : [source];
        ...
        if (typeVariableCount === 1 && !inferenceCircularity) {
          ...
          if (unmatched.length) {
            inferFromTypes(qf.type.get.union(unmatched), nakedTypeVariable!);
            return;
          }
        }
      } else {
    ...
    function inferToMappedType(source: qt.Type, target: qt.MappedType, constraintType: qt.Type): boolean {
      if (constraintType.isa(qt.TypeFlags.Union)) {
        let result = false;
      ...
      if (constraintType.isa(qt.TypeFlags.Index)) {
        ...
        if (inference && !inference.isFixed && !qf.type.is.fromInferenceBlockedSource(source)) {
          ...
          if (inferredType) {
            inferWithPriority(
              inferredType,
              inference.typeParam,
              source.isobj(ObjectFlags.NonInferrableType) ? InferencePriority.PartialHomomorphicMappedType : InferencePriority.HomomorphicMappedType
            );
          }
        }
        return true;
      ...
      if (constraintType.isa(qt.TypeFlags.TypeParam)) {
        inferWithPriority(qf.type.get.index(source), constraintType, InferencePriority.MappedTypeConstraint);
        ...
        inferFromTypes(qf.type.get.union(append(append(propTypes, stringIndexType), numberIndexType)), getTemplateTypeFromMappedType(target));
        return true;
      }
      return false;
    }
    function inferFromObjectTypes(source: qt.Type, target: qt.Type) {
      const isNonConstructorObject = target.isa(qt.TypeFlags.Object) && !(qf.type.is.anonymous(target) && target.symbol && target.symbol.flags & qt.SymbolFlags.Class);
    ...
    function inferFromObjectTypesWorker(source: qt.Type, target: qt.Type) {
      ...
      if (target.isobj(ObjectFlags.Mapped)) {
      ...
      if (!typesDefinitelyUnrelated(source, target)) {
        if (qf.type.is.array(source) || qf.type.is.tuple(source)) {
          if (qf.type.is.tuple(target)) {
            ...
            const sourceRestType = qf.type.is.tuple(source) ? getRestTypeOfTupleType(source) : qf.type.get.elemOfArray(source);
            ...
            if (targetRestType) {
              ...
              if (types.length) inferFromTypes(qf.type.get.union(types), targetRestType);
            }
            return;
          }
    ...
    function inferFromSignatures(source: qt.Type, target: qt.Type, kind: qt.SignatureKind) {
      ...
      const skipParams = (source.isobj(ObjectFlags.NonInferrableType));
      for (let i = 0; i < len; i++) {
  ...
  function reportFlowControlError(node: Node) {
    ...
    diagnostics.add(qf.make.fileDiag(sourceFile, span.start, span.length, qd.msgs.The_containing_function_or_module_body_is_too_large_for_control_flow_analysis));
  }
  ...
  function reportNonexistentProperty(propNode: qc.Identifier | qc.PrivateIdentifier, containingType: qt.Type) {
    ...
    if (propNode.kind !== Syntax.PrivateIdentifier && containingType.isa(qt.TypeFlags.Union) && !(containingType.isa(qt.TypeFlags.Primitive))) {
      for (const subtype of (containingType as qt.UnionType).types) {
        if (!qf.type.get.property(subtype, propNode.escapedText) && !qf.type.get.indexInfo(subtype, IndexKind.String)) {
    ...
    if (typeHasStaticProperty(propNode.escapedText, containingType))
      ...
      if (promisedType && qf.type.get.property(promisedType, propNode.escapedText)) {
        ...
        relatedInfo = qf.make.diagForNode(propNode, qd.msgs.Did_you_forget_to_use_await);
      } else {
        ...
        if (suggestion !== undefined) {
          ...
          relatedInfo = suggestion.valueDeclaration && qf.make.diagForNode(suggestion.valueDeclaration, qd.msgs._0_is_declared_here, suggestedName);
        } else {
    ...
    const resultDiagnostic = qf.make.diagForNodeFromMessageChain(propNode, errorInfo);
  ...
  function typeHasStaticProperty(propName: qu.__String, containingType: qt.Type): boolean {
    const prop = containingType.symbol && qf.type.get.property(containingType.symbol.typeOfSymbol(), propName);
  ...
  function pickLongestCandidateSignature(node: qt.CallLikeExpression, candidates: qt.Signature[], args: readonly qt.Expression[]): qt.Signature {
    ...
    const instantiated = typeArgNodes
      ? qf.make.signatureInstantiation(candidate, getTypeArgsFromNodes(typeArgNodes, typeParams, qf.is.inJSFile(node)))
  ...
  function inferSignatureInstantiationForOverloadFailure(node: qt.CallLikeExpression, typeParams: readonly qt.TypeParam[], candidate: qt.Signature, args: readonly qt.Expression[]): qt.Signature {
    ...
    return qf.make.signatureInstantiation(candidate, typeArgTypes);
  }
  function invocationErrorDetails(errorTarget: Node, apparentType: qt.Type, kind: qt.SignatureKind): { messageChain: qd.MessageChain; relatedMessage: qd.Message | undefined } {
    ...
    const maybeMissingAwait = awaitedType && qf.type.get.signatures(awaitedType, kind).length > 0;
    if (apparentType.isa(qt.TypeFlags.Union)) {
      const types = (apparentType as qt.UnionType).types;
      let hasSignatures = false;
      for (const constituent of types) {
        const signatures = qf.type.get.signatures(constituent, kind);
  ...
  function invocationError(errorTarget: Node, apparentType: qt.Type, kind: qt.SignatureKind, relatedInformation?: qd.DiagnosticRelatedInformation) {
    ...
    const diagnostic = qf.make.diagForNodeFromMessageChain(errorTarget, messageChain);
    if (relatedInfo) addRelatedInfo(diagnostic, qf.make.diagForNode(errorTarget, relatedInfo));
  ...
  function invocationErrorRecovery(apparentType: qt.Type, kind: qt.SignatureKind, diagnostic: qd.Diagnostic) {
    ...
    if (importNode && !is.importCall(importNode)) {
      const sigs = qf.type.get.signatures(apparentType.symbol.links.target!.typeOfSymbol(), kind);
  ...
  function issueMemberSpecificError(node: qt.ClassLikeDeclaration, typeWithThis: qt.Type, baseWithThis: qt.Type, broadDiag: qd.Message) {
    ...
    for (const member of node.members) {
      ...
      if (declaredProp) {
        const prop = qf.type.get.property(typeWithThis, declaredProp.escName);
        const baseProp = qf.type.get.property(baseWithThis, declaredProp.escName);
        if (prop && baseProp) {
          const rootChain = () =>
            chainqd.Messages(
              undefined,
  ...
  function computeExhaustiveSwitchStatement(node: qt.SwitchStatement): boolean {
    if (node.expression.kind === Syntax.TypeOfExpression) {
      ...
      return !!(filterType(type, (t) => (qf.type.get.facts(t) & notEqualFacts) === notEqualFacts).isa(qt.TypeFlags.Never));
    }
    const type = qf.get.typeOfExpression(node.expression);
    if (!qf.type.is.literal(type)) return false;
    const switchTypes = getSwitchClauseTypes(node);
    if (!switchTypes.length || some(switchTypes, (t) => !t.isa(TypeFlags.Unit | TypeFlags.Never))) return false;
  ...
  function contextuallyCheckFunctionExpressionOrObjectLiteralMethod(node: qt.FunctionExpression | qt.ArrowFunction | qt.MethodDeclaration, checkMode?: CheckMode) {
    ...
    if (!(links.flags & NodeCheckFlags.ContextChecked)) {
      ...
      if (!(links.flags & NodeCheckFlags.ContextChecked)) {
        ...
        const signature = firstOrUndefined(qf.type.get.signatures(qf.get.symbolOfNode(node).typeOfSymbol(), qt.SignatureKind.Call));
        if (!signature) return;
  ...
  function widenTypeInferredFromIniter(declaration: qt.HasExpressionIniter, type: qt.Type) {
    const widened = qf.get.combinedFlagsOf(declaration) & NodeFlags.Const || qf.is.declarationReadonly(declaration) ? type : qf.type.get.widenedLiteral(type);
    if (qf.is.inJSFile(declaration)) {
      if (widened.isa(qt.TypeFlags.Nullable)) {
  ...
  function errorUnusedLocal(declaration: qt.Declaration, name: string, addDiagnostic: AddUnusedDiagnostic) {
    ...
    addDiagnostic(declaration, UnusedKind.Local, qf.make.diagForNode(node, message, name));
  }
  ...
  function errorNextVariableOrPropertyDeclarationMustHaveSameType(firstDeclaration: qt.Declaration | undefined, firstType: qt.Type, nextDeclaration: qt.Declaration, nextType: qt.Type): void {
    ...
    if (firstDeclaration) addRelatedInfo(err, qf.make.diagForNode(firstDeclaration, qd.msgs._0_was_also_declared_here, declName));
  }
  ...
  function combineIterationTypes(array: (IterationTypes | undefined)[]) {
    ...
    if (yieldTypes || returnTypes || nextTypes)
      return qf.make.iterationTypes(yieldTypes && qf.type.get.union(yieldTypes), returnTypes && qf.type.get.union(returnTypes), nextTypes && qf.get.intersectionType(nextTypes));
  ...
  function initializeTypeChecker() {
      ...
      if (!is.externalOrCommonJsModule(file)) {
        ...
        if (fileGlobalThisSymbol) {
          for (const declaration of fileGlobalThisSymbol.declarations) {
            diagnostics.add(qf.make.diagForNode(declaration, qd.msgs.Declaration_name_conflicts_with_built_in_global_identifier_0, 'globalThis'));
          }
        }
    ...
    if (autoArrayType === emptyObjectType) autoArrayType = qf.make.anonymousType(undefined, emptySymbols, qu.empty, qu.empty, undefined, undefined);
    ...
    amalgamatedDuplicates.forEach(({ firstFile, secondFile, conflictingSymbols }) => {
      if (conflictingSymbols.size < 8) {
        ...
        diagnostics.add(
          addRelatedInfo(
            qf.make.diagForNode(firstFile, qd.msgs.Definitions_of_the_following_identifiers_conflict_with_those_in_another_file_Colon_0, list),
            qf.make.diagForNode(secondFile, qd.msgs.Conflicts_are_in_this_file)
          )
        );
        diagnostics.add(
          addRelatedInfo(
            qf.make.diagForNode(secondFile, qd.msgs.Definitions_of_the_following_identifiers_conflict_with_those_in_another_file_Colon_0, list),
            qf.make.diagForNode(firstFile, qd.msgs.Conflicts_are_in_this_file)
          )
        );
      }
    });
...
export interface TypeCheckerOld {
  ...
  qf.type.get.baseOfLiteral(type: qt.Type): qt.Type;
  ...
  qf.type.get.nullable(type: qt.Type, flags: qt.TypeFlags): qt.Type;
  ...
  qf.type.get.apparent(type: qt.Type): qt.Type;
  ...
  qf.type.get.elemOfArray(arrayType: qt.Type): qt.Type | undefined;
