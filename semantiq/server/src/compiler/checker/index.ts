...
export function create(host: qt.TypeCheckerHost, produceDiagnostics: boolean): qt.TypeChecker {
      ...
      if (sourcePropType && !check.typeRelatedTo(sourcePropType, targetPropType, relation, undefined)) {
        ...
        if (elaborated) reportedError = true;
          ...
          if (resultObj.errors) {
            ...
            if (!targetProp) {
              const indexInfo =
                (qf.is.typeAssignableToKind(nameType, qt.TypeFlags.NumberLike) && getIndexInfoOfType(target, IndexKind.Number)) || getIndexInfoOfType(target, IndexKind.String) || undefined;
  ...
  function* generateLimitedTupleElems(node: ArrayLiteralExpression, target: Type): ElaborationIterator {
    ...
    if (!len) return;
    for (let i = 0; i < len; i++) {
      if (qf.is.tupleLikeType(target) && !getPropertyOfType(target, ('' + i) as qu.__String)) continue;
