...
export function newMake(f: qt.Frame) {
  ...
  class Fmake {
    ...
    booleanType(ts: readonly qt.Type[]): qt.IntrinsicType & qt.UnionType {
      const type = qf.type.get.union(ts);
    ...
    typeofType() {
      return qf.type.get.union(arrayFrom(typeofEQFacts.keys(), qf.get.literalType));
    }
    ...
    unionOrIntersectionProperty(containingType: qt.UnionOrIntersectionType, name: qu.__String): qt.Symbol | undefined {
      ...
      for (const current of containingType.types) {
        ...
        if (!(type === errorType || type.isa(qt.TypeFlags.Never))) {
          ...
          if (prop) {
            ...
            } else if (type.isobj(ObjectFlags.ObjectLiteral)) {
      ...
      for (const prop of props) {
        ...
        if (type.isa(qt.TypeFlags.Never)) checkFlags |= qt.CheckFlags.HasNeverType;
        propTypes.push(type);
      }
      ...
      if (propTypes.length > 2) {
        ...
        result.type = isUnion ? qf.type.get.union(propTypes) : qf.get.intersectionType(propTypes);
      }
      return result;
    }
    ...
    typeReference(target: qt.GenericType, typeArgs: readonly qt.Type[] | undefined): qt.TypeReference {
      ...
      if (!type) {
        ...
        type.objectFlags |= typeArgs ? qf.type.get.propagatingFlags(typeArgs, 0) : 0;
        type.target = target;
    ...
    tupleTypeOfArity(arity: number, minLength: number, hasRestElem: boolean, readonly: boolean, namedMemberDeclarations: readonly (NamedTupleMember | qt.ParamDeclaration)[] | undefined): qt.TupleType {
      ...
      lengthSymbol.type = hasRestElem ? numberType : qf.type.get.union(literalTypes);
      ...
      type.instantiations.set(qf.type.get.listId(type.typeParams), <qt.GenericType>type);
    ...
    intersectionType(types: qt.Type[], aliasSymbol?: qt.Symbol, aliasTypeArgs?: readonly qt.Type[]) {
      ...
      result.objectFlags = qf.type.get.propagatingFlags(types, qt.TypeFlags.Nullable);
      result.types = types;
    ...
    emptyObjectTypeFromStringLiteral(type: qt.Type) {
      ...
      forEachType(type, (t) => {
        if (!(t.isa(qt.TypeFlags.StringLiteral))) return;
      ...
      const indexInfo = type.isa(qt.TypeFlags.String) ? this.indexInfo(emptyObjectType, false) : undefined;
    ...
    finalArrayType(elemType: qt.Type) {
      return elemType.isa(qt.TypeFlags.Never)
        ? autoArrayType
        : this.arrayType(elemType.isa(qt.TypeFlags.Union) ? qf.type.get.union((<qt.UnionType>elemType).types, qt.UnionReduction.Subtype) : elemType);
    }
    ...
    jsxAttributesTypeFromAttributesProperty(openingLikeElem: qt.JsxOpeningLikeElem, checkMode: CheckMode | undefined) {
      ...
      for (const attributeDecl of attributes.properties) {
        ...
        if (attributeDecl.kind === Syntax.JsxAttribute) {
          ...
          objectFlags |= exprType.isobj(ObjectFlags.PropagatingFlags);
      ...
      if (parent && parent.opening === openingLikeElem && parent.children.length > 0) {
        ...
        if (!hasSpreadAnyType && jsxChildrenPropertyName && jsxChildrenPropertyName !== '') {
          ...
          childrenPropSymbol.type =
            childrenTypes.length === 1 ? childrenTypes[0] : getArrayLiteralTupleTypeIfApplicable(childrenTypes, childrenContextualType, false) || this.arrayType(qf.type.get.union(childrenTypes));
    ...
    unionOfSignaturesForOverloadFailure(candidates: readonly qt.Signature[]): qt.Signature {
      ...
      if (restParamSymbols.length !== 0) {
        const type = this.arrayType(qf.type.get.union(mapDefined(candidates, tryRestType), qt.UnionReduction.Subtype));
    ...
    combinedSymbolFromTypes(sources: readonly qt.Symbol[], types: qt.Type[]): qt.Symbol {
      return this.combinedSymbolForOverloadFailure(sources, qf.type.get.union(types, qt.UnionReduction.Subtype));
    }
    ...
    iterationTypes(yieldType: qt.Type = neverType, returnType: qt.Type = neverType, nextType: qt.Type = unknownType): qt.IterationTypes {
      if (
        yieldType.isa(qt.TypeFlags.Intrinsic) &&
        ...
        const id = qf.type.get.listId([yieldType, returnType, nextType]);
...
export function newInstantiate(f: qt.Frame) {
  ...
  return (qf.instantiate = new (class {
    ...
    anonymousType(type: qt.AnonymousType, mapper: qt.TypeMapper): qt.AnonymousType {
      ...
      if (type.isobj(ObjectFlags.Mapped)) {
    ...
    typeWorker(type: qt.Type, mapper: qt.TypeMapper): qt.Type {
      ...
      if (type.isa(qt.TypeFlags.Object)) {
        ...
        if (objectFlags & (ObjectFlags.Reference | ObjectFlags.Anonymous | ObjectFlags.Mapped)) {
          if (qf.type.is.reference(type) && !(type).node) {
            const resolvedTypeArgs = (type).resolvedTypeArgs;
            const newTypeArgs = this.types(resolvedTypeArgs, mapper);
            return newTypeArgs !== resolvedTypeArgs ? qf.make.typeReference((type).target, newTypeArgs) : type;
          }
          return getObjectTypeInstantiation(<qt.TypeReference | qt.AnonymousType | qt.MappedType>type, mapper);
        }
        return type;
      }
      if (type.isa(qt.TypeFlags.UnionOrIntersection)) {
        const types = (<qt.UnionOrIntersectionType>type).types;
        const newTypes = this.types(types, mapper);
        return newTypes === types
          ? type
          : type.isa(qt.TypeFlags.Intersection)
          ? qf.get.intersectionType(newTypes, type.aliasSymbol, this.types(type.aliasTypeArgs, mapper))
          : qf.type.get.union(newTypes, qt.UnionReduction.Literal, type.aliasSymbol, this.types(type.aliasTypeArgs, mapper));
      }
      ...
      if (type.isa(qt.TypeFlags.Substitution)) {
        ...
        if (maybeVariable.isa(qt.TypeFlags.TypeVariable)) return qf.type.get.substitution(maybeVariable as qt.TypeVariable, this.type((<qt.SubstitutionType>type).substitute, mapper));
        else {
    ...
    instantiableTypes(type: qt.Type, mapper: qt.TypeMapper): qt.Type {
      if (type.isa(qt.TypeFlags.Instantiable)) return this.type(type, mapper);
      if (type.isa(qt.TypeFlags.Union)) {
        return qf.type.get.union(
          map((<qt.UnionType>type).types, (t) => this.instantiableTypes(t, mapper)),
          qt.UnionReduction.None
        );
      }
      if (type.isa(qt.TypeFlags.Intersection)) return qf.get.intersectionType(map((<qt.IntersectionType>type).types, (t) => this.instantiableTypes(t, mapper)));
      return type;
    }
    signatureInContextOf(signature: qt.Signature, contextualSignature: qt.Signature, inferenceContext?: qt.InferenceContext, compareTypes?: qt.TypeComparer): qt.Signature {
      ...
      const mapper = inferenceContext && (restType && restType.isa(qt.TypeFlags.TypeParam) ? inferenceContext.nonFixingMapper : inferenceContext.mapper);
    ...
    typeWithSingleGenericCallSignature(node: qt.Expression | qt.MethodDeclaration | qt.QualifiedName, type: qt.Type, checkMode?: CheckMode) {
      if (checkMode && checkMode & (CheckMode.Inferential | CheckMode.SkipGenericFunctions)) {
        ...
        if (signature && signature.typeParams) {
          ...
          if (contextualType) {
            const contextualSignature = getSingleSignature(qf.type.get.nonNullable(contextualType), callSignature ? SignatureKind.Call : SignatureKind.Construct, false);
...
export function newResolve(f: qt.Frame) {
  ...
  return (qf.resolve = new (class {
    ...
    exportByName(moduleSymbol: qt.Symbol, name: qu.__String, sourceNode: qt.TypeOnlyCompatibleAliasDeclaration | undefined, dontResolveAlias: boolean) {
      ...
      if (exportValue) return qf.type.get.property(exportValue.typeOfSymbol(), name);
      ...
      if (!dontResolveAlias && symbol) {
        ...
        if (compilerOpts.esModuleInterop) {
          ...
          if ((referenceParent.kind === Syntax.ImportDeclaration && qf.get.namespaceDeclarationNode(referenceParent)) || qf.is.importCall(referenceParent)) {
            ...
            let sigs = qf.type.get.structuredSignatures(type, SignatureKind.Call);
            if (!sigs || !sigs.length) sigs = qf.type.get.structuredSignatures(type, SignatureKind.Construct);
            if (sigs && sigs.length) {
    ...
    baseTypesOfClass(type: qt.InterfaceType) {
      ...
      if (type === reducedBaseType || qf.type.has.base(reducedBaseType, type)) {
    ...
    objectTypeMembers(type: qt.ObjectType, source: qt.InterfaceTypeWithDeclaredMembers, typeParams: readonly qt.TypeParam[], typeArgs: readonly qt.Type[]) {
      ...
      const baseTypes = qf.type.get.bases(source);
      if (baseTypes.length) {
        ...
        for (const baseType of baseTypes) {
          ...
          callSignatures = concatenate(callSignatures, qf.type.get.signatures(instantiatedBaseType, SignatureKind.Call));
          constructSignatures = concatenate(constructSignatures, qf.type.get.signatures(instantiatedBaseType, SignatureKind.Construct));
    ...
    unionTypeMembers(type: qt.UnionType) {
      const callSignatures = getUnionSignatures(map(type.types, (t) => (t === globalFunctionType ? [unknownSignature] : qf.type.get.signatures(t, SignatureKind.Call))));
      const constructSignatures = getUnionSignatures(map(type.types, (t) => qf.type.get.signatures(t, SignatureKind.Construct)));
    ...
    intersectionTypeMembers(type: qt.IntersectionType) {
      ...
      for (let i = 0; i < types.length; i++) {
        ...
        if (!mixinFlags[i]) {
          let signatures = qf.type.get.signatures(t, SignatureKind.Construct);
        ...
        callSignatures = appendSignatures(callSignatures, qf.type.get.signatures(t, SignatureKind.Call));
    ...
    anonymousTypeMembers(type: qt.AnonymousType) {
      ...
      if (type.target) {
        ...
        const callSignatures = qf.instantiate.signatures(qf.type.get.signatures(type.target, SignatureKind.Call), type.mapper!);
        const constructSignatures = qf.instantiate.signatures(qf.type.get.signatures(type.target, SignatureKind.Construct), type.mapper!);
        ...
        if (symbol.flags & qt.SymbolFlags.Class) {
          ...
          if (baseConstructorType.flags & (TypeFlags.Object | qt.TypeFlags.Intersection | qt.TypeFlags.TypeVariable)) {
            ...
            addInheritedMembers(members, qf.type.get.properties(baseConstructorType));
          } else if (baseConstructorType === anyType) {
            stringIndexInfo = qf.make.indexInfo(anyType, false);
          }
        }
        const numberIndexInfo =
          symbol.flags & qt.SymbolFlags.Enum && (getDeclaredTypeOfSymbol(symbol).isa(qt.TypeFlags.Enum) || some(type.properties, (prop) => !!(prop.typeOfSymbol().isa(qt.TypeFlags.NumberLike))))
            ? enumNumberIndexInfo
            : undefined;
    ...
    reverseMappedTypeMembers(type: qt.ReverseMappedType) {
      ...
      const members = new qc.SymbolTable();
      for (const prop of qf.type.get.properties(type.source)) {
    ...
    mappedTypeMembers(type: qt.MappedType) {
      ...
      if (isMappedTypeWithKeyofConstraintDeclaration(type)) {
        for (const prop of qf.type.get.properties(modifiersType)) {
          addMemberForKeyType(qf.type.get.literalFromProperty(prop, include));
        }
      ...
      function addMemberForKeyType(t: qt.Type) {
        ...
        if (qf.type.is.usableAsPropertyName(t)) {
          ...
          const modifiersProp = qf.type.get.property(modifiersType, propName);
          const isOptional = !!(
          ...
          if (t.flags & (TypeFlags.Any | qt.TypeFlags.String)) stringIndexInfo = qf.make.indexInfo(propType, !!(templateModifiers & MappedTypeModifiers.IncludeReadonly));
            ...
            numberIndexInfo = qf.make.indexInfo(numberIndexInfo ? qf.type.get.union([numberIndexInfo.type, propType]) : propType, !!(templateModifiers & MappedTypeModifiers.IncludeReadonly));
          }
        }
      }
    }
    structuredTypeMembers(t: qt.StructuredType): qt.ResolvedType {
      if (!(<qt.ResolvedType>t).members) {
        if (qf.type.is.object(t)) {
          if (qf.type.is.reference(t)) this.typeReferenceMembers(t);
          else if (qf.type.is.classOrInterface(t)) this.classOrInterfaceMembers(t);
          else if (qf.type.is.revereseMapped(t)) this.reverseMappedTypeMembers(t);
          else if (qf.type.is.anonymous(t)) this.anonymousTypeMembers(t);
          else if (qf.type.is.mapped(t)) this.mappedTypeMembers(t);
        } else if (qf.type.is.union(t)) this.unionTypeMembers(t);
        else if (qf.type.is.intersection(t)) this.intersectionTypeMembers(t);
      }
      return t as qt.ResolvedType;
    }
    ...
    callExpression(node: qt.CallExpression, candidatesOutArray: qt.Signature[] | undefined, checkMode: CheckMode): qt.Signature {
      ...
      const callSignatures = qf.type.get.signatures(apparentType, SignatureKind.Call);
      const numConstructSignatures = qf.type.get.signatures(apparentType, SignatureKind.Construct).length;
    ...
    newExpression(node: qt.NewExpression, candidatesOutArray: qt.Signature[] | undefined, checkMode: CheckMode): qt.Signature {
      ...
      const constructSignatures = qf.type.get.signatures(expressionType, SignatureKind.Construct);
      ...
      const callSignatures = qf.type.get.signatures(expressionType, SignatureKind.Call);
      if (callSignatures.length) {
