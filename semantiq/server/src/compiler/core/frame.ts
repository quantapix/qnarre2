...
import { EmitFlags, FunctionFlags, Modifier, ModifierFlags, Node, NodeFlags, Nodes, TokenFlags } from '../type';
...
export function newIs(f: qt.Frame) {
  ...
  return (qf.is = new (class {
    templateMiddleOrTailKind(n: Node): n is qc.TemplateMiddle | qt.TemplateTail {
      const k = n.kind;
      return k === Syntax.TemplateMiddle || k === Syntax.TemplateTail;
    }
    stringLiteralLike(n: Node): n is qc.StringLiteralLike {
      const k = n.kind;
      return k === Syntax.StringLiteral || k === Syntax.NoSubstitutionLiteral;
    }
    stringLiteralOrJsxExpressionKind(n: Node): n is qc.StringLiteral | qt.JsxExpression {
      const k = n.kind;
    ...
    stringOrNumericLiteralLike(n: Node): n is qc.StringLiteralLike | qt.NumericLiteral {
      return this.stringLiteralLike(n) || n.kind === Syntax.NumericLiteral;
    }
    identifierTypePredicate(p: qt.TypePredicate): p is qc.IdentifierTypePredicate {
      return p.kind === qt.TypePredicateKind.Identifier;
    }
    thisTypePredicate(p: qt.TypePredicate): p is qc.ThisTypePredicate {
      return p.kind === qt.TypePredicateKind.This;
    }
    getOrSetKind(n: qt.Declaration): n is qc.AccessorDeclaration {
      return n.kind === Syntax.SetAccessor || n.kind === Syntax.GetAccessor;
    }
    toBeCapturedInTempVariable(e: qt.Expression, cache: boolean) {
      const n = qc.skipParentheses(e) as Node;
      switch (n.kind) {
    ...
    bindableObjectDefinePropertyCall(n: qt.CallExpression): n is qc.BindableObjectDefinePropertyCall {
      const e = n.expression as Node;
      if (qu.length(n.arguments) === 3 && e.kind === Syntax.PropertyAccessExpression) {
        const e2 = e.expression as Node;
        if (e2.kind === Syntax.Identifier && qc.idText(e2) === 'Object' && qc.idText(e.name) === 'defineProperty')
          return this.stringOrNumericLiteralLike(n.arguments[1] as Node) && this.bindableStaticNameExpression(n.arguments[0] as Node, true);
      }
      return false;
    }
    commaSequence(e: qt.Expression): e is (qc.BinaryExpression & { operatorToken: qt.CommaToken }) | qc.CommaListExpression {
      const n = e as Node;
    ...
    sameEntityName(name: qt.Expression, init: qt.Expression): boolean {
      ...
      if (n.kind === Syntax.Identifier && this.literalLikeAccess(i)) {
        ...
        if (e.kind === Syntax.ThisKeyword || (e.kind === Syntax.Identifier && (e.escapedText === 'window' || e.escapedText === 'self' || e.escapedText === 'global'))) {
          const a = qf.get.nameOrArgument(i);
          if (a.kind === Syntax.PrivateIdentifier) qu.fail();
          return this.sameEntityName(name, a);
        }
      }
      if (this.literalLikeAccess(n) && this.literalLikeAccess(i))
        return qf.get.elementOrPropertyAccessName(n) === qf.get.elementOrPropertyAccessName(i) && this.sameEntityName(n.expression, i.expression);
      return false;
    }
    ...
    dynamicName(d: qt.Declaration): d is qc.DynamicNamedDeclaration | qt.DynamicNamedBinaryExpression {
    ...
    deleteTarget(n?: Node) {
      ...
      if (k === Syntax.PropertyAccessExpression || k === Syntax.ElementAccessExpression) {
        n = qc.walkUpParenthesizedExpressions(n?.parent);
    ...
    arrayBindingElement(n: Node): n is qt.ArrayBindingElement {
      const k = n.kind;
    ...
    identifierInNonEmittingHeritageClause(n: Node) {
      ...
      const h = qc.findAncestor(n.parent, (p) => {
        switch (p.kind) {
    ...
    docTypeExpressionOrChild(n: Node) {
      return !!qc.findAncestor(n, isDocTypeExpression);
    }
    ...
    privateIdentifierPropertyDeclaration(n?: Node): n is qc.PrivateIdentifierPropertyDeclaration {
...
export function newGet(f: qt.Frame) {
  ...
  const qf = f as Frame;
  return (qf.get = new (class Nget {
    static nextNodeId = 1;
    static nextAutoGenerateId = 1;
    declaration = new (class extends Nget {
      ...
      members(d?: qt.Declaration): Nodes<qt.ClassElement> | Nodes<qt.TypeElement> | Nodes<qt.ObjectLiteralElement> | undefined {
        const n = d as Node | undefined;
        switch (n?.kind) {
          case Syntax.InterfaceDeclaration:
          case Syntax.ClassDeclaration:
          case Syntax.ClassExpression:
        ...
        switch (n.kind) {
          ...
          case Syntax.Identifier:
            if (qf.is.declaration(p)) return p.name === n ? p : undefined;
              ...
              const pp = p?.parent;
              return pp?.kind === Syntax.BinaryExpression && this.assignmentDeclarationKind(pp) !== qc.AssignmentDeclarationKind.None && (pp.left.symbol || pp.symbol) && this.nameOf(pp) === n
                ? pp
                : undefined;
            }
          case Syntax.PrivateIdentifier:
      ...
      internalName(d: qt.Declaration, comments?: boolean, sourceMaps?: boolean) {
        return (d as qc.Declaration).getName(comments, sourceMaps, EmitFlags.LocalName | EmitFlags.InternalName);
      }
      localName(d: qt.Declaration, comments?: boolean, sourceMaps?: boolean) {
        return (d as qc.Declaration).getName(comments, sourceMaps, EmitFlags.LocalName);
      }
      exportName(d: qt.Declaration, comments?: boolean, sourceMaps?: boolean): qt.Identifier {
        return (d as qc.Declaration).getName(comments, sourceMaps, EmitFlags.ExportName);
      }
      name(d: qt.Declaration, comments?: boolean, sourceMaps?: boolean) {
        return (d as qc.Declaration).getName(comments, sourceMaps);
      }
    ...
    toExpression(n: qt.FunctionDeclaration) {
      ...
      const e = new qc.FunctionExpression(n.modifiers, n.asteriskToken, n.name, n.typeParameters, n.parameters, n.type, n.body);
      e.setOriginal(n);
      e.setRange(n);
    ...
    expressionAssociativity(e: qt.Expression) {
      ...
      const n = e as Node;
      const args = n.kind === Syntax.NewExpression && n.arguments !== undefined;
    ...
    entityNameToString(n: qt.EntityNameOrEntityNameExpression | qt.JsxTagNameExpression | qt.PrivateIdentifier): string {
      switch (n.kind) {
        ...
        case Syntax.PrivateIdentifier:
        case Syntax.Identifier:
          return this.fullWidth(n) === 0 ? qc.idText(n) : this.textOf(n);
        case Syntax.QualifiedName:
    ...
    generatedNameForNode(o?: Node, f?: qt.GeneratedIdentifierFlags): qt.Identifier {
      ...
      n.autoGenerateId = Nget.nextAutoGenerateId;
      n.original = o;
      Nget.nextAutoGenerateId++;
      return n;
    }
    ...
    nameOfAccessExpression(n: qt.AccessExpression) {
      ...
      qu.assert(n.kind === Syntax.ElementAccessExpression);
    ...
    firstConstructorWithBody(n: qt.ClassLikeDeclaration): (qt.ConstructorDeclaration & { body: qt.FunctionBody }) | undefined {
      return qu.find(n.members, (m): m is qc.ConstructorDeclaration & { body: qt.FunctionBody } => {
        const n = m as Node;
    ...
    elementOrPropertyAccessArgumentExpressionOrName(n: qt.AccessExpression): qt.Identifier | qt.PrivateIdentifier | qt.StringLiteralLike | qt.NumericLiteral | qt.ElementAccessExpression | undefined {
      if (n.kind === Syntax.PropertyAccessExpression) return n.name;
      const a = qc.skipParentheses(n.argumentExpression) as Node;
    ...
    elementOrPropertyAccessName(n: qt.AccessExpression): qu.__String | undefined {
      ...
      if (n.kind === Syntax.ElementAccessExpression && qf.is.wellKnownSymbolSyntactically(n.argumentExpression as Node))
        return qu.getPropertyNameForKnownSymbolName(qc.idText(n.argumentExpression.name));
      return;
    }
    ...
    assignmentDeclarationKind(e: qt.BinaryExpression | qt.CallExpression): qt.AssignmentDeclarationKind {
      const worker = (e: qt.BinaryExpression | qt.CallExpression): qt.AssignmentDeclarationKind => {
        if (e.kind === Syntax.CallExpression) {
          ...
          if (qf.is.bindableStaticAccessExpression(n) && this.elementOrPropertyAccessName(n) === 'prototype') return qt.AssignmentDeclarationKind.ObjectDefinePrototypeProperty;
    ...
    restParameterElementType(t?: qt.TypeNode) {
    ...
    propertyAssignment(e: qt.ObjectLiteralExpression, k: string, k2?: string): readonly qt.PropertyAssignment[] {
      return e.properties.filter((p): p is qc.PropertyAssignment => {
    ...
    tsConfigPropArrayElementValue(s: qt.TsConfigSourceFile | undefined, k: string, v: string): qt.StringLiteral | undefined {
      return qu.firstDefined(this.tsConfigPropArray(s, k), (p) =>
        isArrayLiteralExpression(p.initer) ? qu.find(p.initer.elements, (e): e is qc.StringLiteral => e.kind === Syntax.StringLiteral && e.text === v) : undefined
      );
    }
    ...
    containingFunction(n: Node): qt.SignatureDeclaration | undefined {
      return qc.findAncestor(n.parent, qf.is.functionLike);
    }
    containingFunctionDeclaration(n: Node): qt.FunctionLikeDeclaration | undefined {
      return qc.findAncestor(n.parent, qf.is.functionLikeDeclaration);
    }
    containingClass(n: Node): qt.ClassLikeDeclaration | undefined {
      return qc.findAncestor(n.parent, qf.is.classLike);
    }
    ...
    enclosingBlockScopeContainer(n: Node): Node {
      return qc.findAncestor(n.parent, (x) => qf.is.blockScope(x, x.parent))!;
    }
    ...
    combinedFlags(n: Node | undefined, cb: (n?: Node) => number): number {
      if (n?.kind === Syntax.BindingElement) n = qc.walkUpBindingElementsAndPatterns(n);
      let flags = cb(n);
    ...
    sourceOfDefaultedAssignment(n: Node) {
      if (n.kind === Syntax.ExpressionStatement) {
        const e = n.expression as Node;
        if (e.kind === Syntax.BinaryExpression && this.assignmentDeclarationKind(e) !== qc.AssignmentDeclarationKind.None) {
    ...
    nonAssignedNameOfDeclaration(d: qt.Declaration | qt.Expression): qt.DeclarationName | undefined {
      ...
      switch (n.kind) {
        ...
        case Syntax.BinaryExpression: {
          switch (this.assignmentDeclarationKind(n)) {
            ...
            case qt.AssignmentDeclarationKind.ThisProperty:
              return this.elementOrPropertyAccessArgumentExpressionOrName(n.left);
        ...
        case Syntax.DocEnumTag:
          return this.nameForNamelessDocTypedef(n);
        case Syntax.ExportAssignment: {
          const { expression } = n;
          return expression.kind === Syntax.Identifier ? expression : undefined;
        }
        case Syntax.ElementAccessExpression:
          if (qf.is.bindableStaticElementAccessExpression(n)) return n.argumentExpression;
      }
      return n.name;
    }
    ...
    doc = new (class {
      ...
      parameterTagsWorker(d: qt.ParameterDeclaration, noCache?: boolean): readonly qt.DocParameterTag[] {
        if (d.name) {
          const n = d.name as Node;
          const p = d.parent as Node | undefined;
          if (n.kind === Syntax.Identifier) {
            const name = n.escapedText;
            return this.tagsWorker(p, noCache).filter((t): t is qc.DocParameterTag => t.kind === Syntax.DocParameterTag && t.name.kind === Syntax.Identifier && t.name.escapedText === name);
          } else {
            const i = p?.parameters.indexOf(d);
            qu.assert(i > -1, "Parameters should always be in their parents' parameter list");
            const ts = this.tagsWorker(p, noCache).filter(qf.is.doc.parameterTag);
            if (i < ts.length) return [ts[i]];
          }
        }
        return qu.empty;
      }
      parameterTags(n: qt.ParameterDeclaration): readonly qt.DocParameterTag[] {
        return this.parameterTagsWorker(n, false);
      }
      parameterTagsNoCache(n: qt.ParameterDeclaration): readonly qt.DocParameterTag[] {
        return this.parameterTagsWorker(n, true);
      }
      typeParameterTagsWorker(n: qt.TypeParameterDeclaration, noCache?: boolean): readonly qt.DocTemplateTag[] {
        const name = n.name.escapedText;
        return this.tagsWorker(n.parent, noCache).filter((t): t is qc.DocTemplateTag => t.kind === Syntax.DocTemplateTag && t.typeParameters.some((p) => p.name.escapedText === name));
      }
      typeParameterTags(n: qt.TypeParameterDeclaration): readonly qt.DocTemplateTag[] {
        return this.typeParameterTagsWorker(n, false);
      }
      typeParameterTagsNoCache(n: qt.TypeParameterDeclaration): readonly qt.DocTemplateTag[] {
        return this.typeParameterTagsWorker(n, true);
      }
      ...
      nameOfTypedef(n: qt.DocTypedefTag): qt.Identifier | qt.PrivateIdentifier | undefined {
        return n.name || nameForNamelessDocTypedef(n);
      }
      ...
      host(n: Node): qt.HasDoc | undefined {
        return qu.checkDefined(qc.findAncestor(n.parent, isDoc)).parent;
      }
      typeParameterDeclarations(n: qt.DeclarationWithTypeParameters): readonly qt.TypeParameterDeclaration[] {
...
export function newHas(f: qt.Frame) {
  ...
  return (qf.has = new (class {
    dynamicName(n: qt.DeclarationName) {
      ...
      const e = (n.kind === Syntax.ElementAccessExpression ? n.argumentExpression : n.expression) as Node;
    ...
    typeArguments(n: Node): n is qt.HasTypeArguments {
      return !!(n as qt.HasTypeArguments).typeArguments;
    }
    ...
    restParameter(n: qt.SignatureDeclaration | qt.DocSignature) {
      const l = qu.lastOrUndefined<qt.ParameterDeclaration | qt.DocParameterTag>(n.parameters);
...
export const fixme = new (class {
  ...
  aggregateChildData(n: Node) {
    if (!(n.flags & NodeFlags.HasAggregatedChildData)) {
      const thisNodeOrAnySubNodesHasError = (n.flags & NodeFlags.ThisNodeHasError) !== 0 || qu.each.child(n, containsParseError);
  ...
  needsScopeMarker(s: qt.Statement) {
    const n = s as Node;
  ...
  sortAndDeduplicateDiagnostics<T extends qd.Diagnostic>(diagnostics: readonly T[]): qu.SortedReadonlyArray<T> {
    return qu.sortAndDeduplicate<T>(diagnostics, compareDiagnostics);
  }
  validateLocaleAndSetLanguage(
    ...
    if (!matchResult) {
      if (errors) {
        errors.push(createCompilerDiagnostic(qd.msgs.Locale_must_be_of_the_form_language_or_language_territory_For_example_0_or_1, 'en', 'ja-jp'));
      }
      return;
    }
  ...
  idText(i: qt.Identifier | qt.PrivateIdentifier): string {
    return qy.get.unescUnderscores(i.escapedText);
  }
  nameForNamelessDocTypedef(d: qt.DocTypedefTag | qt.DocEnumTag): qt.Identifier | qt.PrivateIdentifier | undefined {
    ...
    switch (n.kind) {
      ...
      case Syntax.ExpressionStatement:
        ...
        switch (e.kind) {
          case Syntax.PropertyAccessExpression:
            return e.name;
          case Syntax.ElementAccessExpression:
            const a = e.argumentExpression as Node;
      ...
      case Syntax.LabeledStatement: {
        const s = n.statement as Node;
        if (qf.is.declaration(s) || qf.is.expression(s)) return qf.get.declarationIdentifier(s);
        break;
      }
    }
    return;
...
export function newCreate(f: qt.Frame) {
  ...
  return (qf.create = new (class {
    ...
    reactNamespace(react: string, p: qt.JsxOpeningLikeElement | qt.JsxOpeningFragment) {
      const r = new qc.Identifier(react || 'React');
      r.flags &= ~NodeFlags.Synthesized;
      r.parent = qf.get.parseTreeOf(p);
      return r;
    }
    jsxFactoryExpressionFromEntityName(e: qt.EntityName, p: qt.JsxOpeningLikeElement | qt.JsxOpeningFragment): qt.Expression {
      if (e.kind === Syntax.QualifiedName) {
        const l = this.jsxFactoryExpressionFromEntityName(e.left, p);
        const r = new qc.Identifier(qc.idText(e.right));
        r.escapedText = e.right.escapedText;
        return new qc.PropertyAccessExpression(l, r);
      }
      return this.reactNamespace(qc.idText(e), p);
    }
    jsxFactoryExpression(e: qt.EntityName | undefined, react: string, p: qt.JsxOpeningLikeElement | qt.JsxOpeningFragment): qt.Expression {
      return e ? this.jsxFactoryExpressionFromEntityName(e, p) : new qc.PropertyAccessExpression(this.reactNamespace(react, p), 'createElement');
    }
    ...
    expression(
      e: qt.EntityName | undefined,
      react: string,
      t: qt.Expression,
      ps: qt.Expression,
      cs: readonly qt.Expression[],
      p: qt.JsxOpeningLikeElement,
      r: qu.TextRange
    ): qt.LeftHandSideExpression {
      const args = [t];
      if (ps) args.push(ps);
      if (cs.length) {
    ...
    immediateFunctionExpression(ss: readonly qt.Statement[], p: qt.ParameterDeclaration, v: qt.Expression): qt.CallExpression;
    immediateFunctionExpression(ss: readonly qt.Statement[], p?: qt.ParameterDeclaration, v?: qt.Expression) {
    ...
    immediateArrowFunction(ss: readonly qt.Statement[], p: qt.ParameterDeclaration, v: qt.Expression): qt.CallExpression;
    immediateArrowFunction(ss: readonly qt.Statement[], p?: qt.ParameterDeclaration, v?: qt.Expression) {
    ...
    callBinding(e: qc.Expression, recordTempVariable: (temp: qt.Identifier) => void, _?: qc.ScriptTarget, cacheIdentifiers = false): qc.CallBinding {
      const callee = qc.skipOuterExpressions(e, qc.OuterExpressionKinds.All);
      ...
      if (qf.is.superProperty(callee)) {
        ...
        switch (callee.kind) {
          ...
          case Syntax.ElementAccessExpression: {
            if (qf.is.toBeCapturedInTempVariable((<qt.ElementAccessExpression>callee).expression, cacheIdentifiers)) {
              // for `a[b]()` target is `(_a = a)[b]` and thisArg is `_a`
              thisArg = createTempVariable(recordTempVariable);
              target = new qc.ElementAccessExpression(
                this.assignment(thisArg, (<qt.ElementAccessExpression>callee).expression).setRange((<qt.ElementAccessExpression>callee).expression),
                (<qt.ElementAccessExpression>callee).argumentExpression
              );
              target.setRange(callee);
            } else {
              thisArg = (<qt.ElementAccessExpression>callee).expression;
              target = <qt.ElementAccessExpression>callee;
            }
            break;
          }
          default: {
            ...
            target = qc.parenthesize.forAccess(e);
            break;
          }
        }
      }
    ...
    createExpressionForObjectLiteralElementLike(n: qt.ObjectLiteralExpression, property: qt.ObjectLiteralElementLike, receiver: qt.Expression): qt.Expression | undefined {
    ...
    createFunctionCall(func: qt.Expression, thisArg: qt.Expression, argumentsList: readonly qt.Expression[], location?: qu.TextRange) {
      return setRange(new qc.CallExpression(new qc.PropertyAccessExpression(func, 'call'), undefined, [thisArg, ...argumentsList]), location);
    }
    createFunctionApply(func: qt.Expression, thisArg: qt.Expression, argumentsExpression: qt.Expression, location?: qu.TextRange) {
