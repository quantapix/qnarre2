namespace qnr {
  export namespace Debug {
    ...
    export function formatSymbol(symbol: Symbol): string {
      return `{ name: ${Scanner.unescUnderscores(symbol.escName)}; flags: ${formatSymbolFlags(symbol.flags)}; declarations: ${map(symbol.declarations, (node) => formatSyntax(node.kind))} }`;
    }
