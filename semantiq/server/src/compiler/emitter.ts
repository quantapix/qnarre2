...
export function emitFiles(
  ...
  let exportedModulesFromDeclarationEmit: qt.ExportedModulesFromDeclarationEmit | undefined;
  enter();
  ...
  function emitSourceFileOrBundle({ jsFilePath, sourceMapFilePath, declarationFilePath, declarationMapPath, buildInfoPath }: EmitFileNames, sourceFileOrBundle: qt.SourceFile | qt.Bundle | undefined) {
    ...
    if (buildInfoPath && sourceFileOrBundle && sourceFileOrBundle.kind === Syntax.Bundle) {
  ...
  function emitDeclarationFileOrBundle(
    ...
    const sourceFiles = sourceFileOrBundle.kind === Syntax.SourceFile ? [sourceFileOrBundle] : sourceFileOrBundle.sourceFiles;
    const filesForEmit = forceDtsEmit ? sourceFiles : filter(sourceFiles, isSourceFileNotJson);
    const inputListOrBundle =
      compilerOpts.outFile || compilerOpts.out ? [new qc.Bundle(filesForEmit, !sourceFileOrBundle.kind === Syntax.SourceFile ? sourceFileOrBundle.prepends : undefined)] : filesForEmit;
  ...
  function collectLinkedAliases(node: Node) {
    if (node.kind === Syntax.ExportAssignment) {
      ...
      return;
    } else if (node.kind === Syntax.ExportSpecifier) {
...
export function createPrinter(printerOpts: qt.PrinterOpts = {}, handlers: qt.PrintHandlers = {}): qt.Printer {
  ...
  let autoGenIdToGeneratedName: string[];
  ...
  function writeNode(hint: EmitHint, node: qt.Typing, sourceFile: undefined, output: qt.EmitTextWriter): void;
  ...
  function updateOrPushBundleFileTextLike(pos: number, end: number, kind: qt.BundleFileTextLikeKind) {
  ...
  function reset() {
    ...
    autoGenIdToGeneratedName = [];
    generatedNames = qu.createMap<true>();
    tempFlagsStack = [];
  ...
  function emitJsxAttributeValue(node: qt.StringLiteral | qt.JsxExpression): Node {
    return pipelineEmit(node.kind === Syntax.StringLiteral ? EmitHint.JsxAttributeValue : EmitHint.Unspecified, node);
  }
  ...
  function emitHelpers(node: Node) {
    ...
    for (let i = 0; i < numNodes; i++) {
      ...
      const sourceFile = currentNode.kind === Syntax.SourceFile ? currentNode : currentNode.kind === Syntax.UnparsedSource ? undefined : currentSourceFile!;
      const shouldSkip = printerOpts.noEmitHelpers || (!!sourceFile && qf.emit.hasRecordedExternalHelpers(sourceFile));
      const shouldBundle = (currentNode.kind === Syntax.SourceFile || currentNode.kind === Syntax.UnparsedSource) && !isOwnFileEmit;
      const helpers = currentNode.kind === Syntax.UnparsedSource ? currentNode.helpers : getSortedEmitHelpers(currentNode);
      if (helpers) {
        for (const helper of helpers) {
          if (!helper.scoped) {
  ...
  function writeUnparsedNode(unparsed: qt.UnparsedNode) {
  ...
  function emitEntityName(node: qt.EntityName) {
  ...
  function emitPropertyAccessExpression(node: qt.PropertyAccessExpression) {
    ...
    const token = node.questionDotToken || (createNode(Syntax.DotToken, node.expression.end, node.name.pos) as qt.DotToken);
  ...
  function mayNeedDotDotForPropertyAccess(expression: qt.Expression) {
    ...
    if (expression.kind === Syntax.NumericLiteral) {
      const text = getLiteralTextOfNode(<qt.LiteralExpression>expression, false);
  ...
  function emitBinaryExpression(node: qt.BinaryExpression) {
    ...
    function maybePipelineEmitExpression(next: qt.Expression) {
      ...
      if (pipelinePhase === pipelineEmitWithHint && next.kind === Syntax.BinaryExpression) {
        stackIndex++;
  ...
  function emitBlockStatements(node: qt.BlockLike, forceSingleLine: boolean) {
  ...
  function emitModuleReference(node: qt.ModuleReference) {
    if (node.kind === Syntax.Identifier) {
      emitExpression(node);
    } else {
      emit(node);
  ...
  function emitNamedImportsOrExports(node: qt.NamedImportsOrExports) {
    writePunctuation('{');
    emitList(node, node.elems, ListFormat.NamedImportsOrExportsElems);
    writePunctuation('}');
  }
  function emitImportOrExportSpecifier(node: qt.ImportOrExportSpecifier) {
    if (node.propertyName) {
      emit(node.propertyName);
      writeSpace();
  ...
  function emitJsxOpeningElemOrFragment(node: qt.JsxOpeningElem | qt.JsxOpeningFragment) {
    writePunctuation('<');
    if (node.kind === Syntax.JsxOpeningElem) {
  ...
  function emitJsxClosingElemOrFragment(node: qt.JsxClosingElem | qt.JsxClosingFragment) {
    writePunctuation('</');
    if (node.kind === Syntax.JsxClosingElem) {
      emitJsxTagName(node.tagName);
    }
    writePunctuation('>');
  }
  ...
  function emitJsxTagName(node: qt.JsxTagNameExpression) {
    if (node.kind === Syntax.Identifier) {
      emitExpression(node);
    } else {
      emit(node);
  ...
  function emitSyntheticTripleSlashReferencesIfNeeded(node: qt.Bundle) {
    ...
    for (const prepend of node.prepends) {
      if (prepend.kind === Syntax.UnparsedSource && prepend.syntheticReferences) {
  ...
  function emitPrologueDirectivesIfNeeded(sourceFileOrBundle: qt.Bundle | qt.SourceFile) {
    if (sourceFileOrBundle.kind === Syntax.SourceFile) {
      emitPrologueDirectives(sourceFileOrBundle.statements, sourceFileOrBundle);
    } else {
      const seenPrologueDirectives = qu.createMap<true>();
  ...
  function emitShebangIfNeeded(sourceFileOrBundle: qt.Bundle | qt.SourceFile | qt.UnparsedSource) {
    if (sourceFileOrBundle.kind === Syntax.SourceFile || sourceFileOrBundle.kind === Syntax.UnparsedSource) {
  ...
  function emitTypeAnnotation(node: qt.Typing | undefined) {
    if (node) {
      writePunctuation(':');
      writeSpace();
      emit(node);
  ...
  function emitIniter(node: qt.Expression | undefined, equalCommentStartPos: number, container: Node) {
    if (node) {
      writeSpace();
  ...
  function emitEmbeddedStatement(parent: Node, node: qt.Statement) {
    if (node.kind === Syntax.Block || qf.get.emitFlags(parent) & EmitFlags.SingleLine) {
      writeSpace();
      emit(node);
    } else {
      writeLine();
      increaseIndent();
      if (node.kind === Syntax.EmptyStatement) {
  ...
  function emitTypeArgs(parentNode: Node, typeArgs: Nodes<qt.Typing> | undefined) {
    emitList(parentNode, typeArgs, ListFormat.TypeArgs);
  }
  function emitTypeParams(
    parentNode: qt.SignatureDeclaration | qt.InterfaceDeclaration | qt.TypeAliasDeclaration | qt.ClassDeclaration | qt.ClassExpression,
  ...
  function canEmitSimpleArrowHead(parentNode: qt.FunctionTyping | qt.ArrowFunction, params: Nodes<qt.ParamDeclaration>) {
    ...
    return (
      ...
      parentNode.kind === Syntax.ArrowFunction &&
      !parentNode.type &&
  ...
  function isEmptyBlock(block: qt.BlockLike) {
  ...
  function getLiteralTextOfNode(node: qt.LiteralLikeNode, neverAsciiEscape: boolean | undefined, jsxAttributeEscape: boolean): string {
    if (node.kind === Syntax.StringLiteral && (<qt.StringLiteral>node).textSourceNode) {
      ...
      if (textSourceNode.kind === Syntax.Identifier || textSourceNode.kind === Syntax.NumericLiteral) {
        const text = textSourceNode.kind === Syntax.NumericLiteral ? textSourceNode.text : getTextOfNode(textSourceNode);
  ...
  function reserveNameInNestedScopes(name: string) {
    if (!reservedNames || reservedNames === lastOrUndefined(reservedNamesStack)) {
      reservedNames = qu.createMap<true>();
    }
  ...
  function generateNames(node: Node | undefined) {
    ...
    switch (node.kind) {
      ...
      case Syntax.ForOfStatement:
      case Syntax.ForInStatement:
        generateNames((<qt.ForStatement | qt.ForInOrOfStatement>node).initer);
        generateNames((<qt.ForStatement | qt.ForInOrOfStatement>node).statement);
        break;
      case Syntax.SwitchStatement:
      ...
      case Syntax.CaseClause:
      case Syntax.DefaultClause:
        forEach((<qt.CaseOrDefaultClause>node).statements, generateNames);
        break;
      case Syntax.TryStatement:
      ...
      case Syntax.ObjectBindingPattern:
      case Syntax.ArrayBindingPattern:
        forEach((<qt.BindingPattern>node).elems, generateNames);
        break;
      case Syntax.ImportDeclaration:
  ...
  function generateNameIfNeeded(name: qt.DeclarationName | undefined) {
    if (name) {
      if (qf.is.generatedIdentifier(name)) {
        generateName(name);
      } else if (name.kind === Syntax.BindingPattern) {
        generateNames(name);
      }
    }
  }
  function generateName(name: qt.GeneratedIdentifier) {
    if ((name.autoGenFlags & GeneratedIdentifierFlags.KindMask) === GeneratedIdentifierFlags.Node) return generateNameCached(getNodeForGeneratedName(name), name.autoGenFlags);
    else {
      const autoGenId = name.autoGenId!;
      return autoGenIdToGeneratedName[autoGenId] || (autoGenIdToGeneratedName[autoGenId] = makeName(name));
    }
  }
  ...
  function generateNameForImportOrExportDeclaration(node: qt.ImportDeclaration | qt.ExportDeclaration) {
    ...
    const baseName = expr.kind === Syntax.StringLiteral ? makeIdentifierFromModuleName(expr.text) : 'module';
  ...
  function generateNameForMethodOrAccessor(node: qt.MethodDeclaration | qt.AccessorDeclaration) {
    if (node.name.kind === Syntax.Identifier) return generateNameCached(node.name);
  ...
  function makeName(name: qt.GeneratedIdentifier) {
    switch (name.autoGenFlags & GeneratedIdentifierFlags.KindMask) {
      case GeneratedIdentifierFlags.Auto:
        return makeTempVariableName(TempFlags.Auto, !!(name.autoGenFlags & GeneratedIdentifierFlags.ReservedInNestedScopes));
      case GeneratedIdentifierFlags.Loop:
        return makeTempVariableName(TempFlags._i, !!(name.autoGenFlags & GeneratedIdentifierFlags.ReservedInNestedScopes));
      case GeneratedIdentifierFlags.Unique:
        return makeUniqueName(
          idText(name),
          name.autoGenFlags & GeneratedIdentifierFlags.FileLevel ? isFileLevelUniqueName : isUniqueName,
          !!(name.autoGenFlags & GeneratedIdentifierFlags.Optimistic),
          !!(name.autoGenFlags & GeneratedIdentifierFlags.ReservedInNestedScopes)
        );
    }
  ...
  function getNodeForGeneratedName(name: qt.GeneratedIdentifier) {
    ...
    while (original) {
      node = original;
      if (node.kind === Syntax.Identifier && !!(node.autoGenFlags! & GeneratedIdentifierFlags.Node) && node.autoGenId !== autoGenId) {
        break;
      }
