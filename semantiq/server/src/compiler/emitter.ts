...
export function getOutputDeclarationFileName(inputFileName: string, configFile: ParsedCommandLine, ignoreCase: boolean) {
  qb.assert(!fileExtensionIs(inputFileName, Extension.Dts) && !fileExtensionIs(inputFileName, Extension.Json));
...
export function getOutputFileNames(commandLine: ParsedCommandLine, inputFileName: string, ignoreCase: boolean): readonly string[] {
  ...
  qb.assert(contains(commandLine.fileNames, inputFileName), `Expected fileName to be present in command line`);
  const { addOutput, getOutputs } = createAddOutput();
...
export function emitFiles(
  ...
  function emitJsFileOrBundle(
    ...
    qb.assert(transform.transformed.length === 1, 'Should only see one output from the transform');
  ...
  function emitDeclarationFileOrBundle(
    ...
    if (!declBlocked || forceDtsEmit) {
      qb.assert(declarationTransform.transformed.length === 1, 'Should only see one output from the decl transform');
...
export function createPrinter(printerOptions: PrinterOptions = {}, handlers: PrintHandlers = {}): Printer {
  ...
  let generatedNames: qb.QMap<true>;
  let tempFlagsStack: TempFlags[];
  let tempFlags: TempFlags;
  let reservedNamesStack: qb.QMap<true>[];
  let reservedNames: qb.QMap<true>;
  ...
  function printNode(hint: EmitHint, node: Node, sourceFile: SourceFile): string {
    switch (hint) {
      case EmitHint.SourceFile:
        qb.assert(qc.is.kind(qc.SourceFile, node), 'Expected a SourceFile node.');
        break;
      case EmitHint.IdentifierName:
        qb.assert(qc.is.kind(qc.Identifier, node), 'Expected an Identifier node.');
        break;
    ...
    for (const prepend of bundle.prepends) {
      ...
      if (bundleFileInfo) {
        ...
        if (prepend.oldFileOfCurrentEmit) bundleFileInfo.sections.push(...newSections);
          ...
          newSections.forEach((section) => qb.assert(isBundleFileTextLike(section)));
  ...
  function emitPrologueDirectives(statements: readonly Node[], sourceFile?: SourceFile, seenPrologueDirectives?: qb.QMap<true>, recordBundleFileSection?: true): number {
  ...
  function emitUnparsedPrologues(prologues: readonly UnparsedPrologue[], seenPrologueDirectives: qb.QMap<true>) {
  ...
  function getEffectiveLines(getLineDifference: (includeComments: boolean) => number) {
    qb.assert(!!preserveSourceNewlines);
