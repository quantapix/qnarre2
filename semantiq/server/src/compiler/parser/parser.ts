...
export class Parser {
  ...
  private _startNewParse(fileContents: string, textOffset: number, textLength: number, parseOptions: ParseOptions, diagSink: DiagnosticSink) {
  ...
  private _parseDecorator(): DecoratorNode {
    ...
    while (true) {
      ...
      if (!namePart) {
        ...
        if (callNameExpr) {
          callNameExpr = ErrorNode.create(this._peekToken(), ErrorExpressionCategory.MissingMemberAccessName, callNameExpr);
        } else {
          callNameExpr = ErrorNode.create(this._peekToken(), ErrorExpressionCategory.MissingDecoratorCallName);
        }
        break;
      }
  ...
  private _parseYieldExpression(): YieldNode | YieldFromNode {
    ...
    if (!this._isNextTokenNeverExpression()) {
      exprList = this._parseTestOrStarListAsExpression(true, ErrorExpressionCategory.MissingExpression, Localizer.Diagnostic.expectedYieldExpr());
  ...
  private _parseSubscriptList(): ExpressionNode[] {
    ...
    if (listResult.list.length === 0) {
      return [this._handleExpressionParseError(ErrorExpressionCategory.MissingIndexOrSlice, Localizer.Diagnostic.expectedSliceIndex())];
    }
  ...
  private _parseExpressionStatement(): ExpressionNode {
    let leftExpr = this._parseTestOrStarListAsExpression(false, ErrorExpressionCategory.MissingExpression, Localizer.Diagnostic.expectedExpr());
  ...
  private _getTypeAnnotationComment(): ExpressionNode | undefined {
    ...
    const stringToken = StringToken.create(tokenOffset, typeString.length, StringTokenFlags.None, typeString, 0, undefined);
