namespace qnr {
  ...
  export function isUrl(path: string) {
    return qy_get.encodedRootLength(path) < 0;
  }
  ...
  export function isDiskPathRoot(path: string) {
    const rootLength = qy_get.encodedRootLength(path);
  ...
  export function hasTrailingDirectorySeparator(path: string) {
    return path.length > 0 && qy_is.dirSeparator(path.charCodeAt(path.length - 1));
  }
  ...
  function getAnyExtensionFromPathWorker(path: string, extensions: string | readonly string[], stringEqualityComparer: (a: string, b: string) => boolean) {
    if (typeof extensions === 'string') {
      return qy_get.extensionFrom(path, extensions, stringEqualityComparer) || '';
    }
    for (const extension of extensions) {
      const result = qy_get.extensionFrom(path, extension, stringEqualityComparer);
