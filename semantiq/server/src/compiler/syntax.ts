namespace qnr {
  ...
  export const qy_strToKey = QMap.create(keywords);
  ...
  export const qy_is = new (class {
    ...
    identifierText(s: string, l?: LanguageVariant) {
      ...
      for (let i = qy_get.charSize(c); i < s.length; i += qy_get.charSize(c)) {
  ...
  export const qy_get = new (class {
    ...
    encodedRootLength(path: string) {
      ...
      if (qy_is.volumeChar(c) && path.charCodeAt(1) === Codes.colon) {
      ...
      if (i !== -1) {
        ...
        if (j !== -1) {
          ...
          if (scheme === 'file' && (auth === '' || auth === 'localhost') && qy_is.volumeChar(path.charCodeAt(j + 1))) {
    ...
    extensionFrom(path: string, ext: string, eq: (a: string, b: string) => boolean) {
      ...
      while (pos < t.length) {
        ...
        switch (c) {
          ...
          case Codes.lineFeed:
            ...
            break;
          default:
            if (c > Codes.maxAsciiCharacter && qy_is.lineBreak(c)) {
              ss.push(s);
              s = pos;
            }
            break;
    ...
    lineOf(starts: readonly number[], pos: number, lowerBound?: number) {
    ...
    posOf(starts: readonly number[], line: number, char: number, debug?: string, edits?: true) {
    ...
    leadingCommentRanges(text: string, pos: number): CommentRange[] | undefined {
      return qy_syntax.reduceEachLeadingCommentRange(text, pos, qy_syntax.appendCommentRange, undefined, undefined);
    }
    trailingCommentRanges(text: string, pos: number): CommentRange[] | undefined {
      return qy_syntax.reduceEachTrailingCommentRange(text, pos, qy_syntax.appendCommentRange, undefined, undefined);
    }
  })();

  export const qy_syntax = new (class {
    toString(t: Syntax) {
      return tokStrings[t];
    }
    fromString(s: string) {
    ...
    markerTrivia(s: string, pos: number, e?: (m: DiagnosticMessage, pos?: number, len?: number) => void) {
      if (e) e(Diagnostics.Merge_conflict_marker_encountered, pos, markerLength);
      const c = s.charCodeAt(pos);
      const l = s.length;
      if (c === Codes.lessThan || c === Codes.greaterThan) {
        while (pos < l && !qy_is.lineBreak(s.charCodeAt(pos))) {
          pos++;
        }
      } else {
        ...
        while (pos < l) {
          ...
          if ((c2 === Codes.equals || c2 === Codes.greaterThan) && c2 !== c && qy_is.markerTrivia(s, pos)) break;
          pos++;
        }
      }
      return pos;
    ...
    skipTrivia(s: string, pos: number, stopAfterLineBreak = false, stopAtComments = false) {
        ...
        switch (c) {
          ...
          case Codes.slash:
            ...
            if (s.charCodeAt(pos + 1) === Codes.slash) {
              pos += 2;
              while (pos < s.length) {
                if (qy_is.lineBreak(s.charCodeAt(pos))) break;
                pos++;
              }
              continue;
            }
          ...
          case Codes.equals:
          case Codes.greaterThan:
            if (qy_is.markerTrivia(s, pos)) {
              pos = this.markerTrivia(s, pos);
              continue;
            }
            break;
          case Codes.hash:
            if (pos === 0 && qy_is.shebangTrivia(s, pos)) {
              pos = this.shebangTrivia(s, pos);
              continue;
            }
            break;
          default:
            if (c > Codes.maxAsciiCharacter && qy_is.whiteSpaceLike(c)) {
              pos++;
              continue;
            }
            break;
    ...
    iterateCommentRanges<T, U>(
      ...
      if (pos === 0) {
        collecting = true;
        const shebang = qy_get.shebang(text);
        if (shebang) {
          pos = shebang.length;
        }
      }
      scan: while (pos >= 0 && pos < text.length) {
        ...
        switch (ch) {
          ...
          case Codes.slash:
            ...
            if (nextChar === Codes.slash || nextChar === Codes.asterisk) {
              ...
              if (nextChar === Codes.slash) {
                while (pos < text.length) {
                  if (qy_is.lineBreak(text.charCodeAt(pos))) {
            ...
            break scan;
          default:
            if (ch > Codes.maxAsciiCharacter && qy_is.whiteSpaceLike(ch)) {
              if (hasPendingCommentRange && qy_is.lineBreak(ch)) {
    ...
    forEachLeadingCommentRange<U>(text: string, pos: number, cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean) => U): U | undefined;
    forEachLeadingCommentRange<T, U>(text: string, pos: number, cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean, state: T) => U, state: T): U | undefined;
    forEachLeadingCommentRange<T, U>(text: string, pos: number, cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean, state?: T) => U, state?: T): U | undefined {
      return this.iterateCommentRanges(false, text, pos, false, cb, state);
    }
    forEachTrailingCommentRange<U>(text: string, pos: number, cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean) => U): U | undefined;
    forEachTrailingCommentRange<T, U>(text: string, pos: number, cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean, state: T) => U, state: T): U | undefined;
    forEachTrailingCommentRange<T, U>(text: string, pos: number, cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean, state?: T) => U, state?: T): U | undefined {
      return this.iterateCommentRanges(false, text, pos, true, cb, state);
    }
    appendCommentRange(pos: number, end: number, kind: CommentKind, hasTrailingNewLine: boolean, _?: any, cs?: CommentRange[]) {
      if (!cs) cs = [];
      cs.push({ kind, pos, end, hasTrailingNewLine });
      return cs;
    }
    reduceEachLeadingCommentRange<T, U>(text: string, pos: number, cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean, state?: T, memo?: U) => U, state?: T, initial?: U) {
      return this.iterateCommentRanges(true, text, pos, false, cb, state, initial);
    }
    reduceEachTrailingCommentRange<T, U>(text: string, pos: number, cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean, state?: T, memo?: U) => U, state?: T, initial?: U) {
