namespace qnr {
  ...
  export class Context {
    ...
    static isWhiteSpaceLike(c: number) {
      return this.isWhiteSpaceSingleLine(c) || this.isLineBreak(c);
    }

    static isDigit(c: number) {
      return c >= Codes._0 && c <= Codes._9;
    }

    static isHexDigit(c: number) {
      return this.isDigit(c) || (c >= Codes.A && c <= Codes.F) || (c >= Codes.a && c <= Codes.f);
    }

    static isOctalDigit(c: number) {
      return c >= Codes._0 && c <= Codes._7;
    }

    static isCodePoint(c: number) {
      return c <= 0x10ffff;
    }

    static isDirSeparator(c: number) {
      return c === Codes.slash || c === Codes.backslash;
    }

    static isVolumeChar(c: number) {
      return (c >= Codes.a && c <= Codes.z) || (c >= Codes.A && c <= Codes.Z);
    }

    static isReservedName(x: __String) {
      const n = x as string;
    ...
    private static readonly altDirSeparator = '\\';
    private static readonly urlSchemeSeparator = '://';

    static encodedRootLength(path: string) {
      if (!path) return 0;
      const c = path.charCodeAt(0);
      if (c === Codes.slash || c === Codes.backslash) {
        if (path.charCodeAt(1) !== c) return 1;
        const i = path.indexOf(c === Codes.slash ? dirSeparator : this.altDirSeparator, 2);
        if (i < 0) return path.length;
        return i + 1;
      }
      if (this.isVolumeChar(c) && path.charCodeAt(1) === Codes.colon) {
        const c2 = path.charCodeAt(2);
        if (c2 === Codes.slash || c2 === Codes.backslash) return 3;
        if (path.length === 2) return 2;
      }
      const i = path.indexOf(this.urlSchemeSeparator);
      if (i !== -1) {
        const s = i + this.urlSchemeSeparator.length;
        const j = path.indexOf(dirSeparator, s);
        if (j !== -1) {
          const scheme = path.slice(0, i);
          const auth = path.slice(s, j);
          if (scheme === 'file' && (auth === '' || auth === 'localhost') && this.isVolumeChar(path.charCodeAt(j + 1))) {
    ...
    static extensionFrom(path: string, ext: string, eq: (a: string, b: string) => boolean) {
    ...
    static isOneOf(c: number, cs: readonly number[]) {
    ...
    static isIdentifierPart(c: number, l?: LanguageVariant) {
      return (
        ...
        (c > Codes.maxAsciiCharacter && this.isOneOf(c, this.parts))
      );
    }

    static isIdentifierText(s: string, l?: LanguageVariant) {
      let c = s.codePointAt(0)!;
      if (!this.isIdentifierStart(c)) return false;
      for (let i = charSize(c); i < s.length; i += charSize(c)) {
        if (!this.isIdentifierPart((c = s.codePointAt(i)!), l)) return false;
      }
      return true;
    }

    static couldStartTrivia(s: string, pos: number) {
    ...
    private static readonly markerLength = '<<<<<<<'.length;

    static isMarkerTrivia(s: string, pos: number) {
      assert(pos >= 0);
      if (pos === 0 || this.isLineBreak(s.charCodeAt(pos - 1))) {
        const c = s.charCodeAt(pos);
        if (pos + this.markerLength < s.length) {
          for (let i = 0; i < this.markerLength; i++) {
            if (s.charCodeAt(pos + i) !== c) return false;
          }
          return c === Codes.equals || s.charCodeAt(pos + this.markerLength) === Codes.space;
        }
      }
      return false;
    }

    static scanMarkerTrivia(s: string, pos: number, e?: (m: DiagnosticMessage, pos?: number, len?: number) => void) {
      if (e) e(Diagnostics.Merge_conflict_marker_encountered, pos, this.markerLength);
      const c = s.charCodeAt(pos);
      const l = s.length;
      if (c === Codes.lessThan || c === Codes.greaterThan) {
        while (pos < l && !this.isLineBreak(s.charCodeAt(pos))) {
          pos++;
        }
      } else {
        ...
        while (pos < l) {
          ...
          if ((c2 === Codes.equals || c2 === Codes.greaterThan) && c2 !== c && this.isMarkerTrivia(s, pos)) break;
          pos++;
        }
      }
      return pos;
    ...
    private static readonly shebangRegex = /^#!.*/;

    static getShebang(s: string) {
      const m = this.shebangRegex.exec(s);
      return m ? m[0] : undefined;
    }

    static isShebangTrivia(s: string, pos: number) {
      assert(pos === 0);
      return this.shebangRegex.test(s);
    }

    static scanShebangTrivia(s: string, pos: number) {
      const m = this.shebangRegex.exec(s);
      return pos + (m ? m[0].length : 0);
    }

    static skipTrivia(s: string, pos: number, stopAfterLineBreak = false, stopAtComments = false) {
        ...
        switch (c) {
          ...
          case Codes.slash:
            ...
            if (s.charCodeAt(pos + 1) === Codes.slash) {
              pos += 2;
              while (pos < s.length) {
                if (this.isLineBreak(s.charCodeAt(pos))) break;
                pos++;
              }
              continue;
            }
          ...
          case Codes.equals:
          case Codes.greaterThan:
            if (this.isMarkerTrivia(s, pos)) {
              pos = this.scanMarkerTrivia(s, pos);
              continue;
            }
            break;
          case Codes.hash:
            if (pos === 0 && this.isShebangTrivia(s, pos)) {
              pos = this.scanShebangTrivia(s, pos);
              continue;
            }
            break;
          default:
            if (c > Codes.maxAsciiCharacter && this.isWhiteSpaceLike(c)) {
              pos++;
              continue;
            }
            break;
      ...
      while (pos < t.length) {
        ...
        switch (c) {
          ...
          case Codes.lineFeed:
            ...
            break;
          default:
            if (c > Codes.maxAsciiCharacter && this.isLineBreak(c)) {
              ss.push(s);
              s = pos;
            }
            break;
    ...
    static lineOf(starts: readonly number[], pos: number, lowerBound?: number) {
    ...
    static lineAndCharOf(starts: readonly number[], pos: number): LineAndChar {
      const line = this.lineOf(starts, pos);
      return { line, char: pos - starts[line] };
    }

    static posOf(starts: readonly number[], line: number, char: number, debug?: string, edits?: true) {
    ...
    static iterateCommentRanges<T, U>(
      reduce: boolean,
      text: string,
      pos: number,
      trailing: boolean,
      ...
      if (pos === 0) {
        collecting = true;
        const shebang = this.getShebang(text);
        if (shebang) {
          pos = shebang.length;
        }
      }
      scan: while (pos >= 0 && pos < text.length) {
        ...
        switch (ch) {
          ...
          case Codes.slash:
            ...
            if (nextChar === Codes.slash || nextChar === Codes.asterisk) {
              ...
              if (nextChar === Codes.slash) {
                while (pos < text.length) {
                  if (this.isLineBreak(text.charCodeAt(pos))) {
            ...
            break scan;
          default:
            if (ch > Codes.maxAsciiCharacter && this.isWhiteSpaceLike(ch)) {
              if (hasPendingCommentRange && this.isLineBreak(ch)) {
    ...
    static forEachLeadingCommentRange<U>(text: string, pos: number, cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean) => U): U | undefined;
    static forEachLeadingCommentRange<T, U>(text: string, pos: number, cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean, state: T) => U, state: T): U | undefined;
    static forEachLeadingCommentRange<T, U>(text: string, pos: number, cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean, state?: T) => U, state?: T): U | undefined {
      return this.iterateCommentRanges(/*reduce*/ false, text, pos, /*trailing*/ false, cb, state);
    }

    static forEachTrailingCommentRange<U>(text: string, pos: number, cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean) => U): U | undefined;
    static forEachTrailingCommentRange<T, U>(text: string, pos: number, cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean, state: T) => U, state: T): U | undefined;
    static forEachTrailingCommentRange<T, U>(text: string, pos: number, cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean, state?: T) => U, state?: T): U | undefined {
      return this.iterateCommentRanges(false, text, pos, true, cb, state);
    }

    static appendCommentRange(pos: number, end: number, kind: CommentKind, hasTrailingNewLine: boolean, _?: any, cs?: CommentRange[]) {
      if (!cs) cs = [];
      cs.push({ kind, pos, end, hasTrailingNewLine });
      return cs;
    }

    static reduceEachLeadingCommentRange<T, U>(
      text: string,
      pos: number,
      cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean, state?: T, memo?: U) => U,
      state?: T,
      ...
      return this.iterateCommentRanges(/*reduce*/ true, text, pos, /*trailing*/ false, cb, state, initial);
    }

    static getLeadingCommentRanges(text: string, pos: number): CommentRange[] | undefined {
      return this.reduceEachLeadingCommentRange(text, pos, this.appendCommentRange, undefined, undefined);
    }

    static reduceEachTrailingCommentRange<T, U>(
      text: string,
      pos: number,
      cb: (pos: number, end: number, k: CommentKind, hasTrailingNewLine: boolean, state?: T, memo?: U) => U,
      state?: T,
      ...
      return this.iterateCommentRanges(true, text, pos, true, cb, state, initial);
    }

    static getTrailingCommentRanges(text: string, pos: number): CommentRange[] | undefined {
      return this.reduceEachTrailingCommentRange(text, pos, this.appendCommentRange, /*state*/ undefined, /*initial*/ undefined);
    }

    private static readonly directiveRegExSingleLine = /^\s*\/\/\/?\s*@(ts-expect-error|ts-ignore)/;
    private static readonly directiveRegExMultiLine = /^\s*(?:\/|\*)*\s*@(ts-expect-error|ts-ignore)/;
  }

  export class Scanner2 extends Context {
    pos: number; // Current position (end position of text of current token)
    end: number; // end of text
    text: string;
    token: Syntax;
    tokPos: number; // Start position of text of current token
    startPos: number; // Start position of whitespace before current token
    tokValue: string;
    tokFlags: TokenFlags;
    directives: CommentDirective[] | undefined;
    inJSDocType = 0;

    constructor(public skipTrivia = false, public lang = LanguageVariant.TS, public onError?: ErrorCallback) {
      super();
      /*
    ...
    */
    ...
    setText(t?: string, start?: number, length?: number) {
      text = t ?? '';
    ...
    error(m: DiagnosticMessage): void;
    error(m: DiagnosticMessage, errPos: number, length: number): void;
    error(m: DiagnosticMessage, errPos: number = pos, length?: number): void {
      if (onError) {
        const p = pos;
        pos = errPos;
        onError(m, length ?? 0);
    ...
    speculate<T>(cb: () => T, isLookahead: boolean): T {
      const p = pos;
      const sp = startPos;
      const tp = tokPos;
      const t = token;
    ...
    scanIdentifier(): Syntax.Identifier | KeywordSyntax {
    ...
    scanNumber(): { type: Syntax; value: string } {
      const s = pos;
    ...
    scanHexDigits(min: number, greedy = true, seps = false): string {
      let ds = [] as number[];
      let sep = false;
      let prev = false;
    ...
    scanBinOrOctDigits(base: 2 | 8): string {
      let r = '';
      let sep = false;
      let prev = false;
      while (true) {
    ...
    scanEscSequence(tagged?: boolean): string {
      const s = pos;
      pos++;
      if (pos >= end) {
        error(Diagnostics.Unexpected_end_of_text);
    ...
    scanString(jsxAttr = false): string {
      let r = '';
    ...
    scanTemplateAndSetTokenValue(tagged: boolean): Syntax {
    ...
    appendIfDirective(ds: CommentDirective[] | undefined, t: string, re: RegExp, line: number) {
      const d = directiveFrom(t, re);
      if (d === undefined) return ds;
      return append(ds, { range: { pos: line, end: pos }, type: d });
    }

    directiveFrom(t: string, re: RegExp) {
      const m = re.exec(t);
      if (!m) return;
      switch (m[1]) {
    ...
    private static raw: Scanner | undefined;
    static getRaw() {
      return raw || (raw = new Scanner(true));
    }

    private static readonly sentinel: object = {};

    static process(k: TemplateLiteralToken['kind'], s: string) {
      const r = this.getRaw();
      switch (k) {
