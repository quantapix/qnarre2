...
export function createDynamicPriorityPollingWatchFile(host: { getModifiedTime: NonNullable<System['getModifiedTime']>; setTimeout: NonNullable<System['setTimeout']> }): HostWatchFile {
  ...
  function pollPollingIntervalQueue(queue: PollingIntervalQueue) {
    ...
    if (queue.length) {
      ...
      qf.assert.true(queue.pollIndex === 0);
      queue.pollScheduled = false;
    }
  }
  ...
  function createDirectoryWatcher(dirName: string, dirPath: string, fallbackOpts: qt.WatchOpts | undefined) {
    const watcher = fsWatch(
      ...
      (_eventName: string, relativeFileName) => {
        if (!qf.is.string(relativeFileName)) {
          return;
        }
...
export function createSingleFileWatcherPerName(watchFile: HostWatchFile, useCaseSensitiveFileNames: boolean): HostWatchFile {
  ...
  return (fileName, callback, pollingInterval, opts) => {
    ...
    return {
      close: () => {
        const watcher = qf.check.defined(cache.get(path));
...
export function createDirectoryWatcherSupportingRecursive(host: RecursiveDirectoryWatcherHost): HostWatchDirectory {
  ...
  function createDirectoryWatcher(dirName: string, opts: qt.WatchOpts | undefined, callback?: DirectoryWatcherCallback): ChildDirectoryWatcher {
    ...
    return {
      dirName,
      close: () => {
        const directoryWatcher = qf.check.defined(cache.get(dirPath));
  ...
  function invokeCallbacks(dirPath: qt.Path, fileNameOrInvokeMap: string | Map<true>) {
    ...
    if (qf.is.string(fileNameOrInvokeMap)) {
...
export function createSystemWatchFunctions({
  ...
  function watchFile(fileName: string, callback: FileWatcherCallback, pollingInterval: PollingInterval, opts: qt.WatchOpts | undefined): FileWatcher {
    ...
    const watchFileKind = qf.check.defined(opts.watchFile);
  ...
  function nonRecursiveWatchDirectory(directoryName: string, callback: DirectoryWatcherCallback, recursive: boolean, opts: qt.WatchOpts | undefined): FileWatcher {
    qf.assert.true(!recursive);
    opts = updateOptsForWatchDirectory(opts);
    const watchDirectoryKind = qf.check.defined(opts.watchDirectory);
