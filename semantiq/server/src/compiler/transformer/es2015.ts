...
export function transformES2015(context: qt.TrafoContext) {
  ...
  function insertDefaultValueAssignmentForBindingPattern(statements: qc.Statement[], param: qt.ParamDeclaration, name: qt.BindingPattern, initer: qc.Expression | undefined): boolean {
    if (name.elems.length > 0) {
      ...
      insertStatementAfterCustomPrologue(
        statements,
        qf.emit.setFlags(new qc.ExpressionStatement(qf.create.assignment(qf.get.generatedNameForNode(param), visitNode(initer, visitor, isExpression))), EmitFlags.CustomPrologue)
      );
      return true;
    }
    return false;
      ...
      qf.emit.setFlags(
        new qc.Block([
          new qc.ExpressionStatement(
            qf.emit.setFlags(
              qf.create
                .assignment(qf.emit.setFlags(getMutableClone(name), EmitFlags.NoSourceMap), qf.emit.setFlags(initer, EmitFlags.NoSourceMap | qf.get.emitFlags(initer) | EmitFlags.NoComments))
                .setRange(param),
      ...
      for (const decl of node.declarationList.declarations) {
        ...
        if (decl.initer) {
          ...
          if (decl.name.kind === Syntax.BindingPattern) {
            ...
            assignment = new qc.BinaryExpression(decl.name, Syntax.EqualsToken, visitNode(decl.initer, visitor, isExpression));
  ...
  function convertForOfStatementHead(node: qt.ForOfStatement, boundValue: qc.Expression, convertedLoopBodyStatements: qc.Statement[]) {
    ...
    if (initer.kind === Syntax.VariableDeclarationList) {
      ...
      if (firstOriginalDeclaration && firstOriginalDeclaration.name.kind === Syntax.BindingPattern) {
        ...
        statements.push(
          new qc.VariableStatement(
            undefined,
            setOriginalNode(
              new qc.VariableDeclarationList([new qc.VariableDeclaration(firstOriginalDeclaration ? firstOriginalDeclaration.name : createTempVariable(undefined), undefined, boundValue)]).setRange(
      ...
      if (qf.is.destructuringAssignment(assignment)) {
        ...
        statements.push(new qc.ExpressionStatement(visitNode(assignment, visitor, isExpression)).setRange(moveRangeEnd(initer, -1)));
      }
    }
...
function visitObjectLiteralExpression(node: qt.ObjectLiteralExpression): qc.Expression {
  ...
  if (numInitialProperties !== numProperties) {
    ...
    expressions.push(node.multiLine ? qf.emit.setStartsOnNewLine(getMutableClone(temp)) : temp);
        ...
        new qc.VariableDeclaration(
          ...
          qf.emit.setFlags(
            new qc.FunctionExpression(
              ...
              undefined,
              undefined,
              visitNode(new qc.Block(statements, true), visitor, isBlock)
            ),
            emitFlags
          )
        ),
...
function createFunctionForBodyOfIterationStmt(node: qt.IterationStmt, currentState: ConvertedLoopState, outerState: ConvertedLoopState | undefined): IterationStmtPartFunction<qt.Statement[]> {
  ...
  if (shouldConvertConditionOfForStatement(node) || shouldConvertIncrementorOfForStatement(node)) {
    ...
    if (shouldConvertConditionOfForStatement(node)) {
      statements.push(
        new qc.IfStatement(new qc.PrefixUnaryExpression(Syntax.ExclamationToken, visitNode(node.condition, visitor, isExpression)), visitNode(new qc.BreakStatement(), visitor, qf.is.statement))
      );
    }
  }
