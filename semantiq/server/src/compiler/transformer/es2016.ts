...
export function transformES2016(context: qt.TrafoContext) {
  ...
  function visitExponentiationAssignmentExpression(node: qt.BinaryExpression) {
    ...
    if (left.kind === Syntax.ElemAccessExpression) {
      ...
      const expressionTemp = qf.make.tempVariable(hoistVariableDeclaration);
      const argExpressionTemp = qf.make.tempVariable(hoistVariableDeclaration);
      target = new qc.ElemAccessExpression(
        qf.make.assignment(expressionTemp, left.expression).setRange(left.expression),
        qf.make.assignment(argExpressionTemp, left.argExpression).setRange(left.argExpression)
      ).setRange(left);
      ...
      const expressionTemp = qf.make.tempVariable(hoistVariableDeclaration);
      target = new qc.PropertyAccessExpression(qf.make.assignment(expressionTemp, left.expression).setRange(left.expression), left.name).setRange(left);
    ...
    return qf.make.assignment(target, qf.make.mathPow(value, right, node)).setRange(node);
  }
  function visitExponentiationExpression(node: qt.BinaryExpression) {
    // Transforms `a ** b` into `Math.pow(a, b)`
    const left = qf.visit.node(node.left, visitor, isExpression);
    const right = qf.visit.node(node.right, visitor, isExpression);
    return qf.make.mathPow(left, right, node);
  }
}
