...
export function cast<TOut extends TIn, TIn = any>(value: TIn | undefined, test: (value: TIn) => value is TOut): TOut {
  ...
  return Debug.fail(`Invalid cast. The supplied value ${value} did not pass the test`);
}
...
export namespace Debug {
  ...
  export function fail(m?: string, stackCrawlMark?: AnyFunction): never {
    debugger;
  ...
  export function failBadSyntaxKind(node: qt.Node, m?: string, stackCrawlMark?: AnyFunction): never {
    return fail(`${m || 'Unexpected node.'}\r\nNode ${node.kind} was unexpected.`, stackCrawlMark || failBadSyntaxKind);
  }

  export function assert(expression: unknown, m?: string, verboseDebugInfo?: string | (() => string), stackCrawlMark?: AnyFunction): asserts expression {
    if (!expression) {
      m = m ? `False expression: ${m}` : 'False expression.';
      if (verboseDebugInfo) {
        m += '\r\nVerbose Debug Information: ' + (typeof verboseDebugInfo === 'string' ? verboseDebugInfo : verboseDebugInfo());
      }
      fail(m, stackCrawlMark || assert);
    }
  }
