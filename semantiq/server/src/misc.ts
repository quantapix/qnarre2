...
compile(process.argv.slice(2), {
  noEmitOnError: true,
  noImplicitAny: true,
  target: qts.ScriptTarget.ESNext,
  module: qts.ModuleKind.ESNext,
  declaration: true,
...
function makeFactorialFunction() {
  ...
  return qts.createFunctionDeclaration(
    /*decorators*/ undefined,
    /*modifiers*/ [qts.createToken(qts.SyntaxKind.ExportKeyword)],
    /*asteriskToken*/ undefined,
    functionName,
    /*typeParameters*/ undefined,
    [parameter],
    /*returnType*/ qts.createKeywordTypeNode(qts.SyntaxKind.NumberKeyword),
    qts.createBlock(statements, /*multiline*/ true)
  );
}
...
function generateDocumentation(fs: string[], options: qts.CompilerOptions): void {
  ...
  function isNodeExported(n: qts.Node): boolean {
    return (
      (qts.getCombinedModifierFlags(n as qts.Declaration) & qts.ModifierFlags.Export) !== 0 ||
      (!!n.parent && n.parent.kind === qts.SyntaxKind.SourceFile)
    );
  }
}
